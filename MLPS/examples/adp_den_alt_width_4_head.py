...
# - Heads: 4 (Pg, Qg, Va, Vm), width-phase then depth-phase with per-attempt rollback and physics-first acceptance.
# - Deviations: rollbacks on failed expansions, physics-first acceptance, not forward-only per ADP_algorithms.md.
...
class MLPAlt4Head(nn.Module):
    """Shared input, four branch stacks and four heads."""
    ...
    def __init__(self, in_dim: int, outs: List[int], width: int = 64, depth: int = 1):
        ...
        self.b_fc2 = nn.ModuleList([StackedLinear(self.width, start_depth=max(1, depth)) for _ in range(4)])
        self.heads = nn.ModuleList([nn.Linear(self.width, o) for o in outs])

    @property
    def depth(self) -> int:
        return self.b_fc2[0].depth
    ...
    def _rebuild_width(self, new_width: int) -> None:
        ...
        new_b = nn.ModuleList([StackedLinear(new_width, b.depth).to(device) for b in self.b_fc2])
        for i, b in enumerate(self.b_fc2):
            for j in range(b.depth):
                _copy_overlap_linear(b.layers[j], new_b[i].layers[j])

        new_heads = nn.ModuleList([nn.Linear(new_width, h.out_features).to(device) for h in self.heads])
        for i, h in enumerate(self.heads):
            _copy_overlap_linear(h, new_heads[i])
...
class ADP_ALT_WIDTH_4Head:
    """ADP_ALT_WIDTH forward-only for 4-head DEN."""
...
# - Heads: 4 (Pg, Qg, Va, Vm).
# - Implements ADP_ALT_WIDTH forward-only: width-phase then depth-phase; failed expansions keep marching; restore global best only between phases/end.
# - Acceptance: val loss improvements vs delta thresholds; physics metric tracked for reporting only; head-aware expand_width/expand_depth and train_with_early_stopping.
