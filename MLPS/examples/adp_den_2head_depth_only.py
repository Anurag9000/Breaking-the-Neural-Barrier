...
# - Heads: 2 (PQ, VM) depth-only with rollback; conflicts with forward-only spec.
...
class ADP_DEN_2head_DepthOnly(nn.Module):
    """ADP_DEPTH_ONLY forward-only for a 2-head MLP (PQ, VM)."""
...
# - Uses train_with_early_stopping (patience_es) and snapshot_arch_and_state / restore_arch_and_state; expand_depth appends square hidden layers and resizes both heads.
