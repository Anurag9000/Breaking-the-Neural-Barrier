...
# - Heads: 4, width-only plateau with rollback; conflicts with forward-only spec.
# - Deviations: per-expansion rollback and mixed patience handling.
...
class ADP_DEN_4Head_ExpandTillPlateau(nn.Module):
    """ADP_WIDTH_ONLY forward-only for a 4-head MLP (Pg, Qg, Va, Vm)."""
...
# - Uses train_with_early_stopping (patience_es) and snapshot_arch_and_state / restore_arch_and_state; expand_width widens hidden stack and all heads.
