...
# - Heads: 2 (PQ, VM) width-only with per-attempt rollback; conflicts with forward-only spec.
# - Deviations: rolls back failed width expansions; should keep widening until patience exhaustion, restore best once at end.
...
class ADP_DEN_2Head(nn.Module):
    """ADP_WIDTH_ONLY forward-only for 2-head MLP (PQ, VM)."""
