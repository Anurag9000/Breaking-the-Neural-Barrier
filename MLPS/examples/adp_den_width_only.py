...
# - Heads: 1 (single head) width-only with per-attempt rollback; conflicts with forward-only spec.
# - Deviations: rolls back failed width expansions; should keep widening until patience exhausts, restore best at end.
...
class ADP_DEN(nn.Module):
    """ADP_WIDTH_ONLY forward-only (single head)."""
...
# - Heads: 1 (single head).
# - Implements ADP_WIDTH_ONLY with forward-only expansions (no per-attempt rollback); width_failure_count vs patience_width_exp and delta_width.
# - Uses train_with_early_stopping (patience_es) and snapshot_arch_and_state / restore_arch_and_state; expand_width widens hidden stack and resizes the head.
