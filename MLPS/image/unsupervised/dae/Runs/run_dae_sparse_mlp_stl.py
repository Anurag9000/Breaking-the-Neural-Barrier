...
from ..Models.dae_sparse_mlp_stl import DAESparseMLP, dae_total_neurons
...
def eval_epoch(
    ...
    with torch.no_grad():
        for xb, _ in loader:
            ...
            total += float(recon.item()) + float(sparsity_lambda * sparsity.item())
            n += xb.size(0)
    return total / max(n, 1)


def main() -> None:
    p = argparse.ArgumentParser(description="Sparse MLP DAE STL on CIFAR")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./Runs/dae_sparse_mlp_stl")
