...
from ..Models.dae_lowrank_mlp_stl import DAELowRankMLP, dae_total_neurons
...
def lowrank_penalty(model: DAELowRankMLP) -> torch.Tensor:
    """
    Low-rank-style penalty: sum of nuclear norms of encoder weight matrices.
    """
    total = 0.0
...
def train_one_epoch(
    ...
    for xb, _ in loader:
        ...
        loss = recon_loss + lowrank_lambda * lr_pen
        loss.backward()
        opt.step()
...
def eval_epoch(
    ...
    with torch.no_grad():
        for xb, _ in loader:
            ...
            total += float(recon.item()) + float(lowrank_lambda * lr_pen.item())
            n += xb.size(0)
    return total / max(n, 1)


def main() -> None:
    p = argparse.ArgumentParser(description="Low-rank MLP DAE STL on CIFAR")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./Runs/dae_lowrank_mlp_stl")
    ...
    p.add_argument("--lowrank_lambda", type=float, default=1e-4)
