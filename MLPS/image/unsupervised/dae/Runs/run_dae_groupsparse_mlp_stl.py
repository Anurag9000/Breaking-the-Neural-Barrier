...
from ..Models.dae_groupsparse_mlp_stl import DAEGroupSparseMLP, dae_total_neurons
...
def group_sparsity(z: torch.Tensor, group_size: int) -> torch.Tensor:
    """
    Group sparsity penalty: L2 norm per group, averaged over groups and batch.
    """
    if group_size <= 0:
...
def train_one_epoch(
    ...
    for xb, _ in loader:
        ...
        g_pen = group_sparsity(z, group_size)
        loss = recon_loss + group_lambda * g_pen
        loss.backward()
        opt.step()
...
def eval_epoch(
    ...
    with torch.no_grad():
        for xb, _ in loader:
            ...
            xb_rec, z = model(xb_noisy)
            recon = mse(xb_rec, xb)
            g_pen = group_sparsity(z, group_size) * xb.size(0)
            total += float(recon.item()) + float(group_lambda * g_pen.item())
            n += xb.size(0)
    return total / max(n, 1)


def main() -> None:
    p = argparse.ArgumentParser(description="Group-sparse MLP DAE STL on CIFAR")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./Runs/dae_groupsparse_mlp_stl")
    ...
    p.add_argument("--group_lambda", type=float, default=1e-3)
