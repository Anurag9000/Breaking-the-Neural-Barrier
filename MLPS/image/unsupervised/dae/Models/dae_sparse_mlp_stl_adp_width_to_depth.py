...
from .dae_sparse_mlp_stl import DAESparseMLP, dae_total_neurons
...
def rebuild_model(model: DAESparseMLP, width: int, depth: int, device: torch.device) -> DAESparseMLP:
    new_model = DAESparseMLP(
...
def expand_width(model: DAESparseMLP, ex_k: int, max_width: int, device: torch.device) -> Optional[DAESparseMLP]:
...
def expand_depth(model: DAESparseMLP, max_depth: int, device: torch.device) -> Optional[DAESparseMLP]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAESparseMLP, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAESparseMLP:
    mdl = DAESparseMLP(
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            sparsity = z.abs().mean()
            loss = recon_loss + acfg.sparsity_lambda * sparsity
            loss.backward()
        ...
        with torch.no_grad():
            for xb, _ in dl_val:
                ...
                sparsity = z.abs().mean() * xb.size(0)
                val_total += float(recon.item()) + float(acfg.sparsity_lambda * sparsity.item())
                val_n += xb.size(0)
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAESparseMLP, int, int]:
    mode = acfg.adp_mode
    history: List[float] = []
    ...
    if log_loss:
        plot_loss_vs_epoch(history, results_dir / "loss_vs_epoch.png", title="DAESparseMLP")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAESparseMLP")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Sparse MLP DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_sparse_mlp")
    ...
    model = DAESparseMLP(in_channels=3, img_size=32, width=args.width, depth=args.depth).to(device)
    ...
    logger = ContinuousLogger(results_dir, "dae_sparse_mlp", args.adp_mode)
