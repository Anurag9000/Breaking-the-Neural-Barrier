...
def expand_width(model: DAESparseMLP, ex_k: int, max_width: int, device: torch.device) -> Optional[DAESparseMLP]:
    return expand_unsup_width(DAESparseMLP, model, ex_k, max_width, device)


def expand_depth(model: DAESparseMLP, max_depth: int, device: torch.device, min_new_layer_width: int = 10) -> Optional[DAESparseMLP]:
    return expand_unsup_depth(DAESparseMLP, model, max_depth, device, min_new_layer_width=min_new_layer_width)


def total_neurons(model: DAESparseMLP) -> int:
    return unsup_total_neurons(model)


def snapshot_arch_and_state(model: DAESparseMLP, state_dict=None) -> Dict[str, Any]:
    return snapshot_unsup_model(model, state_dict)


def restore_arch_and_state(model_or_snap, snap=None, device=None) -> DAESparseMLP:
    if snap is None:
        snap = model_or_snap
    return restore_unsup_model(DAESparseMLP, snap, device)
