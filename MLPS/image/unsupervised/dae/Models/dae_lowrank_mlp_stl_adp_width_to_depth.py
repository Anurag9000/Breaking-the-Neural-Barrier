...
from .dae_lowrank_mlp_stl import DAELowRankMLP, dae_total_neurons
...
def rebuild_model(model: DAELowRankMLP, width: int, depth: int, device: torch.device) -> DAELowRankMLP:
    new_model = DAELowRankMLP(
...
def expand_width(model: DAELowRankMLP, ex_k: int, max_width: int, device: torch.device) -> Optional[DAELowRankMLP]:
...
def expand_depth(model: DAELowRankMLP, max_depth: int, device: torch.device) -> Optional[DAELowRankMLP]:
...
def snapshot_arch_and_state(model: DAELowRankMLP, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAELowRankMLP:
    mdl = DAELowRankMLP(
...
def lowrank_penalty(model: DAELowRankMLP) -> torch.Tensor:
    total = 0.0
    for lin in model.encoder_linears():
        total = total + torch.linalg.matrix_norm(lin.weight, ord="nuc")
    return total
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            lr_pen = lowrank_penalty(model)
            loss = recon_loss + acfg.lowrank_lambda * lr_pen
            loss.backward()
        ...
        with torch.no_grad():
            for xb, _ in dl_val:
                ...
                lr_pen = lowrank_penalty(model) * xb.size(0)
                val_total += float(recon.item()) + float(acfg.lowrank_lambda * lr_pen.item())
                val_n += xb.size(0)
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAELowRankMLP, int, int]:
    mode = acfg.adp_mode
    history: List[float] = []
    ...
    if log_loss:
        plot_loss_vs_epoch(history, results_dir / "loss_vs_epoch.png", title="DAELowRankMLP")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAELowRankMLP")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Low-rank MLP DAE on CIFAR")
    ...
    p.add_argument("--lowrank-lambda", type=float, default=1e-4)

    p.add_argument("--results-dir", type=str, default="results_adp_dae_lowrank_mlp")
    ...
    model = DAELowRankMLP(in_channels=3, img_size=32, width=args.width, depth=args.depth).to(device)
    ...
    acfg = ADPConfig(
        ...
        lowrank_lambda=args.lowrank_lambda,
    )

    results_dir = Path(args.results_dir)
    logger = ContinuousLogger(results_dir, "dae_lowrank_mlp", args.adp_mode)
