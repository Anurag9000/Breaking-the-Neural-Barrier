...
from .dae_stacked_mlp_stl import DAEStackedMLP, dae_total_neurons
...
def rebuild_model(model: DAEStackedMLP, width: int, depth: int, device: torch.device) -> DAEStackedMLP:
    new_model = DAEStackedMLP(
...
def expand_width(model: DAEStackedMLP, ex_k: int, max_width: int, device: torch.device) -> Optional[DAEStackedMLP]:
...
def expand_depth(model: DAEStackedMLP, max_depth: int, device: torch.device) -> Optional[DAEStackedMLP]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAEStackedMLP, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEStackedMLP:
    mdl = DAEStackedMLP(
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAEStackedMLP, int, int]:
    mode = acfg.adp_mode
    history: List[float] = []
    ...
    if log_loss:
        plot_loss_vs_epoch(history, results_dir / "loss_vs_epoch.png", title="DAEStackedMLP")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEStackedMLP")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Stacked MLP DAE on CIFAR")
    ...
    p.add_argument("--ex-k", type=int, default=256)
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_stacked_mlp")
    ...
    logger = ContinuousLogger(results_dir, "dae_stacked_mlp", args.adp_mode)
