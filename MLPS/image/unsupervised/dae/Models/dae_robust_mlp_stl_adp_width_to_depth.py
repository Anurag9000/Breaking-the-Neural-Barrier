...
from .dae_robust_mlp_stl import DAERobustMLP, dae_total_neurons
...
def expand_width(model: DAERobustMLP, ex_k: int, max_width: int, device: torch.device) -> Optional[DAERobustMLP]:
    return expand_unsup_width(DAERobustMLP, model, ex_k, max_width, device)


def expand_depth(model: DAERobustMLP, max_depth: int, device: torch.device, min_new_layer_width: int = 10) -> Optional[DAERobustMLP]:
    return expand_unsup_depth(DAERobustMLP, model, max_depth, device, min_new_layer_width=min_new_layer_width)


def total_neurons(model: DAERobustMLP) -> int:
    return unsup_total_neurons(model)


def snapshot_arch_and_state(model: DAERobustMLP, state_dict=None) -> Dict[str, Any]:
    return snapshot_unsup_model(model, state_dict)


def restore_arch_and_state(model_or_snap, snap=None, device=None) -> DAERobustMLP:
    if snap is None:
        snap = model_or_snap
    return restore_unsup_model(DAERobustMLP, snap, device)
...
def train_with_early_stopping(
    ...
    huber = nn.SmoothL1Loss(beta=acfg.huber_delta)
    best_val = float("inf")
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        with torch.no_grad():
            huber_sum = nn.SmoothL1Loss(beta=acfg.huber_delta, reduction="sum")
            for xb, _ in dl_val:
                xb = xb.to(device, non_blocking=True)
                xb_noisy = add_gaussian_noise(xb, acfg.noise_std)
                xb_rec, _ = model(xb_noisy)
                val_total += float(huber_sum(xb_rec, xb).item())
                val_n += xb.size(0)
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAERobustMLP, int, int]:
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Robust (Huber) MLP DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_robust_mlp")
    ...
    model = DAERobustMLP(in_channels=3, img_size=32, width=args.width, depth=args.depth).to(device)
    ...
    logger = ContinuousLogger(results_dir, "dae_robust_mlp", args.adp_mode)
