    ...
    def optimize_width_at_fixed_depth(curr_model: Any) -> Tuple[Any, float, Dict[str, Any]]:
        ...
        while width_fail_limit is None or width_failure_count < width_fail_limit:
            ...
            if v < local_best_val - acfg.delta:
                ...
                if logger is not None:
                    logger.log_console(
                        f"[OPT][WIDTH] no_improve val_loss={v:.6f} widths={curr_model.hidden_widths} fail={width_failure_count}/{width_fail_limit if width_fail_limit is not None else 'inf'}"
                    )
    ...
    def optimize_depth_at_fixed_width(curr_model: Any) -> Tuple[Any, float, Dict[str, Any]]:
        ...
        while depth_fail_limit is None or depth_failure_count < depth_fail_limit:
            ...
            if v < local_best_val - acfg.delta:
                ...
                if logger is not None:
                    logger.log_console(
                        f"[OPT][DEPTH] no_improve val_loss={v:.6f} depth={len(curr_model.hidden_widths)} fail={depth_failure_count}/{depth_fail_limit if depth_fail_limit is not None else 'inf'}"
                    )
