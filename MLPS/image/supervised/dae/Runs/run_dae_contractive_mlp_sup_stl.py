...
from ..Models.dae_contractive_mlp_sup_stl import SupDAEContractiveMLP, sup_dae_total_neurons
...
def contractive_penalty(linears) -> torch.Tensor:
    """
    Simple contractive-style penalty: sum of squared Frobenius norms of encoder weights.
    """
    penalty = 0.0
    for lin in linears:
        penalty = penalty + lin.weight.pow(2).sum()
    return penalty
...
def eval_accuracy(model: SupDAEContractiveMLP, loader: DataLoader, device: torch.device, noise_std: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for xb, yb in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Supervised contractive MLP DAE encoder + classifier")
    # Data / IO
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./Runs/dae_contractive_mlp_sup")
    ...
    p.add_argument("--lambda_contractive", type=float, default=1e-3)
