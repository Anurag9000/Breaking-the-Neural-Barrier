...
from ..Models.dae_robust_mlp_sup_stl import SupDAERobustMLP, sup_dae_total_neurons
...
def eval_epoch(
    ...
    with torch.no_grad():
        for xb, yb in loader:
            ...
            loss_recon = huber(xb_rec, xb) / xb.size(0)
...
def eval_accuracy(model: SupDAERobustMLP, loader: DataLoader, device: torch.device, noise_std: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for xb, yb in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Supervised robust MLP DAE encoder + classifier")
    ...
    p.add_argument("--out_dir", type=str, default="runs/dae_robust_mlp_sup_cifar")
