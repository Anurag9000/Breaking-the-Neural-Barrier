...
from ..Models.dae_lowrank_mlp_sup_stl import SupDAELowRankMLP, sup_dae_total_neurons
...
def lowrank_penalty(model: SupDAELowRankMLP) -> torch.Tensor:
    total = 0.0
...
def train_one_epoch(
    ...
    for xb, yb in loader:
        ...
        loss = lambda_recon * loss_recon + loss_cls + lambda_lowrank * penalty
        loss.backward()
        opt.step()
...
def eval_accuracy(model: SupDAELowRankMLP, loader: DataLoader, device: torch.device, noise_std: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for xb, yb in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Supervised low-rank MLP DAE encoder + classifier")
    ...
    p.add_argument("--lambda_lowrank", type=float, default=1e-4)

    p.add_argument("--out_dir", type=str, default="runs/dae_lowrank_mlp_sup_cifar")
