...
from ..Models.dae_groupsparse_mlp_sup_stl import SupDAEGroupSparseMLP, sup_dae_total_neurons
...
def train_one_epoch(
    ...
    for xb, yb in loader:
        ...
        gs = group_sparsity(z, group_size)
        loss = lambda_recon * loss_recon + loss_cls + group_lambda * gs
        loss.backward()
        opt.step()
...
def eval_accuracy(model: SupDAEGroupSparseMLP, loader: DataLoader, device: torch.device, noise_std: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for xb, yb in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Supervised group-sparse MLP DAE encoder + classifier")
    ...
    p.add_argument("--out_dir", type=str, default="runs/dae_groupsparse_mlp_sup_cifar")
