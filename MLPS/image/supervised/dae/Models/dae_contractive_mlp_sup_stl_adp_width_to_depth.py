...
def expand_width(
    ...
    return expand_sup_width(SupDAEContractiveMLP, model, ex_k, max_width, device)
...
def expand_depth(
    ...
    return expand_sup_depth(SupDAEContractiveMLP, model, max_depth, device, min_new_layer_width=min_new_layer_width)
...
def restore_arch_and_state(model_or_snap, snap=None, device=None) -> SupDAEContractiveMLP:
    if snap is None:
        snap = model_or_snap
    return restore_sup_model(SupDAEContractiveMLP, snap, device)


def total_neurons(model: SupDAEContractiveMLP) -> int:
