...
def expand_width(
    ...
    return expand_sup_width(SupDAEGroupSparseMLP, model, ex_k, max_width, device)
...
def expand_depth(
    ...
    return expand_sup_depth(SupDAEGroupSparseMLP, model, max_depth, device, min_new_layer_width=min_new_layer_width)
...
def restore_arch_and_state(model_or_snap, snap=None, device=None) -> SupDAEGroupSparseMLP:
    if snap is None:
        snap = model_or_snap
    return restore_sup_model(SupDAEGroupSparseMLP, snap, device)


def total_neurons(model: SupDAEGroupSparseMLP) -> int:
