#!/usr/bin/env bash
# run_stl_massive_band_01_03_fresh_gpu_first.sh
# GPU-first dual-gate scheduler for parameter-decade band 10^1–10^3 (fresh run).
# GPU gate: fills VRAM first; reopens only on GPU job completion or >500 MiB VRAM drop.
# CPU gate: normal RAM logic; triggers once GPU is saturated.
# Shared queue, manifest, batch-scale, and hyperparams with pressure_aware counterpart.
set -euo pipefail
...
exec "$PYTHON_BIN" \
  ...
  --run-root MLPS/tabular/shared/dae_dnn/results/parammatched_decade_v1_fresh_v1_gpu_first_param_10pow01_03 \
