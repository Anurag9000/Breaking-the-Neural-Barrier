...
def adp_search(
    ...
    def can_widen(m: DAEGaussianConv) -> bool:
        return m.width < acfg.max_width and (
            dae_total_neurons(m.width + acfg.ex_k, m.depth) <= acfg.max_neurons
        )

    def can_deepen(m: DAEGaussianConv) -> bool:
        return m.depth < acfg.max_depth and (
            dae_total_neurons(m.width, m.depth + 1) <= acfg.max_neurons
        )
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="DAEGaussianConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEGaussianConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Gaussian Conv DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_gaussian_conv")
    ...
    logger = ContinuousLogger(results_dir, "dae_gaussian_conv", args.adp_mode)
