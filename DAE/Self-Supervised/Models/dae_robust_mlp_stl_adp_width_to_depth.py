...
def rebuild_model(model: DAERobustMLP, width: int, depth: int, device: torch.device) -> DAERobustMLP:
    new_model = DAERobustMLP(
...
def snapshot_arch_and_state(model: DAERobustMLP, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAERobustMLP:
    mdl = DAERobustMLP(
...
def adp_search(
    ...
    if log_loss:
        plot_loss_vs_epoch(history, results_dir / "loss_vs_epoch.png", title="DAERobustMLP")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAERobustMLP")
