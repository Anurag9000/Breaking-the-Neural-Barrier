...
class DAEGaussianConv(nn.Module):
    """
    Convolutional denoising autoencoder for CIFAR-like images.

    The architecture mirrors the undercomplete AE used elsewhere in the repo:
    a stack of Conv-BN-ReLU blocks in the encoder with optional pooling,
    and a symmetric decoder with ConvTranspose2d blocks.

    Noise is not applied here; training code is expected to add Gaussian noise
    to the input and use MSE loss between the reconstruction and the clean
    target.
    """
