...
from .dae_saltpepper_conv_stl import DAESaltPepperConv, dae_total_neurons
...
def rebuild_model(model: DAESaltPepperConv, width: int, depth: int, device: torch.device) -> DAESaltPepperConv:
    new_model = DAESaltPepperConv(
...
def expand_width(model: DAESaltPepperConv, ex_k: int, max_width: int, device: torch.device) -> Optional[DAESaltPepperConv]:
...
def expand_depth(model: DAESaltPepperConv, max_depth: int, device: torch.device) -> Optional[DAESaltPepperConv]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAESaltPepperConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAESaltPepperConv:
    mdl = DAESaltPepperConv(
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAESaltPepperConv, int, int]:
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="DAESaltPepperConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAESaltPepperConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Salt-and-pepper Conv DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_saltpepper_conv")
    ...
    model = DAESaltPepperConv(in_channels=3, width=args.width, depth=args.depth, pool_after=pool_after).to(device)
    ...
    logger = ContinuousLogger(results_dir, "dae_saltpepper_conv", args.adp_mode)
