...
def rebuild_model(model: DAEVAEConv, width: int, depth: int, device: torch.device) -> DAEVAEConv:
    new_model = DAEVAEConv(
...
def expand_width(model: DAEVAEConv, ex_k: int, max_width: int, device: torch.device) -> Optional[DAEVAEConv]:
...
def expand_depth(model: DAEVAEConv, max_depth: int, device: torch.device) -> Optional[DAEVAEConv]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAEVAEConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEVAEConv:
    mdl = DAEVAEConv(
...
def adp_search(
...
) -> Tuple[float, DAEVAEConv, int, int]:
    ...
    def can_widen(m: DAEVAEConv) -> bool:
        return (
            dae_vae_total_neurons(m.width + acfg.ex_k, m.depth, m.latent_dim) <= acfg.max_neurons
            and m.width < acfg.max_width
        )

    def can_deepen(m: DAEVAEConv) -> bool:
        return (
            dae_vae_total_neurons(m.width, m.depth + 1, m.latent_dim) <= acfg.max_neurons
    ...
    def optimize_width_at_fixed_depth(
        ...
        while fail < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                fail = 0
                improvements.append((dae_vae_total_neurons(wider.width, wider.depth, wider.latent_dim), v))
    ...
    def optimize_depth_at_fixed_width(
        ...
        while fail < acfg.trials_depth:
            ...
            if v < local_best_val - acfg.delta:
                ...
                fail = 0
                improvements.append((dae_vae_total_neurons(deeper.width, deeper.depth, deeper.latent_dim), v))
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="DAEVAEConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEVAEConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Variational Conv DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_vae_conv")
    ...
    logger = ContinuousLogger(results_dir, "dae_vae_conv", args.adp_mode)
