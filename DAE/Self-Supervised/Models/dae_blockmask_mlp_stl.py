...
class DAEBlockMaskMLP(nn.Module):
    """
    Fully-connected denoising autoencoder with structured block masking
    noise on CIFAR-like flattened inputs.

    The encoder is a stack of Linear-ReLU layers of width `width` and depth
    `depth`. The decoder mirrors the encoder and reconstructs back to the
    flattened image, which is reshaped to (C,H,W) at the output.
    """
...
__all__ = ["DAEBlockMaskMLP", "dae_total_neurons"]
