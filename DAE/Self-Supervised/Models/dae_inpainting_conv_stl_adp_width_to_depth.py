...
from .dae_inpainting_conv_stl import DAEInpaintingConv, dae_total_neurons
...
def rebuild_model(model: DAEInpaintingConv, width: int, depth: int, device: torch.device) -> DAEInpaintingConv:
    new_model = DAEInpaintingConv(
...
def expand_width(
    model: DAEInpaintingConv,
    ex_k: int,
    max_width: int,
    device: torch.device,
) -> Optional[DAEInpaintingConv]:
...
def snapshot_arch_and_state(model: DAEInpaintingConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEInpaintingConv:
    mdl = DAEInpaintingConv(
...
def apply_center_mask(x: torch.Tensor, mask_size: int) -> torch.Tensor:
    if mask_size <= 0:
        return x
    _, _, h, w = x.shape
    ms = min(mask_size, h, w)
    y0 = (h - ms) // 2
    x0 = (w - ms) // 2
    x[:, :, y0 : y0 + ms, x0 : x0 + ms] = 0.0
    return x
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            xb_corrupt = xb.clone()
            xb_corrupt = apply_center_mask(xb_corrupt, acfg.mask_size)

            opt.zero_grad(set_to_none=True)
            xb_rec, _ = model(xb_corrupt)
            loss = mse(xb_rec, xb)
            loss.backward()
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAEInpaintingConv, int, int]:
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_inpainting_conv_cifar")
    ...
    p.add_argument("--mask-size", type=int, default=16)
    ...
    logger = ContinuousLogger(
        ...
        console_prefix="[ADP DAE Inpaint]",
    )
    ...
    base_model = DAEInpaintingConv(in_channels=3, width=args.width, depth=args.depth).to(device)
    logger.log_console(
