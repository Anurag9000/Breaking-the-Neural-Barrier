...
from .dae_tcn_seq_stl import DAETCNSeq, tcn_total_neurons
...
def rebuild_model(model: DAETCNSeq, width: int, depth: int, device: torch.device) -> DAETCNSeq:
    new_model = DAETCNSeq(
...
def snapshot_arch_and_state(model: DAETCNSeq, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAETCNSeq:
    mdl = DAETCNSeq(
        in_channels=snap.get("in_channels", 1),
        width=snap["width"],
        depth=snap["depth"],
    ).to(device)
...
def make_loaders(
    ...
    return dl_train, dl_val, in_channels
...
def adp_search(
    ...
    improvements.append((tcn_total_neurons(best_w, best_d), global_best_val))
    ...
    def optimize_width_at_fixed_depth(
        ...
        while fail < acfg.trials_width:
            ...
            if wider is None or tcn_total_neurons(wider.width, wider.depth) > acfg.max_neurons:
                break
            logger.log_console(
                f"[WIDTH OPT] Trying width={wider.width}, depth={wider.depth}, "
                f"neurons={tcn_total_neurons(wider.width, wider.depth)}"
            )
            ...
            if val + acfg.delta < current_best_val:
                ...
                improvements.append((tcn_total_neurons(wider.width, wider.depth), val))
                logger.log_console(
    ...
    def optimize_depth_at_fixed_width(
        ...
        while fail < acfg.trials_depth:
            ...
            if deeper is None or tcn_total_neurons(deeper.width, deeper.depth) > acfg.max_neurons:
                break
            logger.log_console(
                f"[DEPTH OPT] Trying width={deeper.width}, depth={deeper.depth}, "
                f"neurons={tcn_total_neurons(deeper.width, deeper.depth)}"
            )
            val, state = train_with_early_stopping(
            ...
            if val + acfg.delta < current_best_val:
                ...
                improvements.append((tcn_total_neurons(deeper.width, deeper.depth), val))
                logger.log_console(
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_tcn_seq")
    ...
    p.add_argument("--max-neurons", type=int, default=5000000)
    ...
    logger = ContinuousLogger(
        ...
        console_prefix="[ADP DAE TCN]",
    )
    logger.log_console(f"Initialized ADP search (mode={args.adp_mode})")

    dl_train, dl_val, in_channels = make_loaders(
    ...
    base_model = DAETCNSeq(in_channels=in_channels, width=args.width, depth=args.depth).to(device)
    logger.log_console(
        f"Base model: width={base_model.width}, depth={base_model.depth}, "
        f"neurons={tcn_total_neurons(base_model.width, base_model.depth)}"
    )
    ...
    logger.log_console(
        ...
        f"neurons={tcn_total_neurons(best_w, best_d)}"
    )
