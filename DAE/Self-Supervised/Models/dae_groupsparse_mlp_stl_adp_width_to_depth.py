...
from .dae_groupsparse_mlp_stl import DAEGroupSparseMLP, dae_total_neurons
...
def rebuild_model(model: DAEGroupSparseMLP, width: int, depth: int, device: torch.device) -> DAEGroupSparseMLP:
    new_model = DAEGroupSparseMLP(
...
def expand_width(model: DAEGroupSparseMLP, ex_k: int, max_width: int, device: torch.device) -> Optional[DAEGroupSparseMLP]:
...
def expand_depth(model: DAEGroupSparseMLP, max_depth: int, device: torch.device) -> Optional[DAEGroupSparseMLP]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAEGroupSparseMLP, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEGroupSparseMLP:
    mdl = DAEGroupSparseMLP(
...
def group_sparsity(z: torch.Tensor, group_size: int) -> torch.Tensor:
    if group_size <= 0:
        return z.new_tensor(0.0)
    b, d = z.shape
    g = max(1, d // group_size)
    trimmed = z[:, : g * group_size]
    grouped = trimmed.view(b, g, group_size)
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            g_pen = group_sparsity(z, acfg.group_size)
            loss = recon_loss + acfg.group_lambda * g_pen
            loss.backward()
        ...
        with torch.no_grad():
            for xb, _ in dl_val:
                ...
                g_pen = group_sparsity(z, acfg.group_size) * xb.size(0)
                val_total += float(recon.item()) + float(acfg.group_lambda * g_pen.item())
                val_n += xb.size(0)
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAEGroupSparseMLP, int, int]:
    mode = acfg.adp_mode
    history: List[float] = []
    ...
    if log_loss:
        plot_loss_vs_epoch(history, results_dir / "loss_vs_epoch.png", title="DAEGroupSparseMLP")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEGroupSparseMLP")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Group-sparse MLP DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_groupsparse_mlp")
    ...
    model = DAEGroupSparseMLP(in_channels=3, img_size=32, width=args.width, depth=args.depth).to(device)
    ...
    logger = ContinuousLogger(results_dir, "dae_groupsparse_mlp", args.adp_mode)
