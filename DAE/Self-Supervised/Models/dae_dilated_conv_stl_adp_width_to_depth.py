...
from .dae_dilated_conv_stl import DAEDilatedConv, dae_total_neurons
...
def rebuild_model(model: DAEDilatedConv, width: int, depth: int, device: torch.device) -> DAEDilatedConv:
    new_model = DAEDilatedConv(
...
def expand_width(
    model: DAEDilatedConv,
    ex_k: int,
    max_width: int,
    device: torch.device,
) -> Optional[DAEDilatedConv]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEDilatedConv:
    mdl = DAEDilatedConv(
        in_channels=snap.get("in_channels", 3),
        width=snap["width"],
        depth=snap["depth"],
        dilation=snap.get("dilation", 2),
        pool_after=snap.get("pool_after", []),
    ).to(device)
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAEDilatedConv, int, int]:
    mode = acfg.adp_mode
    history: List[float] = []
    ...
    if log_loss:
        plot_loss_vs_epoch(history, results_dir / "loss_vs_epoch.png", title="DAEDilatedConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEDilatedConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Dilated Conv DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_dilated_conv")
    ...
    logger = ContinuousLogger(results_dir, "dae_dilated_conv", args.adp_mode)
