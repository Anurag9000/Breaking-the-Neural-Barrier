...
from .dae_masking_conv_stl import DAEMaskingConv, dae_total_neurons
...
def rebuild_model(model: DAEMaskingConv, width: int, depth: int, device: torch.device) -> DAEMaskingConv:
    new_model = DAEMaskingConv(
...
def expand_width(model: DAEMaskingConv, ex_k: int, max_width: int, device: torch.device) -> Optional[DAEMaskingConv]:
...
def expand_depth(model: DAEMaskingConv, max_depth: int, device: torch.device) -> Optional[DAEMaskingConv]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAEMaskingConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEMaskingConv:
    mdl = DAEMaskingConv(
...
def apply_mask_noise(x: torch.Tensor, mask_prob: float) -> torch.Tensor:
    if mask_prob <= 0.0:
        return x
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            xb_noisy = apply_mask_noise(xb, acfg.mask_prob)
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAEMaskingConv, int, int]:
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="DAEMaskingConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEMaskingConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Masking Conv DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_masking_conv")
    ...
    model = DAEMaskingConv(in_channels=3, width=args.width, depth=args.depth, pool_after=pool_after).to(device)
    ...
    logger = ContinuousLogger(results_dir, "dae_masking_conv", args.adp_mode)
