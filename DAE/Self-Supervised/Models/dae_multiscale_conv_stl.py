...
class DAEMultiScaleConv(nn.Module):
    """
    Multi-scale convolutional denoising autoencoder for CIFAR-like images.

    This mirrors the plain conv DAE but enforces pooling at multiple depths to
    expose a small multi-scale hierarchy. The `pool_after` indices are derived
    automatically from the depth when not provided.
    """
    ...
    def __init__(
        ...
        if pool_after is None:
            # Default: pool after every layer except the last to build a
            # small Laplacian / multi-scale pyramid.
            pool_after = list(range(1, depth))
