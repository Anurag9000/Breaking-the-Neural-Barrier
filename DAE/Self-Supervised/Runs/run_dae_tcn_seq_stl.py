...
from ..Models.dae_tcn_seq_stl import DAETCNSeq, tcn_total_neurons


class SequenceDataset(Dataset):
    """
    Wrapper around a tensor of shape (N, C, L) or (N, L).
    """
...
def load_sequences(path: Path) -> torch.Tensor:
    ...
    Expected formats:
      - plain tensor (N, C, L) or (N, L)
      - dict with key 'data' containing the tensor.
    """
    obj = torch.load(path)
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_dae_tcn_seq")
    ...
    neurons_metric = tcn_total_neurons(args.width, args.depth)
    best_val = float("inf")
    best_epoch = -1
    es_counter = 0
