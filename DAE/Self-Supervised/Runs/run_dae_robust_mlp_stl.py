...
from ..Models.dae_robust_mlp_stl import DAERobustMLP, dae_total_neurons
...
def eval_epoch(
    ...
    model.eval()
    huber = nn.SmoothL1Loss(beta=huber_delta, reduction="sum")
    total, n = 0.0, 0
    with torch.no_grad():
        for xb, _ in loader:
            xb = xb.to(device, non_blocking=True)
            xb_noisy = add_gaussian_noise(xb, noise_std)
            xb_rec, _ = model(xb_noisy)
            total += float(huber(xb_rec, xb).item())
            n += xb.size(0)
    return total / max(n, 1)


def main() -> None:
    p = argparse.ArgumentParser(description="Robust (Huber) MLP DAE STL on CIFAR")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./runs/dae_robust_mlp_stl")
