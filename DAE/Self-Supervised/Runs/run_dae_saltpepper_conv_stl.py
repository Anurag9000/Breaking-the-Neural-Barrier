...
from ..Models.dae_saltpepper_conv_stl import DAESaltPepperConv, dae_total_neurons
...
def add_salt_pepper_noise(x: torch.Tensor, prob: float, salt_val: float = 1.0, pepper_val: float = -1.0) -> torch.Tensor:
    if prob <= 0.0:
        return x
    # Create a single-channel mask and broadcast across channels
...
def train_one_epoch(
    ...
    for xb, _ in loader:
        ...
        xb_noisy = add_salt_pepper_noise(xb, sp_prob)
...
def main() -> None:
    ...
    p.add_argument("--sp_prob", type=float, default=0.1)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            train_loss = train_one_epoch(model, train_loader, opt, device, sp_prob=args.sp_prob)
            val_loss = eval_epoch(model, val_loader, device, sp_prob=args.sp_prob)
