...
from .dae_groupsparse_mlp_sup_stl import SupDAEGroupSparseMLP, sup_dae_total_neurons
from ..Runs.run_dae_groupsparse_mlp_sup_stl import (  # type: ignore
    build_dataloaders,
    add_gaussian_noise,
    group_sparsity,
)
...
def rebuild_model(
    model: SupDAEGroupSparseMLP,
    num_classes: int,
    width: int,
    depth: int,
    device: torch.device,
) -> SupDAEGroupSparseMLP:
    new_model = SupDAEGroupSparseMLP(
...
def snapshot_arch_and_state(model: SupDAEGroupSparseMLP, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> SupDAEGroupSparseMLP:
    mdl = SupDAEGroupSparseMLP(
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, yb in dl_train:
            ...
            gs = group_sparsity(z, acfg.group_size)
            loss = acfg.lambda_recon * loss_recon + loss_cls + acfg.group_lambda * gs
            loss.backward()
...
def adp_search(
...
) -> Tuple[float, SupDAEGroupSparseMLP, int, int]:
    ...
    def optimize_width_at_fixed_depth(model_curr: SupDAEGroupSparseMLP) -> Tuple[float, Dict[str, torch.Tensor], int]:
        """
    ...
    def optimize_depth_at_fixed_width(model_curr: SupDAEGroupSparseMLP) -> Tuple[float, Dict[str, torch.Tensor], int]:
        """
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="SupDAEGroupSparseMLP")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="SupDAEGroupSparseMLP")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP supervised group-sparse MLP DAE encoder + classifier")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_groupsparse_mlp_sup")
    ...
    logger = ContinuousLogger(results_dir, "sup_dae_groupsparse_mlp", args.adp_mode)
