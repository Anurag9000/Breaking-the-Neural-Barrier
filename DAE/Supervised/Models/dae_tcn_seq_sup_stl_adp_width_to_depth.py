...
def snapshot_arch_and_state(model: SupDAETCNSeq, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> SupDAETCNSeq:
    mdl = SupDAETCNSeq(
...
def adp_search(
...
) -> Tuple[float, SupDAETCNSeq, int, int]:
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="SupDAETCNSeq")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="SupDAETCNSeq")
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_tcn_seq_sup")
    ...
    logger = ContinuousLogger(results_dir, "sup_dae_tcn_seq", args.adp_mode)
