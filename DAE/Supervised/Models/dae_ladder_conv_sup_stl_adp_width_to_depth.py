...
from .dae_ladder_conv_sup_stl import SupDAELadderConv, sup_dae_ladder_total_neurons
from ..Runs.run_dae_ladder_conv_sup_stl import (  # type: ignore
    build_dataloaders,
    add_gaussian_noise,
    kl_div,
)
...
def expand_width(
    ...
    latent_dim: int,
    device: torch.device,
) -> Optional[SupDAELadderConv]:
    new_w = min(max_width, model.width + ex_k)
    if new_w == model.width:
        return None
    return rebuild_model(model, num_classes, new_w, model.depth, latent_dim, device)
...
def expand_depth(
    ...
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, num_classes, model.width, model.depth + 1, latent_dim, device)


def snapshot_arch_and_state(model: SupDAELadderConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> SupDAELadderConv:
    mdl = SupDAELadderConv(
        num_classes=snap["num_classes"],
        in_channels=snap.get("in_channels", 3),
        img_size=snap.get("img_size", 32),
        width=snap["width"],
        depth=snap["depth"],
        latent_dim=snap.get("latent_dim", 128),
    ).to(device)
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, yb in dl_train:
            ...
            x_rec, logits, mu = model(xb_noisy)
            _, mu_back, logvar_back = model.dae(xb_noisy)  # type: ignore[attr-defined]

            loss_recon = mse(x_rec, xb)
            loss_cls = ce(logits, yb)
            loss_kl = kl_div(mu_back, logvar_back)
            loss = acfg.lambda_recon * loss_recon + loss_cls + acfg.lambda_kl * loss_kl
            loss.backward()
...
def adp_search(
...
) -> Tuple[float, SupDAELadderConv, int, int]:
    ...
    def can_widen(m: SupDAELadderConv) -> bool:
        return (
            sup_dae_ladder_total_neurons(m.width + acfg.ex_k, m.depth, acfg.latent_dim, num_classes) <= acfg.max_neurons
            and m.width < acfg.max_width
        )

    def can_deepen(m: SupDAELadderConv) -> bool:
        return (
            sup_dae_ladder_total_neurons(m.width, m.depth + 1, acfg.latent_dim, num_classes) <= acfg.max_neurons
    ...
    def optimize_width_at_fixed_depth(
        ...
        while fail < acfg.trials_width:
            ...
            if not can_widen(curr):
                break
            wider = expand_width(curr, num_classes, acfg.ex_k, acfg.max_width, acfg.latent_dim, device)
            if wider is None:
                break
            ...
            if val_loss < local_best_val - acfg.delta:
                ...
                fail = 0
                neurons = sup_dae_ladder_total_neurons(wider.width, wider.depth, acfg.latent_dim, num_classes)
    ...
    def optimize_depth_at_fixed_width(
        ...
        while fail < acfg.trials_depth:
            ...
            if not can_deepen(curr):
                break
            deeper = expand_depth(curr, num_classes, acfg.max_depth, acfg.latent_dim, device)
            if deeper is None:
                break
            ...
            if val_loss < local_best_val - acfg.delta:
                ...
                fail = 0
                neurons = sup_dae_ladder_total_neurons(deeper.width, deeper.depth, acfg.latent_dim, num_classes)
                improvements.append((neurons, val_loss))
            else:
                fail += 1
        return local_best_snap, local_best_val
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="SupDAELadderConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="SupDAELadderConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP supervised ladder conv DAE encoder + classifier")
    ...
    p.add_argument("--lambda-kl", type=float, default=1e-3)

    p.add_argument("--results-dir", type=str, default="results_adp_dae_ladder_conv_sup")
    ...
    logger = ContinuousLogger(results_dir, "sup_dae_ladder_conv", args.adp_mode)
