...
from .dae_masking_conv_sup_stl import SupDAEMaskingConv, sup_dae_total_neurons
...
def apply_mask_noise(x: torch.Tensor, mask_prob: float) -> torch.Tensor:
    """
    Simple masking noise: randomly zero out a fraction of pixels.
    """
    if mask_prob <= 0.0:
        return x
...
def rebuild_model(
    model: SupDAEMaskingConv,
    num_classes: int,
    width: int,
    depth: int,
    device: torch.device,
) -> SupDAEMaskingConv:
    new_model = SupDAEMaskingConv(
...
def snapshot_arch_and_state(model: SupDAEMaskingConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> SupDAEMaskingConv:
    mdl = SupDAEMaskingConv(
...
def adp_search(
...
) -> Tuple[float, SupDAEMaskingConv, int, int]:
    ...
    def optimize_width_at_fixed_depth(model_curr: SupDAEMaskingConv) -> Tuple[float, Dict[str, torch.Tensor], int]:
        """
    ...
    def optimize_depth_at_fixed_width(model_curr: SupDAEMaskingConv) -> Tuple[float, Dict[str, torch.Tensor], int]:
        """
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="SupDAEMaskingConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="SupDAEMaskingConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP supervised Masking Conv DAE encoder + classifier")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_masking_conv_sup")
    ...
    logger = ContinuousLogger(results_dir, "sup_dae_masking_conv", args.adp_mode)
