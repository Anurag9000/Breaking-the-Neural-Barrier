...
def eval_accuracy(model: SupDAEGaussianConv, loader: DataLoader, device: torch.device, noise_std: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for xb, yb in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Supervised Gaussian Conv DAE encoder + classifier head on CIFAR")
    # Data / IO
