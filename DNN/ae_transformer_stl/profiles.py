"""Profile registry for transformer AE variant."""
from __future__ import annotations

from functools import partial
from typing import Callable, Dict

from Autoencoder.ae_transformer_stl.Models.variant_impl import AETransformerModel


def _builder(meta, args, **overrides):
    cfg = {
        "latent_dim": getattr(args, "latent_dim", 128),
        "tx_dim": getattr(args, "tx_dim", 256),
        "tx_layers": getattr(args, "tx_layers", 4),
        "tx_heads": getattr(args, "tx_heads", 8),
        "hidden": getattr(args, "hidden", 256),
        "proj_newton_steps": getattr(args, "proj_newton_steps", 1),
    }
    cfg.update(overrides)
    return AETransformerModel(meta, **cfg)
