...
def _resize_linear(old: nn.Linear, new_out: int, new_in: int | None = None):
    """Create a *new* Linear(new_in, new_out) layer and copy overlapping weights/biases from *old*."""
    if new_in is None:
...
def expand_width(model: "ADPDepth", inc: int):
...
def expand_depth(model: "ADPDepth") -> None:
    """
    ...
    # ensure head takes the same width d
...
def train_early_stop(
    ...
    *   `max_epochs` is a **global budget** across all calls; the function
    ...
    Returns the best validation loss achieved (restores best weights).
    """
    ...
    while model.global_epoch < max_epochs:
        ...
        # early-stopping bookkeeping (use delta for plateau behavior)
        ...
        logger.info(
            f"Epoch {model.global_epoch:5d} | val_loss={val:.6f} | width={curr_width} | depth={curr_depth} | patience_left={counter}"
        )
...
# ════════════════════════════════════════════════════════════════════════
#  ADP‑Depth: adaptive depth‑&‑width expanding network with expansion patience
# ════════════════════════════════════════════════════════════════════════

class ADPDepth(DEN):
    """Adaptive network that first expands *depth* (with patience), then tries
    width sweeps for the current depth (with its own patience). Stops when neither helps.
    """
    ...
    def __init__(self, config):
        ...
        # failure counters (dimension-specific)
    ...
    def fit_task(
        ...
        *,
        ...
        Δ        = self.delta if delta is None else (0.0 if delta is None else float(delta))
        P        = int(self.patience)
        inc      = int(self.ex_k)
        best_val = float("inf")
        ...
        # ── OUTER LOOP: series of depth attempts, each followed by width sweeps ──
        while not stop_outer:
            # depth guard (before attempting any new depth)
            if len(self.hidden_layers) >= self.max_depth:
                logger.info("Reached max_depth — no further depth expansion.")
                break

            # Establish the *pre-expansion* baseline for the upcoming depth series
            pre_depth_snapshot = copy.deepcopy(self)
            pre_depth_val      = best_val

            # ── DEPTH SERIES with patience ─────────────────────────────────────
            while True:
                # Capacity guard: depth would add one more d×d layer; cap only by max_depth above
                expand_depth(self)
                ...
                if d_val < pre_depth_val - Δ:
                    # accepted depth expansion → reset counter, update global best & baseline
                    self._depth_failures = 0
                    best_val   = d_val
                    best_model = copy.deepcopy(self)
                    # after an accepted depth, we proceed to width sweeps at this depth
                    break
                else:
                    # failure → increment and decide retry vs rollback
                    self._depth_failures += 1
                    k = self._depth_failures
                    N = int(self.trials_depth)
                    if k < N:
                        logger.info(f"No val improvement after depth expansion (trial {k}/{N}); trying another depth expansion.")
                        # keep the added layer and immediately try another depth expansion
                        continue
                    else:
                        logger.info(f"Depth expansions without improvement reached {N}; rolling back and stopping depth.")
                        # rollback to the baseline before this depth series and stop training altogether (depth-first policy)
            ...
            # Baseline for this width series
            ...
            while True:
                ...
                if w_val < pre_width_val - Δ:
                    ...
                    # try another width step within capacity
                    continue
                else:
                    ...
                    if k < N:
                        logger.info(f"No val improvement after width expansion (trial {k}/{N}); trying another width expansion.")
                        # keep the widened net and try another width increment
                        continue
                    else:
                        logger.info(f"Width expansions without improvement reached {N}; rolling back widths and stopping width series.")
                        # rollback to baseline before this width series and stop widths
                        self._restore(pre_width_snapshot)
                        break

            # After finishing width series (either capacity, rollback, or no more gains),
            # loop back to attempt another depth series (subject to max_depth)

        # ── final restore & return ───────────────────────────────────────────
        self._restore(best_model)
        ADPDepth.global_epoch = self.global_epoch  # persist for next task
        return best_val
