...
def expand_width(model: "ADPWidth_2Head", inc: int) -> None:
    """
...
def expand_depth(model: "ADPWidth_2Head") -> None:
    """
...
class ADPWidth_2Head(ADPBase_2Head):
    ...
    def __init__(self, config):
        ...
        # collapsed branch list
        self.branches = ['pg_qg', 'va_vm']

        # new: patience knobs (dict-style with defaults) & counters
        ...
        if all(hasattr(config, k) for k in ('bounds_low','bounds_high','mask')):
            self.bound_layer = BoundedAct(config.bounds_low,
                                          config.bounds_high,
                                          config.mask)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pipeline:
            shared stack → branch-specific fc2 → heads → bounded activation
        Returns concatenated [Pg+Qg, Va+Vm] outputs.
        """
    ...
    def fit_task(
        ...
        *pre-expansion baseline* by at least `delta` (treat `None` as `0.0`).
        ...
        ADPWidth_2Head.global_epoch = self.global_epoch
        return best_V
