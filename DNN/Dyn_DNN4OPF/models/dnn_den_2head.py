...
class DEN_2Heads(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.register_buffer("loss_thr", torch.tensor(0.0, dtype=torch.float32, device=self.device))
        self.register_buffer("spl_thr", torch.tensor(1e-2, dtype=torch.float32, device=self.device))
        ...
        if use_bounds:
            bl = torch.as_tensor(bounds_low, dtype=torch.float32, device=self.device)
            bh = torch.as_tensor(bounds_high, dtype=torch.float32, device=self.device)
