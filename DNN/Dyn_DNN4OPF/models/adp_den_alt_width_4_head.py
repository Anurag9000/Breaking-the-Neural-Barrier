"""
ADP-DEN Alternating (Width→Depth) — 4‑Head
Shared fc1 → 4 branch stacks → 4 heads; concatenated output.
Acceptance: PHYSICS‑FIRST (requires physics_metric). Width proposal first.
"""
...
# -------------------------------
# Controller (Alt‑Width, Physics‑first)
# -------------------------------
class ADP_ALT_WIDTH_4Head:
    ...
    def fit(self, train_loader: Iterable, val_loader: Iterable,
        ...
        assert physics_metric is not None, "4‑Head alt‑width expects a physics_metric for physics‑first acceptance."
        tot = 0
        ...
        while tot < max_global_epochs and not (fw >= self.pw and fd >= self.pd):
            # WIDTH first — physics‑first accept
            if self.model.width >= self.max_neurons:
                fw = self.pw
            else:
                pre = self.model.snapshot(); pp = base_p
