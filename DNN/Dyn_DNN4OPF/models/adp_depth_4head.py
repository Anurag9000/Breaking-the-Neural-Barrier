...
def expand_width(model: "ADPDepth_4Head", inc: int) -> None:
    """
    Widen the network by **inc** neurons:

        • every shared hidden layer in `model.hidden_layers`
        • every branch-specific layer  pg_fc2 | qg_fc2 | va_fc2 | vm_fc2
...
def expand_depth(model: "ADPDepth_4Head") -> None:
    """
...
# ════════════════════════════════════════════════════════════════════════
#  ADP-Depth: adaptive depth-&-width expanding network (+ expansion patience)
# ════════════════════════════════════════════════════════════════════════

class ADPDepth_4Head(ADPBase_4Head):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Masks / timestamps are applied inside `self._gate`.  All tensors stay
        on the model’s device.
        """
        x = x.to(self.device, non_blocking=True)

        # ── 1) shared hidden stack ───────────────────────────────────────
        h = x
        for layer in self.hidden_layers:           # includes newly inserted layers
            h = F.relu(layer(h))

        # ── 2) branch-specific second layer + head per task ──────────────
        outs = []
        for br in self.branches:
            ...
            h2 = self._gate(h2, ts2, m2)           # mask & timestamp gating
    ...
    def fit_task(
        ...
        *,
        ...
        ADPDepth_4Head.global_epoch = self.global_epoch
        return best_V
