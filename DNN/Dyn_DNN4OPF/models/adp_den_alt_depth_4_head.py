"""
ADP-DEN Alternating (Depth→Width) — 4‑Head
Shared fc1 → 4 branch stacks → 4 heads; concatenated output.
Acceptance: PHYSICS‑FIRST (requires physics_metric). Depth proposal first.
"""
...
# -------------------------------
# Controller (Alt‑Depth, Physics‑first)
# -------------------------------
class ADP_ALT_DEPTH_4Head:
    ...
    def fit(self, train_loader: Iterable, val_loader: Iterable,
        ...
        assert physics_metric is not None, "4‑Head alt‑depth expects a physics_metric for physics‑first acceptance."
        tot = 0
        ...
        while tot < max_global_epochs and not (fw >= self.pw and fd >= self.pd):
            # DEPTH first — physics‑first accept
            ...
            # WIDTH next — physics‑first accept
