...
class DNN_Progressive2Head(nn.Module):
    """Progressive Neural Net: **two static columns** (Pg_Qg, Va_Vm)."""
    ...
    def __init__(
        ...
        *,
        ...
        # Derive per-task output dims for two combined heads
        total_out = len(bounds_low)
        n_bus = input_dim // 2
        n_gen = (total_out - 2 * n_bus) // 2
        assert total_out == 2 * n_gen + 2 * n_bus, "Inconsistent bound lengths"
        # head0: Pg+Qg (2*n_gen), head1: Va+Vm (2*n_bus)
        self.output_dims = [2 * n_gen, 2 * n_bus]
        ...
        # Build columns & adapters as before
        self.columns = nn.ModuleList()
        self.adapters = []

        for col_id, odim in enumerate(self.output_dims):
            # slice indices still use helper _slice
            ...
            if col_id == 0:
                ...
                ad1 = nn.ModuleList([
                    AlphaAdapter(hidden_dim).to(device) for _ in range(col_id)
                ])
                ad2 = nn.ModuleList([
                    AlphaAdapter(hidden_dim).to(device) for _ in range(col_id)
                ])
                self.adapters.append((ad1, ad2))
                # Freeze previous columns
    ...
    def forward(self, x: torch.Tensor, task_id: int) -> torch.Tensor:
        """Forward pass for *task_id* using lateral adapters (2 heads)."""
        if not (0 <= task_id < len(self.HEADS)):
            raise IndexError(
                f"task_id must be 0≤id<{len(self.HEADS)} ({', '.join(self.HEADS)})"
            )
    ...
    def get_all_shared_weights(self) -> List[nn.Parameter]:  # noqa: D401
        ...
        for ad_pair in self.adapters:
            if ad_pair[0] is not None:
                weights.extend(p for p in itertools.chain(ad_pair[0].parameters(), ad_pair[1].parameters()) if p.requires_grad)
        return weights
