...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    # build two-head mask: first 2*n_gen, then 2*n_bus
    mask_full = default_mask(n_gen, n_bus)
    config["mask"] = [
        torch.tensor(mask_full[:2*n_gen],      dtype=torch.bool, device=device),
        torch.tensor(mask_full[2*n_gen:],      dtype=torch.bool, device=device),
    ]
    ...
    model = DNN_MTL_2HEAD(
        ...
        bounds_low=[bounds_low[:2*n_gen], bounds_low[2*n_gen:]],
        bounds_high=[bounds_high[:2*n_gen], bounds_high[2*n_gen:]],
    ...
    pg_qg_tr = Y_tr[:, :2*n_gen]
    va_vm_tr = Y_tr[:, 2*n_gen:]
    pg_qg_va = Y_va[:, :2*n_gen]
    va_vm_va = Y_va[:, 2*n_gen:]
    ...
    train_loaders = [
        DataLoader(TensorDataset(X_tr, pg_qg_tr), batch_size=batch, shuffle=True,  pin_memory=True),
        DataLoader(TensorDataset(X_tr, va_vm_tr), batch_size=batch, shuffle=True,  pin_memory=True),
    ]
    val_loaders = [
        DataLoader(TensorDataset(X_va, pg_qg_va), batch_size=batch, shuffle=False, pin_memory=True),
        DataLoader(TensorDataset(X_va, va_vm_va), batch_size=batch, shuffle=False, pin_memory=True),
    ]
    ...
    # --------------------------------------------------------------------- #
    for tid, name in enumerate(["Pg_Qg", "Va_Vm"]):
        test_loss = evaluate(
            test_model,
            test_loader,
            label=f"Test_{name}",
