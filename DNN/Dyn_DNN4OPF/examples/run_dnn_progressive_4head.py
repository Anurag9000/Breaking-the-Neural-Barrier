...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    # Override log-file path inside config (store the absolute path ONCE)
    ...
    logger.info(
        f"Initialized DNN_Progressive(input={input_dim}, hidden={hidden_dim}, use_bounds=True)"
    )
    ...
    # NOTE: DNN_Progressive2Head already applies per-head bounds (use_bounds=True).
    # Wrapping the model in nn.Sequential would break the (x, task_id) call signature.
    if config["clip_test"]:
        model_test = model  # bounds already enforced per head
    else:
        model_test = model
