...
from Dyn_DNN4OPF.models.adp_den_4head_width_only import ADP_DEN_4Head
...
DEFAULT_CONFIG = {
    ...
    "test_samples": 2000,
    # ─── Logging / paths ─────────────────────────────────────────────────────
    "model": "ADP_DEN_4Head_width_only",
    "log_file": str(Path("Results") / "adp_den_4head_width_only_log.csv"),
...
def run_pipeline(overrides: dict | None = None):
    ...
    logger = logging.getLogger("run_adp_den_4head_width_only")
    ...
    model = ADP_DEN_4Head(mcfg).to(device)
        ...
        all_logs.append(
            {
                ...
                "h2_dim": max(getattr(model, f"{br}_fc2").out_features for br in model.branches),
            }
        )
