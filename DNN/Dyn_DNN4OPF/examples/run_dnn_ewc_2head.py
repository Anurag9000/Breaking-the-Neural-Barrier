...
from Dyn_DNN4OPF.training.trainer import train_task_sequential, evaluate
...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    X_tr, Y_tr, OBJ_tr = train_loader.dataset.tensors
    X_v,  Y_v,  OBJ_v  = val_loader.dataset.tensors
    # slice targets: first 2*n_gen dims for Pg+Qg, remaining for Va+Vm
    pg_qg_tr = Y_tr[:, :2*n_gen]
    va_vm_tr = Y_tr[:, 2*n_gen:]
    pg_qg_va = Y_v[:, :2*n_gen]
    va_vm_va = Y_v[:, 2*n_gen:]
    task_train_loaders = [
        DataLoader(
            TensorDataset(X_tr, pg_qg_tr, OBJ_tr),
            batch_size=config["batch_size"], shuffle=True
        ),
        DataLoader(
            TensorDataset(X_tr, va_vm_tr, OBJ_tr),
            batch_size=config["batch_size"], shuffle=True
        ),
    ]
    task_val_loaders = [
        DataLoader(
            TensorDataset(X_v, pg_qg_va, OBJ_v),
            batch_size=config["batch_size"], shuffle=False
        ),
        DataLoader(
            TensorDataset(X_v, va_vm_va, OBJ_v),
            batch_size=config["batch_size"], shuffle=False
        ),
    ]
