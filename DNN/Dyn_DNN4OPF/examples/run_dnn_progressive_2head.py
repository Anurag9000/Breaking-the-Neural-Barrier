...
from Dyn_DNN4OPF.training.trainer import train_progressive_fixed, evaluate
...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    # Override log-file path inside config
    ...
    logger.info(
        f"Initialized DNN_Progressive2Head(input={input_dim}, hidden={hidden_dim}, use_bounds=True)"
    )

    logger.info("Starting fixed-head Progressive training…")
    # replicate loaders for the two fixed heads: Pg_Qg, Va_Vm
    task_train_loaders = [train_loader] * 2
    task_val_loaders   = [val_loader] * 2
    logs = train_progressive_fixed(
        model=model,
        task_loaders=task_train_loaders,
        val_loaders=task_val_loaders,
        task_names=["Pg_Qg", "Va_Vm"],
    ...
    # DNN_Progressive2Head already enforces bounds per head (use_bounds=True).
    # Wrapping in nn.Sequential would break the (x, task_id) signature.
    model_test = model

    for task_id, name in enumerate(["Pg_Qg", "Va_Vm"]):
        test_loss = evaluate(
            model_test,
            test_loader,
            label=f"Test_{name}",
        ...
        logger.info("Final Test MSE %s: %.6f", name, test_loss)
    ...
    with torch.no_grad():
        ...
        X_test, Y_test, _ = test_loader.dataset.tensors
