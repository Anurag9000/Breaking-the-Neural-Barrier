"""
Drop‑in runner: **Alternating ADP (Width→Depth), 4‑Head, PHYSICS‑FIRST acceptance**
Mirrors run_adp_depth_4head.py but uses the ADP_ALT_WIDTH_4Head controller and provides a physics_metric.
...
from Dyn_DNN4OPF.models.adp_den_alt_width_4head import ADP_ALT_WIDTH_4Head
...
DEFAULT_CONFIG: Dict = {
    ...
    "patience": 20,
    "max_epochs": 500,
    "log_file": "adp_alt_width_4head.csv",
    "model": "ADP-ALT-WIDTH-4HEAD",
...
def _physics_metric_factory(eq: Dict, ineq: Dict, n_bus: int, n_gen: int) -> Callable[[torch.Tensor, torch.Tensor], torch.Tensor]:
    ...
    def metric(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # pragmatic physics score: network residual + voltage bound violations
        vmin, vmax = ineq["v_min"], ineq["v_max"]
        v_low = (vmin - vm).clamp_min(0).pow(2).mean()
        v_high = (vm - vmax).clamp_min(0).pow(2).mean()
        return p_calc.pow(2).mean() + q_calc.pow(2).mean() + v_low + v_high
    return metric
...
def run_pipeline(cfg: Dict) -> None:
    ...
    controller = ADP_ALT_WIDTH_4Head(
