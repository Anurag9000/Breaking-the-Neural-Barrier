...
from Dyn_DNN4OPF.models.adp_den_width_only import ADP_DEN
...
DEFAULT_CONFIG = {
    ...
    "trials_depth": 10,  # outer patience (failed expansions)
    "warmup_epochs": 100,
    "spl_thr": 0.25,
    "ex_k": 10,
    "init_width": 10,
    ...
    "test_samples": 2000,
    # ─── Logging / paths ─────────────────────────────────────────────────────
    "model": "ADP_DEN_width_only",
    "log_file": str(Path("Results") / "adp_den_width_only_log.csv"),
...
def run_pipeline(overrides: dict | None = None):
    ...
    logger = logging.getLogger("run_adp_den_width_only")
    ...
    model = ADP_DEN(mcfg).to(device)
    model.patience = config["patience"]
    model.lr = config["lr"]
    model.n_bus = n_bus
    model.n_gen = n_gen
    model.bound_layer.apply_bounds.fill_(False)  # disable clipping in train/val
    ...
    # Final diagnostics on last task (optionally re-enable clipping for test)
