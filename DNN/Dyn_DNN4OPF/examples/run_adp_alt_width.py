"""
Drop‑in runner: **Alternating ADP (Width→Depth), 1‑Head**
Same pipeline as run_adp_depth.py; controller differs + acceptance order.
"""
import logging
import sys
...
from Dyn_DNN4OPF.models.adp_den_alt_width_1head import ADP_ALT_WIDTH_1Head
...
def run_pipeline(cfg: Dict) -> None:
    ...
    controller = ADP_ALT_WIDTH_1Head(in_dim=in_dim, out_dim=out_dim, width=config["init_width"], depth=config["init_depth"], ex_k=config["ex_k"], max_neurons=config["max_neurons"], max_depth=config["max_depth"], delta=config["delta"], patience_width=config["trials_width"], patience_depth=config["trials_depth"], device=device)
