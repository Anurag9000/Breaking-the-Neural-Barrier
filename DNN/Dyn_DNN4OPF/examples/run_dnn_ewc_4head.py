...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    # ── split into four fixed heads: Pg, Qg, Va, Vm ────────────────────────────
    from torch.utils.data import DataLoader, TensorDataset
    # define slices according to output ordering: [Pg | Qg | Va | Vm]
    ...
    for sl in (pg_slice, qg_slice, va_slice, vm_slice):
        task_train_loaders.append(
            DataLoader(
                TensorDataset(X_tr, Y_tr[:, sl], OBJ_tr),
                batch_size=config["batch_size"],
                shuffle=True
            )
        )
        task_val_loaders.append(
            DataLoader(
                TensorDataset(X_v, Y_v[:, sl], OBJ_v),
                batch_size=config["batch_size"]
            )
        )
