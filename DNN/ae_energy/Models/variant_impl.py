...
class EnergyGraphEncoder(nn.Module):
...
class AEEnergyModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.meta = meta
        self.encoder = EnergyGraphEncoder(2, hidden, layers)
        ...
        self.theta_decoder = nn.Sequential(nn.Linear(hidden + meta.n_gen, hidden), nn.ReLU(), nn.Linear(hidden, meta.n_bus))
        self.vm_decoder = nn.Sequential(nn.Linear(hidden + meta.n_gen, hidden), nn.ReLU(), nn.Linear(hidden, meta.n_bus))
    ...
    def forward(self, data) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        conditioned = torch.cat([features, pg], dim=1)
        va = self.theta_decoder(conditioned)
        vm = self.vm_decoder(conditioned)
        outputs = torch.cat([pg, qg, va, vm], dim=1)
        corrected = self.physics(outputs, data.pdqd)
        return corrected, {"latent": latent, "features": features, "pg": pg}
