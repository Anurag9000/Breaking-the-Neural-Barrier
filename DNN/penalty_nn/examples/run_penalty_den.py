...
def run_pipeline(cfg: dict) -> None:
    ...
    """
    # 1) Merge defaults, Optuna params, and user cfg
    ...
    # 3) Bounds & equality data (honor user overrides)
    ...
    # 5) Test-only clip (wrap only when enabled)
    clip_layer = BoundedAct(
        ...
        apply_bounds=False,  # will be set for test only
    ).to(device)
    ...
    # 7) Final evaluation & diagnostics (unchanged)
    model_eval.eval()
    ...
    logging.getLogger(__name__).info(f"Completed penalty DEN. Test loss: {test_loss:.6f}")
