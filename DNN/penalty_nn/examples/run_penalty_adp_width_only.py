...
from penalty_nn.models.penalty_adp_den_width_only import PenaltyADP_DEN
...
DEFAULT_CONFIG = {
    ...
    "model": "Penalty-ADP-DEN-Width-Only",
...
def run_pipeline(cfg: dict) -> None:
    ...
    # Final config fields
    config["dims"] = (in_dim, config["h1_dim"], config["h2_dim"])
    ...
    # Instantiate & train
    model = PenaltyADP_DEN(
        SimpleNamespace(**config),
        lambda_loss=config["lambda_loss"],
        lambda_equality=config["lambda_equality"],
    ...
    for task_id, (tr, va, te, cons) in enumerate(task_loaders, start=1):
        ...
        # Logging: width-only single-head uses self.layers list
        try:
