...
from penalty_nn.models.penalty_progressive_4head import PenaltyDNNProgressive_4HEAD
...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    model = PenaltyDNNProgressive_4HEAD(
        input_dim=input_dim,
    ...
    # Bound layers already exist per column; wrapping the model in nn.Sequential
    # would break the (x, task_id) call signature. Keep the model as-is.
    if config["clip_test"]:
        model_test = model
    else:
        model_test = model
