...
from penalty_nn.models.penalty_mtl_4head import PenaltyDNN_MTL_4HEAD
from penalty_nn.training.penalty_trainer import train_penalty_mtl_incremental, evaluate
...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    logs = train_penalty_mtl_incremental(
        model=model,
    ...
    model = PenaltyDNN_MTL_4HEAD(
        ...
        case_name=config["case_name"], # needed to load physics bounds
        clip_test=config["clip_test"], # enable bounded clipping at test time
        device=device,                 # build model directly on GPU/CPU
    )
