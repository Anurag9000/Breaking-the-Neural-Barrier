"""
Drop‑in runner: **Penalty‑ADP Alternating (Depth→Width), 4‑Head**
- Composite penalty training; early‑stop & accept by **val MSE**.
- Default OPF penalty_fn provided; heads are [PG | QG | VA | VM].
"""
import logging
import sys
...
from Dyn_DNN4OPF.models.adp_penalty_alt_depth_4head import PENALTY_ADP_ALT_DEPTH_4Head
...
DEFAULT_CONFIG: Dict = {
    ...
    "patience": 20,
    "max_epochs": 500,
    "log_file": "penalty_adp_alt_depth_4head.csv",
    "model": "PENALTY-ADP-ALT-DEPTH-4HEAD",
...
def run_pipeline(cfg: Dict) -> None:
    ...
    controller = PENALTY_ADP_ALT_DEPTH_4Head(
        in_dim=in_dim, outs=[n_gen, n_gen, n_bus, n_bus], width=config["init_width"], depth=config["init_depth"], ex_k=config["ex_k"],
