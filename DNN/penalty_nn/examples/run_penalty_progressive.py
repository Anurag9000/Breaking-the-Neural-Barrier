...
from penalty_nn.models.penalty_progressive import PenaltyDNN_Progressive
from Dyn_DNN4OPF.training.trainer import train_progressive_patience_spawn, evaluate
...
def run_pipeline(cfg: Dict[str, Any]) -> None:
    ...
    logs = train_progressive_patience_spawn(
        model=model,
    ...
    if config["clip_test"]:
        ...
        model_test = torch.nn.Sequential(model, clip_layer).to(device)
    else:
        model_test = model
