"""
Drop‑in runner: **Penalty‑ADP Alternating (Width→Depth), 2‑Head**
- Composite penalty training; early‑stop & accept by **val MSE**.
- Default OPF penalty_fn provided.
- Output split: [ (PG,QG)=2*n_gen | (VA,VM)=2*n_bus ].
"""
import logging
import sys
...
from Dyn_DNN4OPF.models.adp_penalty_alt_width_2head import PENALTY_ADP_ALT_WIDTH_2Head
...
DEFAULT_CONFIG: Dict = {
    ...
    "patience": 20,
    "max_epochs": 500,
    "log_file": "penalty_adp_alt_width_2head.csv",
    "model": "PENALTY-ADP-ALT-WIDTH-2HEAD",
...
def _penalty_factory(eq: Dict, ineq: Dict, n_bus: int, n_gen: int) -> Callable[[torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor]:
    ...
    def penalty_fn(pred: torch.Tensor, xb: torch.Tensor, yb: torch.Tensor):
        pq = pred[..., :2*n_gen]; vmblk = pred[..., 2*n_gen:2*n_gen+2*n_bus]
        pg, qg = pq[..., :n_gen], pq[..., n_gen:2*n_gen]
        va, vm = vmblk[..., :n_bus], vmblk[..., n_bus:2*n_bus]
...
def run_pipeline(cfg: Dict) -> None:
    ...
    controller = PENALTY_ADP_ALT_WIDTH_2Head(
        in_dim=in_dim, out_pq=2*n_gen, out_vm=2*n_bus, width=config["init_width"], depth=config["init_depth"], ex_k=config["ex_k"],
