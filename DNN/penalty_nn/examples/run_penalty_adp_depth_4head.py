...
the number mentioned in the ith index is the dimension in the ith hidden layer
these are initialisation parameters model adjusts as it grows
"""
...
from penalty_nn.models.penalty_adp_depth_4head import PenaltyADPDepth_4Head
...
DEFAULT_CONFIG = {
    ...
    "clip_test": False,
    # ─── Logging & checkpoints ───────────────────────────────────────────────
    "log_file": "train_adp_depth.csv",
    # ─── Data / model identifiers ────────────────────────────────────────────
    "model": "ADP-DEPTH-penalty-4head",
...
def run_pipeline(cfg: dict) -> None:
    ...
    for k in soft_ineq:
        soft_ineq[k] = soft_ineq[k].to(device)

    eq["gen_bus_idx"] = eq["gen_bus_idx"].to(device)
    eq["load_bus_idx"]= eq["load_bus_idx"].to(device)
    ...
    # Instantiate & train
    model = PenaltyADPDepth_4Head(SimpleNamespace(**config)).to(device)
        ...
        all_logs.append(
            {
                "task": task_id,
                "test_perf": test_perf,
                "total_hidden": sum(l.out_features for l in model.hidden_layers),
            }
        )
