"""
Penalty‑ADP — Alternating (Width→Depth) — 1‑Head
- Trains with composite penalty loss:  L = λ_loss·MSE + λ_eq·Eq + λ_ineq·Ineq
- Early‑stopping *and* accept/reject use **validation MSE only** (not the penalty terms).
- Alternation cycle: WIDTH proposal → DEPTH proposal, each with its own patience counter.
- Generic: provide a `penalty_fn(pred, xb, yb) -> (eq_term, ineq_term)` returning differentiable tensors.
"""
...
# -------------------------------
class PENALTY_ADP_ALT_WIDTH_1Head:
    def __init__(self, in_dim: int, out_dim: int, width: int = 64, depth: int = 1,
