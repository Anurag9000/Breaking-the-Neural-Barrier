...
from Dyn_DNN4OPF.models.adp_depth import ADPDepth, expand_depth, expand_width
...
def train_early_stop(
    ...
    *,
    ...
    """
    Early-stopping on validation MSE, replacing MSE-loss with penalty loss_fn for training.
...
class PenaltyADPDepth_4Head(ADPDepth):
    """
    Depth-first ADP with physics-informed penalty loss and *patience on expansion*
    for both depth (outer series) and width (inner series).
    """
    def __init__(self, cfg: SimpleNamespace):
        ...
        # Patience knobs (dict-style access with defaults; safe fallbacks)
        try:
    ...
    def fit_task(
        ...
        *,
        ...
        while not stop_outer and len(self.hidden_layers) < self.max_depth:
            # 1) Train current architecture to plateau (establish baseline before depth series)
            ...
            while True:
                ...
                if d_val < preD_val - delta_thr:
                    ...
                    logger.info(
                        "[DEPTH] Expansion accepted; counter reset. %.6f → %.6f (Δ=%.6f)",
                    ...
                    while True:
                        ...
                        if w_val < preW_val - delta_thr:
                            logger.info(
                                "[WIDTH] Expansion accepted; counter reset. %.6f → %.6f (Δ=%.6f)",
                            ...
                            accepted_model = copy.deepcopy(self)
                            accepted_val = w_val
                            # keep attempting more width expansions until fail/guard
                            continue
                        else:
                            ...
                            if self._width_failures >= int(self.trials_width):
                                ...
                                # Keep added capacity; attempt another widening
                                continue

                    # After finishing width series, continue depth series (try more layers)
                    self._restore(preD_model)  # ensure we continue depth from the last accepted depth
                    continue
                    ...
                    if self._depth_failures >= int(self.trials_depth):
                        ...
                        # Keep added layer; try adding another layer in the next loop
                        continue
