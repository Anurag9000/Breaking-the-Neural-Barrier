...
from model_maskfeat_vit import MaskFeatViT
...
def make_loaders(cfg: Config):
    return make_real_image_loaders(cfg.data_root, cfg.batch_size, image_size=cfg.img_size)
...
def train(cfg: Config):
    ...
    model = MaskFeatViT(cfg.img_size, cfg.patch_size, 3, cfg.embed_dim, cfg.depth, cfg.heads, cfg.mlp_ratio,
                        cfg.mask_ratio, cfg.feat_bins).to(cfg.device)
    ...
    for ep in range(cfg.epochs):
        ...
        for imgs,_ in train_loader:
            ...
            pred, feats, mask = model(imgs)
            loss = model.loss((pred, feats, mask))
    ...
    save_plot(tr_curve, 'MaskFeat Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
    save_plot(va_curve, 'MaskFeat Val Loss', os.path.join(cfg.out_dir,'val_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--dataset', type=str, default='imagefolder')
    ...
    a.add_argument('--feat_bins', type=int, default=8)
    ...
    a.add_argument('--out_dir', type=str, default='./outs_maskfeat_vit')
