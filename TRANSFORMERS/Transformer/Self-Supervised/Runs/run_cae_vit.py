...
from model_cae_vit import CAEViT
...
def train(cfg: Config):
    ...
    model = CAEViT(cfg.img_size, cfg.patch_size, 3, cfg.embed_dim, cfg.depth, cfg.heads, cfg.mlp_ratio,
                   cfg.mask_ratio, cfg.latent_dim).to(cfg.device)
    ...
    for ep in range(cfg.epochs):
        ...
        for imgs,_ in train_loader:
            ...
            pred_lat, t_lat, mask = model(imgs)
            loss = model.loss((pred_lat, t_lat, mask))
    ...
    save_plot(tr_curve, 'CAE Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
    save_plot(va_curve, 'CAE Val Loss', os.path.join(cfg.out_dir,'val_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--latent_dim', type=int, default=256)
    ...
    a.add_argument('--out_dir', type=str, default='./outs_cae_vit')
