...
def main():
    ...
    ap.add_argument('--num_prototypes', type=int, default=300)
    ap.add_argument('--tau', type=float, default=0.1)
    ap.add_argument('--eps', type=float, default=0.05)
    ap.add_argument('--sinkhorn_iters', type=int, default=3)
    ap.add_argument('--save', default='swav_vit_best.pt')
    args=ap.parse_args()

    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    tr,va,te=loaders(args.dataset,args.data_dir,args.batch_size)

    cfg=SwAVConfig(args.img,args.patch,args.dim,args.depth,args.heads,args.ratio,
                   args.proj_hidden,args.proj_out,args.num_prototypes,args.tau,args.eps,args.sinkhorn_iters)
    model=SwAVViT(cfg).to(device)
