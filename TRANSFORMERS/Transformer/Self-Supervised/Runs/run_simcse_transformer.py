...
from model_simcse_transformer import SimpleTokenizer, SimCSETransformer
...
class Config:
    ...
    out_dir: str = './outs_simcse_transformer'
    seed: int = 42
...
def make_pairs(lines):
    # unsupervised SimCSE: two dropout/noise views -> here we use simple word-drop augmentation
    def aug(s):
        w = s.split()
        if len(w) > 4:
            k = max(1, int(0.1*len(w)))
            idx = set(random.sample(range(len(w)), k))
            w = [t for i,t in enumerate(w) if i not in idx]
        return ' '.join(w) if w else s
    pairs = [(l, aug(l)) for l in lines]
    return pairs
...
def train(cfg: Config):
    ...
    model = SimCSETransformer(len(tok.itos), cfg.dim, cfg.depth, cfg.heads, cfg.mlp_ratio, cfg.proj_dim).to(cfg.device)
    ...
    for ep in range(cfg.epochs):
        ...
        for x1,x2 in batch_iter():
            ...
            loss = model(x1,x2,cfg.temperature)
    ...
    save_plot(tr_curve, 'SimCSE Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--proj_dim', type=int, default=128)
    a.add_argument('--temperature', type=float, default=0.05)
    ...
    a.add_argument('--out_dir', type=str, default='./outs_simcse_transformer')
