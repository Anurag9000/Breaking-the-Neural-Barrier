...
from model_wavlm_single import WavLMSingle
...
def synth_wavs(sample_len, n=1000):
    for _ in range(n):
        ...
        wav = torch.sin(2*torch.pi*(110+random.random()*660)*t) + 0.1*torch.randn_like(t)
...
def train(cfg: Config):
    set_seed(cfg.seed)
    model = WavLMSingle().to(cfg.device)
    ...
    save_plot(tr_curve, 'WavLM-single Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--out_dir', type=str, default='./outs_wavlm_single')
