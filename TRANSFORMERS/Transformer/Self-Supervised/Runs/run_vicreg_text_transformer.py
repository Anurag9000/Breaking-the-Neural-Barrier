...
from model_vicreg_text_transformer import VICRegTextTransformer, SimpleTokenizer
...
class Config:
    ...
    out_dir: str = './outs_vicreg_text_transformer'
    seed: int = 42
...
def make_pairs(lines):
    def drop_words(s):
        w=s.split();
        if len(w)>4:
            k=max(1,int(0.1*len(w))); idx=set(random.sample(range(len(w)), k));
            w=[t for i,t in enumerate(w) if i not in idx]
        return ' '.join(w) if w else s
    return [(l, drop_words(l)) for l in lines]
...
def train(cfg: Config):
    ...
    def batch_iter():
        ...
        for i in range(0, len(pairs), cfg.batch_size):
            ...
            x1 = torch.stack([tok.encode(a, cfg.max_len) for a,_ in chunk])
            x2 = torch.stack([tok.encode(b, cfg.max_len) for _,b in chunk])
            yield x1, x2

    model = VICRegTextTransformer(len(tok.itos), cfg.dim, cfg.depth, cfg.heads, cfg.mlp_ratio,
    ...
    save_plot(tr_curve, 'VICReg-Text Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--proj_hidden', type=int, default=2048)
    a.add_argument('--proj_out', type=int, default=2048)
    a.add_argument('--sim_coeff', type=float, default=25.0)
    a.add_argument('--var_coeff', type=float, default=25.0)
    a.add_argument('--cov_coeff', type=float, default=1.0)
    ...
    a.add_argument('--out_dir', type=str, default='./outs_vicreg_text_transformer')
