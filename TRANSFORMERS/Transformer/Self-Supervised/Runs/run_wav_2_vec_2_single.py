...
from model_wav2vec2_single import Wav2Vec2Single
...
class Config:
    ...
    out_dir: str = './outs_wav2vec2_single'
    seed: int = 42
...
def wav_loader(wav_dir, sample_len):
    # dummy sine waves to keep this self-contained
    for _ in range(1000):
        t = torch.linspace(0, 1, sample_len)
        wav = torch.sin(2*torch.pi*440*t) + 0.1*torch.randn_like(t)
        yield wav.unsqueeze(0)


def batch_iter(cfg: Config):
    buf=[]
    for wav in wav_loader(cfg.wav_dir, cfg.sample_len):
        buf.append(wav)
...
def train(cfg: Config):
    set_seed(cfg.seed)
    model = Wav2Vec2Single().to(cfg.device)
    ...
    save_plot(tr_curve, 'wav2vec2-single Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--wav_dir', type=str, default='./wav')
    ...
    a.add_argument('--out_dir', type=str, default='./outs_wav2vec2_single')
