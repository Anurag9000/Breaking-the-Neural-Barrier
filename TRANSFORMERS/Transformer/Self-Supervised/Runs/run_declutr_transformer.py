...
from model_declutr_transformer import DeCLUTRTransformer, SimpleTokenizer
...
class Config:
    ...
    out_dir: str = './outs_declutr_transformer'
    seed: int = 42
...
def make_span_pairs(lines):
    pairs = []
    for l in lines:
        w = l.split()
        if len(w) < 4:
            pairs.append((l,l)); continue
        a = ' '.join(w[:max(1, int(0.5*len(w)))])
        b = ' '.join(w[int(0.25*len(w)) : ])
        pairs.append((a,b))
    return pairs
...
def train(cfg: Config):
    ...
    model = DeCLUTRTransformer(len(tok.itos), cfg.dim, cfg.depth, cfg.heads, cfg.mlp_ratio, cfg.proj_dim).to(cfg.device)
    ...
    save_plot(tr_curve, 'DeCLUTR Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--out_dir', type=str, default='./outs_declutr_transformer')
