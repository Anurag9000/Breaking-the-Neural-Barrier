...
from model_mlm_bert import BERTEncoder
...
def train(cfg: Config):
    ...
    model = BERTEncoder(len(tok.itos), cfg.dim, cfg.depth, cfg.heads, cfg.mlp_ratio, cfg.max_len).to(cfg.device)
    ...
    for ep in range(cfg.epochs):
        ...
        for X in make_batches(lines, tok, cfg):
            X=X.to(cfg.device)
            loss = model(X, cfg.mask_token_id, cfg.mask_prob)
    ...
    save_plot(tr_curve, 'MLM Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--mask_token_id', type=int, default=103)
    a.add_argument('--mask_prob', type=float, default=0.15)
    ...
    a.add_argument('--out_dir', type=str, default='./outs_mlm_bert')
