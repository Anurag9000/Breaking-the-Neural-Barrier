...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_wav_2_vec_2_single.py").resolve()
...
ModelClass = baseline_module.ConvFeatureEncoder

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ConvFeatureEncoder model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['dims'] = get_attr(model, ['dims'], 1)
        kwargs['strides'] = get_attr(model, ['strides'], None)
        kwargs['kernels'] = get_attr(model, ['kernels'], None)
