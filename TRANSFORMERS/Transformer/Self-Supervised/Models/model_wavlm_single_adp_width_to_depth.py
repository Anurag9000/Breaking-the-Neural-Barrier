...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_wavlm_single.py").resolve()
...
ModelClass = baseline_module.WavLMSingle

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the WavLMSingle model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['code_dim'] = get_attr(model, ['code_dim'], 1)
        kwargs['code_k'] = get_attr(model, ['code_k'], None)
        kwargs['n_layers'] = depth
        kwargs['n_heads'] = get_attr(model, ['n_heads'], None)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.dim, model.layers), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.dim, curr_model.layers), v))
            else:
    ...
    return global_best_val, model, model.dim, model.layers
