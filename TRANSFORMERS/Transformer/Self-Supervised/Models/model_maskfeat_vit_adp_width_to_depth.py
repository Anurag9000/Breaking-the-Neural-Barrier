...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_maskfeat_vit.py").resolve()
...
ModelClass = baseline_module.MaskFeatViT
from _common_real_image import make_real_image_loaders

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the MaskFeatViT model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['mask_ratio'] = get_attr(model, ['mask_ratio'], None)
        kwargs['feat_bins'] = get_attr(model, ['feat_bins'], None)
