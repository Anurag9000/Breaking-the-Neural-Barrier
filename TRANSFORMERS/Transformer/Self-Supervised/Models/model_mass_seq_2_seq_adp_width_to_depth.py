...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_mass_seq_2_seq.py").resolve()
...
ModelClass = baseline_module.MASSSeq2Seq

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the MASSSeq2Seq model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['dim'] = width
        kwargs['enc_depth'] = get_attr(model, ['enc_depth'], 1)
        kwargs['dec_depth'] = get_attr(model, ['dec_depth'], 1)
