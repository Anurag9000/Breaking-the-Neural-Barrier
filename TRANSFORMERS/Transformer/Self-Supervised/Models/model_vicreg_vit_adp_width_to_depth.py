...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_vicreg_vit.py").resolve()
...
ModelClass = baseline_module.VICRegViT

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the VICRegViT model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['proj_hidden'] = get_attr(model, ['proj_hidden'], None)
        kwargs['proj_out'] = get_attr(model, ['proj_out'], None)
        kwargs['sim_coeff'] = get_attr(model, ['sim_coeff'], None)
        kwargs['var_coeff'] = get_attr(model, ['var_coeff'], None)
        kwargs['cov_coeff'] = get_attr(model, ['cov_coeff'], None)
        kwargs['eps'] = get_attr(model, ['eps'], None)
