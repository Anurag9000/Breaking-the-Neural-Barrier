...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_simcse_transformer.py").resolve()
...
ModelClass = baseline_module.PosEnc

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the PosEnc model.
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    cur_w = width = getattr(model, 'dim', 0) if 'dim' != 'None' else getattr(model.cfg, 'width', 0) if hasattr(model, 'cfg') else 0
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(getattr(model, "dim", 0), getattr(model, 'width', 0)), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(getattr(model, "dim", 0), getattr(model, 'width', 0)), v))
            else:
