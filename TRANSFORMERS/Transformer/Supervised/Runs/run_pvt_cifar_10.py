...
from model_pvt import PyramidVisionTransformer
...
def main():
    ...
    model = PyramidVisionTransformer(num_classes=10).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_pvt_cifar_10'), 'run_pvt_cifar_10', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if acc > best + 1e-6:
            best=acc; bad=0; torch.save({'model': model.state_dict()}, 'PVT_CIFAR10_best.pth')
        else:
            bad+=1
