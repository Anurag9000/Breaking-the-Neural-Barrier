...
def main():
    ...
    model = SwinTiny(num_classes=10).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_swin_cifar_10'), 'run_swin_cifar_10', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if acc > best + 1e-6:
            best = acc; bad = 0
            torch.save({'model': model.state_dict()}, 'SwinTiny_CIFAR10_best.pth')
        else:
            bad += 1
