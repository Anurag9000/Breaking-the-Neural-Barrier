...
def main():
    ap = argparse.ArgumentParser(description="Reformer text classifier on AG News")
    ...
    model = ReformerEncoder(vocab=len(vocab), num_classes=num_classes, dim=256, depth=6, heads=8, bucket=64).to(args.device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict()}, "Reformer_best.pth")
        else:
            bad += 1
            if bad >= patience:
                break
