...
from model_mobilevit import MobileViT
...
def main():
    ...
    ap.add_argument('--epochs', type=int, default=60); ap.add_argument('--lr', type=float, default=3e-4)
    ap.add_argument('--patience', type=int, default=8); ap.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    args=ap.parse_args()
    ...
    model = MobileViT(num_classes=10).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_mobilevit_cifar_10'), 'run_mobilevit_cifar_10', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if acc > best + 1e-6:
            best=acc; bad=0; torch.save({'model': model.state_dict()}, 'MobileViT_CIFAR10_best.pth')
        else:
            bad+=1
