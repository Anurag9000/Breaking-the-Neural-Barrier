...
def main():
    ap = argparse.ArgumentParser(description="Transformer-XL style text classifier on AG News")
    ...
    model = MemTransformerLM(vocab=len(vocab), num_classes=num_classes, d_model=256, nhead=8, layers=6, ff=1024).to(args.device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict(), "vocab": vocab}, "TransformerXL_best.pth")
        else:
            bad += 1
