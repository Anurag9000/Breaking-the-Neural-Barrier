...
# --------------------------------------------------
# MViTv2 (image classification config, simplified)
#  - Multiscale tokens via strided pooling between stages
# --------------------------------------------------

class PatchEmbed(nn.Module):
    def __init__(self, in_ch, dim, patch, stride):
        super().__init__(); self.p=nn.Conv2d(in_ch, dim, patch, stride); self.n=nn.LayerNorm(dim)
...
class MViTv2(nn.Module):
    def __init__(self, num_classes=10, embed=(64,128,256,512), depths=(1,2,9,1), heads=(2,4,8,16)):
        super().__init__(); self.pe=PatchEmbed(3,embed[0],patch=4,stride=4)
