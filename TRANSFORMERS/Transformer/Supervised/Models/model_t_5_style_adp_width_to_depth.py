...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_t_5_style.py").resolve()
...
ModelClass = baseline_module.RMSNorm

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the RMSNorm model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['d'] = get_attr(model, ['d'], None)
