...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_gpt_token_tag.py").resolve()
...
ModelClass = baseline_module.GPTTagger

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the GPTTagger model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['num_tags'] = get_attr(model, ['num_tags'], 1)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, new_w, getattr(model, 'layers', 1) if 'layers' != 'None' else 1, device, cfg)
