...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_mlp_mixer.py").resolve()
...
ModelClass = baseline_module.MixerBlock

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the MixerBlock model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['tokens'] = get_attr(model, ['tokens'], None)
        kwargs['dim'] = width
