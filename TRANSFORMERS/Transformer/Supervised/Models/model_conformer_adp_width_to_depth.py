...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_conformer.py").resolve()
...
ModelClass = baseline_module.ConvModule

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ConvModule model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['expansion'] = get_attr(model, ['expansion'], None)
        kwargs['kernel'] = get_attr(model, ['kernel'], None)
