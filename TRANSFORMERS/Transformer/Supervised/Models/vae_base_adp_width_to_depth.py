...
# Load baseline
BASE_PATH = Path(__file__).with_name("vae_base.py").resolve()
...
ModelClass = baseline_module.ConvEncoder

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ConvEncoder model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['in_ch'] = get_attr(model, ['in_ch'], None)
        kwargs['width'] = width
        kwargs['latent_dim'] = get_attr(model, ['latent_dim'], 1)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    cur = model.width if hasattr(model, 'width') else cfg.max_width # fallback
...
def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, getattr(model, 'width', 64), new_d, device, cfg)
...
def snapshot_arch_and_state(model: ModelClass, state_dict=None) -> Dict[str, Any]:
    ...
    return {
        "width": getattr(model, 'width', 0) if 'width' != 'None' else 0,
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.width, 0), best_val))

    def can_widen(m: ModelClass) -> bool:
        return m.width < acfg.max_width
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.width, 0), v))
            else:
    ...
    return global_best_val, model, model.width, 0
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    return best_val, best_model, int(getattr(best_model, "width", 0)), int(getattr(best_model, "depth", 0))
...
def main():
    ...
    try:
        model = ModelClass(width=args.width).to(device)
    except:
