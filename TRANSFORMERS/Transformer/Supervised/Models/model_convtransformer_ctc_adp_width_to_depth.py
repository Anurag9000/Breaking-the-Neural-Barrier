...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_convtransformer_ctc.py").resolve()
...
ModelClass = baseline_module.ConvFeature

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ConvFeature model.
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    cur = model.channels if hasattr(model, 'channels') else cfg.max_width # fallback
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.channels, 0), best_val))

    def can_widen(m: ModelClass) -> bool:
        return m.channels < acfg.max_width
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.channels, 0), v))
            else:
    ...
    return global_best_val, model, model.channels, 0
...
def main():
    ...
    try:
        model = ModelClass(channels=args.width).to(device)
    except:
