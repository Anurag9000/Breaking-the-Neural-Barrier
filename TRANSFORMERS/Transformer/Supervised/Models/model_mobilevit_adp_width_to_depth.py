...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_mobilevit.py").resolve()
...
ModelClass = baseline_module.MV2Block

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the MV2Block model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['out_ch'] = get_attr(model, ['out_ch'], None)
        kwargs['expand'] = get_attr(model, ['expand'], None)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    cur_w = width = getattr(model, 'width', 0)
