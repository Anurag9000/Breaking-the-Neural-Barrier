...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_perceiver_io.py").resolve()
...
ModelClass = baseline_module.CrossAttention

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the CrossAttention model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['q_dim'] = get_attr(model, ['q_dim'], 1)
        kwargs['kv_dim'] = get_attr(model, ['kv_dim'], 1)
