...
from TRANSFORMERS.Transformer._common_real_benchmark import make_real_transformer_smoke_loaders
...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_vit_lite.py").resolve()
...
ModelClass = baseline_module.ViT_Lite

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ViT_Lite model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['img_size'] = get_attr(model, ['img_size'], 1)
        kwargs['patch_size'] = get_attr(model, ['patch_size'], 1)
        kwargs['in_chans'] = get_attr(model, ['in_chans'], None)
        kwargs['num_classes'] = get_attr(model, ['num_classes', 'head.out_features', 'fc.out_features', 'classifier.out_features'], 1)
        kwargs['embed_dim'] = width
        kwargs['depth'] = depth
        kwargs['num_heads'] = get_attr(model, ['num_heads'], 1)
        kwargs['mlp_ratio'] = get_attr(model, ['mlp_ratio'], None)
        kwargs['drop'] = get_attr(model, ['drop'], None)
        kwargs['attn_drop'] = get_attr(model, ['attn_drop'], None)
        kwargs['proj_drop'] = get_attr(model, ['proj_drop'], None)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    # Align to heads
    H = getattr(model, 'num_heads', 1) or 1
    rem = nxt % H
    if rem != 0: nxt += (H - rem)
...
def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, model.embed_dim, cur + 1, device, cfg)
...
def snapshot_arch_and_state(model: ModelClass, state_dict=None) -> Dict[str, Any]:
    ...
    return {
        ...
        "depth": getattr(model, 'depth', 0) if 'depth' != 'None' else 0,
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.embed_dim, model.depth), best_val))

    def can_widen(m: ModelClass) -> bool:
        return m.embed_dim < acfg.max_width
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.embed_dim, curr_model.depth), v))
            else:
    ...
    return global_best_val, model, model.embed_dim, model.depth
...
def main():
    ...
    p.add_argument("--adp-mode", default="width_to_depth", choices=["width_only", "depth_only", "alt_width", "alt_depth", "width_to_depth", "depth_to_width"])
    ...
    try:
        model = ModelClass(embed_dim=args.width, depth=args.depth).to(device)
    except:
