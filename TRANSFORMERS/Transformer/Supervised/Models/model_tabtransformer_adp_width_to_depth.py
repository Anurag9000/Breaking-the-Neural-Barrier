...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_tabtransformer.py").resolve()
...
ModelClass = baseline_module.TabTransformer

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the TabTransformer model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['num_categories_per_field'] = get_attr(model, ['num_categories_per_field'], 1)
