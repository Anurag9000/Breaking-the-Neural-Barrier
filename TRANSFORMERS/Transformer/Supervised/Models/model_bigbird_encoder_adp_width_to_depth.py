...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_bigbird_encoder.py").resolve()
...
ModelClass = baseline_module.SparseAttentionEncoderLayer

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the SparseAttentionEncoderLayer model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['nhead'] = get_attr(model, ['nhead'], None)
        kwargs['block'] = get_attr(model, ['block'], None)
        kwargs['num_global'] = get_attr(model, ['num_global'], 1)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, new_w, getattr(model, 'depth', 1), device, cfg)
...
def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, getattr(model, 'd_model', 64) if 'd_model' != 'width' else 64, new_d, device, cfg)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(getattr(model, "d_model", 0), getattr(model, 'width', 0)), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(getattr(model, "d_model", 0), getattr(model, 'width', 0)), v))
            else:
