...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_crossvit.py").resolve()
...
ModelClass = baseline_module.CrossViT

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the CrossViT model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['s_dim'] = get_attr(model, ['s_dim'], 1)
        kwargs['s_patch'] = get_attr(model, ['s_patch'], None)
        kwargs['s_depth'] = get_attr(model, ['s_depth'], 1)
        kwargs['s_heads'] = get_attr(model, ['s_heads'], None)
        kwargs['l_dim'] = get_attr(model, ['l_dim'], 1)
        kwargs['l_patch'] = get_attr(model, ['l_patch'], None)
        kwargs['l_depth'] = get_attr(model, ['l_depth'], 1)
        kwargs['l_heads'] = get_attr(model, ['l_heads'], None)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.dim, model.depth), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.dim, curr_model.depth), v))
            else:
