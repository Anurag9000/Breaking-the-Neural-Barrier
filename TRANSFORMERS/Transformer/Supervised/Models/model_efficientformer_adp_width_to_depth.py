...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_efficientformer.py").resolve()
...
ModelClass = baseline_module.SEModule

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the SEModule model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['c'] = get_attr(model, ['c'], None)
        kwargs['r'] = get_attr(model, ['r'], None)
