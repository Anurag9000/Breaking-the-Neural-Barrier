...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_tcn_transformer.py").resolve()
...
ModelClass = baseline_module.Chomp1d

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the Chomp1d model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['chomp_size'] = get_attr(model, ['chomp_size'], 1)
