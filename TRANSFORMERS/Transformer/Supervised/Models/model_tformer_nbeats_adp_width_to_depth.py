...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_tformer_nbeats.py").resolve()
...
ModelClass = baseline_module.NBeatsBlock

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the NBeatsBlock model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['d_in'] = get_attr(model, ['d_in'], None)
        kwargs['width'] = width
