...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_swin_v_2.py").resolve()
...
ModelClass = baseline_module.SwinV2

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the SwinV2 model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['depths'] = get_attr(model, ['depths'], 1)
        kwargs['heads'] = get_attr(model, ['heads'], None)
        kwargs['win'] = get_attr(model, ['win'], None)
