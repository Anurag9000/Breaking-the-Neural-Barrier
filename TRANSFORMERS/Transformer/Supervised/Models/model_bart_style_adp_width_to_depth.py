...
# Load baseline
BASE_PATH = Path(__file__).with_name("model_bart_style.py").resolve()
...
ModelClass = baseline_module.BARTStyle

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the BARTStyle model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['enc_layers'] = get_attr(model, ['enc_layers'], None)
        kwargs['dec_layers'] = get_attr(model, ['dec_layers'], None)
        kwargs['ff'] = get_attr(model, ['ff'], None)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(0, model.layers), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(0, curr_model.layers), v))
            else:
    ...
    return global_best_val, model, 0, model.layers
...
def main():
    ...
    try:
        model = ModelClass(layers=args.depth).to(device)
    except:
