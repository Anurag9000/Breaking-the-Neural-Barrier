...
MAE_BACKBONE_PATH = (
    Path(__file__).resolve().parents[2] / "Self-Supervised" / "Models" / "dae_mae_vit_stl.py"
)
_mae_spec = importlib.util.spec_from_file_location("dae_mae_vit_stl_backbone_runner", MAE_BACKBONE_PATH)
if _mae_spec is None or _mae_spec.loader is None:
    raise ImportError(f"Could not load MAEViT backbone from {MAE_BACKBONE_PATH}")
_mae_backbone = importlib.util.module_from_spec(_mae_spec)
_mae_spec.loader.exec_module(_mae_backbone)
mae_total_neurons = _mae_backbone.mae_total_neurons
MAEViT = _mae_backbone.MAEViT


class SupMAEViTClassifier(nn.Module):
    def __init__(
        self,
        num_classes: int,
        img_size: int = 32,
...
def main() -> None:
    ...
    p.add_argument("--out-dir", type=str, default="runs/dae_mae_vit_sup_stl")
    ...
    neurons = mae_total_neurons(args.embed_dim, args.depth)
