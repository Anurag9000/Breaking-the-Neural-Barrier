...
from utils.text_benchmarks import make_ag_news_semisup_loaders
...
def train_one_epoch(
    ...
    for lab_batch, unlab_batch in zip_longest(labeled_loader, unlabeled_loader):
        ...
        if lab_batch is not None:
            ...
            B, L = ids_lab.shape
            recon_loss_lab = ce(token_logits_lab.view(B * L, -1), ids_lab.view(-1))
        ...
        if unlab_batch is not None:
            ...
            B, L = ids_un.shape
            recon_loss_un = ce(token_logits_un.view(B * L, -1), ids_un.view(-1))
...
def eval_accuracy(model: SupTokenMaskSemiDAE, loader, device: torch.device, mask_token_id: int, mask_prob: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for ids, y in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Semi-supervised token-mask DAE on AG News")
    ...
    p.add_argument("--max-vocab", type=int, default=50000)
    ...
    p.add_argument("--out-dir", type=str, default="runs/dae_tokenmask_text_semisup_agnews")

    args = p.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mask_token_id = args.max_vocab - 1

    lab_loader, unlab_loader, val_loader, test_loader, vocab, num_classes = make_ag_news_semisup_loaders(
    ...
    neurons = sup_token_dae_total_neurons(args.d_model, args.depth, num_classes, len(vocab))
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            val_acc = eval_accuracy(model, val_loader, device, mask_token_id=mask_token_id, mask_prob=args.mask_prob)
    ...
    test_acc = eval_accuracy(model, test_loader, device, mask_token_id=mask_token_id, mask_prob=args.mask_prob)
    report = {
