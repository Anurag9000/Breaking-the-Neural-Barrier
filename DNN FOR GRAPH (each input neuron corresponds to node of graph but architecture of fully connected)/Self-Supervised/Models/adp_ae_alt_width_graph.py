...
@dataclass
class AltCfg:
    ex_k:int=4; delta:float=1e-6; patience_width:int=20; patience_depth:int=20; max_neurons:int=200_000_000; max_width:int=4096

def alternating_width_first(dataset="Cora", init_hidden=64, init_depth=2, seed=42):
...
def main():
    ...
    a=p.parse_args()
    model, v, t = alternating_width_first(a.dataset, a.init_hidden, a.init_depth)
    print(f"[AE ADP Alt Width-first] {a.dataset} ValMSE={v:.6f} TestMSE={t:.6f} H={model.hidden} D={model.depth}")
