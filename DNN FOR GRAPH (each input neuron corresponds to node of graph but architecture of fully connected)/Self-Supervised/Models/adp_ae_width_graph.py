...
class SearchCfg:
    ...
    trials_width:int=100; trials_depth:int=100

def fit_width_then_depth(dataset="Cora", init_hidden=64, init_depth=2, seed=42):
...
def main():
    ...
    a=p.parse_args()
    model, v, t = fit_width_then_depth(a.dataset, a.init_hidden, a.init_depth)
    print(f"[AE ADP Width→Depth] {a.dataset} ValMSE={v:.6f} TestMSE={t:.6f} H={model.hidden} D={model.depth}")
