...
def alternating_width_first(dataset: str = "Cora", init_hidden: int = 64, init_depth: int = 2, seed: int = 42):
    ...
    while True:
        ...
        while wpat > 0:
            ...
            if proj > acfg.max_neurons or (model.hidden + acfg.ex_k) > acfg.max_width:
                break
...
def main():
    ...
    args = p.parse_args()
    model, v, t = alternating_width_first(args.dataset, args.init_hidden, args.init_depth)
    print(f"[ADP-Alt (Width-first)] {args.dataset} final Val={v:.4f} Test@1={t*100:.2f}%  H={model.hidden} D={model.depth}")
