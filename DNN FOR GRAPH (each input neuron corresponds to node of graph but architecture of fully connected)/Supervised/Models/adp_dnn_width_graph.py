...
def fit_width_then_depth(dataset: str = "Cora", init_hidden: int = 64, init_depth: int = 2, seed: int = 42):
    ...
    best_val, test_acc, _ = train_early_stop(model, data, tcfg)
...
def main():
    ...
    args = p.parse_args()
    model, v, t = fit_width_then_depth(args.dataset, args.init_hidden, args.init_depth)
    print(f"[ADP-Width→Depth] {args.dataset} final Val={v:.4f} Test@1={t*100:.2f}%  H={model.hidden} D={model.depth}")
