...
from _common_ucf101 import make_ucf101_loaders
...
def train_epoch(model, loader, opt, crit, device):
    ...
    for frames, lengths, labels in loader:
        frames, lengths, labels = frames.to(device), lengths.to(device), labels.to(device)
...
def main():
    ...
    root = os.environ.get("BBNB_UCF101_ROOT", "./data/UCF101")
    ann = os.environ.get("BBNB_UCF101_ANN", "./data/UCF101/ucfTrainTestlist")
    train_loader, val_loader, test_loader, frames_per_clip, num_classes = make_ucf101_loaders(
        root=root,
        annotation_path=ann,
    ...
    torch.save({"model_state": model.state_dict(), "config": cfg.__dict__, "test_loss": tl, "test_acc": ta}, os.path.join(args.outdir, "convlstm_cls.pt"))
