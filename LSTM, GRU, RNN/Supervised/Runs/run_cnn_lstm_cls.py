...
from cnn_lstm_cls import CNNLSTMClassifier, CNNLSTMConfig
...
def main():
    ...
    ap.add_argument('--conv-channels', type=int, default=128)
    ...
    random.seed(808); torch.manual_seed(808)
    ...
    cfg = CNNLSTMConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, conv_channels=args.conv_channels,
                        conv_kernel=args.conv_kernel, hidden_dim=args.hidden_dim, num_layers=args.num_layers,
                        dropout=args.dropout, num_classes=args.num_classes, pad_idx=args.pad_idx)
    model = CNNLSTMClassifier(cfg).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_cnn_lstm_cls'), 'run_cnn_lstm_cls', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'cnn_lstm_cls.pt'))
