...
from lstm_cls_sap import LSTMSAPClassifier, LSTMSAPConfig
...
def main():
    ...
    random.seed(321); torch.manual_seed(321)
    ...
    cfg = LSTMSAPConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, hidden_dim=args.hidden_dim,
                        num_layers=args.num_layers, dropout=args.dropout, num_classes=args.num_classes,
                        pad_idx=args.pad_idx, bidirectional=False)
    model = LSTMSAPClassifier(cfg, attn_dim=args.attn_dim).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_lstm_cls_sap'), 'run_lstm_cls_sap', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'lstm_cls_sap.pt'))
