...
from lstm_cls_layernorm import LNLSTMClassifier, LNLSTMConfig
...
def main():
    ...
    random.seed(222); torch.manual_seed(222)
    ...
    cfg = LNLSTMConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, hidden_dim=args.hidden_dim,
                       num_layers=args.num_layers, dropout=args.dropout, num_classes=args.num_classes,
                       pad_idx=args.pad_idx)
    model = LNLSTMClassifier(cfg).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_lstm_cls_layernorm'), 'run_lstm_cls_layernorm', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'lstm_cls_layernorm.pt'))
