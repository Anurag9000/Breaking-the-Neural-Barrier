...
from lstm_cls_lstmp import LSTMPClassifier, LSTMPConfig
...
def main():
    ...
    ap.add_argument('--proj-size', type=int, default=256)
    ...
    random.seed(111); torch.manual_seed(111)
    ...
    cfg = LSTMPConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, hidden_dim=args.hidden_dim,
                      proj_size=args.proj_size, num_layers=args.num_layers, dropout=args.dropout,
                      num_classes=args.num_classes, pad_idx=args.pad_idx)
    model = LSTMPClassifier(cfg).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_lstm_cls_lstmp'), 'run_lstm_cls_lstmp', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'lstm_cls_lstmp.pt'))
