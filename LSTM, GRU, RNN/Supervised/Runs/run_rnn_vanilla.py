...
def main():
    ...
    loaders = make_caltech256_loaders(root=root, batch_size=args.batch, seed=args.seed)
    train_loader, val_loader, test_loader, input_dim, num_classes = loaders
    model = RNN_Vanilla(input_dim=input_dim, hidden_size=args.hidden, num_layers=args.layers, num_classes=num_classes)
    train_eval(model, (train_loader, val_loader, test_loader, input_dim, num_classes), device, epochs=args.epochs, lr=args.lr, wd=args.wd, patience=args.patience)
