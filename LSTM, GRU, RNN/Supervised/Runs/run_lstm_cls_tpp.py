...
from lstm_cls_tpp import LSTMTPPClassifier, TPPConfig
...
def main():
    ...
    random.seed(828); torch.manual_seed(828)
    ...
    cfg = TPPConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, hidden_dim=args.hidden_dim,
                    num_layers=args.num_layers, dropout=args.dropout, num_classes=args.num_classes,
                    pad_idx=args.pad_idx, pyramid_levels=(1,2,4))
    model = LSTMTPPClassifier(cfg).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_lstm_cls_tpp'), 'run_lstm_cls_tpp', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'lstm_cls_tpp.pt'))
