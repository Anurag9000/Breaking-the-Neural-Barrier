...
def main():
    ...
    random.seed(717); torch.manual_seed(717)
    ...
    cfg = MILSTMConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, hidden_dim=args.hidden_dim,
    ...
    logger = ContinuousLogger(Path('results_run_lstm_cls_milstm'), 'run_lstm_cls_milstm', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'lstm_cls_milstm.pt'))
