...
from lstm_cls_indylstm import IndyLSTMClassifier, IndyLSTMConfig
...
def main():
    ...
    random.seed(616); torch.manual_seed(616)
    ...
    cfg = IndyLSTMConfig(vocab_size=args.vocab_size, emb_dim=args.emb_dim, hidden_dim=args.hidden_dim,
                         num_layers=args.num_layers, dropout=args.dropout, num_classes=args.num_classes,
                         pad_idx=args.pad_idx)
    model = IndyLSTMClassifier(cfg).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_lstm_cls_indylstm'), 'run_lstm_cls_indylstm', 'train')
    ...
    torch.save({'model_state': model.state_dict(), 'config': cfg.__dict__,
                'test_loss': tl, 'test_acc': ta}, os.path.join(args.outdir, 'lstm_cls_indylstm.pt'))
