...
# Load baseline
BASE_PATH = Path(__file__).with_name("rnn_vanilla.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.RNN_Vanilla

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the RNN_Vanilla model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        # RNN_Vanilla specific
        kwargs['input_dim'] = get_attr(model, ['input_dim', 'rnn.input_size'], 96) # Default 96 for Cifar
        kwargs['hidden_size'] = width
        kwargs['num_layers'] = depth
        kwargs['num_classes'] = get_attr(model, ['num_classes', 'head.out_features'], 10)
        kwargs['dropout'] = get_attr(model, ['dropout', 'rnn.dropout'], 0.1)
        
        new_model = ModelClass(**kwargs).to(device)
    except Exception as e:
        print(f'Rebuild failed: {e}')
        # Fallback for generic models if keys differ
        try:
             new_model = ModelClass(dim=width, num_layers=depth).to(device)
        except:
             return None
...
# Copy make_loaders from run_rnn_vanilla.py
def make_loaders(batch_size=128, num_workers=0): # workers 0 for compat
    from torchvision import datasets, transforms
...
def main():
    ...
    p.add_argument("--max-epochs", type=int, default=20) # Lower default for testing
    args = p.parse_args()
    ...
    try:
        # Instantiate initial model correct args for RNN_Vanilla
        # input_dim=96 (32*3), num_classes=10
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    # Support 'hidden_size' (RNN_Vanilla) or 'dim'
    cur = getattr(model.rnn, 'hidden_size', model.dim if hasattr(model, 'dim') else cfg.max_width)
    nxt = min(max_width, cur + cfg.ex_k)
    if nxt <= cur: return None
    # Assuming depth is maintained
    depth = model.rnn.num_layers if hasattr(model, 'rnn') else model.num_layers
...
def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    # Support 'num_layers'
    cur = model.rnn.num_layers if hasattr(model, 'rnn') else model.num_layers
    if cur >= max_depth: return None
    # Assuming width is maintained
    width = getattr(model.rnn, 'hidden_size', model.dim if hasattr(model, 'dim') else cfg.max_width)
...
def snapshot_arch_and_state(model: ModelClass, state_dict=None) -> Dict[str, Any]:
    ...
    # Handle RNN_Vanilla attributes specifically
    width = getattr(model.rnn, 'hidden_size', getattr(model, 'dim', 0))
    depth = getattr(model.rnn, 'num_layers', getattr(model, 'num_layers', 0))
    return {
        "width": width,
        "depth": depth,
...
def restore_arch_and_state(model: ModelClass, snap: Dict[str, Any], device) -> ModelClass:
    ...
    # We rely on rebuild_model to handle the specific init arguments extraction from 'model' (passed as template)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.dim, model.num_layers), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.dim, curr_model.num_layers), v))
            else:
