...
class TextLSTM(nn.Module):
    def __init__(
        ...
        self.lstm = _make_lstm(emb_dim, hidden_size, num_layers, bidirectional, dropout)
    ...
    @staticmethod
    def from_snapshot(snap: Dict) -> "TextLSTM":
        arch = snap["arch"]
        model = TextLSTM(**arch)
    ...
    def forward(self, x, lengths=None):
        ...
        # Optional: pack padded sequences for speed, if lengths provided
...
class ADP_LSTM_Search:
    def __init__(self, model: TextLSTM, train_cfg: TrainCfg, search_cfg: SearchCfg, device: Optional[torch.device]=None):
        self.model = model
    ...
    def _train_budgeted(self, model: TextLSTM, train_loader, val_loader) -> float:
    ...
    def _grow_width(self, model: TextLSTM, k: int) -> TextLSTM:
        snap = model.snapshot()
        model.widen_hidden(k)
        return model, snap

    def _grow_depth(self, model: TextLSTM) -> TextLSTM:
        snap = model.snapshot()
        model.append_layer()
        return model, snap
    ...
    def search_width_only(self, train_loader, val_loader):
        ...
        while self._budget_left() > 0 and failures < self.search_cfg.trials_width:
            ...
            if self._accept(base_val, prop_val):
                ...
                self.model = TextLSTM.from_snapshot(base_snap)
                failures += 1
        return self.model
            ...
            if self._accept(base_val, prop_val):
                ...
                while self._budget_left() > 0 and d_fails < self.search_cfg.trials_depth:
                    ...
                    if self._accept(base_val, prop2_val):
                        ...
                        self.model = TextLSTM.from_snapshot(series_snap)
                        d_fails += 1
            else:
                self.model = TextLSTM.from_snapshot(base_snap)
                w_fails += 1
