...
class TextGRU(nn.Module):
    def __init__(
        ...
        self.gru = _make_gru(emb_dim, hidden_size, num_layers, bidirectional, dropout)
    ...
    @staticmethod
    def from_snapshot(snap: Dict) -> "TextGRU":
        arch = snap["arch"]
        model = TextGRU(**arch)
...
class ADP_GRU_Search:
    def __init__(self, model: TextGRU, train_cfg: TrainCfg, search_cfg: SearchCfg, device: Optional[torch.device]=None):
        self.model = model
    ...
    def _train_budgeted(self, model: TextGRU, train_loader, val_loader) -> float:
    ...
    def search_width_only(self, train_loader, val_loader):
        ...
        while self._budget_left() > 0 and failures < self.search_cfg.trials_width:
            ...
            if self._accept(base_val, prop_val):
                ...
                self.model = TextGRU.from_snapshot(base_snap)
                failures += 1
        return self.model
            ...
            if self._accept(base_val, prop_val):
                ...
                while self._budget_left() > 0 and d_fails < self.search_cfg.trials_depth:
                    ...
                    if self._accept(base_val, prop2_val):
                        ...
                        self.model = TextGRU.from_snapshot(series_snap)
                        d_fails += 1
            else:
                self.model = TextGRU.from_snapshot(base_snap)
                w_fails += 1
