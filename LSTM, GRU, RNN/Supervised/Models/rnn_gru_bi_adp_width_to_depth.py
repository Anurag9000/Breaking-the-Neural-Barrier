...
# Load baseline
BASE_PATH = Path(__file__).with_name("rnn_gru_bi.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.RNN_GRU_Bi

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the RNN_GRU_Bi model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        # Bidirectional is fixed to True in model definition, but we can pass it if init supports it?
        # Looking at rnn_gru_bi.py: __init__ does NOT take bidirectional arg. It sets it to True inside.
        # But wait, does it?
        # line 6: def __init__(self, input_dim: int, hidden_size: int, num_layers: int, num_classes: int, dropout: float = 0.1):
        # line 14: bidirectional=True
        # So we do NOT pass bidirectional.
...
def main():
    ...
    try:
        # RNN_GRU_Bi: input_dim 96, hidden_size, num_layers, num_classes=10
