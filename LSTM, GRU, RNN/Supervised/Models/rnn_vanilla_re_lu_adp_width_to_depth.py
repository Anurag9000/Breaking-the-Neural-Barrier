...
ModelClass = baseline_module.RNN_Vanilla_ReLU

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the RNN_Vanilla_ReLU model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        # RNN_Vanilla_ReLU specific
        kwargs['input_dim'] = get_attr(model, ['input_dim', 'rnn.input_size'], 96)
...
def main():
    ...
    p.add_argument("--max-epochs", type=int, default=10) # Lower default for testing
    args = p.parse_args()
    ...
    try:
        # Instantiate initial model correct args for RNN_Vanilla_ReLU
