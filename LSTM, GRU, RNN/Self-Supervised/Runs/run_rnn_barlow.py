...
from rnn_barlow import BarlowGRU

class ToyBarlow(Dataset):
...
def main():
    ...
    ap.add_argument('--save', type=str, default='barlow_gru_best.pt')
    args = ap.parse_args()

    random.seed(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)

    ds = ToyBarlow(args.n, args.T, args.D, args.seed)
    ...
    net = BarlowGRU(args.D, args.hidden, args.proj, args.layers).to(device)
    ...
    logger = ContinuousLogger(Path('results_run_rnn_barlow'), 'run_rnn_barlow', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for x in tr:
            ...
            x1,x2 = augment(x)
            z1 = net(x1); z2 = net(x2)
            loss = BarlowGRU.barlow_loss(z1,z2,lambd=5e-3)
