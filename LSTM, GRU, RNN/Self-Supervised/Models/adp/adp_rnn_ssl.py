...
class RNNLanguageModel(nn.Module):
    ...
    def __init__(
        ...
        self.rnn = _make_rnn(emb_dim, hidden_size, num_layers, bidirectional=False, dropout=dropout, nonlinearity=nonlinearity)
    ...
    @staticmethod
    def from_snapshot(snap: Dict) -> "RNNLanguageModel":
        arch = snap["arch"]
        model = RNNLanguageModel(**arch)
...
class ADP_RNN_SSL:
    def __init__(self, model: RNNLanguageModel, train_cfg: TrainCfg, search_cfg: SearchCfg, device: Optional[torch.device]=None, pad_idx: int=0):
        self.model = model
    ...
    def _train_budgeted(self, model: RNNLanguageModel, train_loader, val_loader) -> float:
    ...
    def search_width_only(self, train_loader, val_loader):
        ...
        while self._budget_left() > 0 and failures < self.search_cfg.trials_width:
            ...
            if self._accept(base_val, prop_val):
                ...
                self.model = RNNLanguageModel.from_snapshot(base_snap)
                failures += 1
        return self.model
            ...
            if self._accept(base_val, prop_val):
                ...
                while self._budget_left() > 0 and d_fails < self.search_cfg.trials_depth:
                    ...
                    if self._accept(base_val, prop2_val):
                        ...
                        self.model = RNNLanguageModel.from_snapshot(series_snap)
                        d_fails += 1
            else:
                self.model = RNNLanguageModel.from_snapshot(base_snap)
                w_fails += 1
