...
def _make_gru(input_size: int, hidden_size: int, num_layers: int, bidirectional: bool=False, dropout: float=0.0) -> nn.GRU:
    return nn.GRU(
...
def _resize_gru_hidden(old: nn.GRU, new_hidden: int) -> nn.GRU:
    # Return a new GRU with resized hidden_size, overlap-copying the weights/biases.
    new = _make_gru(
...
def _append_gru_layer(old: nn.GRU) -> nn.GRU:
    # Return a new GRU with num_layers+1, overlap-copying existing layers and
...
class GRULanguageModel(nn.Module):
    ...
    def __init__(
        ...
        self.gru = _make_gru(emb_dim, hidden_size, num_layers, bidirectional=False, dropout=dropout)
    ...
    def widen_hidden(self, delta: int):
        ...
        self.gru = _resize_gru_hidden(self.gru, self.hidden_size)
        ...
        if isinstance(self.proj, nn.Identity) and self.emb_dim != self.hidden_size:
            ...
            self.proj = nn.Linear(self.hidden_size, self.proj.out_features, bias=False)
    ...
    def append_layer(self):
        self.num_layers += 1
        self.gru = _append_gru_layer(self.gru)
    ...
    @staticmethod
    def from_snapshot(snap: Dict) -> "GRULanguageModel":
        arch = snap["arch"]
        model = GRULanguageModel(**arch)
    ...
    def forward(self, tokens):
        ...
        out = self.proj(out)          # (B, T, E) if tied
...
class ADP_GRU_SSL:
    def __init__(self, model: GRULanguageModel, train_cfg: TrainCfg, search_cfg: SearchCfg, device: Optional[torch.device]=None, pad_idx: int=0):
        self.model = model
    ...
    def _train_budgeted(self, model: GRULanguageModel, train_loader, val_loader) -> float:
    ...
    def search_width_only(self, train_loader, val_loader):
        ...
        while self._budget_left() > 0 and failures < self.search_cfg.trials_width:
            ...
            if self._accept(base_val, prop_val):
                ...
                self.model = GRULanguageModel.from_snapshot(base_snap)
                failures += 1
        return self.model
            ...
            if self._accept(base_val, prop_val):
                ...
                while self._budget_left() > 0 and d_fails < self.search_cfg.trials_depth:
                    ...
                    if self._accept(base_val, prop2_val):
                        ...
                        self.model = GRULanguageModel.from_snapshot(series_snap)
                        d_fails += 1
            else:
                self.model = GRULanguageModel.from_snapshot(base_snap)
                w_fails += 1
