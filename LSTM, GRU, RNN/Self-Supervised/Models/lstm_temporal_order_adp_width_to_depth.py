...
# Load baseline
BASE_PATH = Path(__file__).with_name("lstm_temporal_order.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.OrderDataset

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the OrderDataset model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['sequences'] = get_attr(model, ['sequences'], None)
        kwargs['max_len'] = get_attr(model, ['max_len'], None)
        kwargs['segments'] = get_attr(model, ['segments'], None)
        kwargs['shuffle_prob'] = get_attr(model, ['shuffle_prob'], None)
        kwargs['seed'] = get_attr(model, ['seed'], None)
