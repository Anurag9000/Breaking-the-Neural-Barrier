...
# Load baseline
BASE_PATH = Path(__file__).with_name("lstm_siamese_triplet_py_siamese_contrastive_triplet_single_encoder.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.SiameseDataset

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the SiameseDataset model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['augment'] = get_attr(model, ['augment'], None)
