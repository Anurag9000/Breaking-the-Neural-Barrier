...
# - Heads: 2 (PQ, VM) depth-outer/width-inner with per-attempt rollback; conflicts with forward-only spec.
# - Deviations vs updated ADP_algorithms.md: rolls back on failed expansions instead of marching forward and restoring best at end.
...
class ADPDepth_2Head(nn.Module):
    """ADP_DEPTH_OUTER_WIDTH_INNER with forward-only expansions (PQ, VM)."""
...
# - Uses train_with_early_stopping (patience_es) and snapshot_arch_and_state / restore_arch_and_state; expand_width/expand_depth adjust hidden stack and both heads.
