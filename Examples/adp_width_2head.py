...
# - Heads: 2 (PQ, VM via ADPBase_2Head/DEN_2Heads).
# - Mode: width-outer/depth-inner with per-attempt rollback; conflicts with forward-only spec.
# - Deviations vs ADP_algorithms.md (updated): rollback on failed expansions; should keep marching and only restore global best at end.
...
class ADPWidth_2Head(nn.Module):
    """ADP_WIDTH_OUTER_DEPTH_INNER with forward-only expansions (PQ, VM)."""
