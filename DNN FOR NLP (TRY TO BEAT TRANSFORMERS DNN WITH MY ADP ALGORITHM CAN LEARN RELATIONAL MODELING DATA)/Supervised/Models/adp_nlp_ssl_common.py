...
class AdaptiveTextSSL(nn.Module):
    ...
    def _build_modules(self):
        ...
        self.proj = nn.Sequential(
            nn.Linear(self.rep_dim, self.proj_dim),
    ...
    def total_neurons(self): return sum(self.hidden) + self.rep_dim + self.proj_dim
    def depth(self): return len(self.hidden) + 2  # backbone + rep->proj

    def snapshot(self) -> Dict:
        return {"state": {k: v.detach().cpu() for k, v in self.state_dict().items()},
                "hidden": list(self.hidden), "rep_dim": int(self.rep_dim), "proj_dim": int(self.proj_dim)}
    def restore(self, snap: Dict):
        self.hidden = list(snap["hidden"]); self.rep_dim = int(snap["rep_dim"]); self.proj_dim = int(snap["proj_dim"])
    ...
    def append_depth(self):
        ...
        old = {"backbone": self.backbone, "rep": self.rep, "proj": self.proj}
        self._build_modules()
        ...
        # proj is a 2-layer MLP; resize the first Linear's input if rep_dim changed
        self.proj[0] = _resize_linear(old["proj"][0], self.proj[0].in_features, self.proj[0].out_features)
        self.proj[2] = _resize_linear(old["proj"][2], self.proj[2].in_features, self.proj[2].out_features)

    def widen_all(self, ex_k: int):
        self.hidden = [int(w + ex_k) for w in self.hidden]
        self.rep_dim = int(self.rep_dim + ex_k)
        self.proj_dim = int(self.proj_dim + ex_k)
        old = {"backbone": self.backbone, "rep": self.rep, "proj": self.proj}
        self._build_modules()
        for i, (old_blk, new_blk) in enumerate(zip(old["backbone"], self.backbone)):
            new_blk.linear = _resize_linear(old_blk.linear, new_blk.linear.in_features, new_blk.linear.out_features)
