...
# - This file is newly created to implement the ADP algorithms from scratch for the MLPTextAE model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['vocab_size'] = get_attr(model, ['vocab_size', 'tok.num_embeddings', 'encoder.tok.num_embeddings'], 1)
        kwargs['emb_dim'] = width
        kwargs['hidden'] = get_attr(model, ['hidden'], None)
        kwargs['rep_dim'] = get_attr(model, ['rep_dim'], 1)
        kwargs['use_bn'] = get_attr(model, ['use_bn'], None)
...
def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, getattr(model, 'emb_dim', 64) if 'emb_dim' != 'None' else 64, new_d, device, cfg)
...
def snapshot_arch_and_state(model: ModelClass, state_dict=None) -> Dict[str, Any]:
    ...
    return {
        "width": getattr(model, 'emb_dim', 0) if 'emb_dim' != 'None' else 0,
...
def main():
    ...
    dl_train, dl_val, _, vocab = make_ag_news_bow_autoencoder_loaders(batch_size=64, max_len=256, seed=0, num_workers=0)
