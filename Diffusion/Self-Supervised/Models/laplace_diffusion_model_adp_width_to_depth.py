...
# Load baseline
BASE_PATH = Path(__file__).with_name("laplace_diffusion_model.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.LaplaceDiffusion

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the LaplaceDiffusion model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['b_min'] = get_attr(model, ['b_min'], None)
        kwargs['b_max'] = get_attr(model, ['b_max'], None)
