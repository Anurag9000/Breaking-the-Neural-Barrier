...
from Diffusion._common_real_benchmark import make_real_diffusion_smoke_loaders
...
# Load baseline
BASE_PATH = Path(__file__).with_name("score_sde_unified.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.ScoreSDEUnified

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ScoreSDEUnified model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['sde_type'] = get_attr(model, ['sde_type'], None)
        kwargs['sigma_min'] = get_attr(model, ['sigma_min'], None)
        kwargs['sigma_max'] = get_attr(model, ['sigma_max'], None)
        kwargs['T'] = get_attr(model, ['T'], None)
        kwargs['base'] = get_attr(model, ['base'], None)
        kwargs['channels'] = get_attr(model, ['channels'], 3)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    cur = model.base if hasattr(model, 'base') else cfg.max_width # fallback
    nxt = min(max_width, cur + cfg.ex_k)
    if nxt <= cur: return None
    return rebuild_model(model, nxt, infer_adp_depth(model), device, cfg)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.base, 0), best_val))

    def can_widen(m: ModelClass) -> bool:
        return m.base < acfg.max_width
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.base, 0), v))
            else:
    ...
    return global_best_val, model, model.base, 0
...
def main():
    ...
    from Diffusion._common_real_benchmark import make_real_diffusion_smoke_loaders
    dl_train, dl_val, dl_test, _ = make_real_diffusion_smoke_loaders(batch_size=8)
    
    try:
        model = ModelClass(base=args.width).to(device)
    except:
