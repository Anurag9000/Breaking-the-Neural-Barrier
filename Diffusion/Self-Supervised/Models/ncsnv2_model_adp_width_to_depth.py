...
# Load baseline
BASE_PATH = Path(__file__).with_name("ncsnv2_model.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.NCSNv2

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the NCSNv2 model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['sigmas'] = get_attr(model, ['sigmas'], None)
        kwargs['base'] = get_attr(model, ['base'], None)
        kwargs['channels'] = get_attr(model, ['channels'], 3)
        kwargs['spectral_norm'] = get_attr(model, ['spectral_norm'], None)
