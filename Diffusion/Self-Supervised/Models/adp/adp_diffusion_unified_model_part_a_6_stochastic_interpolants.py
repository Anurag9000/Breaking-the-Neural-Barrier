"""
ADP Diffusion – Unified Model (Part A6: Stochastic Interpolants)
Six ADP growth algorithms (w2d, d2w, alt_w, alt_d, width_only, depth_only)
overb a width/depth‑editable UNet that learns the **velocity field** along a
stochastic interpolant between data and noise.

BNB rule: single‑model end‑to‑end; no EMA/teacher.

Interpolant (simple, general):
  x_t = (1 - t) * x0 + t * z + s(t) * w,   with  t ~ U(0,1)
  z ~ N(0, I) (Gaussian noise image),  w ~ N(0, I) (stochastic perturbation)
Velocity target (ground truth):
  \dot{x}_t = (z - x0) + s'(t) * w
We regress f_θ(x_t, t) to \dot{x}_t (MSE). This is **teacher‑free** and unifies
flows/diffusion via choice of s(t).

Two schedules (choose via CLI):
  • poly:   s(t) = sigma_max * t^γ          ⇒ s'(t) = sigma_max * γ * t^{γ-1}
  • softp:  s(t) = sigma_max * (t / (1 + α (1-t)))  (softplus‑like growth)
            s'(t) = derivative closed form implemented below.
"""
from __future__ import annotations
import math
from dataclasses import dataclass
from typing import Dict, Optional

import torch
import torch.nn as nn
import torch.nn.functional as F

# -----------------------------
# Overlap‑copy transplant utils
# -----------------------------

def _copy_overlap_(dst: torch.nn.Parameter, src: torch.nn.Parameter):
    with torch.no_grad():
        sd, ss = list(dst.shape), list(src.shape)
        sl = [min(a, b) for a, b in zip(sd, ss)]
        slices = tuple(slice(0, n) for n in sl)
        dst.zero_(); dst[slices].copy_(src[slices])

def transplant_module(dst: nn.Module, src: nn.Module):
    with torch.no_grad():
        for (dn, dpar), (sn, spar) in zip(dst.named_parameters(), src.named_parameters()):
            try: _copy_overlap_(dpar, spar)
            except Exception: pass
        for (dn, db), (sn, sb) in zip(dst.named_buffers(), src.named_buffers()):
            if db.shape == sb.shape: db.copy_(sb)

# -----------------------------
# UNet‑like backbone that outputs velocity f(x_t, t)
# -----------------------------

class ConvBNAct(nn.Module):
    def __init__(self, cin, cout, k=3, s=1, p=1):
        super().__init__(); self.conv = nn.Conv2d(cin, cout, k, s, p, bias=False); self.bn = nn.BatchNorm2d(cout); self.act = nn.SiLU(inplace=True)
    def forward(self, x): return self.act(self.bn(self.conv(x)))

class ResBlock(nn.Module):
    def __init__(self, c):
        super().__init__(); self.c1 = ConvBNAct(c, c); self.c2 = ConvBNAct(c, c)
    def forward(self, x): return x + self.c2(self.c1(x))

class Down(nn.Module):
    def __init__(self, cin, cout):
        super().__init__(); self.conv = ConvBNAct(cin, cout, 3, 2, 1)
    def forward(self, x): return self.conv(x)

class Up(nn.Module):
    def __init__(self, cin, cout):
        super().__init__(); self.up = nn.ConvTranspose2d(cin, cout, 2, 2); self.post = ConvBNAct(cout, cout)
    def forward(self, x): return self.post(self.up(x))

class TimeEmbed(nn.Module):
    def __init__(self, c):
        super().__init__(); self.fc1 = nn.Linear(1, c); self.fc2 = nn.Linear(c, c); self.act = nn.SiLU()
    def forward(self, t):
        h = t.view(-1,1); h = self.act(self.fc1(h)); return self.fc2(h)

class UNetSI(nn.Module):
    """UNet predicting velocity f_theta(x_t, t), with width/depth growth ops."""
    def __init__(self, in_channels=3, base=32, stages=3, blocks_per_stage=1, out_channels=None):
        super().__init__()
        self.in_channels, self.base, self.stages, self.blocks_per_stage = in_channels, base, stages, blocks_per_stage
        self.out_channels = out_channels or in_channels
        self.time_emb = TimeEmbed(base)
        self.in_conv = ConvBNAct(in_channels, base)
        chans = [base*(2**i) for i in range(stages)]
        self.downs = nn.ModuleList(); self.res_down = nn.ModuleList(); c = base
        for i in range(stages):
            self.res_down.append(nn.Sequential(*[ResBlock(c) for _ in range(blocks_per_stage)]))
            if i < stages-1: self.downs.append(Down(c, c*2)); c *= 2
            else: self.downs.append(nn.Identity())
        self.ups = nn.ModuleList(); self.res_up = nn.ModuleList()
        for i in reversed(range(stages)):
            self.res_up.append(nn.Sequential(*[ResBlock(chans[i]) for _ in range(blocks_per_stage)]))
            if i>0: self.ups.append(Up(chans[i], chans[i-1]))
            else: self.ups.append(nn.Identity())
        self.out = nn.Conv2d(base, self.out_channels, 1)

    # growth ops
    def append_depth(self):
        new = UNetSI(self.in_channels, self.base, self.stages, self.blocks_per_stage+1, self.out_channels)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetSI(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage, self.out_channels)
        transplant_module(new, self); return new

    def forward(self, x, t):
        emb = self.time_emb(t)
        h = self.in_conv(x)
        h = h + emb.view(emb.size(0), -1, 1, 1)
        skips = []; c = h
        for i in range(self.stages):
            c = self.res_down[i](c); skips.append(c); c = self.downs[i](c)
        for i in range(self.stages):
            j = self.stages-1-i
            c = self.res_up[i](c + skips[j]); c = self.ups[i](c)
        return self.out(c)

# -----------------------------
# Stochastic Interpolant objective
# -----------------------------

@dataclass
class SICfg:
    schedule: str = 'poly'     # 'poly' | 'softp'
    sigma_max: float = 1.0
    gamma: float = 1.5         # for poly
    alpha: float = 2.0         # for softp
    device: str = 'cuda'

    def s(self, t: torch.Tensor):
        if self.schedule == 'poly':
            return self.sigma_max * torch.pow(t.clamp(1e-5, 1-1e-5), self.gamma)
        # softplus‑like growth t / (1 + α(1−t)) scaled by sigma_max
        num = t
        den = 1.0 + self.alpha * (1.0 - t)
        return self.sigma_max * (num / den)

    def sp(self, t: torch.Tensor):
        if self.schedule == 'poly':
            t_ = t.clamp(1e-5, 1-1e-5)
            return self.sigma_max * self.gamma * torch.pow(t_, self.gamma - 1.0)
        # derivative of num/den: (den*1 - num*(-α)) / den^2 = (den + α*num)/den^2
        num = t; den = 1.0 + self.alpha * (1.0 - t)
        return self.sigma_max * (den + self.alpha * num) / (den * den)


def si_loss(model: UNetSI, x0: torch.Tensor, cfg: SICfg, rng: torch.Generator):
    B = x0.size(0); device = x0.device
    t = torch.rand(B, device=device, generator=rng) * 0.99 + 0.005
    # base noise and stochastic perturbation
    z = torch.randn_like(x0, generator=rng)
    w = torch.randn_like(x0, generator=rng)
    s = cfg.s(t).view(-1,1,1,1)
    sp = cfg.sp(t).view(-1,1,1,1)
    x_t = (1.0 - t).view(-1,1,1,1) * x0 + t.view(-1,1,1,1) * z + s * w
    v_star = (z - x0) + sp * w
    pred = model(x_t, t)
    return F.mse_loss(pred, v_star)

# -----------------------------
# Trainer + Early stop
# -----------------------------

@dataclass
class TrainCfg:
    lr: float = 2e-4
    weight_decay: float = 1e-4
    max_epochs: int = 50
    patience: int = 5
    grad_clip: float = 1.0
    device: str = 'cuda'
    seed: int = 42

class EarlyStopper:
    def __init__(self, patience: int):
        self.patience = patience; self.best = math.inf; self.bad = 0
        self.chk: Optional[Dict[str, torch.Tensor]] = None
    def step(self, value: float, model: nn.Module):
        if value < self.best:
            self.best = value; self.bad = 0
            self.chk = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
        else:
            self.bad += 1
    def should_stop(self): return self.bad > self.patience


def train_one(model: UNetSI, train_loader, val_loader, tcfg: TrainCfg, scfg: SICfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 2718)
    opt = torch.optim.AdamW(model.parameters(), lr=tcfg.lr, weight_decay=tcfg.weight_decay)
    stopper = EarlyStopper(tcfg.patience)
    for epoch in range(tcfg.max_epochs):
        model.train()
        for x, _ in train_loader:
            x = x.to(device)
            loss = si_loss(model, x, scfg, rng)
            opt.zero_grad(set_to_none=True); loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), tcfg.grad_clip)
            opt.step()
        model.eval(); vloss=0.0; n=0
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += si_loss(model, x, scfg, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
        stopper.step(vloss, model)
        if stopper.should_stop(): break
    if stopper.chk is not None: model.load_state_dict(stopper.chk)
    return stopper.best, {k: v.clone() for k, v in model.state_dict().items()}

# -----------------------------
# ADP search (six algorithms)
# -----------------------------

from dataclasses import dataclass

@dataclass
class SearchCfg:
    max_depth: int = 8
    max_base: int = 192
    trials_width: int = 2
    trials_depth: int = 2
    delta: float = 1e-4

def accept(new_val, old_val, delta): return new_val < (old_val - delta)

def _run(prop, tr, va, tcfg, sicfg): return train_one(prop, tr, va, tcfg, sicfg)

def adp_width_to_depth(model, tr, va, tcfg, sicfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, sicfg)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: w_fail+=1
        if not improved: break
        improved=False
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_to_width(model, tr, va, tcfg, sicfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, sicfg)
    d_fail = w_fail = 0
    while True:
        improved=False
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: w_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_alt_depth(model, tr, va, tcfg, sicfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, sicfg)
    while True:
        any_acc=False
        f=0
        while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; any_acc=True
            else: f+=1
        f=0
        while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; any_acc=True
            else: f+=1
        if not any_acc: break
    model.load_state_dict(best_sd); return model, best_val

def adp_alt_width(model, tr, va, tcfg, sicfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, sicfg)
    while True:
        any_acc=False
        f=0
        while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; any_acc=True
            else: f+=1
        f=0
        while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,sicfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; any_acc=True
            else: f+=1
        if not any_acc: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_only(model, tr, va, tcfg, sicfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, sicfg)
    f=0
    while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
        prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,sicfg)
        if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd
        else: f+=1
    model.load_state_dict(best_sd); return model, best_val

def adp_width_only(model, tr, va, tcfg, sicfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, sicfg)
    f=0
    while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
        prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,sicfg)
        if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd
        else: f+=1
    model.load_state_dict(best_sd); return model, best_val

ADP_REGISTRY = {
    'w2d': adp_width_to_depth,
    'd2w': adp_depth_to_width,
    'alt_d': adp_alt_depth,
    'alt_w': adp_alt_width,
    'depth_only': adp_depth_only,
    'width_only': adp_width_only,
}


def build_model(in_channels=3, base=32, stages=3, blocks=1, out_channels=None) -> UNetSI:
    return UNetSI(in_channels, base, stages, blocks, out_channels or in_channels)
