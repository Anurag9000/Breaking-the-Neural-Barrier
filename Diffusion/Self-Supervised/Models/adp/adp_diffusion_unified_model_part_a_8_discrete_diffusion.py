"""
ADP Diffusion – Unified Model (Part A8: Discrete Diffusion)
Six ADP growth algorithms over a width/depth‑editable UNet that predicts
per‑pixel **categorical logits** (K bins) for a masked discrete corruption
process (teacher‑free D3PM‑style masked channel, simplified).

Forward (masking):
- Quantize x0 ∈ [0,1] to K bins → y ∈ {0..K-1}.
- Sample mask m ~ Bernoulli(t) with t ~ U(0,1).
- Form x_t by replacing masked pixels with a special [MASK] token index K.
Model predicts logits over K classes for all pixels; loss is cross‑entropy on
...
# -----------------------------
# UNet backbone producing K‑way logits per pixel (shared across channels)
# -----------------------------
...
class UNetDiscrete(nn.Module):
    def __init__(self, in_channels=3, base=32, stages=3, blocks_per_stage=1, K=256):
        super().__init__()
        self.in_channels, self.base, self.stages, self.blocks_per_stage, self.K = in_channels, base, stages, blocks_per_stage, K
        ...
        # logits for K classes (shared across rgb by using 1x1 conv to K per pixel per channel then average)
        self.out = nn.Conv2d(base, K, 1)

    # growth ops
    def append_depth(self):
        new = UNetDiscrete(self.in_channels, self.base, self.stages, self.blocks_per_stage+1, self.K)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetDiscrete(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage, self.K)
        transplant_module(new, self); return new
...
def quantize_uint(x: torch.Tensor, K: int):
    # x in [0,1]; return long indices ∈ [0, K-1]
    xq = (x.clamp(0,1) * (K - 1) + 0.5).long()
    return xq


def mask_corrupt(y: torch.Tensor, t: torch.Tensor, K: int):
    # y: (B,C,H,W) long in [0,K-1]; output masked indices ∈ [0,K] with K as MASK
    B, C, H, W = y.shape
    mprob = t.view(-1,1,1,1)
    mask = (torch.rand_like(y.float()) < mprob).long()
    y_masked = y.clone()
    y_masked[mask.bool()] = K
    return y_masked, mask


def disc_loss(model: UNetDiscrete, x0: torch.Tensor, cfg: DiscCfg, rng: torch.Generator):
    B, C, H, W = x0.shape; device = x0.device
    t = torch.rand(B, device=device, generator=rng) * 0.99 + 0.01
    y = quantize_uint((x0 + 1) * 0.5, cfg.K)  # map [-1,1] -> [0,1]
    y_masked, mask = mask_corrupt(y, t, cfg.K)
    # convert masked indices to one‑hot planes (K+1) for conditioning
    onehot = F.one_hot(y_masked.clamp(0, cfg.K), num_classes=cfg.K+1).permute(0,4,1,2,3)  # not efficient; compactify
    # Instead, embed as continuous image by mapping indices back to [0,1]
    x_in = (y_masked.clamp(0, cfg.K-1).float() / (cfg.K-1)).unsqueeze(1).repeat(1,C,1,1)
    logits = model(x_in, t)
    # cross‑entropy on masked pixels for each channel (average across channels)
    loss = 0.0
    for c in range(C):
        target = y[:,c]
...
def train_one(model: UNetDiscrete, train_loader, val_loader, tcfg: TrainCfg, dcfg: DiscCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 4242)
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, _ in train_loader:
            x = x.to(device)
            loss = disc_loss(model, x, dcfg, rng)
        ...
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += disc_loss(model, x, dcfg, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def _run(prop, tr, va, tcfg, dcfg): return train_one(prop, tr, va, tcfg, dcfg)

# (implementations identical structure to previous parts)

def adp_width_to_depth(model, tr, va, tcfg, dcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, dcfg)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,dcfg)
        ...
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,dcfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
...
def build_model(in_channels=3, base=32, stages=3, blocks=1, K=256) -> UNetDiscrete:
    return UNetDiscrete(in_channels, base, stages, blocks, K)
