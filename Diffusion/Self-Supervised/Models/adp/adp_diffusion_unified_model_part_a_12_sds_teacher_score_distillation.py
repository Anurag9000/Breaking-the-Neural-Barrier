"""
ADP Diffusion – Unified Model (Part A12: SDS – Teacher Score Distillation)
Six ADP growth algorithms around a UNet **student** trained to match a
frozen **teacher score/ε** via SDS‑style regression. This is the only part
that (optionally) uses an external frozen teacher; for smoke tests you can use
--teacher rand to avoid any external weights.

Loss (teacher‑free fallback: rand):
  Sample t, form x_t = √ᾱ_t x0 + √(1−ᾱ_t) ε.
  Teacher returns ε_T(x_t, t) (or score). Student predicts ε_S(x_t, t).
  L = E[ w(t) * || ε_S − stopgrad(ε_T) ||² ].

This keeps single‑model ADP search; the teacher is never updated.
"""
...
class UNetSDS(nn.Module):
    ...
    def append_depth(self):
        new = UNetSDS(self.in_channels, self.base, self.stages, self.blocks_per_stage+1)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetSDS(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage)
        transplant_module(new, self); return new
...
class Teacher:
    def __init__(self, mode: str = 'rand'):
        self.mode = mode
    @torch.no_grad()
    def epsilon(self, x_t: torch.Tensor, t: torch.Tensor):
        if self.mode == 'rand':
            g = torch.Generator(device=x_t.device)
            return torch.randn_like(x_t, generator=g)
        else:
            raise NotImplementedError('Provide a real teacher for mode=%s' % self.mode)
...
@dataclass
class SDSCfg:
    teacher_mode: str = 'rand'   # 'rand' or custom
    T: int = 1000
    w_exp: float = 0.5           # w(t) = (ᾱ_t)^w_exp
    device: str = 'cuda'


def sds_loss(student: UNetSDS, teacher: Teacher, x0: torch.Tensor, cfg: SDSCfg, betas, alphas, ac, rng: torch.Generator):
    ...
    tt = (t.float() + 1) / cfg.T
    with torch.no_grad():
        eps_T = teacher.epsilon(xt, tt).detach()
    eps_S = student(xt, tt)
    w = (ac[t] ** cfg.w_exp).view(-1,1,1,1)
    return (w * (eps_S - eps_T)**2).mean()
...
def train_one(model: UNetSDS, teacher: Teacher, train_loader, val_loader, tcfg: TrainCfg, sds: SDSCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 5150)
    sched = SchedCfg(T=sds.T, device=device)
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, _ in train_loader:
            x = x.to(device)
            loss = sds_loss(model, teacher, x, sds, betas, alphas, ac, rng)
        ...
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += sds_loss(model, teacher, x, sds, betas, alphas, ac, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def _run(prop, teacher, tr, va, tcfg, sds): return train_one(prop, teacher, tr, va, tcfg, sds)

def adp_width_to_depth(model, teacher, tr, va, tcfg, sds, scfg):
    best_val, best_sd = _run(model, teacher, tr, va, tcfg, sds)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,teacher,tr,va,tcfg,sds)
        ...
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,teacher,tr,va,tcfg,sds)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_to_width(model, teacher, tr, va, tcfg, sds, scfg):
    best_val, best_sd = _run(model, teacher, tr, va, tcfg, sds)
    d_fail = w_fail = 0
    while True:
        improved=False
...
def adp_alt_depth(model, teacher, tr, va, tcfg, sds, scfg):
    best_val, best_sd = _run(model, teacher, tr, va, tcfg, sds)
    while True:
        any_acc=False
        f=0
...
def adp_alt_width(model, teacher, tr, va, tcfg, sds, scfg):
    best_val, best_sd = _run(model, teacher, tr, va, tcfg, sds)
    while True:
        any_acc=False
        f=0
...
def adp_depth_only(model, teacher, tr, va, tcfg, sds, scfg):
    best_val, best_sd = _run(model, teacher, tr, va, tcfg, sds)
    f=0
    while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
        prop=model.append_depth(); v,sd=_run(prop,teacher,tr,va,tcfg,sds)
...
def adp_width_only(model, teacher, tr, va, tcfg, sds, scfg):
    best_val, best_sd = _run(model, teacher, tr, va, tcfg, sds)
    f=0
    while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
        prop=model.widen_all(1); v,sd=_run(prop,teacher,tr,va,tcfg,sds)
...
def build_model(in_channels=3, base=32, stages=3, blocks=1) -> UNetSDS:
    return UNetSDS(in_channels, base, stages, blocks)
