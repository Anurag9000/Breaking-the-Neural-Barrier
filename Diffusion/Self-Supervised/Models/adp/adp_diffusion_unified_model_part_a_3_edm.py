"""
ADP Diffusion – Unified Model (Part A3: EDM)
Six ADP growth algorithms (w2d, d2w, alt_w, alt_d, width_only, depth_only)
over a width/depth‑editable UNet trained with **EDM** (Elucidated Diffusion Models)
preconditioning and sigma sampling.
...
class UNetEDM(nn.Module):
    """UNet with EDM preconditioning hooks; predicts a denoised image residual."""
    ...
    def append_depth(self):
        new = UNetEDM(self.in_channels, self.base, self.stages, self.blocks_per_stage+1)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetEDM(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage)
        transplant_module(new, self); return new
...
# -----------------------------
# EDM objective (Karras‑style preconditioning)
# -----------------------------
...
class EDMCfg:
    ...
    rho: float = 7.0  # sampling exponent
    device: str = 'cuda'
    ...
    def sample_sigma(self, B: int, generator: torch.Generator, device: torch.device):
        ...
        sigma = (self.sigma_max**inv_rho + u * (self.sigma_min**inv_rho - self.sigma_max**inv_rho)) ** self.rho
        return sigma


def edm_loss(model: UNetEDM, x0: torch.Tensor, cfg: EDMCfg, rng: torch.Generator):
    # assume x0 in [-1,1]
    B = x0.size(0)
    device = x0.device
    sigma = cfg.sample_sigma(B, rng, device)
    n = torch.randn_like(x0, generator=rng)
    x = x0 + n * sigma.view(-1,1,1,1)
    c_in   = 1.0 / torch.sqrt(sigma**2 + 1.0)
    c_out  = sigma
    c_skip = 1.0 / (sigma**2 + 1.0)
    t = (torch.log(sigma) / math.log(cfg.sigma_max)).clamp_(0,1)  # monotone time embedding proxy
    # network predicts a residual; combine with skip for x0 prediction
    pred = model(c_in.view(-1,1,1,1) * x, t)
    x0_hat = c_skip.view(-1,1,1,1) * x + (1 - c_skip).view(-1,1,1,1) * pred
    # weight ~ (sigma^2 + 1) to balance SNR
    w = (sigma**2 + 1.0)
    return (w.view(-1,1,1,1) * (x0_hat - x0)**2).mean()
...
def train_one(model: UNetEDM, train_loader, val_loader, tcfg: TrainCfg, ecfg: EDMCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 999)
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, _ in train_loader:
            x = x.to(device)
            loss = edm_loss(model, x, ecfg, rng)
        ...
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += edm_loss(model, x, ecfg, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def _run(prop, tr, va, tcfg, ecfg): return train_one(prop, tr, va, tcfg, ecfg)

def adp_width_to_depth(model, tr, va, tcfg, ecfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, ecfg)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,ecfg)
        ...
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,ecfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_to_width(model, tr, va, tcfg, ecfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, ecfg)
    d_fail = w_fail = 0
    while True:
        improved=False
...
def adp_alt_depth(model, tr, va, tcfg, ecfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, ecfg)
    while True:
        any_acc=False
        f=0
...
def adp_alt_width(model, tr, va, tcfg, ecfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, ecfg)
    while True:
        any_acc=False
        f=0
...
def adp_depth_only(model, tr, va, tcfg, ecfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, ecfg)
    f=0
    while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
        prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,ecfg)
...
def adp_width_only(model, tr, va, tcfg, ecfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, ecfg)
    f=0
    while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
        prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,ecfg)
...
def build_model(in_channels=3, base=32, stages=3, blocks=1) -> UNetEDM:
    return UNetEDM(in_channels, base, stages, blocks)
