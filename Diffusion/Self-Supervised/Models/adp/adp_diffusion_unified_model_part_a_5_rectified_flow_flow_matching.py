"""
ADP Diffusion – Unified Model (Part A5: Rectified Flow / Flow Matching)
Six ADP growth algorithms (w2d, d2w, alt_w, alt_d, width_only, depth_only)
over a width/depth‑editable UNet‑style **velocity field** f_theta(x_t, t) trained with
(flow/rectified‑flow) matching.

BNB rule: single‑model end‑to‑end; NO teacher/EMA; NO multi‑model distillation.

Two training bridges (choose via run file):
  • linear (deterministic rectified flow):      x_t = (1 - t) * x0 + t * z,   v* = z - x0
  • VE‑stochastic (noise bridge with sigma):    x_t = (1 - t) * x0 + t * (x0 + sigma * n),  v* = (x0 + sigma*n) - x0 = sigma*n
We regress f_theta(x_t, t) to the target velocity v* with MSE. (Teacher‑free.)
"""
...
# -----------------------------
# UNet‑like backbone that outputs velocity field
# -----------------------------
...
class UNetFlow(nn.Module):
    ...
    def append_depth(self):
        new = UNetFlow(self.in_channels, self.base, self.stages, self.blocks_per_stage+1, self.out_channels)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetFlow(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage, self.out_channels)
        transplant_module(new, self); return new
...
# -----------------------------
# Flow / Rectified‑Flow matching objective
# -----------------------------

@dataclass
class FlowCfg:
    bridge: str = 'linear'   # 'linear' | 've'
    sigma: float = 1.0       # used for 've'
    device: str = 'cuda'


def flow_loss(model: UNetFlow, x0: torch.Tensor, cfg: FlowCfg, rng: torch.Generator):
    B = x0.size(0); device = x0.device
    t = torch.rand(B, device=device, generator=rng) * 0.99 + 0.005
    if cfg.bridge == 'linear':
        z = torch.randn_like(x0, generator=rng)
        x_t = (1.0 - t).view(-1,1,1,1) * x0 + t.view(-1,1,1,1) * z
        v_star = z - x0
    else:  # VE stochastic bridge around x0
...
def train_one(model: UNetFlow, train_loader, val_loader, tcfg: TrainCfg, fcfg: FlowCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 314)
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, _ in train_loader:
            x = x.to(device)
            loss = flow_loss(model, x, fcfg, rng)
        ...
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += flow_loss(model, x, fcfg, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def _run(prop, tr, va, tcfg, fcfg): return train_one(prop, tr, va, tcfg, fcfg)

def adp_width_to_depth(model, tr, va, tcfg, fcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, fcfg)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,fcfg)
        ...
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,fcfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_to_width(model, tr, va, tcfg, fcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, fcfg)
    d_fail = w_fail = 0
    while True:
        improved=False
...
def adp_alt_depth(model, tr, va, tcfg, fcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, fcfg)
    while True:
        any_acc=False
        f=0
...
def adp_alt_width(model, tr, va, tcfg, fcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, fcfg)
    while True:
        any_acc=False
        f=0
...
def adp_depth_only(model, tr, va, tcfg, fcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, fcfg)
    f=0
    while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
        prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,fcfg)
...
def adp_width_only(model, tr, va, tcfg, fcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, fcfg)
    f=0
    while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
        prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,fcfg)
...
def build_model(in_channels=3, base=32, stages=3, blocks=1, out_channels=None) -> UNetFlow:
    return UNetFlow(in_channels, base, stages, blocks, out_channels or in_channels)
