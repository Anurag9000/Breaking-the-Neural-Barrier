"""
ADP Diffusion – Unified Model (Part A7: PFGM / PFGM++)
Six ADP growth algorithms (w2d, d2w, alt_w, alt_d, width_only, depth_only)
over a width/depth‑editable UNet trained with a **Poisson Flow** objective
(teacher‑free; single model).

This is a simplified PFGM/PFGM++ style: we sample a noise scale sigma in [s_min,s_max]
with exponent rho, corrupt x0 by x = x0 + sigma * n, and learn a field proportional
to the gradient of a Poisson potential by regressing the normalized direction
n / (sigma^alpha). Weighting follows PFGM++ with beta.

This captures the distinctive **potential‑field** weighting/parameterization while
remaining teacher‑free and architecture‑agnostic for ADP.
"""
...
# -----------------------------
# UNet backbone (width/depth editable), predicting vector field (same channels as input)
# -----------------------------
...
class UNetPFGM(nn.Module):
    ...
    def append_depth(self):
        new = UNetPFGM(self.in_channels, self.base, self.stages, self.blocks_per_stage+1)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetPFGM(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage)
        transplant_module(new, self); return new
...
class PFGMCfg:
    s_min: float = 0.01
    s_max: float = 50.0
    rho: float = 7.0         # exponent for sigma sampling
    alpha: float = 1.0       # direction normalization power (pfgm++)
    beta: float = 1.0        # loss weighting power
    device: str = 'cuda'

    def sample_sigma(self, B: int, generator: torch.Generator, device: torch.device):
        u = torch.rand(B, device=device, generator=generator)
        inv = 1.0 / self.rho
        sigma = (self.s_max**inv + u * (self.s_min**inv - self.s_max**inv)) ** self.rho
        return sigma


def pfgm_loss(model: UNetPFGM, x0: torch.Tensor, cfg: PFGMCfg, rng: torch.Generator):
    B = x0.size(0); device = x0.device
    sigma = cfg.sample_sigma(B, rng, device)
    n = torch.randn_like(x0, generator=rng)
    x = x0 + sigma.view(-1,1,1,1) * n
    t = (torch.log(sigma) / math.log(cfg.s_max)).clamp_(0,1)
    # target field ~ n / sigma^alpha
    target = n / (sigma.view(-1,1,1,1) ** cfg.alpha)
    pred = model(x, t)
    w = (sigma ** cfg.beta).view(-1,1,1,1)
...
def train_one(model: UNetPFGM, train_loader, val_loader, tcfg: TrainCfg, pcfg: PFGMCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 1357)
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, _ in train_loader:
            x = x.to(device)
            loss = pfgm_loss(model, x, pcfg, rng)
        ...
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += pfgm_loss(model, x, pcfg, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def _run(prop, tr, va, tcfg, pcfg): return train_one(prop, tr, va, tcfg, pcfg)

def adp_width_to_depth(model, tr, va, tcfg, pcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, pcfg)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,pcfg)
        ...
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,pcfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_to_width(model, tr, va, tcfg, pcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, pcfg)
    d_fail = w_fail = 0
    while True:
        improved=False
...
def adp_alt_depth(model, tr, va, tcfg, pcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, pcfg)
    while True:
        any_acc=False
        f=0
...
def adp_alt_width(model, tr, va, tcfg, pcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, pcfg)
    while True:
        any_acc=False
        f=0
...
def adp_depth_only(model, tr, va, tcfg, pcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, pcfg)
    f=0
    while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
        prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,pcfg)
...
def adp_width_only(model, tr, va, tcfg, pcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, pcfg)
    f=0
    while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
        prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,pcfg)
...
def build_model(in_channels=3, base=32, stages=3, blocks=1) -> UNetPFGM:
    return UNetPFGM(in_channels, base, stages, blocks)
