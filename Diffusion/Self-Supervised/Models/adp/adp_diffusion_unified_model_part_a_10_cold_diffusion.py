"""
ADP Diffusion – Unified Model (Part A10: Cold Diffusion)
Six ADP growth algorithms over a width/depth‑editable UNet trained with a
**deterministic degradation** process x_t = g_t(x0) (no injected noise).
Model predicts x̂0 from x_t with an anchor recon loss.

Provided degradations (choose by flag):
  • gaussian_blur: σ(t) = σ_max * t
  • box_blur:     k(t) = 1 + 2*⌊k_max * t⌋  (odd kernel)
  • jpeg:         q(t) = q_min + (1−t) * (100−q_min)  (simulated via differentiable clamp)

All are teacher‑free and integrate seamlessly with ADP.
"""
...
# -----------------------------
# UNet predicting x0 from degraded x_t
# -----------------------------
...
class UNetCold(nn.Module):
    ...
    def append_depth(self):
        new = UNetCold(self.in_channels, self.base, self.stages, self.blocks_per_stage+1)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetCold(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage)
        transplant_module(new, self); return new
...
@dataclass
class ColdCfg:
    mode: str = 'gaussian_blur'   # 'gaussian_blur' | 'box_blur' | 'jpeg'
    sigma_max: float = 2.5
    k_max: int = 3
    q_min: float = 25.0
    device: str = 'cuda'
...
def gaussian_blur(x, sigma):
    # simple separable approx with fixed kernel size based on sigma
    radius = max(1, int(3 * float(sigma)))
    k = 2*radius + 1
    if k <= 1: return x
    xpad = F.pad(x, (radius, radius, radius, radius), mode='reflect')
    kernel_1d = torch.arange(-radius, radius+1, device=x.device, dtype=x.dtype)
    kernel_1d = torch.exp(-0.5 * (kernel_1d / (sigma + 1e-6)) ** 2)
    kernel_1d = kernel_1d / kernel_1d.sum()
    kernel_x = kernel_1d.view(1,1,1,k).repeat(x.size(1),1,1,1)
    kernel_y = kernel_1d.view(1,1,k,1).repeat(x.size(1),1,1,1)
    x = F.conv2d(xpad, kernel_x, padding=0, groups=x.size(1))
    x = F.conv2d(x,   kernel_y, padding=0, groups=x.size(1))
    return x
...
def box_blur(x, k):
    ...
    kernel = torch.ones((x.size(1),1,k,k), device=x.device, dtype=x.dtype) / (k*k)
    return F.conv2d(xpad, kernel, padding=0, groups=x.size(1))


def fake_jpeg(x, q):
    # crude differentiable clamp to simulate compression artifacts
    s = (100.0 - q) / 100.0
    y = x + s * torch.tanh(10 * (x - x.detach()))  # keep gradient
    return y.clamp(-1,1)


def degrade(x0: torch.Tensor, t: torch.Tensor, cfg: ColdCfg):
    if cfg.mode == 'gaussian_blur':
        sigma = cfg.sigma_max * t
        return gaussian_blur(x0, sigma.view(-1,1,1,1))
    elif cfg.mode == 'box_blur':
        k = 1 + 2 * (t * cfg.k_max).long()
        # apply per‑example by loop (small batches; acceptable for reference code)
        out = []
        for i in range(x0.size(0)):
            out.append(box_blur(x0[i:i+1], int(k[i].item())))
        return torch.cat(out, dim=0)
    else:  # jpeg
        q = cfg.q_min + (1 - t) * (100.0 - cfg.q_min)
        return fake_jpeg(x0, q.view(-1,1,1,1))


def cold_loss(model: UNetCold, x0: torch.Tensor, cfg: ColdCfg, rng: torch.Generator):
...
def train_one(model: UNetCold, train_loader, val_loader, tcfg: TrainCfg, ccfg: ColdCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 9876)
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, _ in train_loader:
            x = x.to(device)
            loss = cold_loss(model, x, ccfg, rng)
        ...
        with torch.no_grad():
            for x, _ in val_loader:
                x = x.to(device)
                vloss += cold_loss(model, x, ccfg, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def build_model(in_channels=3, base=32, stages=3, blocks=1) -> UNetCold:
    return UNetCold(in_channels, base, stages, blocks)
