"""
ADP Diffusion – Unified Model (Part A13: Integrated Guidance Heads)
Six ADP growth algorithms with a UNet that has **two heads**:
  • denoise head: ε‑prediction (DDPM‑style)
  • guidance head: either a classifier over labels (for classifier guidance) or
    a scalar energy head (for energy guidance)

Loss = L_denoise + λ * L_guidance
  Classifier: cross‑entropy on x0 (optionally on x_t too via mixup)
  Energy: margin ranking: ReLU(E(x_t) − E(x0) + m)

This keeps the model single‑network with multi‑task outputs and is compatible
with ADP growth.
"""
...
class UNetGuidance(nn.Module):
    def __init__(self, in_channels=3, base=32, stages=3, blocks_per_stage=1, num_classes=10):
        super().__init__()
        ...
        # heads
        self.head_eps = nn.Conv2d(base, in_channels, 1)
        self.head_cls = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(base, num_classes)
        )
        self.head_energy = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(base, 1)
        )

    # growth ops
    def append_depth(self):
        new = UNetGuidance(self.in_channels, self.base, self.stages, self.blocks_per_stage+1)
        transplant_module(new, self); return new
    def widen_all(self, delta: int):
        new = UNetGuidance(self.in_channels, self.base+delta, self.stages, self.blocks_per_stage)
        transplant_module(new, self); return new

    def forward_feats(self, x, t):
        emb = self.time_emb(t)
        h = self.in_conv(x)
    ...
    def forward(self, x, t):
        c = self.forward_feats(x, t)
        return self.head_eps(c), self.head_cls(c), self.head_energy(c)
...
@dataclass
class GuideCfg:
    mode: str = 'classifier'   # 'classifier' | 'energy'
    num_classes: int = 10
    lambda_g: float = 0.5
    margin: float = 1.0
    T: int = 1000
...
def guide_loss(model: UNetGuidance, x0: torch.Tensor, y: Optional[torch.Tensor], cfg: GuideCfg, betas, alphas, ac, rng: torch.Generator):
    ...
    tt = (t.float() + 1) / cfg.T
    eps_pred, logits, energy = model(xt, tt)
    L_denoise = F.mse_loss(eps_pred, eps)
    if cfg.mode == 'classifier':
        if y is None:
            # fallback: pseudo‑labels via argmax on logits of clean x0
            with torch.no_grad():
                _, logits_clean, _ = model(x0, torch.ones_like(tt))
                y = logits_clean.argmax(dim=1)
        L_g = F.cross_entropy(logits, y)
    else:
        # energy guidance: encourage lower energy on clean than noisy
        with torch.no_grad():
            _, _, e_clean = model(x0, torch.ones_like(tt))
        L_g = F.relu(energy.view(-1) - e_clean.view(-1) + cfg.margin).mean()
    return L_denoise + cfg.lambda_g * L_g
...
def train_one(model: UNetGuidance, train_loader, val_loader, tcfg: TrainCfg, gcfg: GuideCfg):
    torch.manual_seed(tcfg.seed)
    device = torch.device(tcfg.device if torch.cuda.is_available() else 'cpu')
    model = model.to(device)
    rng = torch.Generator(device=device); rng.manual_seed(tcfg.seed + 6060)
    _, _, ac = gcfg.cosine()
    ...
    for epoch in range(tcfg.max_epochs):
        ...
        for x, y in train_loader:
            x = x.to(device)
            y = y.to(device) if isinstance(y, torch.Tensor) else None
            loss = guide_loss(model, x, y, gcfg, None, None, ac, rng)
        ...
        with torch.no_grad():
            for x, y in val_loader:
                x = x.to(device)
                y = y.to(device) if isinstance(y, torch.Tensor) else None
                vloss += guide_loss(model, x, y, gcfg, None, None, ac, rng).item() * x.size(0)
                n += x.size(0)
        vloss /= max(n,1)
...
def _run(prop, tr, va, tcfg, gcfg): return train_one(prop, tr, va, tcfg, gcfg)

def adp_width_to_depth(model, tr, va, tcfg, gcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, gcfg)
    w_fail = d_fail = 0
    while True:
        improved=False
        while w_fail < scfg.trials_width and model.base + 1 <= scfg.max_base:
            prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,gcfg)
        ...
        while d_fail < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
            prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,gcfg)
            if accept(v,best_val,scfg.delta): model,best_val,best_sd=prop,v,sd; improved=True
            else: d_fail+=1
        if not improved: break
    model.load_state_dict(best_sd); return model, best_val

def adp_depth_to_width(model, tr, va, tcfg, gcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, gcfg)
    d_fail = w_fail = 0
    while True:
        improved=False
...
def adp_alt_depth(model, tr, va, tcfg, gcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, gcfg)
    while True:
        any_acc=False
        f=0
...
def adp_alt_width(model, tr, va, tcfg, gcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, gcfg)
    while True:
        any_acc=False
        f=0
...
def adp_depth_only(model, tr, va, tcfg, gcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, gcfg)
    f=0
    while f < scfg.trials_depth and model.blocks_per_stage + 1 <= scfg.max_depth:
        prop=model.append_depth(); v,sd=_run(prop,tr,va,tcfg,gcfg)
...
def adp_width_only(model, tr, va, tcfg, gcfg, scfg):
    best_val, best_sd = _run(model, tr, va, tcfg, gcfg)
    f=0
    while f < scfg.trials_width and model.base + 1 <= scfg.max_base:
        prop=model.widen_all(1); v,sd=_run(prop,tr,va,tcfg,gcfg)
...
def build_model(in_channels=3, base=32, stages=3, blocks=1, num_classes=10) -> UNetGuidance:
    return UNetGuidance(in_channels, base, stages, blocks, num_classes)
