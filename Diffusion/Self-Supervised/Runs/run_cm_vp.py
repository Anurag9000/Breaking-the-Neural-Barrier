...
from models.cm_vp import VPConsistency
...
p.add_argument('--save', type=str, default='./results/cm_vp')
...
model = VPConsistency(T=args.T).to(device)
...
logger = ContinuousLogger(Path('results_run_cm_vp'), 'run_cm_vp', 'train')
...
with torch.no_grad():
    ...
    save_samples(grid, os.path.join(args.save, 'samples_cm_vp.png'))
    torch.save(model.state_dict(), os.path.join(args.save, 'cm_vp.pth'))
