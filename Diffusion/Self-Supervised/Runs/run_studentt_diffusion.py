# runs/run_studentt_diffusion.py
import os
import argparse
import torch
...
p.add_argument('--save', type=str, default='./results/studentt')
...
model = StudentTDiffusion(nu=args.nu).to(device)
...
with torch.no_grad():
    ...
    save_samples(grid, os.path.join(args.save, 'samples_studentt.png'))

torch.save(model.state_dict(), os.path.join(args.save, 'studentt.pth'))
