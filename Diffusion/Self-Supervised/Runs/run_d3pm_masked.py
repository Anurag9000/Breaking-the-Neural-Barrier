...
from models.d3pm_masked import D3PMMasked
...
p.add_argument('--save', type=str, default='./results/d3pm_masked')
...
model = D3PMMasked(K=args.K).to(device)
...
logger = ContinuousLogger(Path('results_run_d3pm_masked'), 'run_d3pm_masked', 'train')
...
with torch.no_grad():
    # sample in token space then decode to [-1,1] with uniform bin centers
    imgs = model.sample(B=64, steps=args.steps, device=device)
    grid = make_grid((imgs + 1) / 2, nrow=8)  # rescale to [0,1] for saving
    save_samples(grid, os.path.join(args.save, 'samples_d3pm_masked.png'))
    torch.save(model.state_dict(), os.path.join(args.save, 'd3pm_masked.pth'))
