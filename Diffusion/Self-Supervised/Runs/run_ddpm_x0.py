...
from models.ddpm_x0_model import DDPMX0
...
parser.add_argument('--save', type=str, default='./results/ddpm_x0')
...
model = DDPMX0(T=args.T, schedule=args.schedule).to(device)
...
logger = ContinuousLogger(Path('results_run_ddpm_x0'), 'run_ddpm_x0', 'train')
...
with torch.no_grad():
    ...
    save_samples(grid, os.path.join(args.save, 'samples_ddpm_x0.png'))
    torch.save(model.state_dict(), os.path.join(args.save, 'ddpm_x0.pth'))
