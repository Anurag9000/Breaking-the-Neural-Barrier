# runs/run_cold_diffusion.py
import os
import argparse
import torch
import sys
from pathlib import Path
sys.path.append(str(Path(__file__).resolve().parents[3]))
from utils.adp_logging import ContinuousLogger
from torchvision.utils import make_grid
from models.cold_diffusion_model import ColdDiffusion
from runs._common_train_e import make_cifar10_loaders, EarlyStopper, save_samples


# -----------------------
# Arguments
# -----------------------
p = argparse.ArgumentParser()
p.add_argument('--epochs', type=int, default=150)
p.add_argument('--patience', type=int, default=20)
p.add_argument('--lr', type=float, default=2e-4)
p.add_argument('--batch_size', type=int, default=128)
p.add_argument('--device', type=str, default='cuda')
p.add_argument('--save', type=str, default='./results/cold')
p.add_argument('--steps', type=int, default=50)
p.add_argument('--degrade', type=str, default='blur', choices=['blur', 'down_up'])
args = p.parse_args([]) if __name__ == '__main__' else p.parse_args()


# -----------------------
# Data
# -----------------------
train_loader, val_loader, _ = make_cifar10_loaders(batch_size=args.batch_size)


# -----------------------
# Model & Optimizer
# -----------------------
device = torch.device(args.device if torch.cuda.is_available() else 'cpu')
model = ColdDiffusion(degrade=args.degrade).to(device)
opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
stop = EarlyStopper(patience=args.patience)


# -----------------------
# Training Loop
# -----------------------
best_state = None


# Init Logger


logger = ContinuousLogger(Path('results_run_cold_diffusion'), 'run_cold_diffusion', 'train')


for epoch in range(1, args.epochs + 1):
    # Training
    model.train()
    for x, _ in train_loader:
        x = x.to(device)
        loss = model.loss(x)
        opt.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()

    # Validation
    model.eval()
    val_loss, n_samples = 0.0, 0
    with torch.no_grad():
        for x, _ in val_loader:
            x = x.to(device)
            val_loss += model.loss(x).item() * x.size(0)
            n_samples += x.size(0)
    val_loss /= n_samples

    # Log


    msg = f'Epoch {epoch}: val_loss={val_loss:.4f}'


    logger.log_console(msg)


    logger.log_epoch_stats({


        "epoch": epoch,


        "val_loss": val_loss if 'val_loss' in locals() else (loss.item() if 'loss' in locals() else 0),


        "train_loss": loss.item() if 'loss' in locals() else 0


    })

    if stop.step(val_loss):
        best_state = {k: v.cpu() for k, v in model.state_dict().items()}

    if stop.should_stop():
        print("Early stopping triggered.")
        break


# Load best model
if best_state is not None:
    model.load_state_dict(best_state)


# -----------------------
# Sampling & Saving
# -----------------------
os.makedirs(args.save, exist_ok=True)
with torch.no_grad():
    imgs = model.sample(B=64, steps=args.steps, device=device)
    grid = make_grid((imgs + 1) / 2, nrow=8)
    save_samples(grid, os.path.join(args.save, 'samples_cold.png'))

torch.save(model.state_dict(), os.path.join(args.save, 'cold.pth'))
