"""
Runner for ADP Diffusion – Part A4 (Consistency Models)
Flags:
  --family cm
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --sigma-min --sigma-max --lambda-consistency --lambda-anchor

Example (smoke):
python run_adp_diffusion_cm.py --family cm --adp alt_w --lambda-consistency 1.0 --lambda-anchor 0.1 --smoke
"""
import argparse
...
from adp_diffusion_unified_cm_model import (
    CMCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='cm', choices=['cm'])
    ...
    ccfg = CMCfg(sigma_min=args.sigma_min, sigma_max=args.sigma_max, lambda_consistency=args.lambda_consistency, lambda_anchor=args.lambda_anchor)
    ...
    tag = f"cm_cons{args.lambda_consistency}_anc{args.lambda_anchor}_{args.adp}_b{args.base}_k{args.blocks}"
