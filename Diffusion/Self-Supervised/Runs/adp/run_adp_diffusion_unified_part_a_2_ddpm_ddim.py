"""
Runner for ADP Diffusion – Part A2 (DDPM/DDIM)
Select diffusion family and ADP algorithm via flags.
Example (smoke):
python run_adp_diffusion_ddpm.py --family ddpm --adp alt_d --param v --smoke
"""
import argparse
...
from adp_diffusion_unified_ddpm_model import (
    DDPMCfg, TrainCfg, SearchCfg,
    build_model, ADP_REGISTRY
)


def build_loaders(data_root: str, batch_size: int, val_split=0.1, num_workers=2, image_size=32):
    tf_train = T.Compose([
        T.RandomHorizontalFlip(), T.RandomCrop(image_size, padding=4), T.ToTensor()
    ])
...
def main():
    ...
    p.add_argument('--family', type=str, default='ddpm', choices=['ddpm'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))
    p.add_argument('--param', type=str, default='eps', choices=['eps','x0','v'])
    p.add_argument('--schedule', type=str, default='cosine', choices=['cosine','linear','sqrt'])
    ...
    tcfg = TrainCfg(max_epochs=max_epochs, patience=patience)
    dcfg = DDPMCfg(T=args.T, schedule=args.schedule, param=args.param)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, dcfg, scfg)

    out = Path('results_adp_diffusion')
    out.mkdir(parents=True, exist_ok=True)
    tag = f"{args.family}_{args.param}_{args.schedule}_{args.adp}_b{args.base}_k{args.blocks}"
