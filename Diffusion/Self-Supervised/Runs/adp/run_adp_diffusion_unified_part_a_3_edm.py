"""
Runner for ADP Diffusion – Part A3 (EDM)
Flags:
  --family edm
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --sigma-min --sigma-max --rho

Example (smoke):
python run_adp_diffusion_edm.py --family edm --adp alt_d --smoke
"""
import argparse
...
from adp_diffusion_unified_edm_model import (
    EDMCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='edm', choices=['edm'])
    ...
    ecfg = EDMCfg(sigma_min=args.sigma_min, sigma_max=args.sigma_max, rho=args.rho)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, ecfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"{args.family}_rho{args.rho}_{args.adp}_b{args.base}_k{args.blocks}"
