"""
Runner for ADP Diffusion – Part A10 (Cold Diffusion)
Flags:
  --family cold
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --mode {gaussian_blur,box_blur,jpeg}
  --sigma-max --k-max --q-min

Example (smoke):
python run_adp_diffusion_cold.py --family cold --adp alt_w --mode gaussian_blur --sigma-max 2.5 --smoke
"""
import argparse
...
from adp_diffusion_unified_cold_model import (
    ColdCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='cold', choices=['cold'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))

    # Cold cfg
    p.add_argument('--mode', type=str, default='gaussian_blur', choices=['gaussian_blur','box_blur','jpeg'])
    p.add_argument('--sigma-max', type=float, default=2.5)
    p.add_argument('--k-max', type=int, default=3)
    p.add_argument('--q-min', type=float, default=25.0)
    ...
    ccfg = ColdCfg(mode=args.mode, sigma_max=args.sigma_max, k_max=args.k_max, q_min=args.q_min)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, ccfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"cold_{args.mode}_sig{args.sigma_max}_k{args.k_max}_q{args.q_min}_{args.adp}_b{args.base}_k{args.blocks}"
