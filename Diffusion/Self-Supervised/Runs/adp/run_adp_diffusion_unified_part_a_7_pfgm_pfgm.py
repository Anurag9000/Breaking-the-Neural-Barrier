"""
Runner for ADP Diffusion – Part A7 (PFGM / PFGM++)
Flags:
  --family pfgm
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --s-min --s-max --rho --alpha --beta

Example (smoke):
python run_adp_diffusion_pfgm.py --family pfgm --adp alt_w --s-min 0.01 --s-max 50 --rho 7 --alpha 1 --beta 1 --smoke
"""
import argparse
...
from adp_diffusion_unified_pfgm_model import (
    PFGMCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='pfgm', choices=['pfgm'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))

    # PFGM cfg
    p.add_argument('--s-min', type=float, default=0.01)
    p.add_argument('--s-max', type=float, default=50.0)
    ...
    pcfg = PFGMCfg(s_min=args.s_min, s_max=args.s_max, rho=args.rho, alpha=args.alpha, beta=args.beta)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, pcfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"pfgm_rho{args.rho}_a{args.alpha}_b{args.beta}_{args.adp}_b{args.base}_k{args.blocks}"
