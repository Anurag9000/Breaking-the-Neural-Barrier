"""
Runner for ADP Diffusion – Part A1 (Score/SDE)
Two flags choose the method and ADP algorithm:
  --family score_sde                # (this part implements Score/SDE)
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}

Example (smoke):
python run_adp_diffusion_unified.py --family score_sde --adp alt_w --smoke
"""
import argparse
import torch
...
from adp_diffusion_unified_model import (
    SDECfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='score_sde', choices=['score_sde'])
    ...
    # loaders
    train_loader, val_loader = build_loaders(args.data_root, args.batch_size, num_workers=args.num_workers)

    # model
    model = build_model(in_channels=3, base=args.base, stages=args.stages, blocks=args.blocks)

    # train cfg (SDE)
    sde_cfg = SDECfg(kind=args.sde)
    max_epochs = 2 if args.smoke else args.max_epochs
    patience = 1 if args.smoke else args.patience
    tcfg = TrainCfg(sde=sde_cfg, lr=args.lr, weight_decay=args.wd, max_epochs=max_epochs, patience=patience)

    # search cfg
    scfg = SearchCfg(max_depth=args.max_depth, max_base=args.max_base,
                     trials_width=args.trials_width, trials_depth=args.trials_depth, delta=args.delta)

    adp = ADP_REGISTRY[args.adp]
    model, best_val = adp(model, train_loader, val_loader, tcfg, scfg)

    out = Path('results_adp_diffusion')
    out.mkdir(parents=True, exist_ok=True)
    tag = f"{args.family}_{args.sde}_{args.adp}_b{args.base}_k{args.blocks}"
