"""
Runner for ADP Diffusion – Part A6 (Stochastic Interpolants)
Flags:
  --family si
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --schedule {poly,softp}  --sigma-max  --gamma  --alpha

Example (smoke):
python run_adp_diffusion_si.py --family si --adp alt_d --schedule poly --sigma-max 1.0 --gamma 1.5 --smoke
"""
import argparse
...
from adp_diffusion_unified_si_model import (
    SICfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='si', choices=['si'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))

    # SI cfg
    p.add_argument('--schedule', type=str, default='poly', choices=['poly','softp'])
    ...
    sicfg = SICfg(schedule=args.schedule, sigma_max=args.sigma_max, gamma=args.gamma, alpha=args.alpha)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, sicfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"si_{args.schedule}_sig{args.sigma_max}_g{args.gamma}_a{args.alpha}_{args.adp}_b{args.base}_k{args.blocks}"
