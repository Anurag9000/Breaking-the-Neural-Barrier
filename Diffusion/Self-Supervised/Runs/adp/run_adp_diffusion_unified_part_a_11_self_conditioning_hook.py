"""
Runner for ADP Diffusion – Part A11 (Self‑Conditioning Hook)
Flags:
  --family sc
...
Example (smoke):
python run_adp_diffusion_sc.py --family sc --adp alt_w --self-cond-prob 0.5 --smoke
"""
import argparse
...
from adp_diffusion_unified_sc_model import (
    SCCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='sc', choices=['sc'])
    ...
    sccfg = SCCfg(self_cond_prob=args.self_cond_prob, schedule_T=args.T)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, sccfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"sc_p{args.self_cond_prob}_T{args.T}_{args.adp}_b{args.base}_k{args.blocks}"
