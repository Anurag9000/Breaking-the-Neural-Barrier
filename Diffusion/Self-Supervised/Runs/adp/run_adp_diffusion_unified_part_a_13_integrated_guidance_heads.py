"""
Runner for ADP Diffusion – Part A13 (Integrated Guidance Heads)
Flags:
  --family guide
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --mode {classifier,energy}  --lambda-g  --margin  --T

Example (smoke):
python run_adp_diffusion_guide.py --family guide --adp d2w --mode classifier --lambda-g 0.5 --smoke
"""
import argparse
...
from adp_diffusion_unified_guide_model import (
    GuideCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='guide', choices=['guide'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))

    # Guidance cfg
    p.add_argument('--mode', type=str, default='classifier', choices=['classifier','energy'])
    p.add_argument('--lambda-g', type=float, default=0.5)
    ...
    model = build_model(in_channels=3, base=args.base, stages=args.stages, blocks=args.blocks, num_classes=10)
    ...
    gcfg = GuideCfg(mode=args.mode, lambda_g=args.lambda_g, margin=args.margin, T=args.T)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, gcfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"guide_{args.mode}_lam{args.lambda_g}_m{args.margin}_T{args.T}_{args.adp}_b{args.base}_k{args.blocks}"
