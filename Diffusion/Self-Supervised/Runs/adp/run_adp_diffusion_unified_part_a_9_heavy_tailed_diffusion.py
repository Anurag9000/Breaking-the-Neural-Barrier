"""
Runner for ADP Diffusion – Part A9 (Heavy‑Tailed Diffusion)
Flags:
  --family htd
  --adp {w2d,d2w,alt_w,alt_d,depth_only,width_only}
  --family-noise {laplace,studentt} --b <laplace_scale> --nu <df>
  --huber-delta <float>

Example (smoke):
python run_adp_diffusion_htd.py --family htd --adp w2d --family-noise laplace --b 1.0 --huber-delta 0.0 --smoke
"""
import argparse
...
from adp_diffusion_unified_htd_model import (
    HTCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='htd', choices=['htd'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))

    # HT cfg
    p.add_argument('--family-noise', type=str, default='laplace', choices=['laplace','studentt'])
    ...
    hcfg = HTCfg(family=args.family_noise, b=args.b, nu=args.nu)
    ...
    model, best_val = adp(model, tr_loader, va_loader, tcfg, hcfg, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"htd_{args.family_noise}_b{args.b}_nu{args.nu}_{args.adp}_b{args.base}_k{args.blocks}"
