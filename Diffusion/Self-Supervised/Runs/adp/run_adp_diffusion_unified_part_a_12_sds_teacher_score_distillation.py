"""
Runner for ADP Diffusion – Part A12 (SDS – Teacher Score Distillation)
Flags:
  --family sds
...
Example (smoke):
python run_adp_diffusion_sds.py --family sds --adp w2d --teacher rand --smoke
"""
import argparse
...
from adp_diffusion_unified_sds_model import (
    SDSCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY, Teacher, train_one
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='sds', choices=['sds'])
    p.add_argument('--adp', type=str, required=True, choices=list(ADP_REGISTRY.keys()))

    # SDS cfg
    p.add_argument('--teacher', type=str, default='rand', choices=['rand'])
    p.add_argument('--T', type=int, default=1000)
    p.add_argument('--w-exp', type=float, default=0.5)
    ...
    sds = SDSCfg(teacher_mode=args.teacher, T=args.T, w_exp=args.w_exp)
    ...
    teacher = Teacher(mode=args.teacher)
    adp = ADP_REGISTRY[args.adp]
    # ADP wrapper requires signature alignment; we partially apply teacher via lambda
    model, best_val = adp(model, teacher, tr_loader, va_loader, tcfg, sds, scfg)

    out = Path('results_adp_diffusion'); out.mkdir(parents=True, exist_ok=True)
    tag = f"sds_{args.teacher}_T{args.T}_w{args.w_exp}_{args.adp}_b{args.base}_k{args.blocks}"
