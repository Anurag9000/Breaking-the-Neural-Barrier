"""
Runner for ADP Diffusion – Part A8 (Discrete Diffusion)
Flags:
  --family disc
...
Example (smoke):
python run_adp_diffusion_disc.py --family disc --adp alt_d --K 256 --smoke
"""
import argparse
...
from adp_diffusion_unified_disc_model import (
    DiscCfg, TrainCfg, SearchCfg, build_model, ADP_REGISTRY
)
...
def main():
    ...
    p.add_argument('--family', type=str, default='disc', choices=['disc'])
    ...
    model = build_model(in_channels=3, base=args.base, stages=args.stages, blocks=args.blocks, K=args.K)
    ...
    tag = f"disc_K{args.K}_{args.adp}_b{args.base}_k{args.blocks}"
