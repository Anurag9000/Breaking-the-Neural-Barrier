...
from models.laplace_diffusion_model import LaplaceDiffusion
...
p.add_argument('--save', type=str, default='./results/laplace')
...
model = LaplaceDiffusion().to(device)
...
logger = ContinuousLogger(Path('results_run_laplace_diffusion'), 'run_laplace_diffusion', 'train')
...
with torch.no_grad():
    ...
    save_samples(grid, os.path.join(args.save, 'samples_laplace.png'))

torch.save(model.state_dict(), os.path.join(args.save, 'laplace.pth'))
