...
def make_cifar10_loaders(data_root='./data', batch_size=128, val_ratio=0.1, num_workers=4):
    """
    Create CIFAR-10 train/validation/test loaders.
    """
    ...
    # Use proper transform for validation
...
class EarlyStopper:
    """
    Early stopping utility for monitoring validation loss.
    """
    ...
    def step(self, val):
        """
        Update the stopper with the current validation value.
...
def save_samples(grid, path):
    """
    Save a torchvision image grid to the specified path.
    """
