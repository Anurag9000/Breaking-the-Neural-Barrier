...
from models.conditional_flow_matching_model import ConditionalFlowMatching
...
p.add_argument('--save', type=str, default='./results/cfm')
p.add_argument('--steps', type=int, default=50)
p.add_argument('--p_null', type=float, default=0.1)
...
model = ConditionalFlowMatching(p_null=args.p_null).to(device)
...
logger = ContinuousLogger(Path('results_run_cfm'), 'run_cfm', 'train')
...
with torch.no_grad():
    imgs = model.sample(B=64, steps=args.steps, guidance=args.guidance, device=device)
    grid = make_grid((imgs + 1) / 2, nrow=8)
    save_samples(grid, os.path.join(args.save, 'samples_cfm.png'))

torch.save(model.state_dict(), os.path.join(args.save, 'cfm.pth'))
