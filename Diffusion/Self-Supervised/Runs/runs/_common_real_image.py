...
def _build_subset_loaders(dataset_root: Path, batch_size: int, val_ratio: float, test_ratio: float,
                          num_workers: int, image_size: int):
    train_tf, eval_tf = _make_transforms(image_size)
    train_full = datasets.ImageFolder(str(dataset_root), transform=train_tf)
    eval_full = datasets.ImageFolder(str(dataset_root), transform=eval_tf)
    train_idx, val_idx, test_idx = _split_indices(len(train_full), val_ratio, test_ratio)
...
def _build_explicit_split_loaders(train_dir: Path, val_dir: Path, test_dir: Path, batch_size: int,
                                  num_workers: int, image_size: int):
    train_tf, eval_tf = _make_transforms(image_size)
...
def make_real_image_loaders(
    ...
    if _folder_exists(train_dir) and _folder_exists(val_dir) and _folder_exists(test_dir):
        return _build_explicit_split_loaders(train_dir, val_dir, test_dir, batch_size, num_workers, image_size)
    ...
    if _folder_exists(train_dir):
        return _build_subset_loaders(train_dir, batch_size, val_ratio, test_ratio, num_workers, image_size)

    if _folder_exists(root):
        return _build_subset_loaders(root, batch_size, val_ratio, test_ratio, num_workers, image_size)
