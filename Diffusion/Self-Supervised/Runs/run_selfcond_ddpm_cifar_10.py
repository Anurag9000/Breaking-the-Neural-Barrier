...
from selfcond_ddpm_model import UNetSelfCond, SelfCondDDPM, DiffCfg, count_parameters
...
def train_epoch(model: SelfCondDDPM, loader, opt, device):
    model.train(); tot=0.0
    for x,_ in loader:
        x=x.to(device)
...
@torch.no_grad()
def evaluate(model: SelfCondDDPM, loader, device):
    model.eval(); tot=0.0
    for x,_ in loader:
...
@torch.no_grad()
def save_samples(model: SelfCondDDPM, out_dir: Path, device, n=64):
...
def main():
    ...
    ap.add_argument('--save-dir', type=str, default='./artifacts_selfcond_ddpm')
    ...
    net=UNetSelfCond(in_ch=3, base=64, ch_mult=(1,2,4), tdim=256, out_ch=3, self_cond=True)
    model=SelfCondDDPM(net, DiffCfg(args.timesteps, args.beta_start, args.beta_end, True)).to(device)
