...
from edm_model import EDMUNet, EDM, EDMConfig, count_parameters
...
def train_epoch(model: EDM, loader, opt, device):
    model.train(); tot=0.0
    for x,_ in loader:
        x=x.to(device)
        loss=model.loss(x)
...
@torch.no_grad()
def evaluate(model: EDM, loader, device):
    model.eval(); tot=0.0
    for x,_ in loader:
...
@torch.no_grad()
def save_samples(model: EDM, out_dir: Path, device, n=64):
    out_dir.mkdir(parents=True, exist_ok=True)
    x=model.sample((n,3,32,32), device=device, steps=40)
    grid=(x+1)/2
...
def main():
    ...
    ap.add_argument('--save-dir', type=str, default='./artifacts_edm')
    ...
    net=EDMUNet(in_ch=3, base=64, ch_mult=(1,2,4), edim=256, out_ch=3)
    model=EDM(net, EDMConfig()).to(device)
