...
from models.ddpm_v_model_new import DDPMV
...
parser.add_argument('--save', type=str, default='./results/ddpm_v')
...
model = DDPMV(T=args.T, schedule=args.schedule).to(device)
...
logger = ContinuousLogger(Path('results_run_ddpm_v'), 'run_ddpm_v', 'train')
...
with torch.no_grad():
    ...
    save_samples(grid, os.path.join(args.save, 'samples_ddpm_v.png'))
    torch.save(model.state_dict(), os.path.join(args.save, 'ddpm_v.pth'))
