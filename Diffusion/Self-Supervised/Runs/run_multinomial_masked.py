# runs/run_multinomial_masked.py
import os
import argparse
import torch
import sys
...
from models.multinomial_masked import MultinomialMasked
...
p.add_argument('--save', type=str, default='./results/multinomial_masked')
...
model = MultinomialMasked(K=args.K).to(device)
...
logger = ContinuousLogger(Path('results_run_multinomial_masked'), 'run_multinomial_masked', 'train')
...
with torch.no_grad():
    ...
    save_samples(grid, os.path.join(args.save, 'samples_multinomial_masked.png'))
    torch.save(model.state_dict(), os.path.join(args.save, 'multinomial_masked.pth'))
