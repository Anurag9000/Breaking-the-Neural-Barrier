...
# (content truncated for brevity in this comment; full content recreated exactly as intended)
import math
...
class ResBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, emb_dim: int, bias=True):
        super().__init__()
        self.conv1 = ConvBNAct(in_ch, out_ch, bias=bias)
        self.film = FiLM(out_ch, emb_dim)
        self.conv2 = ConvBNAct(out_ch, out_ch, bias=bias)
        self.skip = nn.Conv2d(in_ch, out_ch, 1, bias=bias) if in_ch != out_ch else nn.Identity()
    def forward(self, x: torch.Tensor, e: torch.Tensor) -> torch.Tensor:
        h = self.conv1(x); h = self.film(h, e); h = self.conv2(h); return h + self.skip(x)
...
class AdaptiveUNetSup(nn.Module):
    def __init__(self, in_ch: int, widths: List[int], pooling_indices: List[int], num_classes: int, emb_dim: int = 256, bias: bool = True):
        super().__init__()
        assert len(widths) >= 1
        self.in_ch = in_ch; self.bias = bias; self.pooling_indices = sorted(set(pooling_indices))
        self.emb_dim = emb_dim; self.num_classes = num_classes; self.null_class = num_classes
        self.time_mlp = nn.Sequential(nn.Linear(emb_dim, emb_dim), nn.SiLU(), nn.Linear(emb_dim, emb_dim))
        self.class_emb = nn.Embedding(num_embeddings=num_classes + 1, embedding_dim=emb_dim)
        enc_blocks = nn.ModuleList(); downs = nn.ModuleList(); c = in_ch; self._pools_here = []
        for i, w in enumerate(widths):
            enc_blocks.append(ResBlock(c, w, emb_dim, bias=bias)); c = w
            do_pool = (i in self.pooling_indices); self._pools_here.append(do_pool)
            downs.append(Downsample(c) if do_pool else nn.Identity())
        self.encoder, self.downs = enc_blocks, downs
        self.mid = ResBlock(c, c, emb_dim, bias=bias)
        dec_blocks = nn.ModuleList(); ups = nn.ModuleList()
        for i in range(len(widths)-1, -1, -1):
            w_out = widths[i-1] if i-1 >= 0 else in_ch
            dec_blocks.append(ResBlock(c, w_out, emb_dim, bias=bias)); c = w_out
            ups.append(Upsample(c) if self._pools_here[i] else nn.Identity())
        self.decoder, self.ups = dec_blocks, ups
        self.head = nn.Conv2d(in_ch, in_ch, 3, 1, 1)
    @property
    def widths(self) -> List[int]: return [blk.conv1.bn.num_features for blk in self.encoder]
    def cond_vector(self, t: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        te = sinusoidal_embedding(t, self.emb_dim); te = self.time_mlp(te); ce = self.class_emb(y); return te + ce
    def forward(self, x, t, y):
        e = self.cond_vector(t, y); h = x
        for blk, down in zip(self.encoder, self.downs): h = blk(h, e); h = down(h)
        h = self.mid(h, e)
        for blk, up in zip(self.decoder, self.ups): h = blk(h, e); h = up(h)
        return self.head(h)
    def append_depth(self):
        last_c = self.encoder[-1].conv1.bn.num_features
        self.encoder.append(ResBlock(last_c, last_c, self.emb_dim, bias=self.bias))
        self._pools_here.append(False); self.downs.append(nn.Identity())
        self.decoder.insert(0, ResBlock(last_c, last_c, self.emb_dim, bias=self.bias))
        self.ups.insert(0, nn.Identity())
    def widen_all(self, ex_k: int):
        if ex_k <= 0: return
        prev = self.in_ch
        for blk in self.encoder:
            old_out = blk.conv1.bn.num_features; new_out = old_out + ex_k
            _resize_resblock_(blk, prev, new_out, self.emb_dim); prev = new_out
        _resize_resblock_(self.mid, prev, prev, self.emb_dim)
        enc_widths = [b.conv1.bn.num_features for b in self.encoder]; self._rebuild_decoder(enc_widths)
        _resize_conv2d_(self.head, in_ch=self.in_ch, out_ch=self.in_ch)
    def _rebuild_decoder(self, enc_widths: List[int]):
        old_dec, old_ups = self.decoder, self.ups
        dec, ups = nn.ModuleList(), nn.ModuleList(); c = enc_widths[-1]
        for i in range(len(enc_widths)-1, -1, -1):
            w_out = enc_widths[i-1] if i-1 >= 0 else self.in_ch
            nb = ResBlock(c, w_out, self.emb_dim, bias=self.bias); dec.append(nb); c = w_out
            ups.append(Upsample(c) if self._pools_here[i] else nn.Identity())
        for nb, ob in zip(dec, old_dec): _overlap_resblock_(nb, ob)
        self.decoder, self.ups = dec, ups
    def total_neurons(self) -> int:
        enc = sum([b.conv1.bn.num_features for b in self.encoder]); dec = sum([b.conv1.bn.num_features for b in self.decoder])
        mid = self.mid.conv1.bn.num_features; return enc + dec + mid

def _overlap_copy_(dst, src):
    dims = [min(a, b) for a, b in zip(dst.shape, src.shape)]; dst[tuple(slice(0,d) for d in dims)].copy_(src[tuple(slice(0,d) for d in dims)])
...
class DiffusionHelper:
    ...
    def register(self, betas, alphas, alphas_cumprod, device):
        self.betas = betas.to(device); self.alphas = alphas.to(device); self.alphas_cumprod = alphas_cumprod.to(device)
...
class InnerTrainer:
    def __init__(self, net: AdaptiveUNetSup, tcfg: TrainConfig, dcfg: DiffConfig):
    ...
    def _maybe_drop_labels(self, y: torch.Tensor) -> torch.Tensor:
        if self.tcfg.cond_drop <= 0: return y
        mask = (torch.rand_like(y.float()) < self.tcfg.cond_drop); return y.masked_fill(mask, self.net.null_class)
    def _step_batch(self, x: torch.Tensor, y: torch.Tensor, train=True) -> float:
        B = x.size(0); t = torch.randint(0, self.diff.T, (B,), device=self.device, dtype=torch.long)
        x_noisy, eps = self.diff.q_sample(x, t); y_in = self._maybe_drop_labels(y)
        if train: self.optim.zero_grad(set_to_none=True)
        eps_pred = self.net(x_noisy, t, y_in); loss = F.mse_loss(eps_pred, eps)
        if train:
            loss.backward()
            if self.tcfg.grad_clip is not None: nn.utils.clip_grad_norm_(self.net.parameters(), self.tcfg.grad_clip)
            self.optim.step()
    ...
    def _eval_epoch(self, loader):
        ...
        for x, y in loader:
            ...
            l = self._step_batch(x, y, train=False); b = x.size(0); tot += l * b; n += b
        return tot / max(n,1)
    def fit(self, dl_train, dl_val, max_epochs=50):
        ...
        for _ in range(max_epochs):
            ...
            for x, y in dl_train:
                x = x.to(self.device, non_blocking=True); y = y.to(self.device, non_blocking=True)
                self._step_batch(x, y, train=True)
...
def snapshot(net: AdaptiveUNetSup):
    return {"state": {k: v.detach().cpu().clone() for k,v in net.state_dict().items()},
            "widths": net.widths.copy(), "pools": list(net._pools_here)}

def restore(net: AdaptiveUNetSup, snap):
    curr, target = net.widths, snap["widths"]
    if curr != target:
        new = AdaptiveUNetSup(in_ch=net.in_ch, widths=target, pooling_indices=[i for i,f in enumerate(snap["pools"]) if f],
                              num_classes=net.num_classes, emb_dim=net.emb_dim, bias=net.bias)
        new.load_state_dict(snap["state"])
        net.encoder = new.encoder; net.downs = new.downs; net._pools_here = new._pools_here
        net.mid = new.mid; net.decoder = new.decoder; net.ups = new.ups; net.head = new.head
        net.time_mlp = new.time_mlp; net.class_emb = new.class_emb
    else:
        net.load_state_dict(snap["state"])

def can_widen(net: AdaptiveUNetSup, ex_k: int, scfg: 'SearchConfig') -> bool:
    if ex_k <= 0: return False
    projected = net.total_neurons() + ex_k*(len(net.encoder) + len(net.decoder) + 1)
    if projected > scfg.max_neurons: return False
    if any(w + ex_k > scfg.max_width for w in net.widths): return False
    return True

def can_deepen(net: AdaptiveUNetSup, scfg: 'SearchConfig') -> bool:
...
def _train_eval_val(net: AdaptiveUNetSup, dl_train, dl_val, tcfg: TrainConfig, dcfg: DiffConfig, max_epochs: int) -> float:
    return InnerTrainer(net, tcfg, dcfg).fit(dl_train, dl_val, max_epochs=max_epochs)

def diff_sup_width_to_depth(net, dl_train, dl_val, tcfg, scfg, dcfg, max_epochs=50):
...
def diff_sup_depth_to_width(net, dl_train, dl_val, tcfg, scfg, dcfg, max_epochs=50):
    best_snap = snapshot(net); best_val = _train_eval_val(net, dl_train, dl_val, tcfg, dcfg, max_epochs)
    depth_fails = 0
    while depth_fails < scfg.patience_depth and can_deepen(net, scfg):
...
def diff_sup_alt_depth_first(net, dl_train, dl_val, tcfg, scfg, dcfg, max_epochs=50):
    best_snap = snapshot(net); best_val = _train_eval_val(net, dl_train, dl_val, tcfg, dcfg, max_epochs)
...
def diff_sup_alt_width_first(net, dl_train, dl_val, tcfg, scfg, dcfg, max_epochs=50):
    best_snap = snapshot(net); best_val = _train_eval_val(net, dl_train, dl_val, tcfg, dcfg, max_epochs)
    total = 0; ok = lambda e: scfg.max_total_epochs is None or e < scfg.max_total_epochs
    improved = True
    while improved and ok(total):
...
def diff_sup_depth_only(net, dl_train, dl_val, tcfg, scfg, dcfg, max_epochs=50):
...
def diff_sup_width_only(net, dl_train, dl_val, tcfg, scfg, dcfg, max_epochs=50):
    best_snap = snapshot(net); best_val = _train_eval_val(net, dl_train, dl_val, tcfg, dcfg, max_epochs); fails = 0
