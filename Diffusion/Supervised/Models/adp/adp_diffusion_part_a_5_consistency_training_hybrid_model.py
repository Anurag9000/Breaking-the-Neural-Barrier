# ============================================================
# File: adp_diff_consistency.py  (MODEL)
# Single-model "consistency training"-style diffusion with
# a one-network objective that enforces cross-time consistency
# (no EMA/teacher). Includes all 6 ADP policies.
# ============================================================
...
# -----------------------------
# Adaptive U-Net backbone (width/depth mutable)
# -----------------------------
...
class AdaptiveUNet(nn.Module):
    ...
    def _build(self, in_ch: int):
        ...
        self.out_conv = nn.Conv2d(up_ch, in_ch, 1)  # predicts epsilon noise
...
# -----------------------------
# Single-model consistency diffusion
# -----------------------------

class ConsistencyDiffusionSingleModel(nn.Module):
    """
    Epsilon-prediction network trained with a cross-time consistency loss.
    For a pair (t > s), we enforce that the denoised estimate implied by
    x_t is consistent with the denoised estimate obtained after a deterministic
    DDIM transport to time s and a fresh forward pass at s.

    No teacher/EMA is used — the target is a stop-grad copy of the model's own
    prediction at t, forming a single-model self-consistency constraint.
    """
    def __init__(self, img_ch: int = 3, widths: List[int] = [32,64,96], T: int = 1000,
        ...
        self.net = AdaptiveUNet(in_ch=img_ch, widths=widths, time_ch=128)

    # helpers
    def _a_b(self, t_idx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        a_bar = self.alphas_cumprod[t_idx]
        a = torch.sqrt(a_bar)
        b = torch.sqrt(1.0 - a_bar)
        return a, b
    ...
    def q_sample(self, x0: torch.Tensor, t_idx: torch.Tensor, eps: Optional[torch.Tensor] = None):
        ...
        return torch.sqrt(a_bar) * x0 + torch.sqrt(1.0 - a_bar) * eps, eps

    def _x0_from(self, x_t: torch.Tensor, t_idx: torch.Tensor, eps_pred: torch.Tensor) -> torch.Tensor:
        a_bar = self.alphas_cumprod[t_idx][:, None, None, None]
        return (x_t - torch.sqrt(1.0 - a_bar) * eps_pred) / torch.sqrt(a_bar)

    def _ddim_transport(self, x_t: torch.Tensor, t_idx: torch.Tensor, s_idx: torch.Tensor, eps_pred_t: torch.Tensor) -> torch.Tensor:
        # Deterministic DDIM: x_s = sqrt(a_s) * x0_pred + sqrt(1-a_s) * eps_pred_t
        x0_pred_t = self._x0_from(x_t, t_idx, eps_pred_t)
        a_bar_s = self.alphas_cumprod[s_idx][:, None, None, None]
        return torch.sqrt(a_bar_s) * x0_pred_t + torch.sqrt(1.0 - a_bar_s) * eps_pred_t

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B = x.size(0)
        device = x.device
        # sample t in [1..T-1], s in [0..t-1]
        t_idx = torch.randint(1, self.T, (B,), device=device, dtype=torch.long)
        s_idx = (t_idx * torch.rand(B, device=device)).long()
        t_norm = (t_idx.float() + 0.5) / self.T
        s_norm = (s_idx.float() + 0.5) / self.T

        # noisy pair from same (x0, eps)
        x_t, eps = self.q_sample(x, t_idx)
        # first pass @ t
        eps_pred_t = self.net(x_t, t_norm)
        x0_pred_t = self._x0_from(x_t, t_idx, eps_pred_t)
        # transport to s using deterministic DDIM driven by the same eps_pred_t (no extra net)
        x_s_hat = self._ddim_transport(x_t, t_idx, s_idx, eps_pred_t)
        # second pass @ s
        eps_pred_s = self.net(x_s_hat.detach(), s_norm)
        x0_pred_s = self._x0_from(x_s_hat, s_idx, eps_pred_s)

        # consistency loss (self-target with stop-grad)
        loss_cons = 0.5 * F.mse_loss(x0_pred_s, x0_pred_t.detach())
        # small anchoring to avoid drift
        loss_anchor = 0.5 * F.mse_loss(eps_pred_t, eps)
        return loss_cons + self.lambda_anchor * loss_anchor
    ...
    def sample(self, B: int, img_ch: int, H: int, W: int, device: Optional[torch.device] = None, steps: Optional[int] = None):
        ...
        for i in reversed(range(steps)):
            ...
            eps_pred = self.net(x, t_norm)
            a_bar = self.alphas_cumprod[i]
            x0_pred = (x - torch.sqrt(1.0 - a_bar) * eps_pred) / torch.sqrt(a_bar)
            if i > 0:
                a_bar_prev = self.alphas_cumprod[i-1]
            else:
                a_bar_prev = self.alphas_cumprod[0]
            x = torch.sqrt(a_bar_prev) * x0_pred + torch.sqrt(1.0 - a_bar_prev) * eps_pred
        return torch.clamp(x, -1, 1)

    # ADP passthrough
    def neurons(self) -> int:
        return self.net.neurons()
    def snapshot_state(self):
        return {'net': self.net.snapshot_state()}
    def restore_state(self, snap):
        self.net.restore_state(snap['net'])
    def append_depth(self):
        self.net.append_depth()
    def widen_all(self, ex_k: int):
        self.net.widen_all(ex_k)


# -----------------------------
# ES trainer and ADP policies
# -----------------------------
...
def train_one_model(model: ConsistencyDiffusionSingleModel, train_loader, val_loader, cfg: TrainCfg) -> Tuple[float, Dict]:
...
def adp_depth_then_width(model: ConsistencyDiffusionSingleModel, train_loader, val_loader, tr: TrainCfg, s: SearchCfg):
...
# ============================================================
# End of adp_diff_consistency.py
# ============================================================
