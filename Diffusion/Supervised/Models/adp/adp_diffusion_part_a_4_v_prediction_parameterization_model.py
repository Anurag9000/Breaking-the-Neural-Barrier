# ============================================================
# File: adp_diff_vpred.py  (MODEL)
# Single-model VP/DDPM with v-parameterization (Imagen-style)
# and integrated 6x ADP policies (depth→width, width→depth,
# alt-depth, alt-width, depth-only, width-only). No EMA/teacher.
# ============================================================
...
# -----------------------------
# Cosine schedule (DDPM-compatible) and buffers
# -----------------------------
...
class AdaptiveUNet(nn.Module):
    ...
    def _build(self, in_ch: int):
        ...
        self.out_conv = nn.Conv2d(up_ch, in_ch, 1)  # predicts v
...
# -----------------------------
# Single-model DDPM with v-parameterization
# -----------------------------

class VPredDiffusionSingleModel(nn.Module):
    ...
    # helpers
    def _ab(self, t_idx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        a, b = self._ab(t_idx)
        # v-target = a * eps - b * x0
        v_target = a[:, None, None, None] * eps - b[:, None, None, None] * x
        v_pred = self.net(x_t, t_norm)
        return 0.5 * F.mse_loss(v_pred, v_target)
    ...
    def sample(self, B: int, img_ch: int, H: int, W: int, device: Optional[torch.device] = None, steps: Optional[int] = None):
        ...
        for i in reversed(range(steps)):
            ...
            a = torch.sqrt(a_bar)
            b = torch.sqrt(1.0 - a_bar)
            # recover eps from v and x_t: eps = b * x_t + a * v
            eps_pred = b * x + a * v
            beta = self.betas[i]
            alpha = self.alphas[i]
            ...
            x = (1.0 / torch.sqrt(alpha)) * (x - beta / torch.sqrt(1.0 - alpha_bar) * eps_pred) + torch.sqrt(beta) * noise
...
def train_one_model(model: VPredDiffusionSingleModel, train_loader, val_loader, cfg: TrainCfg) -> Tuple[float, Dict]:
...
def adp_depth_then_width(model: VPredDiffusionSingleModel, train_loader, val_loader, tr: TrainCfg, s: SearchCfg):
