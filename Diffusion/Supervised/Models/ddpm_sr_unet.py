...
# Reuse core UNet blocks but allow extra conditioning channels
...
class SRDDPMUNet(nn.Module):
    """Single-model Super-Resolution Diffusion UNet (epsilon prediction).
    Condition: upsampled low-res image concatenated with x_t at input.
    Input channels = 3 (x_t) + 3 (cond) = 6
    """
    def __init__(self, in_ch_xt: int = 3, in_ch_cond: int = 3, base_ch: int = 64, ch_mults=(1,2,2,4), time_dim: int = 256):
        super().__init__()
        self.time_emb = SinusoidalTimeEmbedding(time_dim)
        self.time_mlp = MLPTime(time_dim, time_dim)
        in_ch = in_ch_xt + in_ch_cond
        ...
        for m, _skip in zip(reversed(ch_mults), reversed(self.skips_ch)):
            out_ch = base_ch * m
            self.ups.append(Up(ch, out_ch, time_dim, use_attn=False))
            ch = out_ch
        self.out_norm = nn.GroupNorm(8, ch)
        self.out_conv = nn.Conv2d(ch, in_ch_xt, 3, padding=1)

    def forward(self, x_t: torch.Tensor, t: torch.Tensor, cond_upsampled: torch.Tensor) -> torch.Tensor:
        t_feat = self.time_mlp(self.time_emb(t))
        x = torch.cat([x_t, cond_upsampled], dim=1)
        x = self.in_conv(x)
        skips = []
        for d in self.downs:
            x, s = d(x, t_feat)
    ...
    @torch.no_grad()
    def sample(self, cond_upsampled: torch.Tensor, betas: torch.Tensor, cfg_scale: float = 0.0, device: Optional[torch.device] = None, eta: float = 0.0):
        device = device or next(self.parameters()).device
        b, _, h, w = cond_upsampled.shape
        ...
        for i in reversed(range(len(betas))):
            ...
            eps = self.forward(x, t, cond_upsampled)
