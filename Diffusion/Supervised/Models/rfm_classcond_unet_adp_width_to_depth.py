...
# Load baseline
BASE_PATH = Path(__file__).with_name("rfm_classcond_unet.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.RectifiedFlowUNet

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the RectifiedFlowUNet model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['base_ch'] = get_attr(model, ['base_ch'], None)
        kwargs['ch_mults'] = get_attr(model, ['ch_mults'], None)
        kwargs['num_classes'] = get_attr(model, ['num_classes', 'head.out_features', 'fc.out_features', 'classifier.out_features'], 1)
        kwargs['time_dim'] = get_attr(model, ['time_dim'], 1)
