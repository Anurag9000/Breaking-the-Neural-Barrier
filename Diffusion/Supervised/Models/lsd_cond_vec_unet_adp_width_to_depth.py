...
# Load baseline
BASE_PATH = Path(__file__).with_name("lsd_cond_vec_unet.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.LatentCondVecUNet

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the LatentCondVecUNet model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['z_ch'] = get_attr(model, ['z_ch'], None)
        kwargs['cond_dim'] = get_attr(model, ['cond_dim'], 1)
