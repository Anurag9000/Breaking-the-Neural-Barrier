...
def make_cifar10_sr_loaders(data_root: str, batch_size: int, vfrac: float = 0.1, seed: int = 42):
    # HR: 32x32, LR: 16x16 (2x downscale)
...
def make_lr_condition(x_hr):
    # Bicubic downscale to 16 and upsample back to 32 to form conditioning
    x = (x_hr + 1) * 0.5  # [0,1]
...
def train(cfg: TrainConfig):
    ...
    train_loader, val_loader, test_loader = make_cifar10_sr_loaders(cfg.data_root, cfg.batch_size)
    ...
    logger = ContinuousLogger(Path('results_run_ddpm_sr_unet'), 'run_ddpm_sr_unet', 'train')
    ...
    # sample: take test set images -> make LR cond -> generate SR
