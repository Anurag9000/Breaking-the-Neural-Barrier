# ============================================================
# File: run_adp_diff_sde.py  (RUN)
# Unified runner for Score-based VP-SDE with 6 ADP policies
# Single-model only. No EMA/teacher.
# ============================================================
...
# Local import when saved as files side-by-side:
# from adp_diff_sde import ScoreSDESingleModel, TrainCfg, SearchCfg, POLICIES
...
def main():
    p = argparse.ArgumentParser(description='Score-based VP-SDE with ADP (single model)')

    # Part fixed to sde here to keep CLI consistent across parts
    p.add_argument('--part', type=str, default='sde', choices=['sde'])
    ...
    model = ScoreSDESingleModel(img_ch=3, widths=args.widths, beta_min=args.beta_min, beta_max=args.beta_max)
    ...
    print(f"[SDE] Finished. Best val loss = {best:.4f}. Final neurons = {model.neurons()}.")
