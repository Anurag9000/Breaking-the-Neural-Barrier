# ============================================================
# File: run_adp_diff_backbones.py  (RUN)
# Runner for single-model DDPM (ε-pred) with pluggable backbones
# and 6 ADP policies. Select with: --backbone {unet,resnet,convnext,vit,dit,swin,unetpp,tiny}
# ============================================================
...
from adp_diff_backbones import EpsDDPMSingleModel, TrainCfg, SearchCfg, POLICIES
...
def main():
    p = argparse.ArgumentParser(description='DDPM (ε-pred) + ADP with pluggable backbones')

    # Backbone & ADP
    p.add_argument('--backbone', type=str, default='unet',
                   choices=['unet','resnet','convnext','vit','dit','swin','unetpp','tiny'])
    ...
    # Model hyperparams
    # For CNN backbones, widths is per-stage channels.
    # For token backbones, widths[0]=embed dim and len(widths)=depth baseline.
    ...
    model = EpsDDPMSingleModel(img_ch=3, widths=args.widths, T=args.T, backbone=args.backbone)
    ...
    print(f"[BACKBONES:{args.backbone}] Best val loss = {best:.4f}. Final neurons = {model.neurons()}.")
...
# ============================================================
# End of run_adp_diff_backbones.py
# ============================================================
