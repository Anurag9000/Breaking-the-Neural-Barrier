...
def main():
    ap = argparse.ArgumentParser(description="BigBird-style text classifier on AG News")
    ...
    best, bad = 0.0, 0
    patience = 2
    logger = ContinuousLogger(Path("results_run_bigbird_text"), "run_bigbird_text", "train")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict(), "vocab": vocab}, "BigBirdEncoder_best.pth")
        else:
            bad += 1
            if bad >= patience:
                break
