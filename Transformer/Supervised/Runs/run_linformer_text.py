...
from model_linformer import LinformerEncoder
...
def main():
    ap = argparse.ArgumentParser(description="Linformer text classifier on AG News")
    ...
    model = LinformerEncoder(vocab=len(vocab), num_classes=num_classes, dim=256, depth=6, heads=8, k=64).to(args.device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr)
    crit = nn.CrossEntropyLoss()

    best, bad, patience = 0.0, 0, 4
    logger = ContinuousLogger(Path("results_run_linformer_text"), "run_linformer_text", "train")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict()}, "Linformer_best.pth")
        else:
            bad += 1
            if bad >= patience:
                break
