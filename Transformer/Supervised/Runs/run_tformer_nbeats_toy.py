...
from model_tformer_nbeats import TNBeats
...
def main():
    ...
    logger = ContinuousLogger(Path('results_run_tformer_nbeats_toy'), 'run_tformer_nbeats_toy', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if rmse + 1e-6 < best:
            best=rmse; bad=0; torch.save({'model': model.state_dict()}, 'TNBeats_best.pth')
        else:
            bad+=1
            if bad>=patience:
