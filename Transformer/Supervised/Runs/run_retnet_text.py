...
def main():
    ap = argparse.ArgumentParser(description="RetNet text classifier on AG News")
    ...
    model = RetNetClassifier(vocab=len(vocab), num_classes=num_classes).to(args.device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict()}, "RetNet_best.pth")
        else:
            bad += 1
            if bad >= patience:
                break
