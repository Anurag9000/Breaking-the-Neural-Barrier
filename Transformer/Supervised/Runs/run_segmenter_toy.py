...
from model_segmenter_vit import SegmenterViT

# Toy segmentation: colored rectangles vs background
class ShapesSeg(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # draw one rectangle of class 1 and one of class 2
        for cls in [1,2]:
...
def mIoU(pred, target, K):
    ious=[]
    for k in range(1,K):  # ignore background 0
        p=(pred==k); t=(target==k)
...
def main():
    ...
    model = SegmenterViT(num_classes=3).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_segmenter_toy'), 'run_segmenter_toy', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if miou > best + 1e-6:
            best=miou; bad=0; torch.save({'model': model.state_dict()}, 'SegmenterViT_best.pth')
        else:
            bad+=1
            if bad>=patience:
