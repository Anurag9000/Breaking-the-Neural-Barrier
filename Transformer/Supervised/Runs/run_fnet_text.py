...
class TSVText(Dataset):
    def __init__(self, n=4000, max_len=128):
        self.samples=[]; g=torch.Generator().manual_seed(1)
        for i in range(n):
...
def main():
    ...
    model = FNetClassifier(vocab=8, num_classes=2).to(args.device)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if acc > best + 1e-6:
            best=acc; bad=0; torch.save({'model': model.state_dict()}, 'FNet_best.pth')
        else:
            bad+=1
            if bad>=patience:
