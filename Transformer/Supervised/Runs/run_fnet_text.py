...
from model_fnet import FNetClassifier
...
def main():
    ap = argparse.ArgumentParser(description="FNet text classifier on AG News")
    ...
    model = FNetClassifier(vocab=len(vocab), num_classes=num_classes).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr)
    crit = nn.CrossEntropyLoss()

    best, bad, patience = 0.0, 0, 3
    logger = ContinuousLogger(Path("results_run_fnet_text"), "run_fnet_text", "train")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict()}, "FNet_best.pth")
        else:
            bad += 1
            if bad >= patience:
                break
