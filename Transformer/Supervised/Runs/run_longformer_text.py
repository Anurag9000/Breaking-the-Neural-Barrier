...
def main():
    ap = argparse.ArgumentParser(description="Longformer text classifier on AG News")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict(), "vocab": vocab}, "LongformerEncoder_best.pth")
        else:
            bad += 1
