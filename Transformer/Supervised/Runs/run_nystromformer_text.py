...
class TSVText(Dataset):
    def __init__(self, n=4000, max_len=256):
        self.samples=[]; g=torch.Generator().manual_seed(5)
        for i in range(n):
...
def main():
    ...
    model = NystromformerEncoder(vocab=16, num_classes=2, dim=256, depth=6, heads=8, landmarks=64).to(args.device)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if acc > best + 1e-6:
            best=acc; bad=0; torch.save({'model': model.state_dict()}, 'Nystromformer_best.pth')
        else:
            bad+=1
            if bad>=patience:
