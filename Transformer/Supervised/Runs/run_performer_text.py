...
def main():
    ap = argparse.ArgumentParser(description="Performer text classifier on AG News")
    ...
    model = PerformerEncoder(vocab=len(vocab), num_classes=num_classes, dim=256, depth=6, heads=8).to(args.device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr)
    crit = nn.CrossEntropyLoss()

    best, bad, patience = 0.0, 0, 4
    logger = ContinuousLogger(Path("results_run_performer_text"), "run_performer_text", "train")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc > best + 1e-6:
            best = acc
            bad = 0
            torch.save({"model": model.state_dict()}, "Performer_best.pth")
        else:
            bad += 1
            if bad >= patience:
                break
