...
from model_t2t_vit import T2T_ViT
...
def loaders(dataset, root, img, batch, workers, seed=42):
    ...
    return (DataLoader(trS,batch,True,num_workers=workers,pin_memory=True),
            DataLoader(vaS,batch,False,num_workers=workers,pin_memory=True),
...
def main():
    ...
    p.add_argument('--save',default='T2T_ViT_best.pth')
    a=p.parse_args()
    ...
    m=T2T_ViT(img_size=a.img_size, num_classes=nc, encoder_dim=a.enc_dim, depth=a.depth, heads=a.heads).to(dev)
