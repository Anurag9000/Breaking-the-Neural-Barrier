...
class ToyForecast(Dataset):
    def __init__(self, n=2000, in_len=96, pred_len=24):
        ...
        for i in range(n):
            f = torch.rand(1, generator=g)*3 + 1
...
def main():
    ...
    model = Informer(in_feats=1, pred_len=24).to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_informer_toy'), 'run_informer_toy', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if rmse + 1e-6 < best:
            best=rmse; bad=0; torch.save({'model': model.state_dict()}, 'Informer_best.pth')
        else:
            bad+=1
            if bad>=patience:
