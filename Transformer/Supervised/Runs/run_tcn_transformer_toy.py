...
def main():
    ...
    model = TCNTransformer().to(args.device)
    ...
    logger = ContinuousLogger(Path('results_run_tcn_transformer_toy'), 'run_tcn_transformer_toy', 'train')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        msg = f"Epoch {epoch}: val_RMSE={rmse:.4f}"
        ...
        if rmse + 1e-6 < best:
            best=rmse; bad=0; torch.save({'model': model.state_dict()}, 'TCNTransformer_best.pth')
        else:
            bad+=1
            if bad>=patience:
