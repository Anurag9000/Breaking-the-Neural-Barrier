...
def main():
    ...
    ap.add_argument('--cutout_p', type=float, default=0.5)
    ap.add_argument('--save', default='dae_vit_best.pt')
    args=ap.parse_args()

    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    tr,va,te=loaders(args.dataset,args.data_dir,args.batch_size)

    cfg=DAEConfig(args.img,args.patch,args.dim,args.depth,args.heads,args.ratio,args.noise_std,args.cutout_p)
    model=ViTDAE(cfg).to(device)
