...
def train(cfg: Config):
    ...
    model = SimCLRViT(cfg.img_size, cfg.patch_size, cfg.embed_dim, cfg.depth, cfg.heads, cfg.mlp_ratio,
    ...
    for epoch in range(cfg.epochs):
        ...
        # val as recon of contrastive objective (use eval views once)
        model.eval(); va_loss=0
        with torch.no_grad():
            for imgs, _ in val_loader:
                x1 = imgs.to(cfg.device)
                x2 = imgs.to(cfg.device)
                z1, z2 = model(x1, x2)
                loss = model.nt_xent(z1, z2, cfg.temperature)
