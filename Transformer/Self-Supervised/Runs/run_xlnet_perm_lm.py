...
from model_xlnet_perm_lm import XLNetPermLM
...
class Config:
    ...
    out_dir: str = './outs_xlnet_perm_lm'
    seed: int = 42
...
def train(cfg: Config):
    ...
    model = XLNetPermLM(len(tok.itos), cfg.dim, cfg.depth, cfg.heads, cfg.mlp_ratio, cfg.max_len).to(cfg.device)
    ...
    save_plot(tr_curve, 'XLNet PermLM Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    a.add_argument('--out_dir', type=str, default='./outs_xlnet_perm_lm')
