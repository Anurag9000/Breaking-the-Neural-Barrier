...
from model_barlow_vit import BarlowViT
...
def train(cfg: Config):
    ...
    model = BarlowViT(cfg.img_size, cfg.patch_size, cfg.embed_dim, cfg.depth, cfg.heads, cfg.mlp_ratio,
                      cfg.proj_hidden, cfg.proj_out).to(cfg.device)
    ...
    for epoch in range(cfg.epochs):
        ...
        for imgs, _ in train_loader:
            ...
            z1, z2 = model(x1, x2)
            loss = BarlowViT.barlow_loss(z1, z2, lambd=5e-3)
            opt.zero_grad()
            loss.backward()
    ...
    save_plot(tr_curve, "Barlow Train Loss", os.path.join(cfg.out_dir, "train_loss.png"), semilogy=True)
    save_plot(va_curve, "Barlow Val Loss", os.path.join(cfg.out_dir, "val_loss.png"), semilogy=True)
