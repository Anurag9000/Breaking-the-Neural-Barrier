...
from model_barlow_vit import BarlowTwinsViT
...
def train(cfg: Config):
    ...
    model = BarlowTwinsViT(cfg.img_size, cfg.patch_size, cfg.embed_dim, cfg.depth, cfg.heads, cfg.mlp_ratio,
                           cfg.proj_hidden, cfg.proj_out, cfg.lambd).to(cfg.device)
    ...
    logger = ContinuousLogger(Path('results_run_barlow_vit'), 'run_barlow_vit', 'train')
    ...
    save_plot(tr_curve, 'Barlow Twins Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
    save_plot(va_curve, 'Barlow Twins Val Loss', os.path.join(cfg.out_dir,'val_loss.png'), semilogy=True)
...
if __name__=='__main__':
    ...
    p.add_argument('--proj_hidden', type=int, default=8192)
    p.add_argument('--proj_out', type=int, default=8192)
    ...
    p.add_argument('--out_dir', type=str, default='./outs_barlow_vit')
