"""Runner: Colorization-ViT
- Converts RGB to Lab to build L input and ab targets.
"""
import argparse, torch, torch.optim as optim
from torch.utils.data import DataLoader, random_split
from torchvision import datasets, transforms
from colorization_vit import ColorizationViT, ColorConfig


def loaders(dataset, data_dir, batch, workers=2):
    # We keep tensors in [0,1] for Lab conversion; normalize only post-colorization if needed.
    ...
    if dataset=='cifar10':
        full=datasets.CIFAR10(data_dir, train=True, transform=base, download=True)
        test=datasets.CIFAR10(data_dir, train=False, transform=plain, download=True)
    else:
        full=datasets.CIFAR100(data_dir, train=True, transform=base, download=True)
...
def main():
    ...
    ap.add_argument('--save', default='colorization_vit_best.pt')
    args=ap.parse_args()

    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    tr,va,te=loaders(args.dataset,args.data_dir,args.batch_size)

    cfg=ColorConfig(args.img,args.patch,args.dim,args.depth,args.heads,args.ratio)
    model=ColorizationViT(cfg).to(device)
    ...
    print(f"Test colorization loss: {test:.4f}")
