import os, json, argparse, random
import numpy as np
import torch
import torch.optim as optim
from dataclasses import dataclass
from model_igpt_patch import iGPTPatch
import matplotlib.pyplot as plt
from _common_real_image import make_real_image_loaders

@dataclass
class Config:
    data_root: str = './data'
    dataset: str = 'imagefolder'
    img_size: int = 32
    patch_size: int = 4
    embed_dim: int = 256
    depth: int = 8
    heads: int = 8
    vocab_size: int = 8192
    batch_size: int = 256
    epochs: int = 200
    lr: float = 3e-4
    weight_decay: float = 0.01
    patience: int = 20
    device: str = 'cuda' if torch.cuda.is_available() else 'cpu'
    out_dir: str = './outs_igpt_patch'
    seed: int = 42


def set_seed(seed):
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


def make_loaders(cfg: Config):
    return make_real_image_loaders(cfg.data_root, cfg.batch_size, image_size=cfg.img_size)


def save_plot(curve, title, path, semilogy=False):
    plt.figure(); plt.semilogy(curve) if semilogy else plt.plot(curve)
    plt.title(title); plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.grid(True)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    plt.savefig(path, bbox_inches='tight'); plt.close()


def train(cfg: Config):
    set_seed(cfg.seed)
    train_loader, val_loader, _ = make_loaders(cfg)
    model = iGPTPatch(cfg.img_size, cfg.patch_size, 3, cfg.embed_dim, cfg.depth, cfg.heads, cfg.vocab_size).to(cfg.device)
    opt = optim.AdamW(model.parameters(), lr=cfg.lr, weight_decay=cfg.weight_decay)
    best_val=float('inf'); best_state=None; patience=cfg.patience
    tr_curve=[]; va_curve=[]

    for ep in range(cfg.epochs):
        model.train(); tr=0.0
        for imgs,_ in train_loader:
            imgs=imgs.to(cfg.device)
            loss = model(imgs)
            opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step()
            tr += loss.item()*imgs.size(0)
        tr/=len(train_loader.dataset); tr_curve.append(tr)

        model.eval(); va=0.0
        with torch.no_grad():
            for imgs,_ in val_loader:
                imgs=imgs.to(cfg.device)
                loss = model(imgs)
                va += loss.item()*imgs.size(0)
        va/=len(val_loader.dataset); va_curve.append(va)
        print(f'Epoch {ep+1}/{cfg.epochs} | train {tr:.4f} | val {va:.4f}')
        if va < best_val - 1e-4:
            best_val=va; best_state={k:v.detach().cpu() for k,v in model.state_dict().items()}; patience=cfg.patience
        else:
            patience-=1
            if patience==0:
                print('Early stopping.'); break

    os.makedirs(cfg.out_dir, exist_ok=True)
    save_plot(tr_curve, 'iGPT Train Loss', os.path.join(cfg.out_dir,'train_loss.png'), semilogy=True)
    save_plot(va_curve, 'iGPT Val Loss', os.path.join(cfg.out_dir,'val_loss.png'), semilogy=True)
    if best_state is not None:
        torch.save(best_state, os.path.join(cfg.out_dir,'best_state.pt'))
        with open(os.path.join(cfg.out_dir,'summary.json'),'w') as f:
            json.dump({'best_val': float(best_val)}, f, indent=2)

if __name__=='__main__':
    a=argparse.ArgumentParser()
    a.add_argument('--dataset', type=str, default='imagefolder')
    a.add_argument('--img_size', type=int, default=32)
    a.add_argument('--patch_size', type=int, default=4)
    a.add_argument('--embed_dim', type=int, default=256)
    a.add_argument('--depth', type=int, default=8)
    a.add_argument('--heads', type=int, default=8)
    a.add_argument('--vocab_size', type=int, default=8192)
    a.add_argument('--batch_size', type=int, default=256)
    a.add_argument('--epochs', type=int, default=200)
    a.add_argument('--lr', type=float, default=3e-4)
    a.add_argument('--weight_decay', type=float, default=0.01)
    a.add_argument('--patience', type=int, default=20)
    a.add_argument('--out_dir', type=str, default='./outs_igpt_patch')
    a.add_argument('--seed', type=int, default=42)
    cfg = Config(**vars(a.parse_args()))
    train(cfg)
