...
from adp_gat_infograph import InfoGraph_GAT, Config
...
def train(m, data, cfg):
    ...
    for ep in range(1, cfg.epochs+1):
        ...
        if loss.item() < best-1e-4:
            best, bad = loss.item(), 0; torch.save(m.state_dict(), cfg.ckpt)
        else: bad += 1
        if ep%10==0: print(f"[InfoGraph-GAT] ep {ep} loss {loss.item():.4f} best {best:.4f}")
...
if __name__=="__main__":
    ...
    m = InfoGraph_GAT(ds.num_features, Config()).to(Config.device); train(m, data, m.cfg)
