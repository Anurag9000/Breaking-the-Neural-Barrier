...
from adp_gtr_attrmask import AttrMask_GTR, Config
...
def train(m, data, cfg):
    ...
    for ep in range(1, cfg.epochs+1):
        ...
        if ep%10==0: print(f"[AttrMask-GTR] ep {ep} loss {loss.item():.4f} best {best:.4f}")
...
if __name__=="__main__":
    ...
    m = AttrMask_GTR(ds.num_features, Config()).to(Config.device); train(m, data, m.cfg)
