...
def build(parser):
    parser.add_argument("--dataset", type=str, default="MUTAG")  # any TUDataset
...
def get_common(args):
    dl_train, dl_val, dl_test, in_dim = make_tu_loader(args.dataset, args.batch_size, args.num_workers, args.val_split, root=args.data_root)
    ...
    net = AdaptiveGNN(in_dim=in_dim, hidden_dims=widths, conv_type=args.conv_type, heads=args.heads,
                      pooling_indices=args.pool-idx if isinstance(args.pool_idx, list) else [], dropout=args.dropout,
    ...
    tcfg = TrainConfig(lr=args.lr, weight_decay=args.weight_decay, es_patience=args.es_patience,
                       ...
                       lambda_ntx=args.lambda_ntx, lambda_barlow=args.lambda_barlow,
...
def final_eval(net, dl_test, device):
    import torch
    from adp_gnn_ssl_core import nt_xent, barlow_twins, TrainConfig
    ...
    with torch.no_grad():
        for batch in dl_test:
            ...
            d1, d2 = adp_gnn_ssl_core.two_view_augment(batch, feat_drop=tcfg.feat_drop, edge_drop=tcfg.edge_drop, node_drop=tcfg.node_drop)
            d1 = d1.to(device); d2 = d2.to(device)
            _, z1 = net(d1); _, z2 = net(d2)
            loss = nt_xent(z1, z2, temperature=tcfg.temperature)
...
def main():
    ...
    net = gnn_alt_width_first(net, dl_train, dl_val, tcfg, sccfg, max_epochs=args.max_epochs)
