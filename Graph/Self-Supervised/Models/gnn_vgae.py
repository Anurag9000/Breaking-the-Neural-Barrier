# Variational Graph Autoencoder (VGAE) — single-model
...
class VGAE(nn.Module):
    def __init__(self, in_dim, hidden, z_dim, num_layers=2, dropout=0.2):
        super().__init__()
        assert num_layers >= 2
        ...
        self.mu = GCNConv(hidden, z_dim)
        self.logvar = GCNConv(hidden, z_dim)
        self.dropout = dropout

    def encode_hidden(self, x, edge_index):
    ...
    def encode(self, x, edge_index):
        h = self.encode_hidden(x, edge_index)
    ...
    def loss(self, z, mu, logvar, pos_edge_index, num_nodes):
    ...
    def forward(self, data):
        ...
        loss = self.loss(z, mu, logvar, edge_index, data.num_nodes)
        return loss
