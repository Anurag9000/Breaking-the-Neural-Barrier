# DeepWalk — single-model skip-gram on random walks (PyTorch)
...
class SkipGram(nn.Module):
    ...
    def forward(self, u, v, neg):
        ...
        neg_e = self.output(neg)  # [B,K,D]
        neg_score = torch.bmm(neg_e, u_e.unsqueeze(-1)).squeeze(-1)  # [B,K]
...
# Utilities to sample walks
@torch.no_grad()
def random_walks(edge_index: torch.Tensor, num_nodes: int, wl: int, wpn: int) -> List[List[int]]:
    ...
    for u in range(num_nodes):
        for _ in range(wpn):
            ...
            for _ in range(wl - 1):
                if len(adj[cur]) == 0:
                    break
                nxt = adj[cur][torch.randint(len(adj[cur]), (1,)).item()]
                path.append(nxt)
                cur = nxt
            walks.append(path)
    return walks
...
def evaluate_ssl(model: nn.Module, pairs: List[Tuple[int,int]], cfg: TrainConfig, num_nodes: int) -> Tuple[float, Dict[str,float]]:
    model.eval();
    with torch.no_grad():
        # simple average loss on a small subset
...
def train_with_early_stop(model: nn.Module, pairs: List[Tuple[int,int]], cfg: TrainConfig, num_nodes: int):
    ...
    # pack all pairs per epoch (toy implementation)
