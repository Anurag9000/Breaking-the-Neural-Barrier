...
def recon_bce(node_h: torch.Tensor, edge_index: torch.Tensor, num_nodes: int, num_neg: Optional[int] = None) -> torch.Tensor:
    device = node_h.device
    if num_neg is None:
        num_neg = edge_index.size(1)
    src, dst = edge_index[0], edge_index[1]
    pos_score = (node_h[src] * node_h[dst]).sum(dim=1)
    pos_loss = F.binary_cross_entropy_with_logits(pos_score, torch.ones_like(pos_score))
    i = torch.randint(0, num_nodes, (num_neg,), device=device)
    j = torch.randint(0, num_nodes, (num_neg,), device=device)
    neg_score = (node_h[i] * node_h[j]).sum(dim=1)
...
def two_view_augment(data, feat_drop=0.2, edge_drop=0.2, node_drop=0.0):
    def aug_once(d):
        ...
        x1 = dropout_features(x, feat_drop)
        ei1 = dropout_edges(ei, edge_drop)
        if node_drop > 0:
...
class GNNBlock(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, conv_type: str = "sage", heads: int = 4, dropout: float = 0.0):
        ...
        if conv_type == "sage":
            ...
            self.proj = nn.Linear(self.out_dim, out_dim) if self.out_dim != out_dim else nn.Identity()
        else:
...
class AdaptiveGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        batch = data.batch if hasattr(data, "batch") and d.batch is not None else torch.zeros(x.size(0), dtype=torch.long, device=x.device)
...
def make_graph_loader(name: str, batch_size: int, num_workers: int = 0, val_split: float = 0.1, root: str = "./data"):
    if not PYG_OK:
        raise RuntimeError(f"torch_geometric is required. Import error: {PYG_ERR}")
    dd = name.lower()
    if dd.startswith("ogbg-"):
        if not OGB_OK:
            raise RuntimeError(f"ogb is required for OGB datasets. Import error: {OGB_ERR}")
        ds = PygGraphPropPredDataset(root=root, name=name.upper())
        split_idx = ds.get_idx_split()
        train_ds = ds[split_idx["train"]]
        valid_ds = ds[split_idx["valid"]]
        test_ds  = ds[split_idx["test"]]
        ...
        dl_val   = DataLoader(valid_ds, batch_size=batch_size, shuffle=False, num_workers=num_workers)
        ...
        train_ds = ds[:n_train]; val_ds = ds[n_train:]; test_ds = ds[n_train:]
        dl_train = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=num_workers)
...
class InnerTrainer:
    ...
    def _compute_loss_batch(self, batch):
        ...
        h1, z1 = self.net(d1); h2, z2 = self.net(d2)
        loss = 0.0
        ...
        if self.tcfg.lambda_recon > 0:
            loss = loss + 0.5 * self.tcfg.lambda_recon * recon_bce(h1, d1.edge_index, d1.x.size(0))
            loss = loss + 0.5 * self.tcfg.lambda_recon * recon_bce(h2, d2.edge_index, d2.x.size(0))
        return loss
