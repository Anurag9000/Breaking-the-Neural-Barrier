# GCA — Graph Contrastive Learning with Adaptive augmentation (single-model)
...
def aug_feature_mask_adaptive(x, p=0.2, degree=None):
    # If degree provided, mask inversely to degree (high-degree nodes less likely to lose features)
    if degree is None:
        mask = torch.rand_like(x) < p
        x2 = x.clone(); x2[mask] = 0.0
        return x2
    # degree: [N]
    prob = p / (degree.unsqueeze(-1) + 1.0)
    prob = prob.clamp(0, 0.9)
...
class GCA(nn.Module):
    ...
    def embed(self, data, feat_p=0.2, edge_p=0.2):
        ...
        if hasattr(data, 'deg'):
            degree = data.deg
        x2 = aug_feature_mask_adaptive(x, feat_p, degree)
