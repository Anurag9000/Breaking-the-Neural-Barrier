...
# Load baseline
BASE_PATH = Path(__file__).with_name("pna_model.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.PNA

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the PNA model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['aggregators'] = get_attr(model, ['aggregators'], None)
        kwargs['scalers'] = get_attr(model, ['scalers'], None)
        kwargs['towers'] = get_attr(model, ['towers'], None)
        kwargs['pre_layers'] = get_attr(model, ['pre_layers'], None)
        kwargs['post_layers'] = get_attr(model, ['post_layers'], None)
        kwargs['divide_input'] = get_attr(model, ['divide_input'], None)
        kwargs['num_layers'] = depth
        kwargs['dropout'] = get_attr(model, ['dropout'], None)
        kwargs['use_batchnorm'] = get_attr(model, ['use_batchnorm'], None)
        kwargs['deg'] = get_attr(model, ['deg'], None)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, new_w, getattr(model, 'num_layers', 1) if 'num_layers' != 'None' else 1, device, cfg)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(0, model.num_layers), best_val))
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(0, curr_model.num_layers), v))
            else:
    ...
    return global_best_val, model, 0, model.num_layers
...
def main():
    ...
    try:
        model = ModelClass(num_layers=args.depth).to(device)
    except:
