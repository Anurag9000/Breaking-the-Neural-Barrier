...
# Load baseline
BASE_PATH = Path(__file__).with_name("graphormer_model.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.GraphTransformer

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the GraphTransformer model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['use_degree_bias'] = get_attr(model, ['use_degree_bias'], None)
        kwargs['max_degree'] = get_attr(model, ['max_degree'], None)
