...
# Load baseline
BASE_PATH = Path(__file__).with_name("graph_han.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.HANNet

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the HANNet model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['hidden_dim'] = width
        kwargs['out_dim'] = get_attr(model, ['out_dim'], 1)
        kwargs['heads'] = get_attr(model, ['heads'], None)
        kwargs['layers'] = depth
        kwargs['metapaths'] = get_attr(model, ['metapaths'], None)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    cur = model.hidden_dim if hasattr(model, 'hidden_dim') else cfg.max_width # fallback
    nxt = min(max_width, cur + cfg.ex_k)
    if nxt <= cur: return None
    return rebuild_model(model, nxt, model.layers, device, cfg)

def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    
    cur = model.layers
    if cur >= max_depth: return None
    return rebuild_model(model, model.hidden_dim, cur + 1, device, cfg)
...
def snapshot_arch_and_state(model: ModelClass, state_dict=None) -> Dict[str, Any]:
    ...
    return {
        "width": getattr(model, 'hidden_dim', 0) if 'hidden_dim' != 'None' else 0,
...
def restore_arch_and_state(model: ModelClass, snap: Dict[str, Any], device) -> ModelClass:
    ...
    try:
        new_model = ModelClass(
            hidden_dim=snap['width'], layers=snap['depth']
        ).to(device)
...
def adp_search(model: ModelClass, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    improvements.append((total_neurons(model.hidden_dim, model.layers), best_val))

    def can_widen(m: ModelClass) -> bool:
        return m.hidden_dim < acfg.max_width

    def can_deepen(m: ModelClass) -> bool:
        return m.layers < acfg.max_depth
    ...
    def optimize_width_at_fixed_depth(curr_model: ModelClass) -> Tuple[ModelClass, float, Dict[str, Any]]:
        ...
        while width_failure_count < acfg.trials_width:
            ...
            if v < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                improvements.append((total_neurons(curr_model.hidden_dim, curr_model.layers), v))
            else:
    ...
    return global_best_val, model, model.hidden_dim, model.layers
...
def main():
    ...
    try:
        model = ModelClass(hidden_dim=args.width, layers=args.depth).to(device)
    except:
