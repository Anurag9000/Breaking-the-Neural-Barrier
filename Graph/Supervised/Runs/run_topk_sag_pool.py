...
from topk_sag_pool_model import TopKOrSAGNet
...
def train_graph_level(args):
    ...
    device = args.device
    model = TopKOrSAGNet(in_channels, args.hidden, out_channels, num_blocks=args.blocks,
                         pool_type=args.pool_type, ratio=args.ratio, dropout=args.dropout,
                         use_batchnorm=not args.no_bn).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"{args.pool_type.upper()}_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title(f'{args.pool_type.upper()} val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"{args.pool_type.upper()}_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_topk_sag')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--batch-size', type=int, default=64)
    p.add_argument('--blocks', type=int, default=3)
    p.add_argument('--hidden', type=int, default=64)
    p.add_argument('--pool-type', dest='pool_type', type=str, default='topk', choices=['topk','sag'])
