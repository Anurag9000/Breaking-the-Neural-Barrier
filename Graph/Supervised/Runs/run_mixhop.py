...
from mixhop_model import MixHop
...
def train_node_level(args):
    ...
    model = MixHop(
        ...
        powers=tuple(map(int, args.powers.split(','))),
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"MixHop_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('MixHop val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"MixHop_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_mixhop')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--layers', type=int, default=2)
    p.add_argument('--hidden', type=int, default=32)
    p.add_argument('--powers', type=str, default='0,1,2')
