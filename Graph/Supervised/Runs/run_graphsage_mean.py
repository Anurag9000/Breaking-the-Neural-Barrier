...
from graphsage_mean_model import GraphSAGE_Mean
...
def train_node_level(args):
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"GraphSAGE_mean_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('GraphSAGE-Mean val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"GraphSAGE_mean_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_graphsage')
