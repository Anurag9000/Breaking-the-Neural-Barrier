...
from graph_gatv2 import GATv2Net
...
def evaluate(model, data):
    ...
    return val_loss, (pred[data.train_mask]==data.y[data.train_mask]).float().mean().item(), \
...
def train(args):
    ...
    model = GATv2Net(dataset.num_features, args.hidden_dim, dataset.num_classes, args.num_layers, args.heads, args.dropout).to(args.device)
