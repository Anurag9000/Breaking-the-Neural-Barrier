...
from graph_unet_model import GraphUNetNet
...
def train_graph_level(args):
    ...
    device = args.device
    model = GraphUNetNet(in_channels, args.hidden, out_channels, depth=args.depth,
                         pool_ratios=args.pool_ratios, dropout=args.dropout).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"GraphUNet_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('Graph U-Net val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"GraphUNet_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_graphunet')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--batch-size', type=int, default=64)
    p.add_argument('--depth', type=int, default=3)
    p.add_argument('--hidden', type=int, default=64)
    p.add_argument('--pool-ratios', dest='pool_ratios', type=float, default=0.5)
