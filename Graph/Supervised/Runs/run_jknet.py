...
def train_node_level(args):
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"JKNet_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('JKNet val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"JKNet_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_jknet')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--layers', type=int, default=4)
    p.add_argument('--hidden', type=int, default=64)
    p.add_argument('--jk-mode', dest='jk_mode', type=str, default='cat', choices=['cat','max','lstm'])
