...
def compute_deg_stats(data):
    d = degree(data.edge_index[1], num_nodes=data.num_nodes)
    hist = torch.bincount(d.long())
    return hist
...
def train_node_level(args):
    ...
    model = PNA(
        ...
        divide_input=not args.no_divide_input,
        deg=deg
    ).to(args.device)
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"PNA_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('PNA val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"PNA_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_pna')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--layers', type=int, default=3)
    p.add_argument('--hidden', type=int, default=64)
    p.add_argument('--towers', type=int, default=4)
    p.add_argument('--pre-layers', dest='pre_layers', type=int, default=1)
    p.add_argument('--post-layers', dest='post_layers', type=int, default=1)
    p.add_argument('--no-divide-input', action='store_true')
