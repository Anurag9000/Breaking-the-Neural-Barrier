...
from diffpool_gin_model import DiffPoolGIN
...
def train_graph_level(args):
    ...
    device = args.device
    model = DiffPoolGIN(in_channels=in_channels, hidden_channels=args.hidden, out_channels=out_channels,
                        cluster_ratio=args.cluster_ratio, dropout=args.dropout, train_eps=args.train_eps).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"DiffPoolGIN_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('DiffPool-GIN val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"DiffPoolGIN_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_diffpool')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--batch-size', type=int, default=32)
    p.add_argument('--hidden', type=int, default=64)
    p.add_argument('--cluster-ratio', dest='cluster_ratio', type=float, default=0.25)
