...
def train_node_level(args):
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"GAT_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('GAT val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"GAT_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_gat')
    ...
    p.add_argument('--attn-dropout', dest='attn_dropout', type=float, default=0.6)
