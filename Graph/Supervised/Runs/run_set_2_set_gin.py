...
from set2set_gin_model import GIN_Set2Set
...
def train_graph_level(args):
    ...
    device = args.device
    model = GIN_Set2Set(in_channels, args.hidden, out_channels, num_layers=args.layers,
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"GIN_Set2Set_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('GIN+Set2Set val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"GIN_Set2Set_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_set2set_gin')
