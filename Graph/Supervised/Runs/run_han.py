...
def build_metapaths(data):
    # IMDB types: ('movie','actor'), ('movie','director') relations.
    # Common metapaths: M-A-M and M-D-M
    return [
...
def train_imdb(args):
    ...
    # Basic preprocessing: self-loops and undirected conversion per relation
    ...
    metapaths = build_metapaths(data)
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"HAN_IMDB.pth"))


def main():
    p = argparse.ArgumentParser()
    p.add_argument('--data-root', type=str, default='data')
    p.add_argument('--out-dir', type=str, default='runs_han')
