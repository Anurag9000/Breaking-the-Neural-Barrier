...
from appnp_model import APPNPNet
...
def train_node_level(args):
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"APPNP_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('APPNP val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"APPNP_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_appnp')
    p.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    p.add_argument('--seed', type=int, default=42)
    p.add_argument('--mlp-layers', dest='mlp_layers', type=int, default=2)
    p.add_argument('--layers', type=int, default=2)  # kept for interface parity, unused
