...
from asapool_model import ASAPNet
...
def train_graph_level(args):
    ...
    device = args.device
    model = ASAPNet(in_channels, args.hidden, out_channels, num_blocks=args.blocks,
                    ratio=args.ratio, dropout=args.dropout, use_batchnorm=not args.no_bn).to(device)
    ...
    torch.save(model.state_dict(), os.path.join(args.out_dir, f"ASAP_{args.dataset}.pth"))
    plt.figure(); plt.semilogy(epochs, val_losses)
    plt.xlabel('epoch'); plt.ylabel('val_loss (log)'); plt.title('ASAPool val loss')
    plt.grid(True); plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, f"ASAP_{args.dataset}_val_loss.png"))
    plt.close()
...
def main():
    ...
    p.add_argument('--out-dir', type=str, default='runs_asap')
