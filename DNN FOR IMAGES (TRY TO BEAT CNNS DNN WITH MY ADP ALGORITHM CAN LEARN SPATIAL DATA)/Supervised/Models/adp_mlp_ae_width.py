...
def adp_search_width_then_depth(model: AdaptiveMLPAE, train_loader, val_loader, device,
                                trials_width: int, trials_depth: int, epochs: int, lr: float, patience: int,
                                delta: float, ex_k: int, max_neurons: int=None, max_depth: int=None, max_width: int=None,
                                denoise_std: float=0.0):
    """
    Greedy search: try width expansions first, then depth expansions.
