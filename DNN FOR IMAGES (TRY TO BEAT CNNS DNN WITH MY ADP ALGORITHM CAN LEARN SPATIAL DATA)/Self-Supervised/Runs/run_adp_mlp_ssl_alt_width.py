# run_adp_mlp_ssl_alt_width.py
import argparse, os, random, torch
from torch.utils.data import DataLoader

from adp_mlp_ssl_alt_depth import AdaptiveMLPSSL, build_data   # reuse your model & data utils
from adp_mlp_ssl_alt_width import adp_search_alternating_width_first
...
def main():
    ...
    path = os.path.join("checkpoints", f"adp_mlp_ssl_alt_width_{args.dataset}.pt")
    torch.save({
        "state": {k: v.cpu() for k, v in model.state_dict().items()},
        "best_val_ntxent": best_val,
        "config": vars(args)
    }, path)
