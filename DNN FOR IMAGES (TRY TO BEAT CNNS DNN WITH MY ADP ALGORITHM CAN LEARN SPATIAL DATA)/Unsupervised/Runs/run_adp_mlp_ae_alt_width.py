# run_adp_mlp_ae_alt_width.py
import argparse, os, random, torch
from torch.utils.data import DataLoader, random_split
from torchvision import datasets, transforms

from adp_mlp_ae_alt_depth import AdaptiveMLPAE  # model class
from adp_mlp_ae_alt_width import adp_search_alternating_width_first
...
def build_data(dataset, data_dir, img_size, val_split):
    ...
    if dataset == "mnist":
        ds = datasets.MNIST(data_dir, train=True, download=True, transform=tfm); C = 1
    elif dataset == "fashionmnist":
        ds = datasets.FashionMNIST(data_dir, train=True, download=True, transform=tfm); C = 1
    elif dataset == "cifar10":
        ds = datasets.CIFAR10(data_dir, train=True, download=True, transform=tfm); C = 3
    elif dataset == "cifar100":
        ds = datasets.CIFAR100(data_dir, train=True, download=True, transform=tfm); C = 3
    else:
...
def main():
    ...
    p.add_argument("--denoise_std", type=float, default=0.0, help=">0 enables denoising AE")
    args = p.parse_args()
    ...
    best_val = adp_search_alternating_width_first(
        model, trl, val, device,
    ...
    path = os.path.join("checkpoints", f"adp_mlp_ae_alt_width_{args.dataset}.pt")
