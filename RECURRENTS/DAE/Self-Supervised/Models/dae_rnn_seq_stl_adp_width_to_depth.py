...
from .dae_rnn_seq_stl import DAERNNSeq, rnn_total_neurons
...
def rebuild_model(model: DAERNNSeq, width: int, depth: int, device: torch.device) -> DAERNNSeq:
    new_model = DAERNNSeq(
...
def snapshot_arch_and_state(model: DAERNNSeq, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAERNNSeq:
    mdl = DAERNNSeq(
        in_channels=snap.get("in_channels", 1),
        width=snap["width"],
        depth=snap["depth"],
        rnn_type=snap.get("rnn_type", "gru"),
    ).to(device)
...
def adp_search(
    ...
    improvements.append((rnn_total_neurons(best_w, best_d), global_best_val))
    ...
    def optimize_width_at_fixed_depth(
        ...
        while fail < acfg.trials_width:
            ...
            if wider is None or rnn_total_neurons(wider.width, wider.depth) > acfg.max_neurons:
                break
            logger.log_console(
                f"[WIDTH OPT] Trying width={wider.width}, depth={wider.depth}, "
                f"neurons={rnn_total_neurons(wider.width, wider.depth)}"
            )
            ...
            if val + acfg.delta < current_best_val:
                ...
                improvements.append((rnn_total_neurons(wider.width, wider.depth), val))
                logger.log_console(
    ...
    def optimize_depth_at_fixed_width(
        ...
        while fail < acfg.trials_depth:
            ...
            if deeper is None or rnn_total_neurons(deeper.width, deeper.depth) > acfg.max_neurons:
                break
            logger.log_console(
                f"[DEPTH OPT] Trying width={deeper.width}, depth={deeper.depth}, "
                f"neurons={rnn_total_neurons(deeper.width, deeper.depth)}"
            )
            val, state = train_with_early_stopping(
            ...
            if val + acfg.delta < current_best_val:
                ...
                improvements.append((rnn_total_neurons(deeper.width, deeper.depth), val))
                logger.log_console(
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_rnn_seq")
    ...
    logger.log_console(
        ...
        f"neurons={rnn_total_neurons(base_model.width, base_model.depth)}"
    )
    ...
    logger.log_console(
        ...
        f"neurons={rnn_total_neurons(best_w, best_d)}"
    )
