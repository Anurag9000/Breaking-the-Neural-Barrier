...
class OrderDataset(Dataset):
    def __init__(self, sequences: List[List[int]], max_len=128, segments=2, shuffle_prob=0.5, seed=1337):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.segments):
            end = start + (n // self.segments)
            if i == self.segments - 1:
                end = n
            splits.append(s[start:end])
            start = end
        correct = torch.tensor(1, dtype=torch.long)
        if self.rng.random() < self.shuffle_prob:
            self.rng.shuffle(splits)
...
class LSTMOrderVerifier(nn.Module):
