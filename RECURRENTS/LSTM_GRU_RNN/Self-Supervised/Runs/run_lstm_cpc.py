...
from lstm_cpc import WhitespaceTokenizer, CPCDataset, pad_ctx, CPCLSTM, TrainConfig, train, evaluate
...
if __name__ == "__main__":
    ...
    ap.add_argument("--ar_hidden", type=int, default=256)
    ...
    train_ds = CPCDataset(train_ids, max_len=args.max_len, context_len=args.context_len, k_future=args.k_future)
    val_ds = CPCDataset(val_ids, max_len=args.max_len, context_len=args.context_len, k_future=args.k_future)
    test_ds = CPCDataset(test_ids, max_len=args.max_len, context_len=args.context_len, k_future=args.k_future)

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, drop_last=True, collate_fn=pad_ctx)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, collate_fn=pad_ctx)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, collate_fn=pad_ctx)
    ...
    model = CPCLSTM(vocab=len(tok.itos), emb_dim=cfg.emb_dim, ar_hidden=cfg.ar_hidden, layers=cfg.layers, k_future=cfg.k_future, dropout=cfg.dropout).to(device)
    best = train(model, train_loader, val_loader, cfg, device)
    test = evaluate(model, test_loader, device, cfg)
    print({
        "vocab": len(tok.itos),
        "best_val_cpc": round(best, 4),
        "test_cpc": round(test, 4),
