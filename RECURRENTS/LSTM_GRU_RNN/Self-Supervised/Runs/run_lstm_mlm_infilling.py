...
from lstm_mlm_infilling import (
    WhitespaceTokenizer,
    SpanInfillingDataset,
    pad_collate,
    LSTMInfilling,
    TrainConfig,
    train,
...
if __name__ == "__main__":
    ...
    def to_ids(blocks: List[str]):
        return [tok.encode(s) for s in blocks]

    train_ids = to_ids(tr)
    val_ids = to_ids(va)
    test_ids = to_ids(te)

    train_ds = SpanInfillingDataset(train_ids, max_len=args.max_len, mask_prob=args.mask_prob, seed=args.seed)
    val_ds = SpanInfillingDataset(val_ids, max_len=args.max_len, mask_prob=args.mask_prob, seed=args.seed)
    test_ds = SpanInfillingDataset(test_ids, max_len=args.max_len, mask_prob=args.mask_prob, seed=args.seed)
    ...
    model = LSTMInfilling(vocab=len(tok.itos), emb_dim=cfg.emb_dim, hidden=cfg.hidden, layers=cfg.layers, dropout=cfg.dropout).to(device)
