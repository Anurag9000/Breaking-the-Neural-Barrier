...
# Load baseline
BASE_PATH = Path(__file__).with_name("lstm_cls_attn_pool.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.AdditiveAttention

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the AdditiveAttention model.
...
def expand_depth(model: ModelClass, max_depth: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, getattr(model, 'hidden', 64) if 'hidden' != 'None' else 64, new_d, device, cfg)
...
def snapshot_arch_and_state(model: ModelClass, state_dict=None) -> Dict[str, Any]:
    ...
    return {
        "width": getattr(model, 'hidden', 0) if 'hidden' != 'None' else 0,
