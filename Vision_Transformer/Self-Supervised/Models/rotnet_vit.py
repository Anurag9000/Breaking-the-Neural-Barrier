"""
RotNet-ViT (single-model)
- Predict rotation angle class in {0°,90°,180°,270°}
- Encoder: ViT -> pooled token -> linear head (4-way)
"""
...
@dataclass
class RotNetConfig:
    img:int=32; patch:int=4; dim:int=384; depth:int=6; heads:int=6; ratio:float=4.0; num_classes:int=4

class RotNetViT(nn.Module):
    def __init__(self, cfg: RotNetConfig):
