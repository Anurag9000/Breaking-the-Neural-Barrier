...
from vicreg_vit import VICRegViT, VICRegConfig
...
def main():
    ...
    ap.add_argument('--save', default='vicreg_vit_best.pt')
    args=ap.parse_args()

    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    tr,va,te=loaders(args.dataset,args.data_dir,args.batch_size)

    cfg=VICRegConfig(args.img,args.patch,args.dim,args.depth,args.heads,args.ratio,
                     args.proj_hidden,args.proj_out,args.w_inv,args.w_var,args.w_cov,args.gamma)
    model=VICRegViT(cfg).to(device)
