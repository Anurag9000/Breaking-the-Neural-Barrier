...
from simsiam_vit import SimSiamViT, SimSiamConfig
...
def main():
    ...
    ap.add_argument('--pred_hidden', type=int, default=512)
    ap.add_argument('--save', default='simsiam_vit_best.pt')
    args=ap.parse_args()

    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    tr,va,te=loaders(args.dataset,args.data_dir,args.batch_size)

    cfg=SimSiamConfig(args.img,args.patch,args.dim,args.depth,args.heads,args.ratio,
                      args.proj_hidden,args.proj_out,args.pred_hidden)
    model=SimSiamViT(cfg).to(device)
