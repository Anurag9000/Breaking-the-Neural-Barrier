...
from maskfeat_vit import MaskFeatViT, MaskFeatConfig
...
def main():
    ...
    ap.add_argument('--save', default='maskfeat_vit_best.pt')
    args=ap.parse_args()

    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    tr,va,te=get_loaders(args.dataset,args.data_dir,args.batch_size)

    cfg=MaskFeatConfig(args.img,args.patch,args.dim,args.depth,args.heads,args.ratio,args.bins,args.mask_ratio)
    model=MaskFeatViT(cfg).to(device)
