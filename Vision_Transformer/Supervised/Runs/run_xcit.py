...
def loaders(dataset, root, img, batch, workers, seed=42):
    ...
    B=DS=root and DS(root=root,train=True,download=True,transform=ev)
...
def main():
    ...
    p.add_argument('--save',default='XCiT_best.pth')
    a=p.parse_args()
    ...
    m=XCiT(num_classes=nc, dim=a.dim, depth=a.depth, heads=a.heads).to(dev)
