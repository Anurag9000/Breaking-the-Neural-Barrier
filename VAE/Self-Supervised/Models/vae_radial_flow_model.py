...
class Radial(nn.Module):
    ...
    def forward(self, z):
        # f(z) = z + beta * h(r) * (z - z0), with h(r)=1/(alpha + r)
        diff = z - self.z0
        r = diff.norm(dim=-1, keepdim=True) + 1e-8
        h = 1.0/(torch.relu(self.alpha)+r)
        z_out = z + self.beta * h * diff
        D = z.size(1)
        # log|det| = (D-1) * log(1 + beta*h) + log(1 + beta*h + (-beta*r*h*h))
        bh = self.beta*h
        log_det = (D-1)*torch.log(torch.abs(1+bh.squeeze(-1))+1e-8) + torch.log(torch.abs(1+bh.squeeze(-1) - self.beta*(r.squeeze(-1))*h.squeeze(-1)*h.squeeze(-1)) + 1e-8)
...
class RadialFlowVAE(nn.Module):
    def __init__(self, in_ch=1, out_ch=1, z_dim=32, n_flows=2, recon='bce'):
        ...
        self.flows = nn.ModuleList([Radial(z_dim) for _ in range(n_flows)])
        self.recon = recon
    @staticmethod
    def reparam(mu, lv):
