...
# VAE with Gaussian likelihood p(x|z)=N(mu(z), diag(sigma^2)), with learned global log-variance
...
class Decoder(nn.Module):
    ...
    def forward(self,z):
        x_mu = self.de(self.fc(z).view(z.size(0),128,4,4))
        return x_mu, self.log_sigma2

class VAE_GaussLik(nn.Module):
    ...
    def forward(self,x):
        ...
        x_mu, log_sigma2 = self.dec(z)
        sigma2 = torch.exp(log_sigma2)
        log_px = -0.5*((x - x_mu).pow(2)/sigma2 + torch.log(2*torch.pi*sigma2)).flatten(1).sum(-1)
