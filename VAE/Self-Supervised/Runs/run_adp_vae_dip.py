...
from adp_vae_dip import DIPVAEConfig, DIPVAE
...
def train(args):
    ...
    cfg=DIPVAEConfig(latent_dim=args.latent, lambda_diag=args.lambda_diag, lambda_offdiag=args.lambda_offdiag,
                     width=args.width, depth=args.depth)
    model=DIPVAE(cfg).to(dev)
    opt=optim.Adam(model.parameters(), lr=args.lr)
    out=Path(args.out); out.mkdir(parents=True, exist_ok=True)

    for ep in range(1,args.epochs+1):
        model.train(); sums={k:0.0 for k in ['loss','recon','kl','dip']}
        for x,_ in tr:
        ...
        n=len(tr); print(f"[Epoch {ep}] loss={sums['loss']/n:.4f} recon={sums['recon']/n:.4f} kl={sums['kl']/n:.4f} dip={sums['dip']/n:.4f}")
        with torch.no_grad():
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/vae_dip')
    ap.add_argument('--epochs', type=int, default=30)
    ap.add_argument('--batch', type=int, default=128)
    ap.add_argument('--lr', type=float, default=2e-3)
    ap.add_argument('--latent', type=int, default=32)
    ap.add_argument('--lambda_diag', type=float, default=10.0)
    ap.add_argument('--lambda_offdiag', type=float, default=5.0)
