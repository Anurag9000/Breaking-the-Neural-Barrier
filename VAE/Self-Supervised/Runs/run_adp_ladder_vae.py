...
from adp_ladder_vae import LadderVAEConfig, LadderVAE
...
def train(args):
    ...
    cfg=LadderVAEConfig(z1_dim=args.z1, z2_dim=args.z2, width=args.width, depth=2)
    model=LadderVAE(cfg).to(dev)
    opt=optim.Adam(model.parameters(), lr=args.lr)
    out=Path(args.out); out.mkdir(parents=True, exist_ok=True)

    for ep in range(1,args.epochs+1):
        model.train(); sums={k:0.0 for k in ['loss','recon','kl_z1','kl_z2']}
        for x,_ in tr:
            x=x.to(dev); xb=renorm(x,dev)
            xr,stats=model(xb)
            L=model.loss_fn(xb,xr,stats)
            opt.zero_grad(set_to_none=True); L['loss'].backward(); opt.step()
            for k in sums: sums[k]+=L[k].item()
        n=len(tr); print(f"[Epoch {ep}] loss={sums['loss']/n:.4f} recon={sums['recon']/n:.4f} kl_z1={sums['kl_z1']/n:.4f} kl_z2={sums['kl_z2']/n:.4f}")
        with torch.no_grad():
            x,_=next(iter(te)); x=x.to(dev); xb=renorm(x,dev); xr,_=model(xb)
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/ladder_vae')
    ap.add_argument('--epochs', type=int, default=40)
    ap.add_argument('--batch', type=int, default=128)
    ap.add_argument('--lr', type=float, default=2e-3)
    ap.add_argument('--z1', type=int, default=32)
    ap.add_argument('--z2', type=int, default=16)
