...
from vae_radial_flow_model import RadialFlowVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/radial')
    args = ap.parse_args()
    ...
    model = RadialFlowVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, n_flows=args.n_flows, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO RadialFlow): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_radial.pt'))
