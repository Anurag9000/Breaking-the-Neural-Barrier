...
from vae_vlae_model import VLAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/vlae')
    args = ap.parse_args()
    ...
    model = VLAE(in_ch=1, out_ch=1, z_dim=args.z_dim, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO VLAE): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_vlae.pt'))
