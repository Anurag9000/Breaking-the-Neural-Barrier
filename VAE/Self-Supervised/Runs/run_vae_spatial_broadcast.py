...
from vae_spatial_broadcast_model import SBDVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/sbd')
    args = ap.parse_args()
    ...
    model = SBDVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO SBD-VAE): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_sbd.pt'))
