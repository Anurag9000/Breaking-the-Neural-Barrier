...
from vae_dequant_model import DequantVAE
...
def add_uniform_dequant(x):
    # map discrete {0,1} to continuous [0,1]
    u = torch.rand_like(x)
    return (x + u/256.0).clamp(0,1)
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/dequant')
    args = ap.parse_args()
    ...
    model = DequantVAE(in_ch=1, out_ch=1, z_dim=args.z_dim).to(device)
    ...
    print(f'TEST (ELBO Dequant): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_dequant.pt'))
