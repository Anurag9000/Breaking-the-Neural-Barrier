...
from vae_mol_likelihood_model import VAE_MoL
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/mol')
    args=ap.parse_args()
    ...
    model=VAE_MoL(in_ch=1,out_ch=1,z_dim=args.z_dim,K=args.K).to(device)
    ...
    print(f'TEST (ELBO MoL): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir,'vae_mol.pt'))
