...
from vae_gauss_likelihood_model import VAE_GaussLik
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/gausslik')
    args = ap.parse_args()
    ...
    model = VAE_GaussLik(in_ch=1, out_ch=1, z_dim=args.z_dim).to(device)
    ...
    print(f'TEST (ELBO GaussLik): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_gausslik.pt'))
