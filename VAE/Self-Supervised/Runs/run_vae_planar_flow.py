...
from vae_planar_flow_model import PlanarFlowVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/planar')
    args = ap.parse_args()
    ...
    model = PlanarFlowVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, n_flows=args.n_flows, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO PlanarFlow): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_planar.pt'))
