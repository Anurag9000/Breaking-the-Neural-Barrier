...
from adp_wae_mmd import WAEMMDConfig, WAEMMD
...
def train(args):
    ...
    cfg=WAEMMDConfig(latent_dim=args.latent, lambda_mmd=args.lambda_mmd, width=args.width, depth=args.depth, noise_std=args.noise)
    model=WAEMMD(cfg).to(dev)
    opt=optim.Adam(model.parameters(), lr=args.lr)
    out=Path(args.out); out.mkdir(parents=True, exist_ok=True)

    for ep in range(1,args.epochs+1):
        model.train(); sums={k:0.0 for k in ['loss','recon','mmd']}
        for x,_ in tr:
            x=x.to(dev); xb=renorm(x,dev)
            xr,z=model(xb)
            L=model.loss_fn(xb,xr,z)
            opt.zero_grad(set_to_none=True); L['loss'].backward(); opt.step()
            for k in sums: sums[k]+=L[k].item()
        n=len(tr); print(f"[Epoch {ep}] loss={sums['loss']/n:.4f} recon={sums['recon']/n:.4f} mmd={sums['mmd']/n:.4f}")
        with torch.no_grad():
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/wae_mmd')
    ap.add_argument('--epochs', type=int, default=30)
    ap.add_argument('--batch', type=int, default=128)
    ap.add_argument('--lr', type=float, default=2e-3)
    ap.add_argument('--latent', type=int, default=32)
    ap.add_argument('--lambda_mmd', type=float, default=10.0)
