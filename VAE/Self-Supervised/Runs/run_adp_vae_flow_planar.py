...
from adp_vae_flow_planar import PlanarFlowVAEConfig, PlanarFlowVAE
...
def train(args):
    ...
    cfg=PlanarFlowVAEConfig(latent_dim=args.latent, n_flows=args.flows, width=args.width, depth=args.depth)
    model=PlanarFlowVAE(cfg).to(dev)
    ...
    for ep in range(1,args.epochs+1):
        ...
        for x,_ in tr:
            ...
            xr,mu,lv,sum_ld=model(xb)
            L=model.loss_fn(xb,xr,mu,lv,sum_ld)
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/vae_flow_planar')
