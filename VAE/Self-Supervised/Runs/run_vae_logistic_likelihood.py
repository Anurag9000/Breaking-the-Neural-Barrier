...
from vae_logistic_likelihood_model import LogisticLikVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/logisticlik')
    args = ap.parse_args()
    ...
    model = LogisticLikVAE(in_ch=1, out_ch=1, z_dim=args.z_dim).to(device)
    ...
    print(f'TEST (ELBO LogisticLik): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_logisticlik.pt'))
