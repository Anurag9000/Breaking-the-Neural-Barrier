...
from vae_autoregressive_prior_model import VAE_AR_Prior
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/ar_prior')
    args = ap.parse_args()
    ...
    model = VAE_AR_Prior(in_ch=1, out_ch=1, z_dim=args.z_dim, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO AR-prior): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_ar_prior.pt'))
