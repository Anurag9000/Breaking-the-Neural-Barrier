...
from vae_laplace_prior_model import LaplacePriorVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/laplace')
    args = ap.parse_args()
    ...
    model = LaplacePriorVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, b=args.b, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO Laplace prior): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_laplace.pt'))
