...
from vae_dip_ii_model import DIPVAE_II
...
def main():
    ...
    ap.add_argument('--lam-off', type=float, default=10.0)
    ap.add_argument('--lam-diag', type=float, default=1.0)
    ap.add_argument('--lam-sigma', type=float, default=0.5)
    ap.add_argument('--recon', type=str, default='bce', choices=['bce','mse'])
    ap.add_argument('--outdir', type=str, default='results_vae/dip_ii')
    args=ap.parse_args()
    ...
    model=DIPVAE_II(in_ch=1,out_ch=1,z_dim=args.z_dim,lam_off=args.lam_off,lam_diag=args.lam_diag,lam_sigma=args.lam_sigma,recon=args.recon).to(device)
    ...
    print(f'TEST (DIP-VAE-II): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir,'vae_dip_ii.pt'))
