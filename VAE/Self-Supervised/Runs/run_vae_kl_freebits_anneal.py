...
from vae_kl_freebits_anneal_model import VAE_KL_Hybrid
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/klhybrid')
    args = ap.parse_args()
    ...
    model = VAE_KL_Hybrid(in_ch=1, out_ch=1, z_dim=args.z_dim, tau=args.tau, beta_max=args.beta_max, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO KL-hybrid): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_klhybrid.pt'))
