...
from vq_vae_st_model import VQVAE_ST
...
def main():
    ...
    ap.add_argument('--z-ch', type=int, default=64)
    ap.add_argument('--K', type=int, default=512)
    ap.add_argument('--beta', type=float, default=0.25)
    ap.add_argument('--recon', type=str, default='bce', choices=['bce','mse'])
    ap.add_argument('--outdir', type=str, default='results_vae/vq_vae_st')
    args=ap.parse_args()
    ...
    model=VQVAE_ST(in_ch=1,out_ch=1,hid=args.hid,z_ch=args.z_ch,K=args.K,beta=args.beta,recon=args.recon).to(device)
    ...
    print(f'TEST (VQ-VAE ST): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir,'vq_vae_st.pt'))
