...
from vae_resnet_model import ResNetVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/resnet')
    args = ap.parse_args()
    ...
    model = ResNetVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO ResNet-VAE): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_resnet.pt'))
