...
from vae_delta_model import DeltaVAE
...
def main():
    ...
    ap.add_argument('--logvar-floor', type=float, default=-2.0)
    ap.add_argument('--recon', type=str, default='bce', choices=['bce','mse'])
    ap.add_argument('--outdir', type=str, default='results_vae/deltavae')
    args = ap.parse_args()
    ...
    model = DeltaVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, logvar_floor=args.logvar_floor, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO DeltaVAE): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_delta.pt'))
