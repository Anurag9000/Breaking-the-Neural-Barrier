...
from vae_dip_i_model import DIPVAE_I
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/dip_i')
    args=ap.parse_args()
    ...
    model=DIPVAE_I(in_ch=1,out_ch=1,z_dim=args.z_dim,lam_off=args.lam_off,lam_diag=args.lam_diag,recon=args.recon).to(device)
    ...
    print(f'TEST (DIP-VAE-I): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir,'vae_dip_i.pt'))
