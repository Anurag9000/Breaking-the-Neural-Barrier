...
from adp_dvae_relaxed import DVAEConfig, DVAE
...
def train(args):
    ...
    cfg=DVAEConfig(n_latents=args.n_lat, width=args.width, depth=args.depth, tau=args.tau)
    model=DVAE(cfg).to(dev)
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/dvae_relaxed')
    ap.add_argument('--epochs', type=int, default=40)
    ap.add_argument('--batch', type=int, default=128)
    ap.add_argument('--lr', type=float, default=2e-3)
    ap.add_argument('--n_lat', type=int, default=64)
