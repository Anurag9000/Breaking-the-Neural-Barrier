...
from vae_continuous_bernoulli_model import ContBernVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/contbern')
    args = ap.parse_args()
    ...
    model = ContBernVAE(in_ch=1, out_ch=1, z_dim=args.z_dim).to(device)
    ...
    print(f'TEST (ELBO ContBern): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_contbern.pt'))
