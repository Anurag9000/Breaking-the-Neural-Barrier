...
from vae_infovae_mmd_model import InfoVAE_MMD
...
def main():
    ...
    ap.add_argument('--mmd-lambda', type=float, default=10.0)
    ap.add_argument('--recon', type=str, default='bce', choices=['bce','mse'])
    ap.add_argument('--outdir', type=str, default='results_vae/infovae_mmd')
    args = ap.parse_args()
    ...
    model = InfoVAE_MMD(in_ch=1, out_ch=1, z_dim=args.z_dim, mmd_lambda=args.mmd_lambda, recon=args.recon).to(device)
    ...
    print(f'TEST (InfoVAE-MMD): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_infommd.pt'))
