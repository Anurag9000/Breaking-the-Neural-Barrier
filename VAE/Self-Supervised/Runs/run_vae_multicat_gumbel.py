...
from vae_multicat_gumbel_model import MultiCatGumbelVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/multicat')
    args = ap.parse_args()
    ...
    model = MultiCatGumbelVAE(in_ch=1, out_ch=1, K=args.K, C=args.C, tau=args.tau, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO multi-cat): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_multicat.pt'))
