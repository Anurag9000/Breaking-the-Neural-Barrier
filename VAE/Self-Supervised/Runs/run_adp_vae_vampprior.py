...
from adp_vae_vampprior import VampPriorConfig, VampPriorVAE
...
def train(args):
    ...
    cfg=VampPriorConfig(latent_dim=args.latent, n_pseudo_inputs=args.pseudo, width=args.width, depth=args.depth)
    model=VampPriorVAE(cfg).to(dev)
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/vae_vampprior')
    ap.add_argument('--epochs', type=int, default=40)
    ap.add_argument('--batch', type=int, default=128)
    ap.add_argument('--lr', type=float, default=2e-3)
    ap.add_argument('--latent', type=int, default=32)
    ap.add_argument('--pseudo', type=int, default=500)
