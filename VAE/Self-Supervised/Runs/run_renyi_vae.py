...
from renyi_vae_model import RenyiVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/renyi')
    args=ap.parse_args()
    ...
    model=RenyiVAE(in_ch=1,out_ch=1,z_dim=args.z_dim,alpha=args.alpha,K=args.K,recon=args.recon).to(device)
    ...
    print(f'TEST (Renyi-α VAE): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir,'renyi_vae.pt'))
