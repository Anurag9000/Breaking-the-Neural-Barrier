...
from adp_vae_beta_tc import BetaTCVAEConfig, BetaTCVAE
...
def train(args):
    ...
    cfg=BetaTCVAEConfig(latent_dim=args.latent, beta=args.beta, width=args.width, depth=args.depth,
                        mi_weight=args.mi_w, dimkl_weight=args.dimkl_w)
    model=BetaTCVAE(cfg).to(dev)
    opt=optim.Adam(model.parameters(), lr=args.lr)
    out=Path(args.out); out.mkdir(parents=True, exist_ok=True)

    for ep in range(1, args.epochs+1):
        model.train(); sums={k:0.0 for k in ['loss','recon','kl','mi','tc','dimkl']}
        for x,_ in tr:
            x=x.to(dev); xb=renorm(x,dev)
            xr,mu,lv,z=model(xb)
            L=model.loss_fn(xb,xr,mu,lv,z)
            opt.zero_grad(set_to_none=True); L['loss'].backward(); opt.step()
            for k in sums: sums[k]+=L[k].item()
        n=len(tr)
        print(f"[Epoch {ep}] loss={sums['loss']/n:.4f} recon={sums['recon']/n:.4f} KL={sums['kl']/n:.4f} MI={sums['mi']/n:.4f} TC={sums['tc']/n:.4f} DIMKL={sums['dimkl']/n:.4f}")
        with torch.no_grad():
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/vae_beta_tc')
    ap.add_argument('--epochs', type=int, default=30)
    ap.add_argument('--batch', type=int, default=128)
    ap.add_argument('--lr', type=float, default=2e-3)
    ap.add_argument('--latent', type=int, default=32)
    ap.add_argument('--beta', type=float, default=6.0)
    ap.add_argument('--mi_w', type=float, default=1.0)
    ap.add_argument('--dimkl_w', type=float, default=1.0)
