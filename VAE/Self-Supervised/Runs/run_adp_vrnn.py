...
from adp_vrnn import VRNNConfig, VRNN
...
def to_seq(x):
    # x: (B,3,32,32) in [0,1]
    B = x.size(0)
    x = x.permute(0,2,3,1).contiguous()  # (B,32,32,3)
    x = x.view(B, 32, 32*3)               # (B, T=32, D=96)
    return x
...
def train(args):
    ...
    cfg=VRNNConfig(input_dim=96, hidden_dim=args.hidden, latent_dim=args.latent, seq_len=32)
    model=VRNN(cfg).to(dev)
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/vrnn')
