...
from vae_vampprior_model import VampPriorVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/vampprior')
    args = ap.parse_args()
    ...
    model = VampPriorVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, K=args.K, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO VampPrior): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_vampprior.pt'))
