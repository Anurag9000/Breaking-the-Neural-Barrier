...
from vae_householder_flow_model import HouseholderFlowVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/householder')
    args = ap.parse_args()
    ...
    model = HouseholderFlowVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, n_flows=args.n_flows, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO Householder): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_householder.pt'))
