...
from vae_beta_capacity_model import BetaCapacityVAE
...
def main():
    ...
    ap.add_argument('--C-max', type=float, default=25.0)
    ap.add_argument('--C-steps', type=int, default=100000)
    ap.add_argument('--recon', type=str, default='bce', choices=['bce','mse'])
    ap.add_argument('--outdir', type=str, default='results_vae/beta_capacity')
    args=ap.parse_args()
    ...
    model=BetaCapacityVAE(in_ch=1,out_ch=1,z_dim=args.z_dim,beta=args.beta,C_max=args.C_max,steps=args.C_steps,recon=args.recon).to(device)
    ...
    print(f'TEST (BetaCapacity): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir,'vae_beta_capacity.pt'))
