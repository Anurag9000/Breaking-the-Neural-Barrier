...
from adp_vade import VaDEConfig, VaDE
...
def train(args):
    ...
    cfg=VaDEConfig(latent_dim=args.latent, n_components=args.k, width=args.width, depth=args.depth)
    model=VaDE(cfg).to(dev)
    opt=optim.Adam(model.parameters(), lr=args.lr)
    out=Path(args.out); out.mkdir(parents=True, exist_ok=True)

    for ep in range(1,args.epochs+1):
        model.train(); sums={k:0.0 for k in ['loss','recon','kl_z','kl_c']}
        for x,_ in tr:
            x=x.to(dev); xb=renorm(x,dev)
            xr,mu,lv,z,qcx=model(xb)
            L=model.loss_fn(xb,xr,mu,lv,z,qcx)
            opt.zero_grad(set_to_none=True); L['loss'].backward(); opt.step()
            for k in sums: sums[k]+=L[k].item()
        n=len(tr); print(f"[Epoch {ep}] loss={sums['loss']/n:.4f} recon={sums['recon']/n:.4f} kl_z={sums['kl_z']/n:.4f} kl_c={sums['kl_c']/n:.4f}")
        with torch.no_grad():
            x,_=next(iter(te)); x=x.to(dev); xb=renorm(x,dev); xr,_,_,_,_=model(xb)
...
if __name__=='__main__':
    ...
    ap.add_argument('--out', type=str, default='./runs/vade')
