...
from vae_gumbel import GumbelVAE, GumbelVAEConfig
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        x=x.to(device); xh, logits, qy = model(x)
        loss, r, kl = model.loss(x, xh, logits, qy)
...
def main(args):
    ...
    cfg = GumbelVAEConfig(width=args.width, K=args.K, groups=args.groups, tau=args.tau)
    model = GumbelVAE(cfg).to(device)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x,_ in tr:
            x=x.to(device); xh, logits, qy = model(x)
            loss, *_ = model.loss(x, xh, logits, qy)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_gumbel_cifar10.pth'))
        with open(os.path.join(args.out,'vae_gumbel_metrics.json'),'w') as f:
