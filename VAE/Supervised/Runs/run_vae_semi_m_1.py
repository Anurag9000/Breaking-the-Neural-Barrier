...
from vae_semi_m1 import VAE_M1, M1Config
...
def evaluate(model, loader, device):
    ...
    for x,y in loader:
        ...
        xh, mu, lv, _ = model(x)
        loss, r, kl, ce = model.loss(x, xh, mu, lv, y)
...
def main(args):
    ...
    cfg = M1Config(width=args.width, latent_dim=args.latent, ce_weight=args.ce_w)
    model = VAE_M1(cfg).to(device)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x,y in tr:
            x,y=x.to(device),y.to(device)
            xh, mu, lv, _ = model(x)
            loss, *_ = model.loss(x, xh, mu, lv, y)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_semi_m1_cifar10.pth'))
        with open(os.path.join(args.out,'vae_semi_m1_metrics.json'),'w') as f:
