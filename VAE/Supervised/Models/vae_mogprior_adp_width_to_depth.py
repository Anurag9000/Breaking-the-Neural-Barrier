...
# Load baseline
BASE_PATH = Path(__file__).with_name("vae_mogprior.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.MoGPriorVAE

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the MoGPriorVAE model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    try:
        ...
        if hasattr(model, 'cfg'):
            ...
            if hasattr(new_cfg, 'None' if True else 'width'):
                setattr(new_cfg, 'None' if True else 'width', width)
            # Update depth
            if hasattr(new_cfg, 'None' if True else 'depth'):
                setattr(new_cfg, 'None' if True else 'depth', depth)
