"""
ADP-CNN Width→Depth (mask-free) for CIFAR-100
──────────────────────────────────────────────
Faithful CNN port of your ADPWidth MLP algorithm (no constraints/bounds):
• Inner loop  : early-stopping on validation loss (CrossEntropy)
• Outer loop  :
    1) Width series — widen ALL conv blocks by `ex_k` per try, patience `trials_width` on acceptance.
    2) After each accepted width, run a Depth series — append one ConvBNReLU (same width as last) per try, patience `trials_depth`.
  Acceptance: best_val_loss < pre_series_val_loss - delta.
  On patience exhaustion, rollback to pre-series baseline (arch + weights).
...
# Ensure a default console handler once (avoid duplicates if caller adds one)
...
# ════════════════════════════════════════════════════════════════════════
# Algorithm: ADP_CNN_Width (Width series → Depth series)
# ════════════════════════════════════════════════════════════════════════
...
class ADP_CNN_Width:
    def __init__(self, config: Optional[Config] = None, device: Optional[torch.device] = None, plot_path: Optional[str] = None):
        ...
        logger.info(
            ...
            f"trials_width={self.trials_width} | trials_depth={self.trials_depth} | "
    ...
    def fit(self, train_loader, val_loader):
        ...
        while True:
            if self.global_epoch[0] >= self.max_epochs:
                logger.info("[Width] global epoch budget exhausted; stopping")
                break

            # Capacity guards before proposing a widen
            ...
            logger.info(
                f"[Width] after widen_all | widths_after={_widths_list(self.model)} | "
                f"depth_after={_depth(self.model)} | neurons_after={_total_neurons(self.model)}"
            )
            ...
            if width_loss + 1e-12 < pre_width_loss - self.delta:
                ...
                # ────────────────────── DEPTH SERIES (after accepted width) ──────────────────────
                depth_failures = 0
                pre_depth_snapshot = baseline_snapshot
                pre_depth_loss     = baseline_loss
                while True:
                    if self.global_epoch[0] >= self.max_epochs:
                        logger.info("[Depth] global epoch budget exhausted; stop depth series.")
                        break

                    # Depth capacity guards
                    ...
                    arch_snapshot_before_d = copy.deepcopy(self.model)
                    ...
                    if depth_loss + 1e-12 < pre_depth_loss - self.delta:
                        ...
                        self.model = arch_snapshot_before_d.to(device)
                        ...
                        if depth_failures >= self.trials_depth:
                            logger.info("[Depth] patience exhausted; rollback to pre-depth-series baseline and stop depth series.")
                            self.model.restore_state(pre_depth_snapshot)
                            baseline_snapshot = pre_depth_snapshot
                            baseline_loss     = pre_depth_loss
                            break

                # Ensure we are at best accepted baseline after depth series
                self.model.restore_state(baseline_snapshot)

            else:
                logger.info("[Width] REJECT proposal (no sufficient improvement); rolling back")
                ...
                if width_failures >= self.trials_width:
                    logger.info("[Width] patience exhausted; stopping outer loop.")
                    break
