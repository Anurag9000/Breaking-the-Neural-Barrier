...
# Load baseline
BASE_PATH = Path(__file__).with_name("mo_co_v_2_cnn_model_self_supervised.py").resolve()
    ...
    for _ in range(acfg.max_epochs):
        ...
        if logger:
            logger.log_epoch_stats({
                ...
                "width": 0,
                "depth": 0
                "neurons": total_neurons(getattr(model, "None", 0), getattr(model, "None", 0)),
                "val_loss": val,
                "best_val": best_val,
...
def main():
    ...
    # Initialize Logger
    logger = ContinuousLogger(Path("results_adp_mo_co_v_2_cnn_model_self_supervised"), "mo_co_v_2_cnn_model_self_supervised", args.adp_mode)
