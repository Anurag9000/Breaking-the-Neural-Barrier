...
# Load baseline
BASE_PATH = Path(__file__).with_name("cnn_dpn.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.DPNBlock

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the DPNBlock model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['in_res'] = get_attr(model, ['in_res'], None)
        kwargs['in_dense'] = get_attr(model, ['in_dense'], None)
        kwargs['R'] = get_attr(model, ['R'], None)
        kwargs['k'] = get_attr(model, ['k'], None)
        kwargs['groups'] = get_attr(model, ['groups'], None)
        kwargs['mid'] = get_attr(model, ['mid'], None)
