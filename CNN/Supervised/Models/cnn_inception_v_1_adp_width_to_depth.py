...
# Load baseline
BASE_PATH = Path(__file__).with_name("cnn_inception_v_1.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.InceptionModule

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the InceptionModule model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['c1x1'] = get_attr(model, ['c1x1'], None)
        kwargs['c3x3_reduce'] = get_attr(model, ['c3x3_reduce'], None)
        kwargs['c3x3'] = get_attr(model, ['c3x3'], None)
        kwargs['c5x5_reduce'] = get_attr(model, ['c5x5_reduce'], None)
        kwargs['c5x5'] = get_attr(model, ['c5x5'], None)
        kwargs['pool_proj'] = get_attr(model, ['pool_proj'], None)
