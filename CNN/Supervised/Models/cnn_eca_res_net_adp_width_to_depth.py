...
# Load baseline
BASE_PATH = Path(__file__).with_name("cnn_eca_res_net.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.ECALayer

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the ECALayer model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['k_size'] = get_attr(model, ['k_size'], 1)
        kwargs['gamma'] = get_attr(model, ['gamma'], None)
        kwargs['b'] = get_attr(model, ['b'], None)
