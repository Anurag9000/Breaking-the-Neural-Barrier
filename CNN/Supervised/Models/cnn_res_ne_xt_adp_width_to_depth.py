...
# Load baseline
BASE_PATH = Path(__file__).with_name("cnn_res_ne_xt.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.BottleneckX

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the BottleneckX model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['in_planes'] = get_attr(model, ['in_planes'], None)
        kwargs['planes'] = get_attr(model, ['planes'], None)
        kwargs['stride'] = get_attr(model, ['stride'], None)
        kwargs['cardinality'] = get_attr(model, ['cardinality'], None)
        kwargs['base_width'] = get_attr(model, ['base_width'], 1)
        kwargs['downsample'] = get_attr(model, ['downsample'], None)
...
def expand_width(model: ModelClass, ex_k: int, max_width: int, device, cfg: ADPConfig) -> Optional[ModelClass]:
    ...
    return rebuild_model(model, nxt, _model_depth(model, 1), device, cfg)
