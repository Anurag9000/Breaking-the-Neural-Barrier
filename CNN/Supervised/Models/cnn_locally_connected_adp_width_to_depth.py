...
# Load baseline
BASE_PATH = Path(__file__).with_name("cnn_locally_connected.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.LocallyConnected2d

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the LocallyConnected2d model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        ...
        kwargs['kernel_size'] = get_attr(model, ['kernel_size'], 1)
        kwargs['stride'] = get_attr(model, ['stride'], None)
        kwargs['padding'] = get_attr(model, ['padding'], None)
        kwargs['input_size'] = get_attr(model, ['input_size'], 1)
        kwargs['bias'] = get_attr(model, ['bias'], None)
