"""
Runner for NFNet (CIFAR) — single-model supervised.
Supports presets {L0, L1, L2}, β residual scale, SWS γ, and drop-path.
...
from CNN_NFNet import make_nfnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_nfnet_cifar(num_classes=num_classes, in_channels=3, preset=args.preset,
                             beta=args.beta, drop_path_rate=args.drop_path, gamma=args.gamma, act=args.act).to(device)
    ...
    torch.save({
        "model": f"NFNet({args.preset}, beta={args.beta}, gamma={args.gamma}, drop_path={args.drop_path}, act={args.act})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train NFNet (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/nfnet_L0_cifar10.pth")

    p.add_argument("--preset", type=str, default="L0", choices=["L0","L1","L2"])
    p.add_argument("--beta", type=float, default=1.0, help="Residual scaling for NF blocks")
    p.add_argument("--gamma", type=float, default=1.0, help="SWS scaling constant")
    p.add_argument("--act", type=str, default="silu", choices=["silu","gelu"])
