"""
Runner for RegNetX (CIFAR) — single-model supervised.
Supports popular presets {regnetx_200mf, 400mf, 600mf, 800mf, 1.6gf}. Standard early-stopping loop,
...
from CNN_RegNetX import make_regnetx_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_regnetx_cifar(num_classes=num_classes, in_channels=3, preset=args.preset).to(device)
    ...
    torch.save({
        "model": f"RegNetX({args.preset}) CIFAR",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train RegNetX (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/regnetx_400mf_cifar10.pth")

    p.add_argument("--preset", type=str, default="regnetx_400mf", choices=[
        "regnetx_200mf", "regnetx_400mf", "regnetx_600mf", "regnetx_800mf", "regnetx_1.6gf"
    ])
