"""
Runner for All-CNN-C on CIFAR-10/100 with early stopping.
Follows your runner conventions: deterministic split w/ separate train/val datasets, AdamW, CE (optional label smoothing),
cosine schedule flag, grad-clip, best-checkpoint restore, final test evaluation.
"""
...
def train(args):
    ...
    # Model/opt/loss
    model = AllCNN(num_classes=num_classes, in_channels=3, p_drop=args.dropout).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train All-CNN-C with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/allcnn_cifar10.pth")
