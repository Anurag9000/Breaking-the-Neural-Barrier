"""
Runner for ResNet v2 (CIFAR pre-activation 6n+2) — single-model supervised.
Supports depths: 20, 32, 44, 56, 110. Standard training loop with early stopping.
"""
...
from CNN_ResNetV2 import make_resnetv2_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_resnetv2_cifar(depth=args.depth, num_classes=num_classes, in_channels=3).to(device)
    ...
    torch.save({
        "model": f"ResNetV2-{args.depth}(CIFAR)",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ResNet v2 (CIFAR 6n+2 pre-activation) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/resnetv2_20_cifar10.pth")

    p.add_argument("--depth", type=int, default=20, choices=[20,32,44,56,110])
