"""
Runner for GC-ResNet (CIFAR) — single-model supervised.
Supports depths 18/34/50, reduction ratio, and fusion mode (add | mul | add_mul).
...
from CNN_GC_ResNet import make_gc_resnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_gc_resnet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3,
                                 reduction=args.reduction, fusion=args.fusion).to(device)
    ...
    torch.save({
        "model": f"GC-ResNet-{args.depth}(CIFAR, r={args.reduction}, fusion={args.fusion})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train GC-ResNet (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/gc_resnet18_cifar10.pth")

    p.add_argument("--depth", type=int, default=18, choices=[18,34,50])
    p.add_argument("--reduction", type=int, default=16, help="GC bottleneck reduction ratio")
    p.add_argument("--fusion", type=str, default="add", choices=["add","mul","add_mul"], help="GC fusion mode")
