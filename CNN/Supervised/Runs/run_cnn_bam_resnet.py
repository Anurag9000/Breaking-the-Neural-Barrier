"""
Runner for BAM-ResNet (CIFAR) — single-model supervised.
Supports depths 18/34/50, reduction ratio, spatial dilations. Standard early-stopping loop,
deterministic split, AdamW, CE ± label smoothing, optional cosine schedule, grad-clip, restore best, final test.
"""
...
from CNN_BAM_ResNet import make_bam_resnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    dilations: List[int] | None = None if len(args.spatial_dilations) == 0 else [int(d) for d in args.spatial_dilations]
    model = make_bam_resnet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3,
                                  reduction=args.reduction, spatial_dilations=dilations).to(device)
    ...
    torch.save({
        "model": f"BAM-ResNet-{args.depth}(CIFAR, r={args.reduction}, dilations={dilations if dilations else 'default'})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train BAM-ResNet (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/bam_resnet18_cifar10.pth")

    p.add_argument("--depth", type=int, default=18, choices=[18,34,50])
    p.add_argument("--reduction", type=int, default=16, help="BAM reduction ratio")
    p.add_argument("--spatial_dilations", nargs='*', default=[], help="Space-separated dilations for BAM spatial branch; empty → default [1,2,4]")
