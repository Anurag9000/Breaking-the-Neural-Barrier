"""
Runner for MobileNet V2 (CIFAR) — single-model supervised.
...
from CNN_MobileNetV2 import make_mobilenet_v2_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_mobilenet_v2_cifar(num_classes=num_classes, in_channels=3, width_mult=args.width_mult, dropout=args.dropout).to(device)
    ...
    torch.save({
        "model": f"MobileNetV2(CIFAR, width_mult={args.width_mult}, dropout={args.dropout})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train MobileNet V2 (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/mobilenetv2_cifar10.pth")
