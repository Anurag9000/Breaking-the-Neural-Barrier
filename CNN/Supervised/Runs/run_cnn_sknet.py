"""
Runner for SKNet-ResNet (CIFAR) — single-model supervised.
Supports depths 18/34/50, reduction ratio, groups. Standard early-stopping loop, deterministic split,
...
from CNN_SKNet import make_sknet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_sknet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3,
                             reduction=args.reduction, groups=args.groups).to(device)
    ...
    torch.save({
        "model": f"SKNet-ResNet-{args.depth}(CIFAR, r={args.reduction}, g={args.groups})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train SKNet-ResNet (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/sknet_resnet18_cifar10.pth")

    p.add_argument("--depth", type=int, default=18, choices=[18,34,50])
    p.add_argument("--reduction", type=int, default=16, help="SK attention reduction ratio")
    p.add_argument("--groups", type=int, default=1, help="Grouped convs inside SK branches")
