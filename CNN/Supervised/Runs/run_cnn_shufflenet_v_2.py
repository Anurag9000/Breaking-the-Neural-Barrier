"""
Runner for ShuffleNet V2 (CIFAR) — single-model supervised.
Supports width multiplier {0.5,1.0,1.5,2.0}. Standard early-stopping loop, deterministic split,
...
from CNN_ShuffleNetV2 import make_shufflenet_v2_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_shufflenet_v2_cifar(num_classes=num_classes, in_channels=3, width_mult=args.width_mult).to(device)
    ...
    torch.save({
        "model": f"ShuffleNetV2(CIFAR, width_mult={args.width_mult})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ShuffleNet V2 (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/shufflenetv2_1.0x_cifar10.pth")

    p.add_argument("--width_mult", type=float, default=1.0, choices=[0.5,1.0,1.5,2.0])
