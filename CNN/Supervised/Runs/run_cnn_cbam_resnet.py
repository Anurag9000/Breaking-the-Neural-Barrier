"""
Runner for CBAM-ResNet (CIFAR) — single-model supervised.
Supports depths 18/34/50, reduction ratio, spatial kernel. Standard early-stopping loop, deterministic split,
...
from CNN_CBAM_ResNet import make_cbam_resnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_cbam_resnet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3,
                                   reduction=args.reduction, spatial_kernel=args.spatial_kernel).to(device)
    ...
    torch.save({
        "model": f"CBAM-ResNet-{args.depth}(CIFAR, r={args.reduction}, k={args.spatial_kernel})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train CBAM-ResNet (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/cbam_resnet18_cifar10.pth")

    p.add_argument("--depth", type=int, default=18, choices=[18,34,50])
    p.add_argument("--reduction", type=int, default=16, help="Channel attention reduction r")
    p.add_argument("--spatial_kernel", type=int, default=7, choices=[3,7], help="Spatial attention kernel size")
