"""
Runner for Wide-ResNet (CIFAR) — single-model supervised.
Supports WRN-<depth>-<widen_factor>, e.g., WRN-28-10. Includes early stopping, deterministic val split,
...
from CNN_WideResNet import make_wrn_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_wrn_cifar(depth=args.depth, widen_factor=args.width, num_classes=num_classes, in_channels=3, p_drop=args.dropout).to(device)
    ...
    torch.save({
        "model": f"WRN-{args.depth}-{args.width}(CIFAR)",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train Wide-ResNet (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/wrn_28_10_cifar10.pth")

    p.add_argument("--depth", type=int, default=28, help="Total depth (6*N+4)")
    p.add_argument("--width", type=int, default=10, help="Widen factor k")
    p.add_argument("--dropout", type=float, default=0.3, help="Dropout in WRN blocks")
