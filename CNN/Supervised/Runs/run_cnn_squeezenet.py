"""
Runner for SqueezeNet (CIFAR) — single-model supervised.
Supports versions 1.1 (default) and 1.0 (CIFAR-adapted). Standard early-stopping loop, deterministic split,
...
from CNN_SqueezeNet import make_squeezenet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_squeezenet_cifar(num_classes=num_classes, in_channels=3, version=args.version).to(device)
    ...
    torch.save({
        "model": f"SqueezeNet-{args.version}(CIFAR)",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train SqueezeNet (CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/squeezenet11_cifar10.pth")

    p.add_argument("--version", type=str, default="1.1", choices=["1.1","1.0"])
