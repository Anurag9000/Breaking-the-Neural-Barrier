"""
Runner for MnasNet (CIFAR) — single-model supervised.
...
def train(args):
    ...
    # Model/opt/loss
    model = make_mnasnet_cifar(num_classes=num_classes, in_channels=3, width_mult=args.width_mult, dropout=args.dropout).to(device)
    ...
    torch.save({
        "model": f"MnasNet(CIFAR, width_mult={args.width_mult}, dropout={args.dropout})",
