"""
Runner for Inception v1 (GoogLeNet, CIFAR variant) on CIFAR-10/100 with early stopping.
Follows your runner conventions and supports optional auxiliary losses (single-model):
  Loss = main + 0.3 * (aux1 + aux2) when --aux is set.
"""
...
from CNN_InceptionV1 import InceptionV1
...
@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, criterion: nn.Module, use_aux: bool) -> Tuple[float, float]:
    model.eval()
    total_loss, total_correct, total = 0.0, 0, 0
    for images, targets in loader:
        images, targets = images.to(device), targets.to(device)
        logits, a1, a2 = model(images)
        loss = criterion(logits, targets)
        if use_aux and (a1 is not None) and (a2 is not None):
            loss = loss + 0.3 * (criterion(a1, targets) + criterion(a2, targets))
...
def train(args):
    ...
    # Model/opt/loss
    model = InceptionV1(num_classes=num_classes, in_channels=3, aux_logits=args.aux, dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for images, targets in train_loader:
            ...
            logits, a1, a2 = model(images)
            loss = criterion(logits, targets)
            if args.aux and (a1 is not None) and (a2 is not None):
                loss = loss + 0.3 * (criterion(a1, targets) + criterion(a2, targets))
            loss.backward()
            if args.grad_clip > 0:
        ...
        val_loss, val_acc = evaluate(model, val_loader, device, criterion, use_aux=args.aux)
    ...
    # Test evaluation
    test_loss, test_acc = evaluate(model, test_loader, device, criterion, use_aux=args.aux)
    elapsed = time() - start_time

    os.makedirs(os.path.dirname(args.save_path), exist_ok=True)
    torch.save({
        "model": "InceptionV1(GoogLeNet-CIFAR)",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train Inception v1 (GoogLeNet, CIFAR) with early stopping (single-model).")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_root", type=str, default="./data")
    p.add_argument("--save_path", type=str, default="./checkpoints/inception_v1_cifar10.pth")
    ...
    p.add_argument("--aux", action="store_true", help="Enable auxiliary classifiers (loss includes aux heads)")
