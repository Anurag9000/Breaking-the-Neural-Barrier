"""
Self-supervised training script for SimSiam + CNN encoder (CIFAR-10 by default)
-------------------------------------------------------------------------------
Features
  • Strong SimCLR-style augmentations to produce two views
  • AMP training, AdamW optimizer (optionally SGD)
  • Cosine LR schedule (optional)
  • Checkpointing of encoder+heads
  • Optional linear evaluation with frozen encoder (--linear-eval)

Usage:
  python run_simsiam_cnn.py --data ./data --epochs 200 --batch-size 256 \
      --width 64 --depth 4 --pool-after "1,3" --proj-dim 2048 \
      --lr 3e-4 --wd 1e-4 --save-dir ./ckpt_simsiam

Linear eval after SSL pretrain:
  python run_simsiam_cnn.py --linear-eval --load ./ckpt_simsiam/best.pth
"""
...
# Local import (place simsiam_cnn_model.py next to this script)
from simsiam_cnn_model import SimSiam, ConvEncoder, total_neurons
...
def build_model(args: Args) -> SimSiam:
    ...
    model = SimSiam(encoder=encoder, proj_hidden=args.proj_hidden, proj_dim=args.proj_dim,
                    pred_hidden=args.pred_hidden)
    return model


def save_checkpoint(model: SimSiam, optimizer: optim.Optimizer, scaler: GradScaler, epoch: int, path: str):
...
def evaluate_ssl_loss(model: SimSiam, loader: DataLoader, device: torch.device, max_batches: int = 5) -> float:
    model.eval()
...
def parse_args() -> Args:
    ...
    p.add_argument('--save-dir', type=str, default='./ckpt_simsiam')
    ...
    p.add_argument('--pred-hidden', type=int, default=None)
