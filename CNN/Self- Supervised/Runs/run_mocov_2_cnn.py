"""
Self-supervised training script for MoCo v2 + CNN encoder (CIFAR-10 by default)
-------------------------------------------------------------------------------
Features
  • Strong SimCLR-style augmentations producing two views (q, k)
  • Momentum (EMA) key encoder and a large queue of negatives
  • AMP training, AdamW optimizer
  • Optional cosine momentum schedule toward 1.0
  • Checkpointing of query/key encoders and queue state
  • Optional linear evaluation with frozen encoder (--linear-eval)

Usage:
  python run_mocov2_cnn.py --data ./data --epochs 400 --batch-size 256 \
      --width 64 --depth 4 --pool-after "1,3" --K 16384 --m 0.99 --T 0.2 \
      --lr 3e-4 --wd 1e-4 --save-dir ./ckpt_moco

Linear eval after SSL pretrain:
  python run_mocov2_cnn.py --linear-eval --load ./ckpt_moco/best.pth
"""
...
# Local import (place mocov2_cnn_model.py next to this script)
from mocov2_cnn_model import MoCoV2, ConvEncoder, total_neurons
...
class PairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img, _ = self.base[idx]
        x1 = self.t1(img)  # query view
        x2 = self.t2(img)  # key view
        return x1, x2
...
def build_model(args: Args) -> MoCoV2:
    ...
    model = MoCoV2(encoder_q=encoder, proj_hidden=args.proj_hidden, proj_dim=args.proj_dim,
                   K=args.K, m=args.m, T=args.T)
    return model


def save_checkpoint(model: MoCoV2, optimizer: optim.Optimizer, scaler: GradScaler, epoch: int, path: str):
...
def cosine_schedule(base: float, step: int, total_steps: int) -> float:
    t = min(step / max(1, total_steps), 1.0)
    return 1.0 - (1.0 - base) * (0.5 * (1 + math.cos(math.pi * t)))
...
def train(args: Args):
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        running = 0.0
        for step, (xq, xk) in enumerate(train_loader, start=1):
            xq = xq.to(device, non_blocking=True)
            xk = xk.to(device, non_blocking=True)

            # momentum schedule for key encoder
            if args.cosine_m:
                m = cosine_schedule(args.m, global_step, total_steps)
                model.update_momentum(m)
            else:
                model.update_momentum(args.m)

            optimizer.zero_grad(set_to_none=True)
            with autocast(enabled=(device.type == 'cuda')):
                loss = model.loss(xq, xk)
        ...
        # Epoch summary (quick estimate)
        epoch_loss = evaluate_ssl_loss(model, train_loader, device, max_batches=5)
        print(f"Epoch {epoch:03d} | SSL loss: {epoch_loss:.4f} | neurons (q-enc): {total_neurons(model):,}")
    ...
    if args.linear_eval:
        acc = linear_eval(args, model.encoder_q, test_loader, device)
        print(f"Linear eval (frozen encoder) accuracy: {acc*100:.2f}%")


def evaluate_ssl_loss(model: MoCoV2, loader: DataLoader, device: torch.device, max_batches: int = 5) -> float:
    model.eval()
...
def parse_args() -> Args:
    ...
    p.add_argument('--save-dir', type=str, default='./ckpt_moco')
    ...
    p.add_argument('--m', type=float, default=0.99)
    p.add_argument('--T', type=float, default=0.2)
    p.add_argument('--cosine-m', action='store_true', default=True)
    p.add_argument('--no-cosine-m', dest='cosine_m', action='store_false')
