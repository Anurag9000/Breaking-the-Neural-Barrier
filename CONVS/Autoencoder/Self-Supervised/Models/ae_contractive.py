...
# -----------------------------
# Contractive Convolutional Autoencoder (single-model)
# - 0-based pooling indices during forward (mirrors  models)
# - Contractive penalty approximates ||∂h/∂x||_F^2 via autograd on encoder output h wrt input x
# -----------------------------

class ContractiveConvAE(nn.Module):
    def __init__(
        self,
        in_ch: int = 3,
        ...
        # Decoder (mirror widths in reverse order)
    ...
    def decode(self, h: torch.Tensor, up_times: int) -> torch.Tensor:
        z = h
        # We upsample "up_times" times in total across the decoder pathway
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # count how many downsamples happened to mirror on decode
        down_ct = 0
        h = x
        for i, blk in enumerate(self.encoder):
            h = blk(h)
...
# -----------------------------
# Trainer with contractive penalty
# -----------------------------
...
class CAETrainer:
    def __init__(
        ...
        es_patience: int = 30,
        max_epochs: int = 300,
        results_dir: str = "results_ae_contractive",
    ...
    def _contractive_penalty(self, x: torch.Tensor) -> torch.Tensor:
        # Approximate ||∂h/∂x||_F^2 by computing gradient of h.sum() wrt x
        x = x.detach().clone().requires_grad_(True)
        h = self.model.encode(x)
        s = h.sum()
        grads = torch.autograd.grad(s, x, create_graph=True)[0]
    ...
    def _eval_epoch(self, loader) -> float:
        ...
        for x, _ in loader:
            ...
            y = self.model(x)
            recon = self.crit(y, x)
            total = recon  # no contractive penalty on validation for fair early stopping
            bs = x.size(0)
    ...
    def _maybe_plot(self):
        ...
        plt.title(f"CAE Val Loss | neurons={self.model.total_neurons()} depth={self.model.depth()} widths={self.model.widths_list()} lam_c={self.lam_c}")
        plt.grid(True, which="both", ls=":")
        out = os.path.join(self.results_dir, "CAE_neuron_loss_plot.png")
        plt.tight_layout()
        fig.savefig(out)
        plt.close(fig)
    ...
    def fit(self, train_loader, val_loader):
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            for x, _ in train_loader:
                ...
                y = self.model(x)
                recon = self.crit(y, x)
                cpen = self._contractive_penalty(x)
                loss = recon + self.lam_c * cpen
    ...
    def load(self, path: str, map_location=None):
        ...
        self.lam_c = float(blob.get("lam_c", self.lam_c))
