...
from ae_contractive import ContractiveConvAE, CAETrainer
...
def main():
    ...
    tc = TrainConfig(
        ...
        es_patience=30,
        max_epochs=300,
        results_dir="results_ae_contractive",
        lam_contractive=1e-3,
    )
    ...
    trainer = CAETrainer(
        ...
        lam_contractive=tc.lam_contractive,
    )

    best_val = trainer.fit(train_loader, val_loader)
    test_loss = trainer.evaluate(test_loader)

    ckpt = os.path.join(tc.results_dir, "CAE_best.pth")
    trainer.save(ckpt)
