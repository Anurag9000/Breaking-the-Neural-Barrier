...
from ae_unet import UNetConvAE, UNetAETrainer
...
def main():
    ...
    model = UNetConvAE(
        ...
        use_bilinear=mc.use_bilinear,
    )

    trainer = UNetAETrainer(
        model=model,
        device=device,
        lr=tc.lr,
    ...
    ckpt = os.path.join(tc.results_dir, "UNetAE_best.pth")
    trainer.save(ckpt)
