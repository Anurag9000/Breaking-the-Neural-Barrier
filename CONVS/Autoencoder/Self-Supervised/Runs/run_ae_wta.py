...
from ae_wta import WTAConvAE, WTAAETrainer
...
def main():
    ...
    model = WTAConvAE(
        ...
        k_spatial=mc.k_spatial,
    )

    trainer = WTAAETrainer(
        model=model,
        device=device,
        lr=tc.lr,
    ...
    ckpt = os.path.join(tc.results_dir, "WTAAE_best.pth")
    trainer.save(ckpt)
