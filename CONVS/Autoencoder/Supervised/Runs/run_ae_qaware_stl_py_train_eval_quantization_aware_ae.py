...
from AE_QAWARE_STL import AE_QAWARE_STL, ae_qaware_total_neurons

# -----------------------------------------------------------------------------
# Runner for AE_QAWARE_STL
# Quantizes the latent z with an STE-based fake-quant module during training.
# Loss = MSE reconstruction; no extra penalty needed.
# -----------------------------------------------------------------------------
...
def main():
    ...
    p.add_argument('--out_dir', type=str, default='./Runs/ae_qaware_stl')
    ...
    p.add_argument('--quant_everywhere', action='store_true')
    # Train
    ...
    model = AE_QAWARE_STL(in_channels=3, width=args.width, depth=args.depth,
                          pool_after=pool_after, n_bits=args.n_bits,
                          per_channel=bool(args.per_channel),
                          quant_everywhere=bool(args.quant_everywhere)).to(device)
    ...
    report = {
        ...
        'pool_after': pool_after, 'n_bits': args.n_bits,
        'per_channel': bool(args.per_channel), 'quant_everywhere': bool(args.quant_everywhere),
        'neurons_metric': ae_qaware_total_neurons(args.width, args.depth),
