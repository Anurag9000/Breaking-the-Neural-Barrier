"""
Runner for DPN (Dual Path Network, CIFAR) — single-model supervised.
Supports compact and dpn92lite configs, early stopping, deterministic split,
...
from CNN_DPN import make_dpn_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_dpn_cifar(config=args.config, num_classes=num_classes, in_channels=3, groups=args.groups).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train DPN (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/dpn_compact_cifar10.pth")

    p.add_argument("--config", type=str, default="compact", choices=["compact", "dpn92lite"])
    p.add_argument("--groups", type=int, default=16, help="Grouped conv cardinality in DPN blocks")
    ...
    p.add_argument("--max_epochs", type=int, default=260)
    p.add_argument("--patience", type=int, default=26)
