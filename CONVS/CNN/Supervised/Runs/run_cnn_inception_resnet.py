"""
Runner for Inception-ResNet (CIFAR variant) — single-model.
Uses early stopping, deterministic val split, AdamW, optional cosine schedule, grad-clip, and final test eval.
"""
...
from CNN_InceptionResNet import InceptionResNet
...
def train(args):
    ...
    # Model/opt/loss
    model = InceptionResNet(num_classes=num_classes, in_channels=3, dropout=args.dropout, scale=args.scale).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train Inception-ResNet (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/inception_resnet_cifar10.pth")
    ...
    p.add_argument("--max_epochs", type=int, default=320)
    p.add_argument("--patience", type=int, default=32)
    ...
    p.add_argument("--scale", type=float, default=0.1, help="Residual scaling factor")
