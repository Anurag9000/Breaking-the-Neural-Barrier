"""
Runner for SparseNet (CIFAR) — single-model supervised.
Supports growth rate k, layers per block n, compression θ, dropout. Standard early-stopping loop,
...
from CNN_SparseNet import make_sparsenet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_sparsenet_cifar(growth_rate=args.growth, layers_per_block=args.layers_per_block,
                                 compression=args.compression, drop_rate=args.dropout,
    ...
    torch.save({
        "model": f"SparseNet(LPB={args.layers_per_block}, k={args.growth})-CIFAR",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train SparseNet (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/sparsenet_l16_k12_cifar10.pth")

    p.add_argument("--layers_per_block", type=int, default=16, help="n per block")
    p.add_argument("--growth", type=int, default=12, help="Growth rate k")
    p.add_argument("--compression", type=float, default=0.5, help="Transition compression θ")
    p.add_argument("--dropout", type=float, default=0.0, help="Dropout after 3×3 conv in sparse layers")
    ...
    p.add_argument("--max_epochs", type=int, default=240)
