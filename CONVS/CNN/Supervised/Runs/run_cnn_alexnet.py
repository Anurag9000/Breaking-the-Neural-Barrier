"""
Runner for AlexNet (CIFAR variant) on CIFAR-10/100 with early stopping.
Keeps parity with your runners: deterministic split, separate train/val datasets, AdamW, CE (optional label smoothing),
cosine schedule flag, grad-clip, best-checkpoint restore, final test.
"""
...
def train(args):
    ...
    # Model/opt/loss
    model = AlexNet(num_classes=num_classes, in_channels=3, dropout=args.dropout).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train AlexNet (CIFAR variant) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/alexnet_cifar10.pth")
