"""
Runner for Inception v3 (CIFAR variant) with optional aux logits — single-model.
...
from CNN_InceptionV3 import InceptionV3
...
def train(args):
    ...
    # Model/opt/loss
    model = InceptionV3(num_classes=num_classes, in_channels=3, aux_logits=args.aux, dropout=args.dropout).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train Inception v3 (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/inception_v3_cifar10.pth")
