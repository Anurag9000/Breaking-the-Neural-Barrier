"""
Runner for Locally Connected CNN (LCLNet) on CIFAR-10/100 with early stopping.
Adheres to your runner conventions: deterministic split, separate train/val datasets (no transform leakage),
AdamW, CE (optional label smoothing), cosine flag, grad-clip, best-checkpoint restore, final test.

NOTE: The LocallyConnected2d layer expects fixed 16x16 input at its position. This runner uses a first max-pool
      to ensure that (32x32 -> 16x16) before the LC layer.
"""
...
from CNN_LocallyConnected import LCLNet
...
def train(args):
    ...
    # Model/opt/loss
    model = LCLNet(num_classes=num_classes, in_channels=3, p_drop=args.dropout).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train Locally Connected CNN with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/lclnet_cifar10.pth")
