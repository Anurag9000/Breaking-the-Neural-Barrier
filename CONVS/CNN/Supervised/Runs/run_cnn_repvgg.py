"""
Runner for RepVGG (CIFAR) — single-model supervised.
Supports presets {A0, A1, B0} and an optional --deploy flag to load in re-parameterized form after training.
Standard early-stopping loop, deterministic split, AdamW, CE ± label smoothing, optional cosine schedule,
grad-clip, restore best, final test. Optionally fuses to deploy and re-saves a *_deploy.pth.
"""
...
from CNN_RepVGG import make_repvgg_cifar, RepVGGCIFAR
...
def train(args):
    ...
    # Model/opt/loss
    model = make_repvgg_cifar(num_classes=num_classes, in_channels=3, preset=args.preset, deploy=False).to(device)
    ...
    # Save training-form checkpoint
    os.makedirs(os.path.dirname(args.save_path), exist_ok=True)
    torch.save({
        "model": f"RepVGG({args.preset}) CIFAR (train-form)",
    ...
    # Optionally fuse to deploy and save a second checkpoint
    if args.export_deploy:
        if isinstance(model, RepVGGCIFAR):
            model.reparameterize()
        deploy_path = args.save_path.replace('.pth', '_deploy.pth')
        torch.save({
            "model": f"RepVGG({args.preset}) CIFAR (deploy)",
            "state_dict": model.state_dict(),
            "num_classes": num_classes,
            "best_val_loss": best_val,
            "test_loss": test_loss,
        ...
        print(f"Exported deployable model to: {deploy_path}")
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train RepVGG (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/repvgg_A0_cifar10.pth")

    p.add_argument("--preset", type=str, default="A0", choices=["A0","A1","B0"])
    p.add_argument("--export_deploy", action="store_true", help="Fuse branches and save *_deploy.pth after training")
