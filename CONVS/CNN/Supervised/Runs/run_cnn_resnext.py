"""
Runner for ResNeXt (CIFAR) — single-model supervised.
Supports depth (e.g., 29), cardinality (groups), base width per group, early stopping, deterministic split,
...
from CNN_ResNeXt import make_resnext_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_resnext_cifar(depth=args.depth, cardinality=args.cardinality, base_width=args.base_width,
    ...
    torch.save({
        "model": f"ResNeXt-{args.depth}({args.cardinality}x{args.base_width}d)-CIFAR",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ResNeXt (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/resnext_29_8x64d_cifar10.pth")

    p.add_argument("--depth", type=int, default=29)
    p.add_argument("--cardinality", type=int, default=8, help="Number of groups")
    p.add_argument("--base_width", type=int, default=64, help="Width per group")
