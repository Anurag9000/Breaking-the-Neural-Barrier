"""
Runner for ConvNeXt (CIFAR) — single-model supervised.
Supports presets {tiny, small, base}, width multiplier, layer scale, and drop-path.
...
from CNN_ConvNeXt import make_convnext_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_convnext_cifar(num_classes=num_classes, in_channels=3, preset=args.preset,
                                width_mult=args.width_mult, layer_scale_init=args.layer_scale,
    ...
    torch.save({
        "model": f"ConvNeXt({args.preset}, width_mult={args.width_mult}, layer_scale={args.layer_scale}, drop_path={args.drop_path})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ConvNeXt (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/convnext_tiny_cifar10.pth")
