"""
Runner for Inception v4 (CIFAR variant) with optional aux logits — single-model.
Loss = main + 0.3 * aux (when --aux is enabled).
"""
...
from CNN_InceptionV4 import InceptionV4
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device, criterion: nn.Module, use_aux: bool) -> Tuple[float, float]:
    ...
    for images, targets in loader:
        ...
        if use_aux and (aux is not None):
            loss = loss + 0.3 * criterion(aux, targets)
...
def train(args):
    ...
    # Model/opt/loss
    model = InceptionV4(num_classes=num_classes, in_channels=3, aux_logits=args.aux, dropout=args.dropout).to(device)
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for images, targets in train_loader:
            ...
            logits, aux = model(images)
            loss = criterion(logits, targets)
            if args.aux and (aux is not None):
                loss = loss + 0.3 * criterion(aux, targets)
            loss.backward()
            if args.grad_clip > 0:
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train Inception v4 (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/inception_v4_cifar10.pth")
    ...
    p.add_argument("--max_epochs", type=int, default=350)
    ...
    p.add_argument("--aux", action="store_true", help="Enable auxiliary classifier (loss includes aux)")
