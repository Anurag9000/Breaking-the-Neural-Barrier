"""
Runner for GhostNet (CIFAR) — single-model supervised.
Supports width multiplier, ghost ratio, and dropout. Standard early-stopping loop, deterministic split,
...
from CNN_GhostNet import make_ghostnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_ghostnet_cifar(num_classes=num_classes, in_channels=3, width_mult=args.width_mult,
                                ghost_ratio=args.ghost_ratio, dropout=args.dropout).to(device)
    ...
    torch.save({
        "model": f"GhostNet(CIFAR, width_mult={args.width_mult}, ghost_ratio={args.ghost_ratio}, dropout={args.dropout})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train GhostNet (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type:str, default="./checkpoints/ghostnet_cifar10.pth")

    p.add_argument("--width_mult", type=float, default=1.0)
    p.add_argument("--ghost_ratio", type=int, default=2)
