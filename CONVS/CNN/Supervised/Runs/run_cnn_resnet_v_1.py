"""
Runner for ResNet v1 (CIFAR 6n+2) — single-model supervised.
...
from CNN_ResNetV1 import make_resnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_resnet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3).to(device)
    ...
    torch.save({
        "model": f"ResNetV1-{args.depth}(CIFAR)",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ResNet v1 (CIFAR 6n+2) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/resnetv1_20_cifar10.pth")
