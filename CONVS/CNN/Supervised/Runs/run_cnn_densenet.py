"""
Runner for DenseNet-BC (CIFAR) — single-model supervised.
Supports L=6n+4 via `--layers_per_block` (n), growth rate k, compression θ, dropout, early stopping,
...
from CNN_DenseNet import make_densenet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_densenet_cifar(growth_rate=args.growth, layers_per_block=args.layers_per_block,
    ...
    torch.save({
        "model": f"DenseNet-BC(L={6*args.layers_per_block+4}, k={args.growth})-CIFAR",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train DenseNet-BC (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/densenet_bc_l100_k12_cifar10.pth")

    p.add_argument("--layers_per_block", type=int, default=16, help="n so that L=6n+4 (e.g., 16 → L=100)")
    ...
    p.add_argument("--dropout", type=float, default=0.0, help="Dropout after 3×3 conv in dense layers")
