"""
Runner for SE-ResNet (CIFAR) — single-model supervised.
Supports depths 18/34/50 and reduction ratio. Standard early-stopping loop, deterministic split,
...
from CNN_SE_ResNet import make_se_resnet_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_se_resnet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3, reduction=args.reduction).to(device)
    ...
    torch.save({
        "model": f"SE-ResNet-{args.depth}(CIFAR, r={args.reduction})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train SE-ResNet (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/se_resnet18_cifar10.pth")

    p.add_argument("--depth", type=int, default=18, choices=[18,34,50])
    p.add_argument("--reduction", type=int, default=16, help="SE reduction ratio r")
