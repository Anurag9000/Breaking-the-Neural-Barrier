"""
Runner for ResNet-D (CIFAR) — single-model supervised.
Supports depths 18/34/50. Standard loop with early stopping, deterministic val split,
...
from CNN_ResNetD import make_resnetd_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_resnetd_cifar(depth=args.depth, num_classes=num_classes, in_channels=3).to(device)
    ...
    torch.save({
        "model": f"ResNetD-{args.depth}(CIFAR)",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ResNet-D (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/resnetd_18_cifar10.pth")
