"""
Runner for ECA-ResNet (CIFAR) — single-model supervised.
Supports depths 18/34/50, optional fixed ECA kernel k or adaptive via (gamma,b).
Standard early-stopping loop, deterministic split, AdamW, CE ± label smoothing, optional cosine
schedule, grad-clip, restore best, final test.
"""
...
from CNN_ECA_ResNet import make_eca_resnet_cifar
...
def train(args):
    ...
    model = make_eca_resnet_cifar(depth=args.depth, num_classes=num_classes, in_channels=3,
                                  k_size=(None if args.k_size <= 0 else args.k_size), gamma=args.gamma, b=args.b).to(device)
    ...
    torch.save({
        "model": f"ECA-ResNet-{args.depth}(CIFAR, k={args.k_size if args.k_size>0 else 'adaptive'}, gamma={args.gamma}, b={args.b})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ECA-ResNet (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/eca_resnet18_cifar10.pth")

    p.add_argument("--depth", type=int, default=18, choices=[18,34,50])
    p.add_argument("--k_size", type=int, default=0, help="Fixed odd kernel for ECA 1D conv; <=0 uses adaptive rule")
    p.add_argument("--gamma", type=float, default=2.0, help="Adaptive kernel rule divisor")
    p.add_argument("--b", type=float, default=1.0, help="Adaptive kernel rule bias")
