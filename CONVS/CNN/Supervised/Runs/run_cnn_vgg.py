"""
Runner for VGG (A/D/E, optional BN) on CIFAR-10/100 with early stopping.
Parallels your runner style: deterministic split with separate train/val datasets, AdamW, CE (optional label smoothing),
cosine schedule flag, grad-clip, best-checkpoint restore, and final test evaluation.
"""
...
def train(args):
    ...
    # Model/opt/loss
    model = make_vgg(variant=args.variant, num_classes=num_classes, in_channels=3, batch_norm=not args.no_bn, dropout=args.dropout).to(device)
    ...
    torch.save({
        "model": f"VGG-{args.variant}{'-BN' if not args.no_bn else ''}",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train VGG (A/D/E, optional BN) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/vggD_bn_cifar10.pth")

    p.add_argument("--variant", type=str, default="D", choices=["A", "D", "E"], help="VGG-11(A), VGG-16(D), VGG-19(E)")
    p.add_argument("--no_bn", action="store_true", help="Disable BatchNorm (default: BN enabled)")
