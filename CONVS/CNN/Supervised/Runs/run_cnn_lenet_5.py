"""
Training script for LeNet-5 on CIFAR-10/100 with early stopping.
- Mirrors the runner style in your uploaded files: deterministic split, separate train/val datasets (no transform leakage),
  AdamW, CE loss, gradient clipping, best-checkpoint restore, and final test evaluation.
- Optional cosine schedule and label smoothing via flags.
"""
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train LeNet-5 on CIFAR with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/lenet5_cifar10.pth")
