"""
Runner for ShuffleNet V1 (CIFAR) — single-model supervised.
Supports group count (1,2,3,4,8) and width multiplier. Standard early-stopping loop,
deterministic split, AdamW, CE ± label smoothing, optional cosine schedule, grad-clip,
...
from CNN_ShuffleNetV1 import make_shufflenet_v1_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_shufflenet_v1_cifar(num_classes=num_classes, in_channels=3, groups=args.groups, width_mult=args.width_mult).to(device)
    ...
    torch.save({
        "model": f"ShuffleNetV1(CIFAR, groups={args.groups}, width_mult={args.width_mult})",
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train ShuffleNet V1 (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/shufflenetv1_g3_cifar10.pth")

    p.add_argument("--groups", type=int, default=3, choices=[1,2,3,4,8])
