"""
Runner for Network in Network (NiN) on CIFAR-10/100 with early stopping.
Matches your runner style: deterministic split with separate train/val datasets, AdamW, CE (optional label smoothing),
...
def train(args):
    ...
    # Model/opt/loss
    model = NiN(num_classes=num_classes, in_channels=3, p_drop=args.dropout).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train NiN with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/nin_cifar10.pth")
