"""
Runner for CSPDarknet-53 (CIFAR) — single-model supervised.
Standard early-stopping loop, deterministic val split, AdamW, CE ± label smoothing, optional cosine schedule,
grad-clip, restore best, and final test.
"""
...
from CNN_CSPNet import make_cspdarknet53_cifar
...
def train(args):
    ...
    # Model/opt/loss
    model = make_cspdarknet53_cifar(num_classes=num_classes, in_channels=3).to(device)
...
def build_argparser():
    p = argparse.ArgumentParser(description="Train CSPDarknet-53 (CIFAR) with early stopping (single-model).")
    ...
    p.add_argument("--save_path", type=str, default="./checkpoints/cspdarknet53_cifar10.pth")
