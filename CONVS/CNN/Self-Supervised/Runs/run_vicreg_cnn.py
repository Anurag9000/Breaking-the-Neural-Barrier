"""
Self-supervised training script for VICReg + CNN encoder (CIFAR-10 by default)
------------------------------------------------------------------------------
Features
  • Strong SimCLR-style augmentations producing two views
  • 3-layer projector (BN on last layer without affine)
...
Usage:
  python run_vicreg_cnn.py --data ./data --epochs 300 --batch-size 256 \
      --width 64 --depth 4 --pool-after "1,3" --proj-dim 2048 \
      --alpha 25 --beta 25 --mu 1 --lr 1e-3 --wd 1e-6 --save-dir ./ckpt_vicreg

Linear eval after SSL pretrain:
  python run_vicreg_cnn.py --linear-eval --load ./ckpt_vicreg/best.pth
"""
...
# Local import (place vicreg_cnn_model.py next to this script)
from vicreg_cnn_model import VICReg, ConvEncoder, vicreg_loss, total_neurons
...
def build_model(args: Args) -> VICReg:
    ...
    model = VICReg(encoder=encoder, proj_hidden=args.proj_hidden, proj_dim=args.proj_dim)
    return model


def save_checkpoint(model: VICReg, optimizer: optim.Optimizer, scaler: GradScaler, epoch: int, path: str):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, (x1, x2) in enumerate(train_loader, start=1):
            ...
            with autocast(enabled=(device.type == 'cuda')):
                z1, z2 = model(x1, x2)
                loss = vicreg_loss(z1, z2, alpha=args.alpha, beta=args.beta, mu=args.mu, gamma=args.gamma)
        ...
        # Epoch summary
        epoch_loss = evaluate_ssl_loss(model, train_loader, device, args, max_batches=5)
...
def evaluate_ssl_loss(model: VICReg, loader: DataLoader, device: torch.device, args: Args, max_batches: int = 5) -> float:
    model.eval()
...
def parse_args() -> Args:
    ...
    p.add_argument('--save-dir', type=str, default='./ckpt_vicreg')
