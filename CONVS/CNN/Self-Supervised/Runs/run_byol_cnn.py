"""
Self-supervised training script for BYOL + CNN encoder (CIFAR-10 by default)
----------------------------------------------------------------------------
Features
  • Strong SimCLR-style augmentations producing two views
  • Online (encoder→projector→predictor) vs. EMA target (encoder→projector) with stop-grad
  • EMA momentum schedule (cosine to 1.0)
  • AMP training, AdamW optimizer, optional cosine LR
  • Checkpointing of online/target branches
  • Optional linear evaluation with frozen *target* encoder (--linear-eval)

Usage:
  python run_byol_cnn.py --data ./data --epochs 400 --batch-size 256 \
      --width 64 --depth 4 --pool-after "1,3" --proj-dim 256 --pred-hidden 128 \
      --m-ema 0.996 --lr 3e-4 --wd 1e-4 --save-dir ./ckpt_byol

Linear eval after SSL pretrain:
  python run_byol_cnn.py --linear-eval --load ./ckpt_byol/best.pth
"""
...
# Local import (place byol_cnn_model.py next to this script)
from byol_cnn_model import BYOL, ConvEncoder, total_neurons
...
def build_model(args: Args) -> BYOL:
    ...
    model = BYOL(encoder=encoder, proj_hidden=args.proj_hidden, proj_dim=args.proj_dim,
                 pred_hidden=args.pred_hidden, m_ema=args.m_ema)
    return model


def save_checkpoint(model: BYOL, optimizer: optim.Optimizer, scaler: GradScaler, epoch: int, path: str):
...
def cosine_momentum(base_m: float, step: int, total_steps: int) -> float:
    # schedule momentum from base_m toward ~1.0
    t = min(step / max(1, total_steps), 1.0)
    return 1.0 - (1.0 - base_m) * (0.5 * (1 + math.cos(math.pi * t)))
...
def train(args: Args):
        ...
        for step, (x1, x2) in enumerate(train_loader, start=1):
            ...
            if args.cosine:
                ...
                for pg in optimizer.param_groups:
                    pg['lr'] = lr
            m = cosine_momentum(args.m_ema, global_step, total_steps)
    ...
    if args.linear_eval:
        acc = linear_eval(args, model.encoder_k, test_loader, device)
        print(f"Linear eval (frozen EMA target) accuracy: {acc*100:.2f}%")


def evaluate_ssl_loss(model: BYOL, loader: DataLoader, device: torch.device, max_batches: int = 5) -> float:
    model.eval()
...
def linear_eval(args: Args, encoder: nn.Module, test_loader: DataLoader, device: torch.device) -> float:
    ...
    feat_dim = getattr(encoder, 'out_dim', None)
    if feat_dim is None:
        # assume ConvEncoder with GAP -> width
        feat_dim = next(p for n,p in encoder.named_parameters() if n.endswith('features.0.conv.weight')).shape[0]
...
def parse_args() -> Args:
    ...
    p.add_argument('--save-dir', type=str, default='./ckpt_byol')
    ...
    p.add_argument('--m-ema', type=float, default=0.996)
    p.add_argument('--proj-dim', type=int, default=256)
    p.add_argument('--proj-hidden', type=int, default=None)
    p.add_argument('--pred-hidden', type=int, default=128)
