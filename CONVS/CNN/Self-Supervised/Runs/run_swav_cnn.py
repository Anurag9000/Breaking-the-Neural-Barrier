"""
Self-supervised training script for SwAV + CNN encoder (CIFAR-10 by default)
----------------------------------------------------------------------------
Features
  • Strong SimCLR-style augmentations producing two large crops (extendable to multi-crop)
  • Online Sinkhorn-Knopp to obtain balanced codes per batch
  • AMP training, AdamW optimizer, optional cosine LR
  • Checkpointing of encoder, projector, and prototypes
  • Optional linear evaluation with frozen encoder (--linear-eval)

Usage:
  python run_swav_cnn.py --data ./data --epochs 400 --batch-size 256 \
      --width 64 --depth 4 --pool-after "1,3" --proj-dim 128 --prototypes 300 \
      --tau-p 0.1 --tau-s 0.1 --lr 3e-4 --wd 1e-4 --save-dir ./ckpt_swav

Linear eval after SSL pretrain:
  python run_swav_cnn.py --linear-eval --load ./ckpt_swav/best.pth
"""
...
# Local import (place swav_cnn_model.py next to this script)
from swav_cnn_model import SwAV, ConvEncoder, total_neurons
...
def build_model(args: Args) -> SwAV:
    ...
    model = SwAV(encoder=encoder, proj_hidden=args.proj_hidden, proj_dim=args.proj_dim,
                 n_prototypes=args.prototypes, tau_p=args.tau_p, tau_s=args.tau_s)
    return model


def save_checkpoint(model: SwAV, optimizer: optim.Optimizer, scaler: GradScaler, epoch: int, path: str):
...
def evaluate_ssl_loss(model: SwAV, loader: DataLoader, device: torch.device, max_batches: int = 5) -> float:
    model.eval()
...
def parse_args() -> Args:
    ...
    p.add_argument('--save-dir', type=str, default='./ckpt_swav')
    ...
    p.add_argument('--tau-p', type=float, default=0.1)
