"""
Self-supervised training script for PIRL (Jigsaw invariance) + CNN encoder (CIFAR-10 by default)
------------------------------------------------------------------------------------------------
Features
  • Strong base augmentations + Jigsaw pretext transform (3x3 grid with permutation bank)
  • Symmetric batch-contrastive objective between clean and jigsaw views (NT-Xent / InfoNCE)
...
Usage:
  python run_pirl_cnn.py --data ./data --epochs 300 --batch-size 256 \
      --width 64 --depth 4 --pool-after "1,3" --proj-dim 128 --tau 0.2 \
      --lr 3e-4 --wd 1e-4 --save-dir ./ckpt_pirl

Linear eval after SSL pretrain:
  python run_pirl_cnn.py --linear-eval --load ./ckpt_pirl/best.pth
"""
...
# Local import (place pirl_cnn_model.py next to this script)
from pirl_cnn_model import PIRL, Jigsaw, ConvEncoder, total_neurons


# ----------------------------
# Data: base augs + paired (clean, jigsaw)
# ----------------------------
...
class PIRLDataset(Dataset):
    def __init__(self, base: Dataset, clean_tf: T.Compose, jigsaw_p: float = 1.0):
        self.base = base
        self.clean_tf = clean_tf
        self.jigsaw = Jigsaw(p=jigsaw_p)
    def __len__(self):
        return len(self.base)
    def __getitem__(self, idx):
        img, _ = self.base[idx]
        x = self.clean_tf(img)             # clean view (tensor CHW)
        x_jig = self.jigsaw(x.clone())     # jigsaw view on the *tensor*
        return x, x_jig
...
def build_dataloaders(args: Args):
    ...
    pirl_train = PIRLDataset(base_train, clean_tf=tf, jigsaw_p=1.0)
    train_loader = DataLoader(pirl_train, batch_size=args.batch_size, shuffle=True,
...
def build_model(args: Args) -> PIRL:
    ...
    model = PIRL(encoder=encoder, proj_hidden=args.proj_hidden, proj_dim=args.proj_dim, temperature=args.tau)
    return model


def save_checkpoint(model: PIRL, optimizer: optim.Optimizer, scaler: GradScaler, epoch: int, path: str):
...
def train(args: Args):
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        running = 0.0
        for step, (x, x_jig) in enumerate(train_loader, start=1):
            x = x.to(device, non_blocking=True)
            x_jig = x_jig.to(device, non_blocking=True)
...
def evaluate_ssl_loss(model: PIRL, loader: DataLoader, device: torch.device, max_batches: int = 5) -> float:
    model.eval()
    total_loss, n_seen, batches = 0.0, 0, 0
    with torch.no_grad():
        for x, x_jig in loader:
            x = x.to(device, non_blocking=True)
            x_jig = x_jig.to(device, non_blocking=True)
            loss = model.loss(x, x_jig)
            bsz = x.size(0)
...
def parse_args() -> Args:
    ...
    p.add_argument('--save-dir', type=str, default='./ckpt_pirl')
