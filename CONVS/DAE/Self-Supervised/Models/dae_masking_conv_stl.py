...
class DAEMaskingConv(nn.Module):
    """
    Convolutional denoising autoencoder with masking noise.

    Inputs are CIFAR-style images; training code is expected to apply a random
    binary mask to the input (dropout-style corruption) and use MSE loss
    between reconstruction and the clean target.
    """
