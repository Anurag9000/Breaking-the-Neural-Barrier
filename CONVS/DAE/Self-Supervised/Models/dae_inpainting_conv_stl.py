...
class DAEInpaintingConv(nn.Module):
    """
    Convolutional inpainting denoising autoencoder for CIFAR‑like images.

    Architecture mirrors the plain conv DAE used elsewhere (stack of
    Conv‑BN‑ReLU blocks with occasional pooling on the encoder and a symmetric
    decoder). The corruption (masking a block / region) is applied in the
    training loop; this module simply maps corrupted inputs to reconstructions.
    """

    def __init__(self, in_channels: int = 3, width: int = 64, depth: int = 4, pool_after=None):
        super().__init__()
        assert depth >= 1
