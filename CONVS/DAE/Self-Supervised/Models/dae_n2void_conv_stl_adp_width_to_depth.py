...
from .dae_gaussian_conv_stl import DAEGaussianConv, dae_total_neurons
...
def sample_mask(x: torch.Tensor, mask_prob: float) -> torch.Tensor:
    if mask_prob <= 0.0:
        return torch.zeros_like(x[:, :1, :, :], dtype=torch.bool)
    return torch.rand_like(x[:, :1, :, :]) < mask_prob
...
def rebuild_model(model: DAEGaussianConv, width: int, depth: int, device: torch.device) -> DAEGaussianConv:
    new_model = DAEGaussianConv(
...
def expand_width(model: DAEGaussianConv, ex_k: int, max_width: int, device: torch.device) -> Optional[DAEGaussianConv]:
...
def expand_depth(model: DAEGaussianConv, max_depth: int, device: torch.device) -> Optional[DAEGaussianConv]:
    if model.depth >= max_depth:
        return None
    return rebuild_model(model, model.width, model.depth + 1, device)


def snapshot_arch_and_state(model: DAEGaussianConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEGaussianConv:
    mdl = DAEGaussianConv(
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            mask = sample_mask(xb, acfg.mask_prob)
            xb_masked = xb.clone()
            xb_masked[mask.expand_as(xb)] = 0.0

            opt.zero_grad(set_to_none=True)
            xb_rec, _ = model(xb_masked)
            diff = (xb_rec - xb) ** 2
            loss = diff[mask.expand_as(diff)].sum() / max(mask.sum().item(), 1)
            loss.backward()
...
def adp_search(
...
) -> Tuple[float, DAEGaussianConv, int, int]:
    ...
    def can_widen(m: DAEGaussianConv) -> bool:
        return dae_total_neurons(m.width + acfg.ex_k, m.depth) <= acfg.max_neurons and m.width < acfg.max_width

    def can_deepen(m: DAEGaussianConv) -> bool:
        return dae_total_neurons(m.width, m.depth + 1) <= acfg.max_neurons and m.depth < acfg.max_depth
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="DAEN2VoidConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEN2VoidConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Noise2Void Conv DAE on CIFAR")
    ...
    p.add_argument("--pool-after", type=int, nargs="*", default=[2])
    ...
    p.add_argument("--mask-prob", type=float, default=0.1)

    p.add_argument("--results-dir", type=str, default="results_adp_dae_n2void_conv")
    ...
    pool_after = args.pool_after or []
    model = DAEGaussianConv(in_channels=3, width=args.width, depth=args.depth, pool_after=pool_after).to(device)
    ...
    logger = ContinuousLogger(results_dir, "dae_n2void_conv", args.adp_mode)
