...
from .dae_multiscale_conv_stl import DAEMultiScaleConv, dae_total_neurons
...
def rebuild_model(model: DAEMultiScaleConv, width: int, depth: int, device: torch.device) -> DAEMultiScaleConv:
    new_model = DAEMultiScaleConv(
...
def expand_depth(
    ...
    if model.depth >= max_depth:
        return None
    # By default keep pooling at all but last depth for the new model.
    pool_after = list(model.pool_after)
    if (model.depth) not in pool_after:
        pool_after.append(model.depth)
    new_model = DAEMultiScaleConv(
        in_channels=model.in_channels,
        width=model.width,
        depth=model.depth + 1,
...
def snapshot_arch_and_state(model: DAEMultiScaleConv, state: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, Any]:
...
def restore_arch_and_state(snap: Dict[str, Any], device: torch.device) -> DAEMultiScaleConv:
    mdl = DAEMultiScaleConv(
...
def adp_search(
    ...
    log_loss: bool,
    log_neurons: bool,
) -> Tuple[float, DAEMultiScaleConv, int, int]:
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_multiscale_conv_cifar")
    ...
    logger = ContinuousLogger(
        ...
        console_prefix="[ADP DAE MultiScale]",
    )
    ...
    base_model = DAEMultiScaleConv(in_channels=3, width=args.width, depth=args.depth).to(device)
    logger.log_console(
