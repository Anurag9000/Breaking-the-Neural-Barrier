...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, _ in dl_train:
            ...
            x1 = add_gaussian_noise(xb, acfg.noise_std)
            x2 = add_gaussian_noise(xb, acfg.noise_std)
        ...
        with torch.no_grad():
            for xb, _ in dl_val:
                ...
                x2 = add_gaussian_noise(xb, acfg.noise_std)
                x1_rec, _ = model(x1)
                total += float(mse(x1_rec, x2).item())
                n += xb.size(0)
...
def adp_search(
    ...
    if log_loss:
        plot_loss_vs_epoch(val_history, results_dir / "loss_vs_epoch.png", title="DAEN2NConv")
    if log_neurons and improvements:
        ns = [n for n, _ in improvements]
        vs = [v for _, v in improvements]
        plot_loss_vs_neurons(ns, vs, results_dir / "loss_vs_neurons.png", title="DAEN2NConv")
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="ADP Noise2Noise Conv DAE on CIFAR")
    ...
    p.add_argument("--results-dir", type=str, default="results_adp_dae_n2n_conv")
    ...
    logger = ContinuousLogger(results_dir, "dae_n2n_conv", args.adp_mode)
