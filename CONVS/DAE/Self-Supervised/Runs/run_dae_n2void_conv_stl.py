...
from ..Models.dae_gaussian_conv_stl import DAEGaussianConv, dae_total_neurons
...
def sample_mask(x: torch.Tensor, mask_prob: float) -> torch.Tensor:
    ...
    return torch.rand_like(x[:, :1, :, :]) < mask_prob  # broadcast over channels
...
def train_one_epoch(
    ...
    """
    Noise2Void-style objective: mask random pixels, predict their clean value.
    Loss is computed only on masked locations.
    """
    model.train()
    ...
    for xb, _ in loader:
        ...
        mask = sample_mask(xb, mask_prob)  # (B,1,H,W)
        xb_masked = xb.clone()
        ...
        diff = (xb_rec - xb) ** 2
        masked_loss = diff[mask.expand_as(diff)].sum() / max(mask.sum().item(), 1)
        masked_loss.backward()
        opt.step()

        total += float(masked_loss.item()) * xb.size(0)
        n += xb.size(0)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Noise2Void-style Conv DAE STL on CIFAR")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./Runs/dae_n2void_conv_stl")
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            train_loss = train_one_epoch(model, train_loader, opt, device, mask_prob=args.mask_prob)
            val_loss = eval_epoch(model, val_loader, device, mask_prob=args.mask_prob)
    ...
    report = {
        ...
        "objective": "noise2void_masked_pixels",
    }
