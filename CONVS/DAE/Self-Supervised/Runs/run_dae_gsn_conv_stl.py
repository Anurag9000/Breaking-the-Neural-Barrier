...
def train_one_epoch(
    ...
    """
    GSN-style walkback training: for each minibatch, run K transition steps
    (corrupt+denoise) and penalise deviation from the original clean data
    across all steps.
    """
    model.train()
    ...
    for xb, _ in loader:
        ...
        loss = 0.0
        for _ in range(max(walkback_steps, 1)):
            x_t_noisy = add_gaussian_noise(x_t, noise_std)
            x_t_rec, _ = model(x_t_noisy)
            loss_step = mse(x_t_rec, xb) / xb.size(0)
            loss += loss_step
            x_t = x_t_rec.detach()
        loss.backward()
        opt.step()
...
def main() -> None:
    p = argparse.ArgumentParser(description="GSN / walkback Conv DAE STL on CIFAR")
    p.add_argument("--dataset", type=str, default="cifar10", choices=["cifar10", "cifar100"])
    p.add_argument("--data_dir", type=str, default="./data")
    p.add_argument("--out_dir", type=str, default="./Runs/dae_gsn_conv_stl")
    ...
    p.add_argument("--walkback_steps", type=int, default=3)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            train_loss = train_one_epoch(
                model, train_loader, opt, device, noise_std=args.noise_std, walkback_steps=args.walkback_steps
            )
            val_loss = eval_epoch(
                model, val_loader, device, noise_std=args.noise_std, walkback_steps=args.walkback_steps
            )
    ...
    report = {
        ...
        "walkback_steps": args.walkback_steps,
        "objective": "GSN_walkback",
    }
