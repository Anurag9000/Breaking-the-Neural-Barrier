...
from ..Models.dae_inpainting_conv_stl import DAEInpaintingConv, dae_total_neurons
...
def apply_center_mask(x: torch.Tensor, mask_size: int) -> torch.Tensor:
    """
    Zero a central square block of side `mask_size` pixels.
    """
    if mask_size <= 0:
        return x
    _, _, h, w = x.shape
...
def train_one_epoch(
    ...
    for xb, _ in dl:
        ...
        xb_corrupt = xb.clone()
        xb_corrupt = apply_center_mask(xb_corrupt, mask_size)
...
def main() -> None:
    ...
    p.add_argument("--results-dir", type=str, default="results_dae_inpainting_conv_cifar")
    ...
    model = DAEInpaintingConv(in_channels=3, width=args.width, depth=args.depth).to(device)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            train_loss = train_one_epoch(model, dl_train, opt, device, args.mask_size)
            val_loss = eval_epoch(model, dl_val, device, args.mask_size)
