...
from utils.adp_plot import (  # type: ignore
    plot_loss_vs_epoch,
    plot_loss_vs_neurons,
)

from .dae_metric_conv_sup_stl import (
...
def supervised_contrastive_loss(
    ...
    """
    Supervised contrastive loss (Khosla et al.) on a single batch.
    """
    if z.ndim != 2:
        z = z.view(z.size(0), -1)
...
def train_with_early_stopping(
    ...
    for epoch in range(1, acfg.max_epochs + 1):
        ...
        for xb, yb in dl_train:
            ...
            loss_metric = supervised_contrastive_loss(z, yb, acfg.temperature)
            loss = acfg.lambda_recon * loss_recon + acfg.lambda_contrast * loss_metric
            loss.backward()
...
def main():
    ...
    parser.add_argument("--results-dir", type=str, default="results_dae_metric_conv_sup_adp")
    ...
    logger = ContinuousLogger(
        experiment_name="dae_metric_conv_sup",
        mode=args.adp_mode,
