...
from ..Models.dae_ladder_conv_sup_stl import SupDAELadderConv, sup_dae_ladder_total_neurons
...
def kl_div(mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
    # Standard KL for diagonal Gaussian vs N(0, I).
...
def train_one_epoch(
    ...
    for xb, yb in loader:
        ...
        # Underlying ladder VAE conv returns (x_rec, mu, logvar)
        x_rec, logits, mu = model(xb_noisy)
        # We need logvar from backbone; access via dae if present.
        ...
        loss = lambda_recon * loss_recon + loss_cls + lambda_kl * loss_kl
        loss.backward()
        opt.step()
...
def eval_accuracy(model: SupDAELadderConv, loader: DataLoader, device: torch.device, noise_std: float) -> float:
    model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for xb, yb in loader:
...
def main() -> None:
    p = argparse.ArgumentParser(description="Supervised ladder conv DAE encoder + classifier")
    ...
    p.add_argument("--out_dir", type=str, default="runs/dae_ladder_conv_sup_cifar")
    ...
    best_epoch = -1
    epochs_no_improve = 0
    neurons = sup_dae_ladder_total_neurons(args.width, args.depth, args.latent_dim, num_classes)
