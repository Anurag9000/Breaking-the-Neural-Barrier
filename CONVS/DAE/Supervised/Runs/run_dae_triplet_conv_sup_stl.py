...
from CONVS.DAE.Supervised.Models.dae_metric_conv_sup_stl import (
...
def _make_triplets(z: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    n_vec = z[torch.tensor(negatives, device=device)]
    return a, p, n_vec
...
def train_one_epoch(
    ...
    for xb, yb in loader:
        ...
        if a.numel() > 0:
            ...
            loss = lambda_recon * loss_recon + lambda_triplet * loss_trip
        else:
...
def main() -> None:
    ...
    p.add_argument("--out-dir", type=str, default="runs/dae_triplet_conv_sup_stl")
    ...
    # Evaluate reconstruction + embedding stability on test (same scalar loss)
    if ckpt_path.exists():
