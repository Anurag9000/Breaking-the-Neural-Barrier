...
# Load baseline
BASE_PATH = Path(__file__).with_name("vae_vq.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
ModelClass = baseline_module.VectorQuantizer

# ADP REVIEW (BEFORE REFACTOR)
# - This file is newly created to implement the ADP algorithms from scratch for the VectorQuantizer model.
...
def rebuild_model(model: ModelClass, width: int, depth: int, device, cfg: ADPConfig) -> ModelClass:
    ...
    try:
        kwargs = {}
        kwargs['n_e'] = get_attr(model, ['n_e'], None)
        kwargs['e_dim'] = get_attr(model, ['e_dim'], 1)
        kwargs['beta'] = get_attr(model, ['beta'], None)
