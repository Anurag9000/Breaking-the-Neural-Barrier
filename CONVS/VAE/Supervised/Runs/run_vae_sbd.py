...
from vae_sbd import SBDVAE, SBDConfig
...
def main(args):
    ...
    cfg = SBDConfig(width=args.width, latent_dim=args.latent)
    model = SBDVAE(cfg).to(device)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_sbd_cifar10.pth'))
        with open(os.path.join(args.out,'vae_sbd_metrics.json'),'w') as f:
