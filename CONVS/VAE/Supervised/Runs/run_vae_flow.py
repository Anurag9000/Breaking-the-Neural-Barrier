...
from vae_flow import FlowVAE, FlowVAEConfig
...
def evaluate(model, loader, device):
    model.eval(); tot=recon_t=kl_t=0.0
    for x,_ in loader:
        x=x.to(device); xh, mu, lv, z, ld = model(x)
        loss, r, kl = model.loss(x, xh, mu, lv, z, ld)
        bs=x.size(0); tot+=loss.item()*bs; recon_t+=r.item()*bs; kl_t+=kl.item()*bs
    n=len(loader.dataset); return tot/n, recon_t/n, kl_t/n


def main(args):
    device = torch.device('cuda' if torch.cuda.is_available() and not args.cpu else 'cpu')
    tr, va = loaders(args.data, args.batch_size, args.workers)
    cfg = FlowVAEConfig(width=args.width, latent_dim=args.latent, n_flows=args.n_flows)
    model = FlowVAE(cfg).to(device)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x,_ in tr:
            x=x.to(device); xh, mu, lv, z, ld = model(x)
            loss, *_ = model.loss(x, xh, mu, lv, z, ld)
            opt.zero_grad(set_to_none=True); loss.backward(); nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step()
            run += loss.item()*x.size(0)
        run/=len(tr.dataset)
        vloss, vrecon, vkl = evaluate(model, va, device)
        if vloss+1e-12<best:
            best=vloss; best_state={'model':model.state_dict(),'cfg':cfg.__dict__,'epoch':ep}; bad=0
        else:
            bad+=1
        if args.verbose:
            print(f"ep {ep:03d} train {run:.4f} | val {vloss:.4f} (recon {vrecon:.4f} kl {vkl:.4f})")
        if bad>=args.patience:
            if args.verbose: print('early stop'); break

    if best_state is not None:
        os.makedirs(args.out, exist_ok=True)
        torch.save(best_state, os.path.join(args.out, 'vae_flow_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_flow_metrics.json'), 'w') as f:
...
if __name__=='__main__':
    ...
    ap.add_argument('--n-flows', type=int, default=4)
