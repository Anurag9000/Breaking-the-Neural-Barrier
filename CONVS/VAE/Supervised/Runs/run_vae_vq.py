...
from vae_vq import VQVAE, VQVAEConfig
...
def evaluate(model, loader, device):
    model.eval(); tot=recon_t=vq_t=0.0
    for x,_ in loader:
        x=x.to(device)
        xh, vq_loss = model(x)
        loss, r, vq = model.loss(x, xh, vq_loss)
        bs=x.size(0); tot+=loss.item()*bs; recon_t+=r.item()*bs; vq_t+=vq.item()*bs
    n=len(loader.dataset); return tot/n, recon_t/n, vq_t/n


def main(args):
    device = torch.device('cuda' if torch.cuda.is_available() and not args.cpu else 'cpu')
    tr, va = loaders(args.data, args.batch_size, args.workers)
    cfg = VQVAEConfig(width=args.width, embed_dim=args.embed_dim, n_embed=args.n_embed)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x,_ in tr:
            x=x.to(device)
            xh, vq_loss = model(x)
            loss, *_ = model.loss(x, xh, vq_loss)
            opt.zero_grad(set_to_none=True); loss.backward(); torch.nn.utils.clip_grad_norm_(model.parameters(),1.0); opt.step()
            run+=loss.item()*x.size(0)
        run/=len(tr.dataset)
        vloss, vrecon, vvq = evaluate(model, va, device)
        if vloss+1e-12<best:
            best=vloss; best_state={'model':model.state_dict(),'cfg':cfg.__dict__,'epoch':ep}; bad=0
        else: bad+=1
        if args.verbose:
            print(f"ep {ep:03d} train {run:.4f} | val {vloss:.4f} (recon {vrecon:.4f} vq {vvq:.4f})")
        if bad>=args.patience:
            if args.verbose: print('early stop'); break

    if best_state:
        os.makedirs(args.out, exist_ok=True)
        torch.save(best_state, os.path.join(args.out,'vae_vq_cifar10.pth'))
        with open(os.path.join(args.out,'vae_vq_metrics.json'),'w') as f:
...
if __name__=='__main__':
    ...
    ap.add_argument('--n-embed',type=int,default=512)
