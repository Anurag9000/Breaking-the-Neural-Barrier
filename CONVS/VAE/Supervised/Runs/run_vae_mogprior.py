...
from vae_mogprior import MoGPriorVAE, MoGPriorConfig
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        ...
        # reuse internal loss computation
        loss, r, kl = model.loss(x, xh, mu, lv, z)
...
def main(args):
    ...
    cfg = MoGPriorConfig(width=args.width, latent_dim=args.latent, K=args.K)
    model = MoGPriorVAE(cfg).to(device)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x,_ in tr:
            x=x.to(device); xh, mu, lv, z = model(x)
            loss, *_ = model.loss(x, xh, mu, lv, z)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_mogprior_cifar10.pth'))
        with open(os.path.join(args.out,'vae_mogprior_metrics.json'),'w') as f:
