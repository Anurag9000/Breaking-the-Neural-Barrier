...
from vae_iaf import IAFVAE, IAFVAEConfig
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        x=x.to(device); xh, mu, lv, zK, logdet = model(x)
        loss, r, kl = model.loss(x, xh, mu, lv, zK, logdet)
...
def main(args):
    ...
    cfg = IAFVAEConfig(width=args.width, latent_dim=args.latent, n_flows=args.n_flows)
    model = IAFVAE(cfg).to(device)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x,_ in tr:
            x=x.to(device); xh, mu, lv, zK, logdet = model(x)
            loss, *_ = model.loss(x, xh, mu, lv, zK, logdet)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_iaf_cifar10.pth'))
        with open(os.path.join(args.out,'vae_iaf_metrics.json'),'w') as f:
...
if __name__=='__main__':
    ...
    ap.add_argument('--n-flows',type=int,default=2)
