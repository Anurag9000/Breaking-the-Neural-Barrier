...
from nvae import NVAE, NVAEConfig
...
def main(args):
    ...
    cfg = NVAEConfig(width=args.width, z_groups=3, z_per_group=args.z)
    model = NVAE(cfg).to(device)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'nvae_cifar10.pth'))
        with open(os.path.join(args.out,'nvae_metrics.json'),'w') as f:
