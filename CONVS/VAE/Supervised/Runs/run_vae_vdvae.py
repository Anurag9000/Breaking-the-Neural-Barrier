...
from vae_vdvae import VDVAE, VDVAEConfig
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        x=x.to(device); xh, kl = model(x)
        loss, r, kl = model.loss(x, xh, kl)
...
def main(args):
    ...
    cfg = VDVAEConfig(width=args.width, groups=args.groups, z_dim=args.z)
    model = VDVAE(cfg).to(device)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_vdvae_cifar10.pth'))
        with open(os.path.join(args.out,'vae_vdvae_metrics.json'),'w') as f:
