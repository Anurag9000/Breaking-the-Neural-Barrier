...
from vae_ladder import LVAE, LVAEConfig
...
def main(args):
    ...
    cfg = LVAEConfig(width=args.width, z1_dim=args.z1, z2_dim=args.z2)
    model = LVAE(cfg).to(device)
    ...
    if best_state is not None:
        ...
        torch.save(best_state, os.path.join(args.out, 'vae_ladder_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_ladder_metrics.json'), 'w') as f:
