...
from vae_vamp import VampPriorVAE, VampConfig
...
def main(args):
    ...
    cfg = VampConfig(width=args.width, latent_dim=args.latent, K=args.K)
    model = VampPriorVAE(cfg).to(device)
    ...
    if best_state is not None:
        ...
        torch.save(best_state, os.path.join(args.out, 'vae_vamp_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_vamp_metrics.json'), 'w') as f:
