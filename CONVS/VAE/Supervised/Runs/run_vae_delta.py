...
from vae_delta import DeltaVAE, DeltaVAEConfig
...
def main(args):
    ...
    cfg = DeltaVAEConfig(width=args.width, latent_dim=args.latent, delta=args.delta)
    model = DeltaVAE(cfg).to(device)
    ...
    if best_state:
        ...
        torch.save(best_state, os.path.join(args.out,'vae_delta_cifar10.pth'))
        with open(os.path.join(args.out,'vae_delta_metrics.json'),'w') as f:
