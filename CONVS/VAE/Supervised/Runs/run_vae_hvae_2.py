...
from vae_hvae2 import HVAE2, HVAE2Config
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        x=x.to(device); xh, stats = model(x)
        loss, r, kl = model.loss(x, xh, stats)
...
def main(args):
    ...
    cfg = HVAE2Config(width=args.width, z1_dim=args.z1, z2_dim=args.z2)
    model = HVAE2(cfg).to(device)
    ...
    if best_state is not None:
        ...
        torch.save(best_state, os.path.join(args.out, 'vae_hvae2_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_hvae2_metrics.json'), 'w') as f:
...
if __name__=='__main__':
    ...
    ap.add_argument('--z1', type=int, default=32)
    ap.add_argument('--z2', type=int, default=32)
