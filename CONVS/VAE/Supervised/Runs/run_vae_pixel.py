...
from vae_pixel import PixelVAE, PixelVAEConfig
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        x=x.to(device); xh, mu, lv = model(x)
        loss, r, kl = model.loss(x, xh, mu, lv)
...
def main(args):
    ...
    cfg = PixelVAEConfig(width=args.width, latent_dim=args.latent)
    model = PixelVAE(cfg).to(device)
    ...
    if best_state is not None:
        ...
        torch.save(best_state, os.path.join(args.out, 'vae_pixel_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_pixel_metrics.json'), 'w') as f:
