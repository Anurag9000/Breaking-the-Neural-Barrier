...
from vae_beta import BetaVAE, BetaVAEConfig
...
def get_loaders(root, bs, workers):
    ...
    full_tr = datasets.CIFAR10(root=root, train=True, download=True, transform=tf_tr)
    n_tr = int(0.9*len(full_tr)); n_val = len(full_tr)-n_tr
    tr_idx = torch.Generator().manual_seed(42)
    tr_set, _ = random_split(full_tr, [n_tr, n_val], generator=tr_idx)
...
def evaluate(model, loader, device):
    ...
    for x, _ in loader:
        ...
        bs = x.size(0)
        total += loss.item()*bs; recon_t += recon.item()*bs; kl_t += kl.item()*bs
...
def main(args):
    ...
    cfg = BetaVAEConfig(width=args.width, latent_dim=args.latent, beta=args.beta)
    ...
    for ep in range(1, args.epochs+1):
        ...
        for x, _ in tr:
            x = x.to(device)
            x_hat, mu, logvar = model(x)
            loss, _, _ = model.loss(x, x_hat, mu, logvar)
        ...
        if vloss + 1e-12 < best:
            ...
            bad += 1
        if args.verbose:
            print(f"ep {ep:03d} train {running:.4f} | val {vloss:.4f} (recon {vrecon:.4f} kl {vkl:.4f})")
    ...
    if best_state is not None:
        ...
        torch.save(best_state, os.path.join(args.out, 'vae_beta_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_beta_metrics.json'), 'w') as f:
