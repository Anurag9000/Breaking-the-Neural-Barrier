...
from vae_wae_mmd import WAE_MMD, WAEConfig
...
def evaluate(model, loader, device):
    ...
    for x,_ in loader:
        x=x.to(device); xh, z = model(x)
        loss, r, p = model.loss(x, xh, z)
        bs=x.size(0); tot+=loss.item()*bs; recon_t+=r.item()*bs; mmd_t+=p.item()*bs
    n=len(loader.dataset); return tot/n, recon_t/n, mmd_t/n


def main(args):
    device = torch.device('cuda' if torch.cuda.is_available() and not args.cpu else 'cpu')
    tr, va = loaders(args.data, args.batch_size, args.workers)
    cfg = WAEConfig(width=args.width, latent_dim=args.latent, mmd_weight=args.mmd_w, sigma=args.sigma)
    model = WAE_MMD(cfg).to(device)
    ...
    if best_state is not None:
        ...
        torch.save(best_state, os.path.join(args.out, 'vae_wae_mmd_cifar10.pth'))
        with open(os.path.join(args.out, 'vae_wae_mmd_metrics.json'), 'w') as f:
