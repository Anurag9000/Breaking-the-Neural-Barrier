...
# Rényi-α VAE: Monte Carlo bound
# L_α = 1/(1-α) * log E_q [ (p(x,z)/q(z|x))^{1-α} ] , α in (0,1) U (1,∞)
...
class RenyiVAE(nn.Module):
    def __init__(self, in_ch=1, out_ch=1, z_dim=32, alpha=0.5, K=5, recon='bce'):
        super().__init__()
        self.enc=Encoder(in_ch,z_dim)
        self.dec=Decoder(out_ch,z_dim)
        self.alpha=alpha; self.K=K; self.recon=recon
    def forward(self,x):
        B=x.size(0); K=self.K; a=self.alpha
        mu,lv=self.enc(x)
        std=(0.5*lv).exp()
        ...
        if self.recon=='bce':
            log_px_z = -F.binary_cross_entropy_with_logits(x_logits, x.unsqueeze(0).expand_as(x_logits), reduction='none').flatten(2).sum(-1)
        else:
        ...
        t = (1 - a)
        # log E[(w)^t] via logmeanexp
        logmean = torch.logsumexp(t*log_w, dim=0) - torch.log(torch.tensor(float(K), device=x.device))
        L = (1.0/t) * logmean
        return -(L.mean())
