...
from vae_mog_prior_model import MoGPriorVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/mogprior')
    args = ap.parse_args()
    ...
    model = MoGPriorVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, K=args.K, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO MoG prior): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_mogprior.pt'))
