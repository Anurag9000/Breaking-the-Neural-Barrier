...
from vae_denoising_model import DenoisingVAE
...
def corrupt(x, sigma=0.3, drop=0.1):
    noise = sigma*torch.randn_like(x)
    mask = (torch.rand_like(x) > drop).float()
    x_n = (x + noise).clamp(0,1)
    x_n = x_n*mask
    return x_n
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/denoising')
    args = ap.parse_args()
    ...
    model = DenoisingVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO DenoisingVAE): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_denoising.pt'))
