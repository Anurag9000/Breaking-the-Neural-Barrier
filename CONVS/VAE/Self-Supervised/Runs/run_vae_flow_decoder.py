...
from vae_flow_decoder_model import FlowDecoderVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/flowdec')
    args = ap.parse_args()
    ...
    model = FlowDecoderVAE(in_ch=1, z_dim=args.z_dim).to(device)
    ...
    print(f'TEST (ELBO flow-dec): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_flowdec.pt'))
