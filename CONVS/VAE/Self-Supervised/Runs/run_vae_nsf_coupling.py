...
from vae_nsf_coupling_model import VAE_NSF_Coupling
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/nsf_coupling')
    args = ap.parse_args()
    ...
    model = VAE_NSF_Coupling(in_ch=1, out_ch=1, z_dim=args.z_dim, n_flows=args.n_flows, K=args.K, B=args.B, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO NSF-Coupling): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_nsf_coupling.pt'))
