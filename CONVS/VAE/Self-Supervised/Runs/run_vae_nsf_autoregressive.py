...
from vae_nsf_autoregressive_model import VAE_NSF_AR
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/nsf_ar')
    args = ap.parse_args()
    ...
    model = VAE_NSF_AR(in_ch=1, out_ch=1, z_dim=args.z_dim, n_flows=args.n_flows, K=args.K, B=args.B, recon=args.recon).to(device)
    ...
    print(f'TEST (ELBO NSF-AR): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_nsf_ar.pt'))
