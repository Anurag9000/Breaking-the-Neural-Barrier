...
from vae_betatc_model import BetaTCVAE
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/betatc')
    args = ap.parse_args()
    ...
    model = BetaTCVAE(in_ch=1, out_ch=1, z_dim=args.z_dim, beta=args.beta, recon=args.recon).to(device)
    ...
    print(f'TEST (β-TC loss): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'vae_betatc.pt'))
