...
from pixelvae_plus_model import PixelVAEPlus
...
def main():
    ...
    ap.add_argument('--outdir', type=str, default='results_vae/pixel_plus')
    args = ap.parse_args()
    ...
    model = PixelVAEPlus(in_ch=1, out_ch=1, z_dim=args.z_dim, recon=args.recon, depth=args.depth).to(device)
    ...
    print(f'TEST (ELBO PixelVAE+): {te_loss:.4f}')

    torch.save(model.state_dict(), os.path.join(args.outdir, 'pixelvae_plus.pt'))
