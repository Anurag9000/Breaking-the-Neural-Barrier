...
from lstm_temporal_jigsaw import (
    WhitespaceTokenizer,
    JigsawDataset,
    pad_collate,
    LSTMJigsaw,
...
if __name__ == "__main__":
    ...
    ap.add_argument("--max_len", type=int, default=180)
    ...
    train_ds = JigsawDataset(train_ids, max_len=args.max_len, seed=args.seed)
    val_ds = JigsawDataset(val_ids, max_len=args.max_len, seed=args.seed)
    test_ds = JigsawDataset(test_ids, max_len=args.max_len, seed=args.seed)
    ...
    model = LSTMJigsaw(vocab=len(tok.itos), emb_dim=cfg.emb_dim, hidden=cfg.hidden, layers=cfg.layers, dropout=cfg.dropout).to(device)
