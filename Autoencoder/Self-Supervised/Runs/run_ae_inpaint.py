...
from ae_inpaint import InpaintingConvAE, InpaintingAETrainer
...
def main():
    ...
    tc = TrainConfig(
        ...
        es_patience=30,
        max_epochs=300,
        results_dir="results_ae_inpaint",
        holes_per_image=1,
        min_hole_frac=0.15,
        max_hole_frac=0.35,
    )
    ...
    trainer = InpaintingAETrainer(
        ...
        holes_per_image=tc.holes_per_image,
        min_hole_frac=tc.min_hole_frac,
        max_hole_frac=tc.max_hole_frac,
    )

    best_val = trainer.fit(train_loader, val_loader)
    test_loss = trainer.evaluate(test_loader)

    ckpt = os.path.join(tc.results_dir, "InpaintAE_best.pth")
    trainer.save(ckpt)

    report = {
        "device": str(device),
        "best_val_hole_mse": float(best_val),
        "test_hole_mse": float(test_loss),
