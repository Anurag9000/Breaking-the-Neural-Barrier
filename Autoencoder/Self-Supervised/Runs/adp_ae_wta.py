...
# -----------------------------
# Winner-Take-All Convolutional Autoencoder (single-model)
# - Enforces *competitive sparsity* in the latent by keeping only the top-k spatial activations per channel
#   (default k=1 -> classic WTA: only the strongest spatial location in each feature map survives)
# - 0-based pooling indices during forward (mirrors ADP models)
# -----------------------------

class WTAConvAE(nn.Module):
    def __init__(
        self,
        in_ch: int = 3,
        widths: List[int] = [32, 64, 128],
        pooling_indices: List[int] = [0, 2],
        k_spatial: int = 1,  # keep top-k spatial positions per channel per sample
    ):
        super().__init__()
        assert len(widths) >= 1
        self.in_ch = in_ch
    ...
    def _wta_spatial(self, h: torch.Tensor) -> torch.Tensor:
        """Keep top-k spatial activations per (B,C). k=1 -> single winner per channel map.
        h: (B,C,H,W)
        """
        if self.k_spatial <= 0:
            return h
        B, C, H, W = h.shape
        k = min(self.k_spatial, H * W)
        flat = h.view(B, C, -1)
        # values, indices of top-k per (B,C)
        vals, idx = torch.topk(flat, k, dim=2)
        mask = torch.zeros_like(flat)
        # scatter ones to top-k indices
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h_wta = self._wta_spatial(h)
        out = self.decode(h_wta, down_ct)
        return out

# -----------------------------
# Trainer: reconstruction loss + hard WTA in forward (no extra penalty)
# -----------------------------
...
class WTAAETrainer:
    def __init__(
        ...
        es_patience: int = 30,
        max_epochs: int = 300,
        results_dir: str = "results_adp_ae_wta",
        loss_type: str = "mse",
    ):
    ...
    def _maybe_plot(self):
        ...
        plt.title(f"WTAAE Val | neurons={self.model.total_neurons()} depth={self.model.depth()} widths={self.model.widths_list()} k={self.model.k_spatial}")
        plt.grid(True, which="both", ls=":")
        out = os.path.join(self.results_dir, "WTAAE_neuron_loss_plot.png")
        plt.tight_layout()
        fig.savefig(out)
        plt.close(fig)
    ...
    def save(self, path: str):
        torch.save({
            ...
            "k_spatial": int(self.model.k_spatial),
