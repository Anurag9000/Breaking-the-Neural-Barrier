# run_adp_ae_spatial_core.py
# Core runner for spatial/geometric SSL AEs (category D).
# Works with CIFAR10/STL10/ImageFolder. Saves best ckpt & previews.
...
from adp_ae_spatial_core import SAEConfig, build_model, SUPPORTED_SPATIAL
...
def main(default_algo=None):
    ap = argparse.ArgumentParser("Spatial AE Runner (core)")
    # data
    ...
    # algo
    ap.add_argument("--algo", type=str, default=default_algo, choices=sorted(list(SUPPORTED_SPATIAL)))
    # train
    ...
    ap.add_argument("--out-dir", type=str, default="./results_spatial")
    args = ap.parse_args()

    assert args.algo in SUPPORTED_SPATIAL
    set_seed(args.seed)
    ...
    cfg = SAEConfig(in_channels=3, base_channels=args.base_ch, depth=args.depth,
                    latent_dim=args.latent_dim, patch_grid=args.patch_grid, jigsaw_classes=args.jigsaw_classes,
                    scale_bins=args.scale_bins, trans_max_px=args.trans_max_px,
    ...
    for ep in range(1, args.epochs+1):
        ...
        if va_m["loss"] < best - 1e-6:
            best = va_m["loss"]
            p = os.path.join(ck, f"SPAT_AE_{args.algo}_{args.dataset}_best.pth")
