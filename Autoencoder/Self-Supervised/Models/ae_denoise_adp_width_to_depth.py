...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_denoise.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
DenoisingConvAE = baseline_module.DenoisingConvAE  # type: ignore
...
# - Inner training: train_with_patience merges ES and acceptance using delta; no separate patience_es; noise injection but best tracking uses delta.
# - Width expansion: widen_all mutates in place; trials_width used as counter; no snapshot/restore abstraction; delta shared for both dimensions.
# - Depth expansion: append_depth mutates encoder; rollback via state only; no architecture snapshot; trials_depth counter only.
# - 2D/ALT: width_to_depth/depth_to_width/alt_* just toggle modes on no improvement; missing structured outer/inner or phase saturation from spec.
...
def _build_model(in_ch: int, widths: List[int], pooling_indices: List[int], device) -> DenoisingConvAE:
    return DenoisingConvAE(in_ch=in_ch, widths=widths, pooling_indices=pooling_indices).to(device)


def rebuild_model(model: DenoisingConvAE, widths: List[int], device) -> DenoisingConvAE:
...
def snapshot_arch_and_state(model: DenoisingConvAE):
    return {
...
def restore_arch_and_state(model: DenoisingConvAE, snapshot, device) -> DenoisingConvAE:
...
def total_neurons(model: DenoisingConvAE) -> int:
    return sum(b.conv.out_channels for b in model.encoder) + sum(b.conv.out_channels for b in model.decoder)


def expand_width(model: DenoisingConvAE, ex_k_width: int, max_width: int, device) -> DenoisingConvAE:
    new_widths = [min(max_width, w + ex_k_width) for w in model.widths]
    return rebuild_model(model, new_widths, device)


def expand_depth(model: DenoisingConvAE, ex_k_depth: int, device) -> DenoisingConvAE:
    if ex_k_depth <= 0:
        return model
...
def train_with_early_stopping(model: DenoisingConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list) -> Tuple[float, dict, None]:
...
def adp_search(model: DenoisingConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def can_widen(widths: List[int], base_model: DenoisingConvAE) -> bool:
    ...
    def can_deepen(widths: List[int], base_model: DenoisingConvAE) -> bool:
    ...
    def width_search(local_model: DenoisingConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: DenoisingConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="ADP Denoising Conv AE width/depth search")
    ...
    p.add_argument("--results-dir", type=Path, default=Path("results_adp_denoise"))
    ...
    model = DenoisingConvAE(in_ch=3, widths=args.widths, pooling_indices=args.pool_idx).to(device)
    ...
    print(f"[ADP Denoise AE] mode={args.adp_mode} best_val={best_val:.6f} widths={widths} depth={len(widths)}")
