...
def rebuild_decoder(model: DenoisingConvAE, widths: List[int]):
...
def widen_all(model: DenoisingConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: DenoisingConvAE):
...
def train_with_patience(model: DenoisingConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list):
...
def main():
    ...
    print(f"[ADP Denoise AE] mode={args.adp_mode} best_val={best:.6f} widths={model.widths} depth={len(model.widths)}")
