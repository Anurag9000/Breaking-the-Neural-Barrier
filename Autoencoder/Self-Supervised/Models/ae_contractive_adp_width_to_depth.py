...
def rebuild_decoder(model: ContractiveConvAE, widths: List[int]):
...
def widen_all(model: ContractiveConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: ContractiveConvAE):
