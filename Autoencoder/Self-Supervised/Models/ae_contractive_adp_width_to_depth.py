...
ContractiveConvAE = baseline_module.ContractiveConvAE  # type: ignore
...
# - Inner training: train_with_patience merges ES and acceptance using delta; no separate patience_es; penalty handled but best tracking uses delta.
...
def _build_model(in_ch: int, widths: List[int], pooling_indices: List[int], device) -> ContractiveConvAE:
    return ContractiveConvAE(in_ch=in_ch, widths=widths, pooling_indices=pooling_indices).to(device)


def rebuild_model(model: ContractiveConvAE, widths: List[int], device) -> ContractiveConvAE:
...
def snapshot_arch_and_state(model: ContractiveConvAE):
    return {
...
def restore_arch_and_state(model: ContractiveConvAE, snapshot, device) -> ContractiveConvAE:
...
def total_neurons(model: ContractiveConvAE) -> int:
    return sum(b.conv.out_channels for b in model.encoder) + sum(b.conv.out_channels for b in model.decoder)


def expand_width(model: ContractiveConvAE, ex_k_width: int, max_width: int, device) -> ContractiveConvAE:
    new_widths = [min(max_width, w + ex_k_width) for w in model.widths]
    return rebuild_model(model, new_widths, device)


def expand_depth(model: ContractiveConvAE, ex_k_depth: int, device) -> ContractiveConvAE:
    if ex_k_depth <= 0:
        return model
...
def contractive_penalty(model: ContractiveConvAE, x: torch.Tensor) -> torch.Tensor:
    x_req = x.detach().clone().requires_grad_(True)
    h = model.encode(x_req)
    s = h.sum()
    grads = torch.autograd.grad(s, x_req, create_graph=True)[0]
    penalty = (grads ** 2).sum(dim=(1, 2, 3)).mean()
    return penalty


def train_with_early_stopping(model: ContractiveConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list) -> Tuple[float, dict, None]:
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            ...
            pen = contractive_penalty(model, x)
            loss = recon + acfg.lam_contractive * pen
            loss.backward()
...
def adp_search(model: ContractiveConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def can_widen(widths: List[int], base_model: ContractiveConvAE) -> bool:
    ...
    def can_deepen(widths: List[int], base_model: ContractiveConvAE) -> bool:
    ...
    def width_search(local_model: ContractiveConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: ContractiveConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="ADP Contractive Conv AE width/depth search")
    ...
    model = ContractiveConvAE(in_ch=3, widths=args.widths, pooling_indices=args.pool_idx).to(device)
    ...
    print(f"[ADP Contractive AE] mode={args.adp_mode} best_val={best_val:.6f} widths={widths} depth={len(widths)}")
