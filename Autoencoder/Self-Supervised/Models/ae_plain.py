...
# -----------------------------
# Plain / Undercomplete Convolutional Autoencoder (single-model)
# - Deterministic encoder-decoder bottleneck, no noise/penalties
# - 0-based pooling indices during forward (mirrors  models)
# -----------------------------

class PlainConvAE(nn.Module):
    def __init__(
        self,
        in_ch: int = 3,
...
class PlainAETrainer:
    def __init__(
        ...
        es_patience: int = 30,
        max_epochs: int = 300,
        results_dir: str = "results_ae_plain",
    ...
    def _maybe_plot(self):
        ...
        plt.title(f"PlainAE Val | neurons={self.model.total_neurons()} depth={self.model.depth()} widths={self.model.widths_list()} loss={self.loss_type}")
        plt.grid(True, which="both", ls=":")
        out = os.path.join(self.results_dir, "PlainAE_neuron_loss_plot.png")
        plt.tight_layout()
        fig.savefig(out)
        plt.close(fig)
