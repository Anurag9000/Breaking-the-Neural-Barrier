...
def adp_search(model: FeatureConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def can_widen(widths: List[int], base_model: FeatureConvAE) -> bool:
    ...
    def can_deepen(widths: List[int], base_model: FeatureConvAE) -> bool:
    ...
    def width_search(local_model: FeatureConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: FeatureConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
