...
# -----------------------------
# Robust (Huber) Convolutional Autoencoder (single-model)
# - Same architecture as Plain AE; training uses SmoothL1 (Huber) loss to reduce sensitivity to outliers
# - 0-based pooling indices during forward (mirrors  models)
# -----------------------------

class RobustConvAE(nn.Module):
    def __init__(
        self,
        in_ch: int = 3,
...
# -----------------------------
# Trainer: Huber (SmoothL1) reconstruction, early stopping, plotting
# -----------------------------
...
class RobustAETrainer:
    def __init__(
        ...
        huber_beta: float = 0.1,   # delta parameter ("beta" arg in SmoothL1Loss)
        reduction: str = "mean",
    ):
        ...
        self.crit = nn.SmoothL1Loss(beta=self.huber_beta, reduction=reduction)
    ...
    def _maybe_plot(self):
        ...
        plt.xlabel("log step")
        plt.ylabel("Val Huber (log)")
        plt.title(f"RobustAE Val | neurons={self.model.total_neurons()} depth={self.model.depth()} widths={self.model.widths_list()} beta={self.huber_beta}")
        plt.grid(True, which="both", ls=":")
        out = os.path.join(self.results_dir, "RobustAE_neuron_loss_plot.png")
        plt.tight_layout()
        fig.savefig(out)
        plt.close(fig)
    ...
    def load(self, path: str, map_location=None):
        ...
        self.huber_beta = float(blob.get("huber_beta", self.huber_beta))
        self.reduction = blob.get("reduction", self.reduction)
        self.crit = nn.SmoothL1Loss(beta=self.huber_beta, reduction=self.reduction)
