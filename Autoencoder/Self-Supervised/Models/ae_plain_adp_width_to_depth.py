...
def rebuild_decoder(model: PlainConvAE, widths: List[int]):
...
def widen_all(model: PlainConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: PlainConvAE):
...
def train_with_patience(model: PlainConvAE, dl_train, dl_val, acfg: ADPConfig, device):
...
def adp_search(model: PlainConvAE, dl_train, dl_val, acfg: ADPConfig, device):
    def can_widen():
    ...
    while improved:
        ...
        if mode == "width_to_depth" and not improved:
            mode = "depth"
            pd = acfg.trrials_depth if hasattr(acfg, "trrials_depth") else acfg.trials_depth
            improved = True
...
def main():
    ...
    print(f"[ADP Plain AE] mode={args.adp_mode} best_val={best:.6f} widths={model.widths} depth={len(model.widths)}")
