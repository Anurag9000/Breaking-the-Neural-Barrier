...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_sparse.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
SparseConvAE = baseline_module.SparseConvAE  # type: ignore
...
def rebuild_decoder(model: SparseConvAE, widths: List[int]):
...
def widen_all(model: SparseConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: SparseConvAE):
...
def restore_arch_and_state(model: SparseConvAE, snapshot, device) -> SparseConvAE:
    restored = SparseConvAE(in_ch=model.in_ch, widths=snapshot["widths"], pooling_indices=snapshot["pooling_indices"]).to(device)
...
def sparse_loss(model: SparseConvAE, x: torch.Tensor, crit, lam_l1: float):
    h, down_ct = model.encode(x)
    h_s = model.apply_sparsity(h)
    recon = model.decode(h_s, down_ct)
    loss_recon = crit(recon, x)
    if model.mode == "l1":
        loss_sparse = lam_l1 * h.abs().mean()
    else:
        loss_sparse = torch.tensor(0.0, device=x.device)
    return loss_recon + loss_sparse, loss_recon.detach(), loss_sparse.detach()


def train_with_patience(model: SparseConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            x = x.to(device)
            loss, _, _ = sparse_loss(model, x, crit, acfg.lam_l1)
        ...
        with torch.no_grad():
            ...
            for x, _ in dl_val:
                x = x.to(device)
                l, _, _ = sparse_loss(model, x, crit, acfg.lam_l1)
                val += l.item() * x.size(0)
                n += x.size(0)
            val = val / max(n, 1)
        history.append(val)
...
def adp_search(model: SparseConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def width_search(local_model: SparseConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: SparseConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
...
def main():
    ...
    model = SparseConvAE(in_ch=3, widths=args.widths, pooling_indices=args.pool_idx).to(device)
    acfg = ADPConfig(
    ...
    print(f"[ADP Sparse AE] mode={args.adp_mode} best_val={best_val:.6f} widths={widths} depth={len(widths)}")
