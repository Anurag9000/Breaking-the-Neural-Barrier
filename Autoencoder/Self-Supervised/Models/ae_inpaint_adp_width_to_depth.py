...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_inpaint.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
InpaintingConvAE = baseline_module.InpaintingConvAE  # type: ignore
...
# - Inner training: train_with_patience uses delta for ES; no separate patience_es; inpainting mask loss handled but best tracking uses delta.
...
def _build_model(in_ch: int, widths: List[int], pooling_indices: List[int], device) -> InpaintingConvAE:
    return InpaintingConvAE(in_ch=in_ch, widths=widths, pooling_indices=pooling_indices).to(device)


def rebuild_model(model: InpaintingConvAE, widths: List[int], device) -> InpaintingConvAE:
...
def snapshot_arch_and_state(model: InpaintingConvAE):
    return {
...
def restore_arch_and_state(model: InpaintingConvAE, snapshot, device) -> InpaintingConvAE:
...
def total_neurons(model: InpaintingConvAE) -> int:
    return sum(b.conv.out_channels for b in model.encoder) + sum(b.conv.out_channels for b in model.decoder)


def expand_width(model: InpaintingConvAE, ex_k_width: int, max_width: int, device) -> InpaintingConvAE:
    new_widths = [min(max_width, w + ex_k_width) for w in model.widths]
    return rebuild_model(model, new_widths, device)


def expand_depth(model: InpaintingConvAE, ex_k_depth: int, device) -> InpaintingConvAE:
    if ex_k_depth <= 0:
        return model
...
def rect_mask(x: torch.Tensor, holes: int, min_frac: float, max_frac: float):
    B, C, H, W = x.shape
    mask = torch.zeros(B, 1, H, W, device=x.device)
    area = H * W
    for b in range(B):
        for _ in range(holes):
            target = torch.empty(1).uniform_(min_frac, max_frac).item() * area
            aspect = torch.empty(1).uniform_(0.5, 2.0).item()
            h = int(round((target * aspect) ** 0.5))
            w = int(round((target / aspect) ** 0.5))
            h = max(1, min(H, h)); w = max(1, min(W, w))
...
def train_with_patience(model: InpaintingConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            x = x.to(device)
            mask = rect_mask(x, acfg.holes_per_image, acfg.min_hole_frac, acfg.max_hole_frac)
            x_in = x * (1 - mask)
...
def adp_search(model: InpaintingConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def can_widen(widths: List[int], base_model: InpaintingConvAE) -> bool:
    ...
    def can_deepen(widths: List[int], base_model: InpaintingConvAE) -> bool:
    ...
    def width_search(local_model: InpaintingConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: InpaintingConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
    ...
    if acfg.adp_mode in ("width_only","width"):
    ...
    elif acfg.adp_mode in ("depth_only","depth"):
    ...
    elif acfg.adp_mode == "depth_to_width":  # ADP_DEPTH_OUTER_WIDTH_INNER
        model, best_val, best_state, best_widths = width_search(model, initial_val=best_val, initial_state=best_state, log_improvement=True)
        depth_failure_count = 0
        while depth_failure_count < patience_depth_exp and can_deepen(best_widths, model):
            ...
            if cand_val < best_val - delta_depth:
                ...
                depth_failure_count += 1
    elif acfg.adp_mode == "width_to_depth":  # ADP_WIDTH_OUTER_DEPTH_INNER
        model, best_val, best_state, best_widths = depth_search(model, initial_val=best_val, initial_state=best_state, log_improvement=True)
        width_failure_count = 0
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="ADP Inpainting AE width/depth search")
    ...
    p.add_argument("--results-dir", type=Path, default=Path("results_adp_inpaint"))
    ...
    model = InpaintingConvAE(in_ch=3, widths=args.widths, pooling_indices=args.pool_idx).to(device)
