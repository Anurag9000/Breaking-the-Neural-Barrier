...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_tied.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
TiedConvAE = baseline_module.TiedConvAE  # type: ignore
...
# - Expansions: widen_all/append_depth mutate in place; rollback on failure; delta shared for width/depth.
...
def rebuild_decoder(model: TiedConvAE, widths: List[int]):
    # decoder uses BN/act only; weights are tied from encoder
    rev = list(reversed(widths))
    dec_bn = []
    dec_act = []
    ch = rev[0]
    old_bn = list(model.dec_bn)
    old_act = list(model.dec_act)
    for w in rev[1:]:
        bn = nn.BatchNorm2d(w) if model.use_decoder_bn else nn.Identity()
        if old_bn:
            old_b = old_bn.pop(0)
            if isinstance(bn, nn.BatchNorm2d) and isinstance(old_b, nn.BatchNorm2d):
                bn = _resize_bn(bn, bn.num_features)
                with torch.no_grad():
                    r = min(bn.weight.numel(), old_b.weight.numel())
                    bn.weight[:r] = old_b.weight[:r]
                    bn.bias[:r] = old_b.bias[:r]
                    bn.running_mean[:r] = old_b.running_mean[:r]
                    bn.running_var[:r] = old_b.running_var[:r]
        dec_bn.append(bn)
        dec_act.append(nn.ReLU(inplace=True))
        ch = w
    model.dec_bn = nn.ModuleList(dec_bn)
    model.dec_act = nn.ModuleList(dec_act)
    model.head = nn.Conv2d(ch, model.in_ch, kernel_size=1, stride=1, padding=0)


def widen_all(model: TiedConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
    for blk in model.encoder:
        out = min(max_width, blk.conv.out_channels + ex_k)
        blk.conv = _resize_conv(blk.conv, prev, out)
        blk.bn = _resize_bn(blk.bn, out)
...
def append_depth(model: TiedConvAE):
...
def total_neurons(model: TiedConvAE) -> int:
    return sum(b.conv.out_channels for b in model.encoder)


def snapshot_arch_and_state(model: TiedConvAE):
    return {
...
def restore_arch_and_state(model: TiedConvAE, snapshot, device) -> TiedConvAE:
    restored = TiedConvAE(in_ch=model.in_ch, widths=snapshot["widths"], pooling_indices=snapshot["pooling_indices"], use_decoder_bn=model.use_decoder_bn).to(device)
...
def train_with_patience(model: TiedConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list):
...
def adp_search(model: TiedConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def width_search(local_model: TiedConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: TiedConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
...
def main():
    ...
    model = TiedConvAE(in_ch=3, widths=args.widths, pooling_indices=args.pool_idx).to(device)
    acfg = ADPConfig(
    ...
    print(f"[ADP Tied AE] mode={args.adp_mode} best_val={best_val:.6f} widths={widths} depth={len(widths)}")
