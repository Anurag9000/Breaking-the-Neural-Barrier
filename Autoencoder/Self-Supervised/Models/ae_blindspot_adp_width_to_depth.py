...
# - Modes: width_only/width, depth_only/depth, width_to_depth, depth_to_width, alt_width, alt_depth handled via ad hoc while-loop toggling modes.
# - Inner training: train_with_patience mixes ES and acceptance via delta; no distinct patience_es; no phys metric.
# - Width expansion: widen_all mutates in place; uses trials_width as failure counter; no snapshot/restore abstraction; delta shared for width/depth.
# - Depth expansion: append_depth mutates encoder; rollback via state only (architecture changes not captured); uses trials_depth as counter.
# - 2D/ALT: width_to_depth/depth_to_width and alt_* simply toggle on no improvement; missing structured inner/outer loops and phase saturation per spec.
# - Patiences: no separate patience_width_exp / patience_depth_exp applied per context; stopping on generic improved flag.
...
def _build_model(in_ch: int, widths: List[int], pooling_indices: List[int], device) -> BlindSpotConvAE:
    return BlindSpotConvAE(in_ch=in_ch, widths=widths, pooling_indices=pooling_indices).to(device)


def rebuild_model(model: BlindSpotConvAE, widths: List[int], device) -> BlindSpotConvAE:
...
def snapshot_arch_and_state(model: BlindSpotConvAE):
    return {
...
def restore_arch_and_state(model: BlindSpotConvAE, snapshot, device) -> BlindSpotConvAE:
...
def expand_width(model: BlindSpotConvAE, ex_k_width: int, max_width: int, device) -> BlindSpotConvAE:
    new_widths = [min(max_width, w + ex_k_width) for w in model.widths]
    return rebuild_model(model, new_widths, device)


def expand_depth(model: BlindSpotConvAE, ex_k_depth: int, device) -> BlindSpotConvAE:
    if ex_k_depth <= 0:
        return model
...
def train_with_early_stopping(model: BlindSpotConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list) -> Tuple[float, dict, None]:
...
def adp_search(model: BlindSpotConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    ...
    def can_widen(widths: List[int], base_model: BlindSpotConvAE) -> bool:
    ...
    def can_deepen(widths: List[int], base_model: BlindSpotConvAE) -> bool:
    ...
    def width_search(local_model: BlindSpotConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: BlindSpotConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
...
def main():
    ...
    p.add_argument("--results-dir", type=Path, default=Path("results_adp_blindspot"))
    ...
    print(f"[ADP Blindspot AE] mode={args.adp_mode} best_val={best_val:.6f} widths={widths} depth={len(widths)}")
