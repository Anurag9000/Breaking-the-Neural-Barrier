...
def rebuild_decoder(model: EnergyConvAE, widths: List[int]):
...
def widen_all(model: EnergyConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: EnergyConvAE):
...
def train_with_patience(model: EnergyConvAE, dl_train, dl_val, acfg: ADPConfig, device):
...
def adp_search(model: EnergyConvAE, dl_train, dl_val, acfg: ADPConfig, device):
    def can_widen():
...
def main():
    ...
    print(f"[ADP Energy AE] mode={args.adp_mode} best_val={best:.6f} widths={model.widths} depth={len(model.widths)}")
