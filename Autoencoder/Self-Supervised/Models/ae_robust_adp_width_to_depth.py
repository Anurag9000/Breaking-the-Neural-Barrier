...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_robust.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
RobustConvAE = baseline_module.RobustConvAE  # type: ignore
...
# - Expansions: widen_all/append_depth mutate in place; per-failure rollback; single delta for width/depth.
# - 2D/ALT: toggles modes on no improvement; lacks forward-only expansion and context-end restore per updated spec.
# - Missing snapshot/restore abstractions and forward-only patience usage.
...
def rebuild_decoder(model: RobustConvAE, widths: List[int]):
...
def widen_all(model: RobustConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: RobustConvAE):
...
def restore_arch_and_state(model: RobustConvAE, snapshot, device) -> RobustConvAE:
    restored = RobustConvAE(in_ch=model.in_ch, widths=snapshot["widths"], pooling_indices=snapshot["pooling_indices"]).to(device)
...
def huber_loss(y: torch.Tensor, target: torch.Tensor, delta: float) -> torch.Tensor:
    return torch.nn.functional.smooth_l1_loss(y, target, beta=delta, reduction="mean")


def train_with_patience(model: RobustConvAE, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            x = x.to(device)
            y = model(x)
            loss = huber_loss(y, x, acfg.huber_delta)
        ...
        with torch.no_grad():
            ...
            for x, _ in dl_val:
                x = x.to(device)
                y = model(x)
                l = huber_loss(y, x, acfg.huber_delta)
                val += l.item() * x.size(0)
                n += x.size(0)
            val = val / max(n, 1)
        history.append(val)
...
def adp_search(model: RobustConvAE, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp")):
    val_history = []
    improvements = []
    ...
    def width_search(local_model: RobustConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: RobustConvAE, initial_val=None, initial_state=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_state = initial_state
        local_best_widths = list(local_model.widths)
        if local_best_val is None or local_best_state is None:
...
# - Modes implement forward-only ADP spec (no per-expansion rollback; restore global best at context end) for width_only/depth_only, width_to_depth, depth_to_width, alt_width, alt_depth.


def main():
    import argparse

    p = argparse.ArgumentParser(description="ADP Robust (Huber) AE (width/depth search)")
    ...
    model = RobustConvAE(in_ch=3, widths=args.widths, pooling_indices=args.pool_idx).to(device)
    acfg = ADPConfig(
