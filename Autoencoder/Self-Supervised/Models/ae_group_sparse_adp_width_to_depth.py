...
def rebuild_decoder(model: GroupSparseConvAE, widths: List[int]):
...
def widen_all(model: GroupSparseConvAE, ex_k: int, max_width: int):
    new_widths = []
    prev = model.in_ch
...
def append_depth(model: GroupSparseConvAE):
...
def train_with_patience(model: GroupSparseConvAE, dl_train, dl_val, acfg: ADPConfig, device):
...
def adp_search(model: GroupSparseConvAE, dl_train, dl_val, acfg: ADPConfig, device):
    def can_widen():
...
def main():
    ...
    print(f"[ADP GroupSparse AE] mode={args.adp_mode} best_val={best:.6f} widths={model.widths} depth={len(model.widths)} G={model.group_size}")
