...
BASE_PATH = Path(__file__).with_name("ae_over_stl_py_overcomplete_autoencoder_w_activity_regularization.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
AE_OVER_STL = baseline_module.AE_OVER_STL  # type: ignore
...
class ADPConfig:
    ...
    latent_mult: float = 2.0
    lam_activity: float = 1e-5


def rebuild_model(width: int, depth: int, latent_mult: float, pool_after: List[int]) -> AE_OVER_STL:
    return AE_OVER_STL(in_channels=3, width=width, depth=depth, latent_mult=latent_mult, pool_after=pool_after)


def widen_model(model: AE_OVER_STL, ex_k: int, max_width: int):
    new_w = min(max_width, model.width + ex_k)
    if new_w == model.width:
        return
    new_model = rebuild_model(new_w, model.depth, model.latent_mult, list(model.pool_after))
    new_model.load_state_dict(model.state_dict(), strict=False)
    return new_model


def deepen_model(model: AE_OVER_STL):
    new_model = rebuild_model(model.width, model.depth + 1, model.latent_mult, list(model.pool_after))
    new_model.load_state_dict(model.state_dict(), strict=False)
    return new_model


def total_neurons(model: AE_OVER_STL) -> int:
    return model.width * (model.depth + 1)


def snapshot_arch_and_state(model: AE_OVER_STL, state_dict=None):
    state = state_dict if state_dict is not None else model.state_dict()
    return {
        "width": model.width,
        "depth": model.depth,
        "latent_mult": model.latent_mult,
        "pool_after": list(model.pool_after),
        "state": copy.deepcopy(state),
    }


def restore_arch_and_state(model: AE_OVER_STL, snap, device):
    restored = rebuild_model(snap["width"], snap["depth"], snap["latent_mult"], list(snap["pool_after"])).to(device)
...
def activity_loss(z: torch.Tensor) -> torch.Tensor:
    return z.abs().mean()


def train_with_early_stopping(model: AE_OVER_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            ...
            x_rec, z = model(x)
            rec = crit(x_rec, x)
            pen = acfg.lam_activity * activity_loss(z)
            loss = rec + pen
...
def adp_search(model: AE_OVER_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history = []
    improvements = []

    def can_widen(local_model: AE_OVER_STL):
    ...
    def can_deepen(local_model: AE_OVER_STL):
    ...
    def width_search(local_model: AE_OVER_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_OVER_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
...
def main():
    ...
    p = argparse.ArgumentParser(description="ADP Overcomplete AE (Supervised) width/depth search")
    p.add_argument("--width", type=int, default=64)
    p.add_argument("--depth", type=int, default=4)
    p.add_argument("--pool-after", type=int, nargs="*", default=[])
    p.add_argument("--latent-mult", type=float, default=2.0)
    p.add_argument(
        "--adp-mode",
        type=str,
    ...
    p.add_argument("--lam-activity", type=float, default=1e-5)
    ...
    model = AE_OVER_STL(in_channels=3, width=args.width, depth=args.depth, latent_mult=args.latent_mult, pool_after=args.pool_after).to(device)
    acfg = ADPConfig(
        ...
        lam_activity=args.lam_activity,
    )
    results_dir = Path(f"results_{BASE_PATH.stem}")
    # Initialize Logger
    logger = ContinuousLogger(results_dir, "ae_over_stl_py_overcomplete_autoencoder_w_activity_regularization", args.adp_mode)
    ...
    logger.close()
    print(f"[ADP Overcomplete AE STL] mode={args.adp_mode} best_val={best_val:.6f} width={width} depth={depth}")
