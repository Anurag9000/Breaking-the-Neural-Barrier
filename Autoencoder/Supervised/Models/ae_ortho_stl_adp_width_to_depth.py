...
def train_with_early_stopping(model: AE_ORTHO_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            ...
            x_rec, _ = model(x)
            rec = crit(x_rec, x)
            pen = acfg.lam_ortho * orthogonality_penalty(list(model._enc_convs + model._dec_convs))  # type: ignore
            loss = rec + pen
...
def adp_search(model: AE_ORTHO_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history = []
    improvements = []

    def can_widen(local_model: AE_ORTHO_STL):
    ...
    def can_deepen(local_model: AE_ORTHO_STL):
    ...
    def width_search(local_model: AE_ORTHO_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_ORTHO_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
