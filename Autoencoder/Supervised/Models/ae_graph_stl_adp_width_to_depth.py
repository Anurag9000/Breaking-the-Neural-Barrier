...
# - Modes: width_only/width, depth_only/depth, width_to_depth, depth_to_width, alt_width, alt_depth driven by single loop with per-expansion rollback.
...
def adp_search(model: AE_GRAPH_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    ...
    def can_widen(local_model: AE_GRAPH_STL):
        widened_neurons = ae_graph_total_neurons(local_model.dim + acfg.ex_k, local_model.depth)
        return (local_model.dim + acfg.ex_k) <= acfg.max_width and widened_neurons <= acfg.max_neurons

    def can_deepen(local_model: AE_GRAPH_STL):
        deeper_neurons = ae_graph_total_neurons(local_model.dim, local_model.depth + 1)
    ...
    def width_search(local_model: AE_GRAPH_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_GRAPH_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
    ...
    if mode in ("width_only", "width"):
        ...
        best_dim, best_depth = best_snap["dim"], best_snap["depth"]
        ...
        while depth_failure_count < pd and can_deepen(model):
            ...
            if cand_val < best_val - acfg.delta:
                best_val = cand_val
                best_snap = cand_snap
                best_dim, best_depth = cand_snap["dim"], cand_snap["depth"]
                depth_failure_count = 0
    ...
    return best_val, model, best_snap["dim"], best_snap["depth"]
