...
BASE_PATH = Path(__file__).with_name("ae_dilated_stl_py_dilated_atrous_convolution_autoencoder.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
AE_DILATED_STL = baseline_module.AE_DILATED_STL  # type: ignore

# ADP REVIEW (BEFORE REFACTOR)
# - Modes: width_only/width, depth_only/depth, width_to_depth, depth_to_width, alt_width, alt_depth share a single loop with per-step rollback.
# - Inner training: train_with_patience ties ES reset to delta and reloads immediately.
# - Expansions: widen/deepen rollback on failure; single delta/patience for both dimensions.
# - Control flow: toggles modes on no improvement; lacks forward-only march and context-end restore as per updated spec.
# - No snapshot/restore helpers; width/depth patiences not separated from ES patience.
...
def rebuild_model(width: int, depth: int, dilations: List[int], pool_after: List[int]) -> AE_DILATED_STL:
    return AE_DILATED_STL(in_channels=3, width=width, depth=depth, dilations=dilations, pool_after=pool_after)


def widen_model(model: AE_DILATED_STL, ex_k: int, max_width: int):
    new_w = min(max_width, model.width + ex_k)
    if new_w == model.width:
        return
    new_model = rebuild_model(new_w, model.depth, list(model.dilations), list(model.pool_after))
    new_model.load_state_dict(model.state_dict(), strict=False)
    return new_model


def deepen_model(model: AE_DILATED_STL):
    new_dil = list(model.dilations) + [model.dilations[-1]]
    new_model = rebuild_model(model.width, model.depth + 1, new_dil, list(model.pool_after))
    new_model.load_state_dict(model.state_dict(), strict=False)
    return new_model


def total_neurons(model: AE_DILATED_STL) -> int:
    return model.width * (model.depth + 1)


def snapshot_arch_and_state(model: AE_DILATED_STL, state_dict=None):
...
def restore_arch_and_state(model: AE_DILATED_STL, snap, device):
    restored = rebuild_model(snap["width"], snap["depth"], list(snap["dilations"]), list(snap["pool_after"])).to(device)
...
def train_with_early_stopping(model: AE_DILATED_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
...
def adp_search(model: AE_DILATED_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history = []
    improvements: List[tuple] = []

    def can_widen(local_model: AE_DILATED_STL):
    ...
    def can_deepen(local_model: AE_DILATED_STL):
    ...
    def width_search(local_model: AE_DILATED_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_DILATED_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
...
# - alt_depth/alt_width -> Alternating phases starting with depth or width; each phase forward-only on that dimension, starting from global best and restoring it at phase end; stops when both dimensions saturate.


def main():
    import argparse

    p = argparse.ArgumentParser(description="ADP Dilated AE (Supervised) width/depth search")
    p.add_argument("--width", type=int, default=64)
    p.add_argument("--depth", type=int, default=4)
    p.add_argument("--dilations", type=int, nargs="*", default=None)
    ...
    dilations = args.dilations if args.dilations else [1] * args.depth
    dl_train, dl_val = make_loaders(args.batch_size, 0.1)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = AE_DILATED_STL(in_channels=3, width=args.width, depth=args.depth, dilations=dilations, pool_after=args.pool_after).to(device)
    acfg = ADPConfig(
    ...
    # Initialize Logger
    logger = ContinuousLogger(results_dir, "ae_dilated_stl_py_dilated_atrous_convolution_autoencoder", args.adp_mode)
