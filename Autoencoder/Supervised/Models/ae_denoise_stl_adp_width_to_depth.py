...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_denoise_stl.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
AE_DENOISE_STL = baseline_module.AE_DENOISE_STL  # type: ignore

# ADP REVIEW (BEFORE REFACTOR)
# - Modes: width_only/width, depth_only/depth, width_to_depth, depth_to_width, alt_width, alt_depth all run through a single loop with per-step rollback.
# - Inner training: train_with_patience ties ES reset to delta and restores immediately.
# - Expansions: widen/deepen with rollback on every failed attempt; width/depth share one delta and patience count.
# - Control flow: mode toggles on no improvement; lacks forward-only march and context-level restore per updated ADP spec.
# - Missing snapshot/restore helpers and separation of ES vs expansion patiences.
...
def rebuild_model(width: int, depth: int, pool_after: List[int]) -> AE_DENOISE_STL:
    return AE_DENOISE_STL(in_channels=3, width=width, depth=depth, pool_after=pool_after)


def widen_model(model: AE_DENOISE_STL, ex_k: int, max_width: int):
...
def deepen_model(model: AE_DENOISE_STL):
    new_d = model.depth + 1
...
def total_neurons(model: AE_DENOISE_STL) -> int:
    return model.width * (model.depth + 1)


def snapshot_arch_and_state(model: AE_DENOISE_STL, state_dict=None):
    state = state_dict if state_dict is not None else model.state_dict()
    return {"width": model.width, "depth": model.depth, "pool_after": list(model.pool_after), "state": copy.deepcopy(state)}


def restore_arch_and_state(model: AE_DENOISE_STL, snap, device):
...
def train_with_early_stopping(model: AE_DENOISE_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        with torch.no_grad():
            ...
            for x, _ in dl_val:
                x = x.to(device)
                x_noisy = x  # eval without extra noise
                x_rec, _ = model(x_noisy)
                l = crit(x_rec, x)
                val += l.item() * x.size(0)
                n += x.size(0)
...
def adp_search(model: AE_DENOISE_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history = []
    improvements: List[tuple] = []

    def can_widen(local_model: AE_DENOISE_STL):
    ...
    def can_deepen(local_model: AE_DENOISE_STL):
    ...
    def width_search(local_model: AE_DENOISE_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_DENOISE_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
...
# - width_only/width -> ADP_WIDTH_ONLY: forward-only width expansions with width_failure_count < trials_width; restore best snapshot once at end.
# - depth_only/depth -> ADP_DEPTH_ONLY: forward-only depth expansions with depth_failure_count < trials_depth; restore best snapshot once at end.
# - depth_to_width -> ADP_DEPTH_OUTER_WIDTH_INNER: outer depth loop marches forward; inner width_search forward-only; accept if delta-improved else keep going until depth patience then restore global best.
# - width_to_depth -> ADP_WIDTH_OUTER_DEPTH_INNER: outer width loop marches forward; inner depth_search forward-only; acceptance by delta then restore global best after loop.
# - alt_depth/alt_width -> Alternating phases starting with depth or width; each phase uses forward-only search on that dimension, starts from global best, restores global best at phase end; loop ends when both dimensions saturated.


def main():
    import argparse

    p = argparse.ArgumentParser(description="ADP Denoising AE (Supervised) width/depth search")
    ...
    model = AE_DENOISE_STL(in_channels=3, width=args.width, depth=args.depth, pool_after=args.pool_after).to(device)
    acfg = ADPConfig(
    ...
    # Initialize Logger
    logger = ContinuousLogger(results_dir, "ae_denoise_stl", args.adp_mode)
    ...
    logger.close()
    print(f"[ADP Denoise AE STL] mode={args.adp_mode} best_val={best_val:.6f} width={width} depth={depth}")
