...
def train_with_patience(model: AE_SPARSE_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
...
def adp_search(model: AE_SPARSE_STL, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp_sparse_stl")):
    ...
    def can_widen(width: int, depth: int):
        ...
        return new_w > width and ae_sparse_total_neurons(new_w, depth) <= acfg.max_neurons

    def can_deepen(width: int, depth: int):
        return depth + 1 <= acfg.max_depth and ae_sparse_total_neurons(width, depth + 1) <= acfg.max_neurons

    cur_w, cur_d = model.width, model.depth
    best_val = train_with_patience(model, dl_train, dl_val, acfg, device, val_history)
    best_state = copy.deepcopy(model.state_dict())
    improvements.append((ae_sparse_total_neurons(cur_w, cur_d), best_val))
...
def main():
    ...
    print(f"[ADP Sparse AE] mode={args.adp_mode} best_val={best:.6f} width={w} depth={d}")
