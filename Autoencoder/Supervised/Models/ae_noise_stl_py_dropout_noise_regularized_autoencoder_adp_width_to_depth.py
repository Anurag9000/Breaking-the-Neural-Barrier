...
AE_NOISE_STL = baseline_module.AE_NOISE_STL  # type: ignore
...
class ADPConfig:
    ...
    p_drop_enc: float = 0.0
    p_drop_dec: float = 0.0
    feat_sigma_enc: float = 0.0
    feat_sigma_dec: float = 0.0


def rebuild_model(width: int, depth: int, pool_after: List[int], p_drop_enc: float, p_drop_dec: float, feat_sigma_enc: float, feat_sigma_dec: float) -> AE_NOISE_STL:
    return AE_NOISE_STL(
        in_channels=3,
        width=width,
        depth=depth,
        pool_after=pool_after,
        p_drop_enc=p_drop_enc,
        p_drop_dec=p_drop_dec,
        feat_sigma_enc=feat_sigma_enc,
        feat_sigma_dec=feat_sigma_dec,
    )


def widen_model(model: AE_NOISE_STL, ex_k: int, max_width: int):
    new_w = min(max_width, model.width + ex_k)
    if new_w == model.width:
        return
    new_model = rebuild_model(new_w, model.depth, list(model.pool_after), model.p_drop_enc, model.p_drop_dec, model.feat_sigma_enc, model.feat_sigma_dec)
    new_model.load_state_dict(model.state_dict(), strict=False)
    return new_model


def deepen_model(model: AE_NOISE_STL):
    new_model = rebuild_model(model.width, model.depth + 1, list(model.pool_after), model.p_drop_enc, model.p_drop_dec, model.feat_sigma_enc, model.feat_sigma_dec)
    new_model.load_state_dict(model.state_dict(), strict=False)
    return new_model


def total_neurons(model: AE_NOISE_STL) -> int:
    return model.width * (model.depth + 1)


def snapshot_arch_and_state(model: AE_NOISE_STL, state_dict=None):
    state = state_dict if state_dict is not None else model.state_dict()
    return {
        "width": model.width,
        "depth": model.depth,
        "pool_after": list(model.pool_after),
        "p_drop_enc": model.p_drop_enc,
        "p_drop_dec": model.p_drop_dec,
        "feat_sigma_enc": model.feat_sigma_enc,
        "feat_sigma_dec": model.feat_sigma_dec,
        "state": copy.deepcopy(state),
    }


def restore_arch_and_state(model: AE_NOISE_STL, snap, device):
    restored = rebuild_model(
        snap["width"],
        snap["depth"],
        list(snap["pool_after"]),
        snap["p_drop_enc"],
        snap["p_drop_dec"],
        snap["feat_sigma_enc"],
        snap["feat_sigma_dec"],
...
def train_with_early_stopping(model: AE_NOISE_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
...
def adp_search(model: AE_NOISE_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history = []
    improvements = []

    def can_widen(local_model: AE_NOISE_STL):
    ...
    def can_deepen(local_model: AE_NOISE_STL):
    ...
    def width_search(local_model: AE_NOISE_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_NOISE_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
...
def main():
    ...
    p = argparse.ArgumentParser(description="ADP Dropout/Noise AE (Supervised) width/depth search")
    p.add_argument("--width", type=int, default=64)
    p.add_argument("--depth", type=int, default=4)
    p.add_argument("--pool-after", type=int, nargs="*", default=[])
    p.add_argument("--p-drop-enc", type=float, default=0.0)
    ...
    model = AE_NOISE_STL(
        ...
        p_drop_enc=args.p_drop_enc,
        p_drop_dec=args.p_drop_dec,
    ...
    # Initialize Logger
    logger = ContinuousLogger(results_dir, "ae_noise_stl_py_dropout_noise_regularized_autoencoder", args.adp_mode)
    ...
    logger.close()
    print(f"[ADP Noise AE STL] mode={args.adp_mode} best_val={best_val:.6f} width={width} depth={depth}")
