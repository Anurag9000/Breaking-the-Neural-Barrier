...
# Load baseline
BASE_PATH = Path(__file__).with_name("ae_robust_stl.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
AE_ROBUST_STL = baseline_module.AE_ROBUST_STL  # type: ignore
ConvBlock = baseline_module.ConvBlock  # type: ignore
DeconvBlock = baseline_module.DeconvBlock  # type: ignore
ae_robust_total_neurons = baseline_module.ae_robust_total_neurons  # type: ignore
...
def rebuild_model(model: AE_ROBUST_STL, width: int, depth: int, device, pool_after: List[int]) -> AE_ROBUST_STL:
    new_model = AE_ROBUST_STL(in_channels=model.in_channels, width=width, depth=depth, pool_after=pool_after).to(device)
    merged = _merge_state(new_model.state_dict(), model.state_dict())
    new_model.load_state_dict(merged, strict=False)
    return new_model


def widen_model(model: AE_ROBUST_STL, ex_k: int, max_width: int, device):
...
def deepen_model(model: AE_ROBUST_STL, device):
    return rebuild_model(model, model.width, model.depth + 1, device, list(model.pool_after))


def snapshot_arch_and_state(model: AE_ROBUST_STL, state_dict=None):
    state = state_dict if state_dict is not None else model.state_dict()
    return {"width": model.width, "depth": model.depth, "pool_after": list(model.pool_after), "state": copy.deepcopy(state)}


def restore_arch_and_state(model: AE_ROBUST_STL, snap, device):
    restored = AE_ROBUST_STL(in_channels=model.in_channels, width=snap["width"], depth=snap["depth"], pool_after=list(snap["pool_after"])).to(device)
    restored.load_state_dict(snap["state"])
    return restored


def train_with_early_stopping(model: AE_ROBUST_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            ...
            rec, _ = model(x)
            loss = F.huber_loss(rec, x, delta=acfg.huber_delta)
            loss.backward()
        ...
        with torch.no_grad():
            ...
            for x, _ in dl_val:
                x = x.to(device)
                rec, _ = model(x)
                l = F.huber_loss(rec, x, delta=acfg.huber_delta)
                val += l.item(); n += 1
            val = val / max(n,1)
        history.append(val)
...
def adp_search(model: AE_ROBUST_STL, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp_robust_stl")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history: List[float] = []
    improvements: List[tuple[int, float]] = []

    def can_widen(local_model: AE_ROBUST_STL):
        new_w = min(acfg.max_width, local_model.width + acfg.ex_k)
        return new_w > local_model.width and ae_robust_total_neurons(new_w, local_model.depth) <= acfg.max_neurons

    def can_deepen(local_model: AE_ROBUST_STL):
        return local_model.depth + 1 <= acfg.max_depth and ae_robust_total_neurons(local_model.width, local_model.depth + 1) <= acfg.max_neurons
    ...
    improvements.append((ae_robust_total_neurons(model.width, model.depth), best_val))
    pw, pd = acfg.trials_width, acfg.trials_depth

    def width_search(local_model: AE_ROBUST_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        ...
        while width_failure_count < pw and can_widen(local_model):
            ...
            if cand_val < local_best_val - acfg.delta:
                ...
                width_failure_count = 0
                if log_improvement:
                    improvements.append((ae_robust_total_neurons(local_model.width, local_model.depth), local_best_val))
            else:
    ...
    def depth_search(local_model: AE_ROBUST_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
        ...
        while not (depth_saturated and width_saturated):
            if phase == "depth":
                ...
                if cand_val < best_val - acfg.delta:
                    ...
                    best_width, best_depth = cand_snap["width"], cand_snap["depth"]
                    depth_saturated = False
                    improvements.append((ae_robust_total_neurons(cand_model.width, cand_model.depth), best_val))
                else:
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="ADP Robust AE (Huber) width/depth search")
    ...
    model = AE_ROBUST_STL(in_channels=3, width=args.width, depth=args.depth, pool_after=args.pool_after).to(device)
    acfg = ADPConfig(adp_mode=args.adp_mode, delta=args.delta, patience=args.patience, trials_width=args.trials_width,
                     ...
                     max_neurons=args.max_neurons, max_epochs=args.max_epochs, huber_delta=args.huber_delta,
                     pool_after=args.pool_after)
    ...
    print(f"[ADP Robust AE] mode={args.adp_mode} best_val={best_val:.6f} width={w} depth={d}")
