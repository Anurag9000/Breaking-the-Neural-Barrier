...
def train_with_patience(model: AE_ROBUST_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
...
def adp_search(model: AE_ROBUST_STL, dl_train, dl_val, acfg: ADPConfig, device, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path("results_adp_robust_stl")):
    ...
    def can_widen(width: int, depth: int):
        ...
        return new_w > width and ae_robust_total_neurons(new_w, depth) <= acfg.max_neurons

    def can_deepen(width: int, depth: int):
        return depth + 1 <= acfg.max_depth and ae_robust_total_neurons(width, depth + 1) <= acfg.max_neurons

    cur_w, cur_d = model.width, model.depth
    best_val = train_with_patience(model, dl_train, dl_val, acfg, device, val_history)
    best_state = copy.deepcopy(model.state_dict())
    improvements.append((ae_robust_total_neurons(cur_w, cur_d), best_val))
    ...
    while improved:
        ...
        if mode in ("width_only","width","width_to_depth","alt_width"):
            if can_widen(cur_w, cur_d) and pw > 0:
                ...
                model = rebuild_model(model, cur_w, cur_d, device, list(model.pool_after))
...
def main():
    ...
    print(f"[ADP Robust AE] mode={args.adp_mode} best_val={best:.6f} width={w} depth={d}")
