...
BASE_PATH = Path(__file__).with_name("ae_lowrank_stl_py_low_rank_nuclear_norm_regularized_ae.py").resolve()
_spec = importlib.util.spec_from_file_location("baseline_module", BASE_PATH)
baseline_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(baseline_module)
AE_LOWRANK_STL = baseline_module.AE_LOWRANK_STL  # type: ignore
...
def rebuild_model(width: int, depth: int, pool_after: List[int]) -> AE_LOWRANK_STL:
    return AE_LOWRANK_STL(in_channels=3, width=width, depth=depth, pool_after=pool_after)


def widen_model(model: AE_LOWRANK_STL, ex_k: int, max_width: int):
...
def nuclear_norm_loss(z: torch.Tensor) -> torch.Tensor:
    # z: (B,C,H,W) -> flatten spatial
    B, C, H, W = z.shape
    z_flat = z.view(B, C, -1)  # (B,C,HW)
    # nuclear norm per-sample (sum singular values)
    loss = 0.0
    for i in range(B):
        s = torch.linalg.svdvals(z_flat[i])
        loss = loss + s.sum()
    return loss / float(B)


def train_with_early_stopping(model: AE_LOWRANK_STL, dl_train, dl_val, acfg: ADPConfig, device, history: list):
    ...
    for _ in range(acfg.max_epochs):
        ...
        for x, _ in dl_train:
            ...
            x_rec, z = model(x)
            rec = crit(x_rec, x)
            pen = acfg.lam_nuc * nuclear_norm_loss(z)
            loss = rec + pen
...
def adp_search(model: AE_LOWRANK_STL, dl_train, dl_val, acfg: ADPConfig, device, logger: ContinuousLogger, log_loss: bool = False, log_neurons: bool = False, results_dir: Path = Path(\"results_adp\")):
    results_dir.mkdir(parents=True, exist_ok=True)
    val_history = []
    improvements: List[tuple] = []

    def can_widen(local_model: AE_LOWRANK_STL):
    ...
    def can_deepen(local_model: AE_LOWRANK_STL):
    ...
    def width_search(local_model: AE_LOWRANK_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
    ...
    def depth_search(local_model: AE_LOWRANK_STL, initial_val=None, initial_snap=None, log_improvement: bool = False):
        local_best_val = initial_val
        local_best_snap = initial_snap
        if local_best_val is None or local_best_snap is None:
            local_best_val, local_best_state = train_with_early_stopping(local_model, dl_train, dl_val, acfg, device, val_history, logger=logger)
...
def main():
    ...
    p = argparse.ArgumentParser(description="ADP Low-Rank (nuclear norm) AE (Supervised)")
    ...
    p.add_argument("--lam-nuc", type=float, default=1e-3)
    ...
    model = AE_LOWRANK_STL(in_channels=3, width=args.width, depth=args.depth, pool_after=args.pool_after).to(device)
    acfg = ADPConfig(
        ...
        lam_nuc=args.lam_nuc,
    )
    results_dir = Path(f"results_{BASE_PATH.stem}")
    # Initialize Logger
    logger = ContinuousLogger(results_dir, "ae_lowrank_stl_py_low_rank_nuclear_norm_regularized_ae", args.adp_mode)
    ...
    logger.close()
    print(f"[ADP LowRank AE STL] mode={args.adp_mode} best_val={best_val:.6f} width={width} depth={depth}")
