...
from AE_DENOISE_STL import AE_DENOISE_STL, ae_denoise_total_neurons

# -----------------------------------------------------------------------------
# Runner for AE_DENOISE_STL (Gaussian Denoising Autoencoder)
# Adds Gaussian noise only during training; validates/tests on clean inputs.
# -----------------------------------------------------------------------------
...
def train_one_epoch(model, loader, opt, device, sigma: float, scaler=None):
    model.train()
    mse = nn.MSELoss()
    total, n = 0.0, 0
    for x, _ in loader:
...
def eval_epoch(model, loader, device):
    ...
    with torch.no_grad():
        for x, _ in loader:
            x = x.to(device, non_blocking=True)
            x_rec, _ = model(x)  # evaluate with clean input
...
def main():
    ...
    p.add_argument('--out_dir', type=str, default='./runs/ae_denoise_stl')
    ...
    # Denoising
    p.add_argument('--noise_sigma', type=float, default=0.1, help='std-dev of Gaussian noise added during training (input is normalized)')
    ...
    model = AE_DENOISE_STL(in_channels=3, width=args.width, depth=args.depth, pool_after=pool_after).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(model, train_loader, optimizer, device, args.noise_sigma, scaler if args.amp else None)
    ...
    report = {
        ...
        'neurons_metric': ae_denoise_total_neurons(args.width, args.depth),
