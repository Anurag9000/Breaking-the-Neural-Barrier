...
# -----------------------------------------------------------------------------
# Runner for AE_ORTHO_STL
# Objective: MSE + lam_ortho * sum(||W W^T - I||_F^2) over conv & deconv kernels.
# -----------------------------------------------------------------------------
...
def train_one_epoch(model, loader, opt, device, lam_ortho: float, scaler=None):
    ...
    for x, _ in loader:
        ...
        if scaler is None:
            ...
            loss = mse(x_rec, x)
            if lam_ortho > 0:
                # collect conv modules
                convs = [m for m in model.modules() if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d))]
                loss = loss + lam_ortho * orthogonality_penalty(convs)
            loss.backward()
            opt.step()
        else:
...
def eval_epoch(model, loader, device, lam_ortho: float):
    model.eval()
    ...
    with torch.no_grad():
        for x, _ in loader:
            ...
            if lam_ortho > 0:
                convs = [m for m in model.modules() if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d))]
                # include penalty in validation objective
                loss = loss + lam_ortho * orthogonality_penalty(convs)
...
def main():
    ...
    # Ortho
    p.add_argument('--lam_ortho', type=float, default=1e-6)
    ...
    model = AE_ORTHO_STL(in_channels=3, width=args.width, depth=args.depth, pool_after=pool_after).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(model, train_loader, optimizer, device, args.lam_ortho, scaler if args.amp else None)
        val_loss = eval_epoch(model, val_loader, device, args.lam_ortho)
    ...
    report = {
        'dataset': args.dataset, 'width': args.width, 'depth': args.depth,
        'pool_after': pool_after, 'lam_ortho': args.lam_ortho,
        'neurons_metric': ae_ortho_total_neurons(args.width, args.depth),
        'best_val_obj': best_val, 'best_epoch': best_epoch, 'test_mse': test_mse,
    }
