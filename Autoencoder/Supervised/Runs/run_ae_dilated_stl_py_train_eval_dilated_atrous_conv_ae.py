...
from AE_DILATED_STL import AE_DILATED_STL, ae_dilated_total_neurons

# -----------------------------------------------------------------------------
# Runner for AE_DILATED_STL
# Adds dilation schedule for encoder/decoder blocks; MSE reconstruction loss.
# -----------------------------------------------------------------------------
...
def _parse_dilations(s: str, depth: int):
    if s.strip() == '':
        return [1] * depth
    ...
    if len(vals) == 1:
        return vals * depth
    if len(vals) != depth:
        raise ValueError(f"Expected {depth} dilation values; got {len(vals)}")
    return vals
...
def main():
    ...
    p.add_argument('--out_dir', type=str, default='./runs/ae_dilated_stl')
    p.add_argument('--seed', type=int, default=1337)
    # Model
    p.add_argument('--width', type=int, default=64)
    p.add_argument('--depth', type=int, default=4)
    p.add_argument('--pool_after', type=str, default='')
    p.add_argument('--dilations', type=str, default='1,2,4,1', help='per-block dilation list or single int')
    # Train
    ...
    dilations = _parse_dilations(args.dilations, args.depth)
    ...
    model = AE_DILATED_STL(in_channels=3, width=args.width, depth=args.depth,
                           dilations=dilations, pool_after=pool_after).to(device)
    ...
    report = {
        ...
        'dilations': dilations,
        'neurons_metric': ae_dilated_total_neurons(args.width, args.depth),
