...
from AE_TCN_STL import AE_TCN_STL, ae_tcn_total_neurons

# -----------------------------------------------------------------------------
# Runner for AE_TCN_STL (treat rows as sequence; TCN encoder/decoder).
# CIFAR 32x32 assumed for reshape logic.
# -----------------------------------------------------------------------------
...
def main():
    ...
    p.add_argument('--dilations', type=str, default='1,2,4,1')
    ...
    dilations = [int(x) for x in args.dilations.split(',')] if args.dilations.strip() else [1]*args.depth
    ...
    model = AE_TCN_STL(in_channels=3, width=args.width, depth=args.depth, dilations=dilations).to(device)
    ...
    report = {
        ...
        'dilations': dilations, 'neurons_metric': ae_tcn_total_neurons(args.width, args.depth),
