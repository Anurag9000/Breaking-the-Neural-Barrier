...
from AE_NOISE_STL import AE_NOISE_STL, ae_noise_total_neurons

# -----------------------------------------------------------------------------
# Runner for AE_NOISE_STL: internal regularization via Dropout2d and/or
# GaussianFeatureNoise inside the network. Reconstruction loss = MSE.
# -----------------------------------------------------------------------------
...
def main():
    ...
    p.add_argument('--out_dir', type=str, default='./Runs/ae_noise_stl')
    p.add_argument('--seed', type=int, default=1337)
    # Model
    p.add_argument('--width', type=int, default=64)
    p.add_argument('--depth', type=int, default=4)
    p.add_argument('--pool_after', type=str, default='2')
    p.add_argument('--p_drop_enc', type=float, default=0.1)
    p.add_argument('--p_drop_dec', type=float, default=0.0)
    p.add_argument('--feat_sigma_enc', type=float, default=0.0)
    p.add_argument('--feat_sigma_dec', type=float, default=0.0)
    # Train
    ...
    model = AE_NOISE_STL(in_channels=3, width=args.width, depth=args.depth,
                         pool_after=pool_after,
                         p_drop_enc=args.p_drop_enc, p_drop_dec=args.p_drop_dec,
                         feat_sigma_enc=args.feat_sigma_enc, feat_sigma_dec=args.feat_sigma_dec).to(device)
    ...
    report = {
        ...
        'depth': args.depth,
        'pool_after': pool_after,
        'neurons_metric': ae_noise_total_neurons(args.width, args.depth),
